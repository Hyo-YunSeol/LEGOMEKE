...
const THEMES = Object.freeze({
  'body-guide': { label: '레고 체형도감', sky: '#fff7ed', floor: '#fde68a', accent: '#ea580c', pool: ['crown','heart','star','apple','cake','cat','balloon','flower','book','moon','gift','clock'] },
  'lego-room': { label: '레고의 방', sky: '#eff6ff', floor: '#e7e5e4', accent: '#2563eb', pool: ['clock','plant','book','cup','cat','gamepad','camera','gift','heart','cake','star','moon'] },
  convenience: { label: '편의점', sky: '#ecfdf5', floor: '#d1fae5', accent: '#059669', pool: ['bottle','apple','cake','cup','camera','gift','star','mushroom','book','heart','clock','balloon'] },
  beach: { label: '바닷가', sky: '#e0f2fe', floor: '#fef3c7', accent: '#0284c7', pool: ['sun','cloud','fish','umbrella','balloon','apple','star','moon','camera','bottle','heart','flower'] },
  ...
  festival: { label: '축제', sky: '#fdf2f8', floor: '#fbcfe8', accent: '#db2777', pool: ['balloon','gift','star','crown','cake','heart','camera','apple','rocket','flower','moon','cup'] },
  'space-lab': { label: '우주 연구소', sky: '#eef2ff', floor: '#c7d2fe', accent: '#4338ca', pool: ['rocket','moon','star','gamepad','bottle','clock','camera','crown','heart','plant','fish','gift'] }
});
...
function renderCandidate(type, hitbox, index, changed, accent) {
  ...
  if (mode === 0) return '';
  if (mode === 1) return `<g transform="translate(${x} ${y})" filter="url(#spotHue)">${base}</g>`;
...
export function renderSpotDifferenceScene(puzzle, { changed = false, foundIds = [], revealAll = false, ariaLabel = '' } = {}) {
  ...
  const answers = hitboxes.filter((hitbox) => differences.has(hitbox.id) && (revealAll || found.has(hitbox.id))).map((hitbox) => {
    ...
    return `<circle cx="${hitbox.x * W}" cy="${hitbox.y * H}" r="${radius}" fill="none" stroke="#ef4444" stroke-width="8"/><circle cx="${hitbox.x * W}" cy="${hitbox.y * H}" r="${Math.max(10, radius - 12)}" fill="none" stroke="#fff" stroke-width="3" opacity=".92"/>`;
  }).join('');
  return `<svg class="spot-scene-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="${String(ariaLabel || theme.label).replace(/[&<>\"]/g, '')}"><defs><filter id="spotHue"><feColorMatrix type="hueRotate" values="145"/></filter></defs>${themeBackground(puzzle.themeId, puzzle.variant)}${objects}<rect x="12" y="12" width="776" height="496" rx="20" fill="none" stroke="#0f172a" stroke-opacity=".14" stroke-width="4"/>${answers}</svg>`;
}
