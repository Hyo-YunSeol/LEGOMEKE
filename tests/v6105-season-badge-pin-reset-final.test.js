...
test('이미 배포된 v27의 잘못된 다매 사천성왕을 레알다매레고로 한 번만 정정하고 v28로 올린다', async () => {
  ...
  assert.equal(firstLoad.meta.version, 28);
  ...
  assert.equal(secondLoad.meta.version, 28);
  assert.equal(secondLoad.pets[secondLoad.users['real-damae-user'].currentPetId].seasonBadges.sichuan, null, 'v28 정정은 다시 실행되면 안 된다');
});
