...
test('v26 저장 데이터는 다매에게 사천성왕을 한 번만 소급 부여하고 v27로 올린다', async () => {
  const storage = new MemoryStorage();
  const writer = new DurableJsonStore(storage);
  const state = initialState();
  state.meta.version = 26;
  const user = {
  ...
  const migratedPet = firstLoad.pets[firstLoad.users['damae-user'].currentPetId];
  assert.equal(firstLoad.meta.version, 27);
  assert.equal(migratedPet.seasonBadges.sichuan, firstLoad.gameRankingSeason.endsAt);

  migratedPet.seasonBadges.sichuan = null;
  ...
  assert.equal(secondLoad.meta.version, 27);
  assert.equal(secondLoad.pets[secondLoad.users['damae-user'].currentPetId].seasonBadges.sichuan, null, 'v27에서는 소급 지급이 다시 실행되면 안 된다');
});
