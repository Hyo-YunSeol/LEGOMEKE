...
test('사천성 3연승 이상은 시작 10초간 그림만 가리고 서버 패 입력은 즉시 허용한다', () => {
  ...
  assert.ok(pair);
  const allowedWhileHidden = playSichuanPair(state, host, room.id, { matchId: room.matchId, first: pair[0], second: pair[1], actionId: 'penalty-hidden-allowed' }, at(5));
  assert.equal(allowedWhileHidden.ok, true);
  assert.equal(allowedWhileHidden.removed, true);
  assert.equal(room.players[host.id].removedCount, 2);
});
