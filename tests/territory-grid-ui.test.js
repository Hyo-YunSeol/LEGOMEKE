...
test('영토전 화면은 5×5를 행 4개·각 행 4칸으로 렌더링한다', () => {
...
test('영토전 UI는 진행 중 황금 위치를 숨기고 종료된 지난 시즌 결과에서만 공개한다', () => {
  assert.match(appJs, /territory-home-icon/);
  assert.match(appJs, /황금 영토는 게임 종료 후 공개됩니다/);
  assert.match(appJs, /종료된 시즌의 황금 영토는 이제 공개됩니다/);
  assert.match(appJs, /last\.goldenCell/);
  assert.doesNotMatch(appJs, /territory-golden-countdown/);
