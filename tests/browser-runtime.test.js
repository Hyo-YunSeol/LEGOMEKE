...
// 공유 CI에서 여러 Node 테스트 파일과 JSDOM이 동시에 CPU를 사용할 때 2.5초는
// 정상 UI도 간헐적으로 실패시켰다. 기능 실패는 그대로 잡되 환경 스케줄링 여유를 둔다.
...
async function createBrowser({ token = '', fetchImpl, socketSendHook = null }) {
  ...
  class MockWebSocket extends window.EventTarget {
    ...
    send(value) {
      ...
      window.__sentSocketMessages.push(parsed);
      if (typeof socketSendHook === 'function') socketSendHook(parsed, this, window);
    }
    emit(payload) {
...
test('PC 확성기 입력은 실시간 bootstrap 갱신이 와도 작성 중 DOM과 값을 잃지 않는다', async (t) => {
  const { room } = await createRoom();
  const token = await register(room, '확성기PC');
  const runtime = await createBrowser({ token, fetchImpl: (window) => roomFetch(window, room) });
  t.after(() => runtime.dom.window.close());
  const { window } = runtime;

  await waitFor(() => !window.document.querySelector('#app-shell')?.classList.contains('hidden'), '앱 화면이 열리지 않았습니다.');
  window.document.querySelector('[data-action="open-shop"]')?.click();
  await waitFor(() => window.document.querySelector('#loudspeaker-message-input'), '확성기 입력창이 열리지 않았습니다.');

  const input = window.document.querySelector('#loudspeaker-message-input');
  input.focus();
  // PC 한글 IME의 이벤트 경계처럼 DOM 값은 바뀌었지만 input 이벤트가 아직 앱 상태에 반영되지 않은 순간을 재현한다.
  input.value = '입력하다 사라지면 안 됨';
  input.blur();
  window.__testSockets[0].emit({ type: 'refresh', reason: 'presence', at: Date.now() });
  await new Promise((resolve) => setTimeout(resolve, 260));

  const after = window.document.querySelector('#loudspeaker-message-input');
  assert.equal(after, input, '실시간 bootstrap이 확성기 input DOM을 교체하면 PC 한글 입력이 끊길 수 있다');
  assert.equal(after.value, '입력하다 사라지면 안 됨');
  assert.deepEqual(errorsToMessages(runtime.errors), []);
});
...
test('모바일 사과게임은 포기하기를 누르면 현재 포인트를 정산하고 결과창을 연다', async (t) => {
  const { room } = await createRoom();
  const token = await register(room, '사과모바일');
  const started = await responseJson(await room.fetch(authRequest('/api/minigames/start', token, {
    method: 'POST', body: JSON.stringify({ gameId: 'apple' })
  })));
  const state = await room.store.load();
  const challenge = state.miniGameChallenges[started.data.challenge.id];
  challenge.applePendingPoints = 11;
  challenge.appleScore = 440;
  challenge.appleRemovedCount = 22;
  await room.store.save(state);
  ...
  await waitFor(() => window.document.querySelector('[data-action="resume-mini"]'), '사과게임 이어하기가 보이지 않습니다.');
  window.document.querySelector('[data-action="resume-mini"]').click();
  await waitFor(() => window.document.querySelector('#apple-board'), '사과게임판이 열리지 않았습니다.');

  const abandon = window.document.querySelector('.apple-abandon-controls [data-action="abandon-mini"]');
  assert.ok(abandon);
  assert.equal(abandon.textContent.trim(), '포기하기');
  abandon.click();
  await waitFor(() => window.document.querySelector('.mini-result-card'), '사과게임 포기 결과가 열리지 않았습니다.');

  const latest = await room.store.load();
  const pet = Object.values(latest.pets)[0];
  assert.equal(latest.miniGameChallenges[challenge.id].completed, true);
  assert.equal(pet.stats.points, 11);
  assert.equal(pet.daily.miniGamesPlayed, 1);
  assert.match(window.document.querySelector('.mini-result-card').textContent, /게임을 포기했습니다/);
  assert.deepEqual(errorsToMessages(runtime.errors), []);
});

test('블록게임은 문양·공용 empty 클래스 없이 색상만 표시하고 포기하기를 정산한다', async (t) => {
  const { room } = await createRoom();
  ...
  const stop = window.document.querySelector('.block-controls [data-action="abandon-mini"]');
  assert.ok(stop);
  assert.equal(stop.textContent.trim(), '포기하기');
  stop.click();
  await waitFor(() => window.document.querySelector('.mini-result-card'), '포기하기 결과가 열리지 않았습니다.');
  const latest = await room.store.load();
  const pet = Object.values(latest.pets)[0];
  assert.equal(latest.miniGameChallenges[challenge.id].completed, true);
...
test('테트리스대전은 늦은 서버 상태가 와도 확인되지 않은 좌우 입력을 되돌리지 않는다', async (t) => {
  ...
  // 이 테스트는 '늦은 서버 상태가 좌우 입력을 되돌리는지'만 검증한다.
  // 공유 CI가 느려져도 서버 중력 catch-up이 먼저 발생해 수직 위치까지 바뀌는
  // 별개의 시간 의존성이 섞이지 않도록 마지막 중력 시각을 테스트 동안 미래로 고정한다.
  const battleRoom = state.blockBattle?.rooms?.[created.data.roomId];
  for (const player of Object.values(battleRoom?.players || {})) {
    player.lastGravityAt = new Date(Date.now() + 60_000).toISOString();
  }
  await room.store.save(state);
  state = await room.store.load();
...
test('장시간 플레이에도 700ms 고정 낙하를 유지하고 자동 tick은 누적되지 않는다', async (t) => {
  const { room } = await createRoom();
  ...
  assert.ok(second.actions.includes('left'), '장시간 플레이에도 좌우 입력이 자동 낙하보다 먼저 보존되어야 한다');
...
test('고지연 테트리스대전은 첫 Space ACK 전에 누른 두 번째 독립 Space를 다음 블록에 정확히 한 번 보존한다', async (t) => {
  const { room } = await createRoom();
  const hostToken = await register(room, '연속스페이스방장');
  const guestToken = await register(room, '연속스페이스손님');
  let state = await room.store.load();
  for (const pet of Object.values(state.pets)) pet.stats.points = 1_000;
  await room.store.save(state);
  const created = await responseJson(await room.fetch(authRequest('/api/block-battle/rooms', hostToken, {
  ...
  state = await room.store.load();
  const battle = state.blockBattle.rooms[created.data.roomId];
  for (const player of Object.values(battle.players)) player.lastGravityAt = new Date(Date.now() + 60_000).toISOString();
  const host = Object.values(state.users).find((user) => user.nickname === '연속스페이스방장');
  const initialPieces = Number(battle.players[host.currentPetId].pieces || 0);
  await room.store.save(state);
  ...
  await waitFor(() => !window.document.querySelector('#app-shell')?.classList.contains('hidden'), '앱 화면이 열리지 않았습니다.');
  window.document.querySelector('.nav-item[data-tab="games"]').dispatchEvent(new window.PointerEvent('pointerdown', {
    bubbles: true, cancelable: true, pointerType: 'touch', pointerId: 71, isPrimary: true
  }));
  await waitFor(() => window.document.querySelector('[data-action="block-battle-control"][data-value="hardDrop"]'), '테트리스 하드드롭 조작이 열리지 않았습니다.');

  const hardDropMessages = () => (window.__sentSocketMessages || []).filter((item) => item.type === 'block-battle-input' && item.actions?.includes('hardDrop'));
  const paintedCells = () => window.document.querySelectorAll('.block-battle-player.mine .block-battle-cell[class*="piece-"]').length;
  assert.equal(paintedCells(), 4, '시작 블록은 활성 블록 4칸만 보여야 한다');

  window.document.dispatchEvent(new window.KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: ' ', code: 'Space' }));
  window.document.dispatchEvent(new window.KeyboardEvent('keyup', { bubbles: true, key: ' ', code: 'Space' }));
  await waitFor(() => hardDropMessages().length === 1, '첫 번째 Space 하드드롭이 전송되지 않았습니다.');
  assert.equal(paintedCells(), 8, '첫 ACK 전에도 첫 블록 잠금과 다음 블록 4칸이 즉시 보여야 한다');

  window.document.dispatchEvent(new window.KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: ' ', code: 'Space' }));
  window.document.dispatchEvent(new window.KeyboardEvent('keyup', { bubbles: true, key: ' ', code: 'Space' }));
  window.document.dispatchEvent(new window.KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: ' ', code: 'Space', repeat: true }));
  window.document.dispatchEvent(new window.KeyboardEvent('keyup', { bubbles: true, key: ' ', code: 'Space' }));
  window.document.querySelector('[data-action="block-battle-control"][data-value="hardDrop"]').dispatchEvent(
    new window.MouseEvent('click', { bubbles: true, cancelable: true, detail: 0 })
  );
  await new Promise((resolve) => setTimeout(resolve, 25));
  assert.equal(hardDropMessages().length, 1, '첫 ACK 전에는 두 번째 입력을 버리지 말고 로컬 큐에 보존해야 한다');
  assert.equal(paintedCells(), 12, '두 번째 독립 Space는 다음 블록을 한 번 잠그고 세 번째 활성 블록을 보여야 한다');

  const firstMessage = hardDropMessages()[0];
  const firstProcessed = await responseJson(await room.fetch(authRequest(`/api/block-battle/rooms/${created.data.roomId}/input`, hostToken, {
    method: 'POST', body: JSON.stringify(firstMessage)
  })));
  const firstConfirmedRoom = firstProcessed.data.blockBattle.rooms.find((item) => item.id === created.data.roomId);
  window.__testSockets[0].emit({ type: 'block-battle-state', room: firstConfirmedRoom, serverTime: firstProcessed.data.blockBattle.serverTime });
  await waitFor(() => hardDropMessages().length === 2, '첫 ACK 뒤 보존된 두 번째 Space가 순서대로 전송되지 않았습니다.');

  const secondMessage = hardDropMessages()[1];
  assert.notEqual(secondMessage.requestId, firstMessage.requestId);
  assert.deepEqual(secondMessage.actions, ['hardDrop']);
  const secondProcessed = await responseJson(await room.fetch(authRequest(`/api/block-battle/rooms/${created.data.roomId}/input`, hostToken, {
    method: 'POST', body: JSON.stringify(secondMessage)
  })));
  const secondConfirmedRoom = secondProcessed.data.blockBattle.rooms.find((item) => item.id === created.data.roomId);
  window.__testSockets[0].emit({ type: 'block-battle-state', room: secondConfirmedRoom, serverTime: secondProcessed.data.blockBattle.serverTime });
  await new Promise((resolve) => setTimeout(resolve, 30));

  state = await room.store.load();
  assert.equal(Number(state.blockBattle.rooms[created.data.roomId].players[host.currentPetId].pieces || 0), initialPieces + 2, '서로 다른 두 블록이 서버에서도 정확히 한 번씩 잠겨야 한다');
  assert.equal(hardDropMessages().length, 2, 'repeat 또는 Space 직후 합성 click이 세 번째 하드드롭을 만들면 안 된다');
  assert.deepEqual(errorsToMessages(runtime.errors), []);
});

test('전직 사용자 상세 프로필은 API의 selected assetKey와 전직 이름을 그대로 렌더링한다', async (t) => {
  const { room } = await createRoom();
  const viewerToken = await register(room, '전직프로필관전자');
  await register(room, '전직프로필대상');
  const state = await room.store.load();
  const targetUser = Object.values(state.users).find((user) => user.nickname === '전직프로필대상');
  const targetPet = state.pets[targetUser.currentPetId];
  targetPet.stats.body = 8_750;
  targetPet.records.maxBody = 8_750;
  targetPet.bodyAdvancementUnlockedAt = '2026-09-02T12:00:00.000Z';
  targetPet.bodyAdvancement = { key: 'babyDino', selectedAt: '2026-09-02T12:01:00.000Z' };
  await room.store.save(state);

  const profileResponse = await responseJson(await room.fetch(authRequest(`/api/profiles/${targetPet.id}`, viewerToken)));
  assert.equal(profileResponse.data.profile.bodyAdvancement.selected.assetKey, 'form-baby-dino');
  assert.equal(profileResponse.data.profile.bodyAdvancement.selected.label, '아기공룡');

  const runtime = await createBrowser({ token: viewerToken, fetchImpl: (window) => roomFetch(window, room) });
  t.after(() => runtime.dom.window.close());
  const { window } = runtime;
  await waitFor(() => !window.document.querySelector('#app-shell')?.classList.contains('hidden'), '앱 화면이 열리지 않았습니다.');
  window.document.querySelector('.nav-item[data-tab="social"]').dispatchEvent(new window.PointerEvent('pointerdown', {
    bubbles: true, cancelable: true, pointerType: 'touch', pointerId: 72, isPrimary: true
  }));
  await waitFor(() => window.document.querySelector('#resident-toggle'), '주민 목록 버튼이 보이지 않습니다.');
  window.document.querySelector('#resident-toggle').click();
  await waitFor(() => window.document.querySelector(`[data-action="profile"][data-id="${targetPet.id}"]`), '전직 대상 주민 프로필이 보이지 않습니다.');
  window.document.querySelector(`[data-action="profile"][data-id="${targetPet.id}"]`).click();
  await waitFor(() => window.document.querySelector('.profile-detail .pet-avatar'), '상세 프로필이 열리지 않았습니다.');

  const avatar = window.document.querySelector('.profile-detail .pet-avatar');
  const detail = window.document.querySelector('.profile-detail').textContent;
  assert.equal(avatar.getAttribute('src'), '/pets/form-baby-dino.svg?v=6101250');
  assert.equal(avatar.getAttribute('alt'), '아기공룡');
  assert.match(detail, /아기공룡 · 몸집 8750/);
  assert.doesNotMatch(detail, /레비아탄레고 · 몸집 8750/);
  assert.deepEqual(errorsToMessages(runtime.errors), []);
});

test('블록게임은 첫 서버 응답이 지연돼도 빠른 두 클릭과 자동 bootstrap을 보존하고 서버에 순서대로 적용한다', async (t) => {
  const { room } = await createRoom();
  const token = await register(room, '블록연속클릭');
  const started = await responseJson(await room.fetch(authRequest('/api/minigames/start', token, {
    method: 'POST', body: JSON.stringify({ gameId: 'block' })
  })));
  const state = await room.store.load();
  const challenge = state.miniGameChallenges[started.data.challenge.id];
  challenge.blockBoard = Array.from({ length: 12 }, () => Array(10).fill(null));
  for (const [col, color] of [[0, 0], [1, 1], [2, 2]]) {
    challenge.blockBoard[10][col] = color;
    challenge.blockBoard[11][col] = color;
  }
  challenge.blockBoardVersion = 1;
  challenge.blockPendingPoints = 0;
  challenge.blockRemovedCount = 0;
  challenge.blockMoveCount = 0;
  challenge.blockProcessedRequestIds = [];
  await room.store.save(state);
  ...
  const runtime = await createBrowser({
    token,
    fetchImpl: (window) => {
      const base = roomFetch(window, room);
      return async (input, options = {}) => {
        const url = new URL(String(input), window.location.href);
        if (url.pathname === '/api/minigames/block/select') {
          selectBodies.push(JSON.parse(String(options.body || '{}')));
          if (selectBodies.length === 1) await firstGate;
        }
  ...
  await waitFor(() => !window.document.querySelector('#app-shell')?.classList.contains('hidden'), '앱 화면이 열리지 않았습니다.');
  window.document.querySelector('.nav-item[data-tab="games"]').dispatchEvent(new window.PointerEvent('pointerdown', {
    bubbles: true, cancelable: true, pointerType: 'touch', pointerId: 73, isPrimary: true
  }));
  await waitFor(() => window.document.querySelector('[data-action="resume-mini"]'), '블록게임 이어하기가 보이지 않습니다.');
  window.document.querySelector('[data-action="resume-mini"]').click();
  await waitFor(() => window.document.querySelector('#block-board')?.children.length === 120, '블록판이 열리지 않았습니다.');
  ...
  const modal = window.document.querySelector('#modal-content');
  board.querySelector('[data-row="11"][data-col="0"]').click();
  await waitFor(() => selectBodies.length === 1, '첫 블록 요청이 시작되지 않았습니다.');
  board.querySelector('[data-row="11"][data-col="1"]').click();
  assert.equal(selectBodies.length, 1, '두 번째 요청은 첫 응답 전에 서버로 병렬 전송하면 안 된다');
  assert.equal(window.document.querySelector('#block-remaining').textContent.trim(), '2개', '두 클릭은 서버 ACK 전에도 로컬 판에 즉시 반영되어야 한다');
  assert.equal(window.document.querySelector('#block-pending').textContent.trim(), '10P');

  window.__testSockets[0].emit({ type: 'refresh', reason: 'presence', at: Date.now() });
  await new Promise((resolve) => setTimeout(resolve, 260));
  assert.equal(window.document.querySelector('#modal-content'), modal, '미확정 블록 입력 중 bootstrap이 모달 전체를 교체하면 안 된다');
  assert.equal(window.document.querySelector('#block-board'), board, '미확정 블록 입력 중 bootstrap이 로컬 블록판 DOM을 교체하면 안 된다');
  assert.equal(window.document.querySelector('#block-remaining').textContent.trim(), '2개', '서버의 이전 판이 미확정 로컬 두 클릭을 덮으면 안 된다');

  releaseFirst();
  await waitFor(() => selectBodies.length === 2, '첫 응답 뒤 두 번째 블록 요청이 이어서 전송되지 않았습니다.');
  assert.deepEqual(selectBodies.map((item) => item.boardVersion), [1, 2]);
  assert.equal(new Set(selectBodies.map((item) => item.requestId)).size, 2, '각 블록 클릭은 서로 다른 requestId를 가져야 한다');
  await waitFor(async () => {
    const latest = await room.store.load();
    return latest.miniGameChallenges[challenge.id]?.blockMoveCount === 2;
  }, '빠른 두 클릭이 서버에 순서대로 적용되지 않았습니다.');
  assert.deepEqual(errorsToMessages(runtime.errors), []);
});

test('블록게임 서버 오류는 남은 낙관적 큐를 취소하고 최신 서버 판으로 한 번 교정한다', async (t) => {
  const { room } = await createRoom();
  const token = await register(room, '블록오류교정');
  const started = await responseJson(await room.fetch(authRequest('/api/minigames/start', token, {
    method: 'POST', body: JSON.stringify({ gameId: 'block' })
  })));
  const state = await room.store.load();
    ...
    fetchImpl: (window) => {
      ...
      return async (input, options = {}) => {
        ...
        if (url.pathname === '/api/minigames/block/select') {
          ...
          if (selectCalls === 1) {
            await failureGate;
            return new Response(JSON.stringify({ ok: false, message: '강제 블록 오류' }), {
  ...
  await waitFor(() => !window.document.querySelector('#app-shell')?.classList.contains('hidden'), '앱 화면이 열리지 않았습니다.');
  window.document.querySelector('.nav-item[data-tab="games"]').dispatchEvent(new window.PointerEvent('pointerdown', {
    bubbles: true, cancelable: true, pointerType: 'touch', pointerId: 74, isPrimary: true
  }));
  await waitFor(() => window.document.querySelector('[data-action="resume-mini"]'), '블록게임 이어하기가 보이지 않습니다.');
  window.document.querySelector('[data-action="resume-mini"]').click();
  await waitFor(() => window.document.querySelector('#block-board')?.children.length === 120, '블록판이 열리지 않았습니다.');
  ...
  const board = window.document.querySelector('#block-board');
  board.querySelector('[data-row="11"][data-col="0"]').click();
  await waitFor(() => window.document.querySelector('#block-remaining')?.textContent.trim() === '4개', '오류 전 첫 낙관적 제거가 즉시 표시되지 않았습니다.');
  board.querySelector('[data-row="11"][data-col="1"]').click();
  await waitFor(() => window.document.querySelector('#block-remaining')?.textContent.trim() === '2개', '첫 요청 대기 중 두 번째 낙관적 입력이 로컬 판에 반영되지 않았습니다.');
  assert.equal(selectCalls, 1, '첫 응답 전 두 번째 입력을 병렬 서버 전송하면 안 된다');

  releaseFailure();
  await waitFor(() => window.document.querySelector('#block-remaining')?.textContent.trim() === '6개', '서버 오류 뒤 authoritative 판으로 교정되지 않았습니다.');
  assert.equal(selectCalls, 1, '첫 요청 실패 뒤 남은 낙관적 큐는 서버로 보내지 않고 폐기해야 한다');
  assert.equal(window.document.querySelectorAll('.toast.error').length, 1, '같은 교정 오류 토스트가 연속 도배되면 안 된다');
  assert.ok(window.document.querySelector('#block-board [data-row="11"][data-col="0"]'), '서버에서 제거되지 않은 첫 그룹이 다시 보여야 한다');
  assert.ok(window.document.querySelector('#block-board [data-row="11"][data-col="1"]'), '취소된 두 번째 낙관적 그룹도 authoritative 판에서 다시 보여야 한다');

  window.document.querySelector('#block-board [data-row="11"][data-col="1"]').click();
  await waitFor(async () => {
    const latest = await room.store.load();
    return latest.miniGameChallenges[challenge.id]?.blockMoveCount === 1;
  }, '오류 교정 후 새 입력을 다시 처리하지 못했습니다.');
  assert.equal(selectCalls, 2);
  assert.deepEqual(errorsToMessages(runtime.errors), []);
});

test('운영자 자동 bootstrap은 스크롤·후원 문구·체크박스·포커스·커서를 보존하며 목록만 갱신한다', async (t) => {
  const created = await createRoom();
  const token = await register(created.room, '자동운영자');
  const state = await created.room.store.load();
  const adminUser = Object.values(state.users).find((user) => user.nickname === '자동운영자');
  created.room.env.ADMIN_USER_IDS = adminUser.id;
  const runtime = await createBrowser({ token, fetchImpl: (window) => roomFetch(window, created.room) });
  t.after(() => runtime.dom.window.close());
  const { window } = runtime;
  await waitFor(() => !window.document.querySelector('#app-shell')?.classList.contains('hidden'), '앱 화면이 열리지 않았습니다.');
  window.document.querySelector('.nav-item[data-tab="records"]').dispatchEvent(new window.PointerEvent('pointerdown', {
    bubbles: true, cancelable: true, pointerType: 'touch', pointerId: 75, isPrimary: true
  }));
  await waitFor(() => window.document.querySelector('[data-action="open-admin"]'), '운영자 버튼이 보이지 않습니다.');
  window.document.querySelector('[data-action="open-admin"]').click();
  await waitFor(() => window.document.querySelector('#admin-support-message'), '운영자 창이 열리지 않았습니다.');

  const modal = window.document.querySelector('#modal-content');
  const list = window.document.querySelector('#admin-member-list');
  const textarea = window.document.querySelector('#admin-support-message');
  const toggle = window.document.querySelector('#admin-support-form input[name="enabled"]');
  textarea.value = '저장 전 작성 중인 후원 문구는 자동 갱신으로 사라지면 안 됩니다.';
  textarea.focus();
  textarea.setSelectionRange(7, 14);
  toggle.checked = !toggle.checked;
  const editedChecked = toggle.checked;
  modal.scrollTop = 219;

  await register(created.room, '자동갱신신규회원');
  window.__testSockets[0].emit({ type: 'refresh', reason: 'presence', at: Date.now() });
  await new Promise((resolve) => setTimeout(resolve, 280));

  assert.equal(window.document.querySelector('#modal-content'), modal);
  assert.equal(window.document.querySelector('#admin-member-list'), list, '자동 갱신은 회원 목록 컨테이너 자체를 교체하지 않아야 한다');
  assert.match(list.textContent, /자동갱신신규회원레고/);
  assert.equal(window.document.querySelector('#admin-support-message'), textarea, '자동 갱신은 후원 textarea DOM을 교체하면 안 된다');
  assert.equal(textarea.value, '저장 전 작성 중인 후원 문구는 자동 갱신으로 사라지면 안 됩니다.');
  assert.equal(toggle.checked, editedChecked);
  ...
  assert.equal(modal.scrollTop, 219);

  window.document.querySelector('[data-action="admin-refresh"]').click();
  await waitFor(() => window.document.querySelector('[data-action="admin-refresh"]')?.textContent.includes('완료'), '운영자 작업 응답이 완료되지 않았습니다.');
  assert.equal(modal.scrollTop, 219, '운영자 작업 응답 뒤에도 보던 스크롤 위치를 유지해야 한다');
  assert.equal(textarea.value, '저장 전 작성 중인 후원 문구는 자동 갱신으로 사라지면 안 됩니다.');
  assert.deepEqual(errorsToMessages(runtime.errors), []);
});
