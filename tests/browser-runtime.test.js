...
test('게임 화면은 라이어게임 대신 블럭대전 3방 로비를 표시한다', async (t) => {
  const { room } = await createRoom();
  ...
  await waitFor(() => window.document.querySelector('[data-action="block-battle-create"]'), '블럭대전 방 만들기 버튼이 보이지 않습니다.');
  ...
  await waitFor(() => window.document.querySelector('#block-battle-create-form'), '블럭대전 방 만들기 창이 열리지 않았습니다.');
  window.document.querySelector('#block-battle-create-form').requestSubmit();
  await waitFor(() => window.document.querySelector('[data-action="block-battle-back"]'), '생성한 블럭대전 방 화면이 열리지 않았습니다.');
...
test('블록게임은 문양·공용 empty 클래스 없이 색상만 표시하고 그만하기를 정산한다', async (t) => {
  ...
  assert.ok(stop);
  assert.match(stop.textContent, /5P/);
  stop.click();
