...
test('모바일 숫자맞히기는 오답 제출 후에도 입력창·초점·모달 스크롤을 그대로 유지한다', async (t) => {
  ...
  window.__testSockets[0].emit({ type: 'refresh', reason: 'presence', at: Date.now() });
  await new Promise((resolve) => setTimeout(resolve, 260));
  assert.equal(window.document.querySelector('#number-game-form input[name="guess"]'), input, '실시간 갱신 때 숫자 입력 DOM을 교체하면 안 된다');
  assert.equal(input.value, '37', '실시간 갱신 때문에 작성 중 숫자가 지워지면 안 된다');
