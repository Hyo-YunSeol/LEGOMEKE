...
test('레고도쿠 확정 사양은 8×8·3분·5실수·3방이며 판돈 규칙은 기존 대전과 같다', () => {
...
test('두 플레이어는 같은 문제를 받고 상대 선수에게는 위치를 숨기되 관전자는 양쪽 현재 판을 볼 수 있다', () => {
  ...
  assert.deepEqual(spectatorView.players[ps[0].id].confirmed, [correctIndex]);
  assert.equal(guestView.players[ps[0].id].foundCount, 1);
  assert.equal(spectatorView.players[ps[0].id].foundCount, 1);
});

test('오답은 실수를 올리고 5번째 오답에서 즉시 패배·판돈 정산하며 중복 actionId는 다시 처리하지 않는다', () => {
  ...
  for (let index = 1; index < 4; index += 1) {
    const result = playLegodokuCell(state, ps[0], roomId, { matchId: room.matchId, index: wrong[index], actionId: `wrong-${index + 1}` }, at(2 + index));
    assert.equal(result.finished, undefined);
  }
  const fifth = playLegodokuCell(state, ps[0], roomId, { matchId: room.matchId, index: wrong[4], actionId: 'wrong-5' }, at(6));
  assert.equal(fifth.finished, true);
  assert.equal(fifth.correct, false);
  assert.match(fifth.message, /실수 5회/);
