...
test('다빈치 랭킹은 승수와 정답을 표시하고 공통 3일 시즌 영역에 들어간다', () => {
  ...
  assert.match(workerSource, /rankings: \{ \.\.\.rankingsView\(state, pet\.id\), blockBattle: blockBattleRankings\(state, pet\.id\), sichuan: sichuanRanking\(state, pet\.id\), davinci: davinciRanking\(state, pet\.id\) \}/);
});
