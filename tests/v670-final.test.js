...
test('플렉스 8종과 기존 상점 유지권의 최종 가격이 정확하다', () => {
  ...
  assert.equal(SHOP_ITEMS.staminaHour.price, 300);
  assert.equal(SHOP_ITEMS.hungerHour.price, 500);
});
...
test('플렉스 구매는 24시간 장착·교체·멱등 구매 방지·만료 정리를 보장한다', () => {
  ...
  assert.equal(pet.stats.points, 7_500);
...
test('플렉스 SVG 8종과 모바일 미니게임 2×2 레이아웃이 배포 자산에 포함된다', async () => {
