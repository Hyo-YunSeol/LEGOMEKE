...
test('v6.9.0 배포 버전과 캐시는 690으로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=69201/);
  assert.match(html, /app\.js\?v=69201/);
  assert.match(app, /\/sw\.js\?v=69201/);
  assert.match(sw, /lego-life-v69201-minesweeper-input-final/);
  assert.match(sw, /VERSION = '69201'/);
  assert.match(worker, /APP_VERSION = '6\.9\.2-minesweeper-input-final'/);
  assert.equal(JSON.parse(pkg).version, '6.9.2');
});
