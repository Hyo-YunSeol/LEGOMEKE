...
test('v6.9.0 배포 버전과 캐시는 690으로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=610118/);
  assert.match(html, /app\.js\?v=610118/);
  assert.match(app, /\/sw\.js\?v=610118/);
  assert.match(sw, /lego-life-v610118-spotdiff-combo-apple-stable-final/);
  assert.match(sw, /VERSION = '610118'/);
  assert.match(worker, /APP_VERSION = '6\.10\.18-spotdiff-combo-apple-stable-final'/);
