...
test('v6.9.0 배포 버전과 캐시는 690으로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=69001/);
  assert.match(html, /app\.js\?v=69001/);
  assert.match(app, /\/sw\.js\?v=69001/);
  assert.match(sw, /lego-life-v69001-final/);
  assert.match(sw, /VERSION = '69001'/);
  assert.match(worker, /APP_VERSION = '6\.9\.0-final'/);
  assert.equal(JSON.parse(pkg).version, '6.9.0');
});
