...
test('v6.9.0 배포 버전과 캐시는 690으로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=610114/);
  assert.match(html, /app\.js\?v=610114/);
  assert.match(app, /\/sw\.js\?v=610114/);
  assert.match(sw, /lego-life-v610114-spotdiff-sichuan-myth-final/);
  assert.match(sw, /VERSION = '610114'/);
  assert.match(worker, /APP_VERSION = '6\.10\.14-spotdiff-sichuan-myth-final'/);
  assert.equal(JSON.parse(pkg).version, '6.10.14');
});
