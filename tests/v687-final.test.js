...
test('v6.8.9 진행 중 실시간 게임은 최초·같은 게임 탭 pointerdown 안에서 즉시 렌더링한다', async () => {
...
test('v6.8.9 테트리스 시작 직후 null 공격 상태와 부분 보드는 안전 처리한다', async () => {
  const source = await readFile(APP, 'utf8');
...
test('v6.8.9 배포 버전과 캐시는 689로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=68945/);
  assert.match(html, /app\.js\?v=68945/);
  assert.match(app, /\/sw\.js\?v=68945/);
  assert.match(sw, /lego-life-v68945-final/);
  assert.match(sw, /VERSION = '68945'/);
  assert.match(worker, /APP_VERSION = '6\.8\.9-final'/);
  assert.equal(JSON.parse(pkg).version, '6.8.9');
});
