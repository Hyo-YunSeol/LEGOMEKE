...
test('v6.8.7 진행 중 실시간 게임은 게임 탭 pointerdown 안에서 즉시 렌더링한다', async () => {
  const source = await readFile(APP, 'utf8');
  assert.match(source, /const realtimeGameOpen = tabName === 'games' && Boolean\(currentOmokRoom\(\) \|\| currentBlockBattleRoom\(\)\)/);
...
test('v6.8.7 테트리스 시작 직후 null 공격 상태와 부분 보드는 안전 처리한다', async () => {
  const source = await readFile(APP, 'utf8');
...
test('v6.8.7 배포 버전과 캐시는 687로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=687/);
  assert.match(html, /app\.js\?v=687/);
  assert.match(app, /\/sw\.js\?v=687/);
  assert.match(sw, /lego-life-v687-final/);
  assert.match(sw, /VERSION = '687'/);
  assert.match(worker, /APP_VERSION = '6\.8\.7-final'/);
  assert.equal(JSON.parse(pkg).version, '6.8.7');
});
