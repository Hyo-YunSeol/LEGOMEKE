...
test('v6.8.8 진행 중 실시간 게임은 최초·같은 게임 탭 pointerdown 안에서 즉시 렌더링한다', async () => {
...
test('v6.8.8 테트리스 시작 직후 null 공격 상태와 부분 보드는 안전 처리한다', async () => {
  const source = await readFile(APP, 'utf8');
...
test('v6.8.8 배포 버전과 캐시는 688로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=688/);
  assert.match(html, /app\.js\?v=688/);
  assert.match(app, /\/sw\.js\?v=688/);
  assert.match(sw, /lego-life-v688-final/);
  assert.match(sw, /VERSION = '688'/);
  assert.match(worker, /APP_VERSION = '6\.8\.8-final'/);
  assert.equal(JSON.parse(pkg).version, '6.8.8');
});
