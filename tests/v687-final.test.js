...
test('v6.9.0 배포 버전과 캐시는 690으로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=610117/);
  assert.match(html, /app\.js\?v=610117/);
  assert.match(app, /\/sw\.js\?v=610117/);
  assert.match(sw, /lego-life-v610117-spotdiff-adult-atlas-final/);
  assert.match(sw, /VERSION = '610117'/);
  assert.match(worker, /APP_VERSION = '6\.10\.17-spotdiff-adult-atlas-final'/);
