...
test('v6.9.0 배포 버전과 캐시는 690으로 동기화된다', async () => {
  ...
  assert.match(html, /styles\.css\?v=69401/);
  assert.match(html, /app\.js\?v=69401/);
  assert.match(app, /\/sw\.js\?v=69401/);
  assert.match(sw, /lego-life-v69401-game-repair-final/);
  assert.match(sw, /VERSION = '69401'/);
  assert.match(worker, /APP_VERSION = '6\.9\.4-game-repair-final'/);
  assert.equal(JSON.parse(pkg).version, '6.9.4');
});
