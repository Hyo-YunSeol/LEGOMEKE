...
test('1대1 테트리스는 lock 결과를 서버에서만 확정하고 NEXT와 큰 보드를 유지한다', async () => {
  const [app, css] = await Promise.all([readFile(appUrl, 'utf8'), readFile(cssUrl, 'utf8')]);
  assert.match(app, /function blockBattleVisualSelf\(/);
  assert.match(app, /lock\/줄삭제\/다음 블록 생성은 서버 ACK에서만 확정한다/);
  ...
  assert.doesNotMatch(queue, /previewBlockBattleLock\(/);
