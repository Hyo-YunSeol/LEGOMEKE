...
test('모바일 오목 수정본은 app·CSS·서비스워커 캐시를 v648으로 강제 갱신한다', async () => {
  const sw = await readFile(new URL('../public/sw.js', import.meta.url), 'utf8');
  assert.match(html, /styles\.css\?v=647/);
  assert.match(html, /app\.js\?v=647/);
  assert.match(app, /sw\.js\?v=647/);
  assert.match(sw, /lego-life-v648-final/);
  assert.match(sw, /styles\.css\?v=647/);
  assert.match(sw, /app\.js\?v=647/);
});
...
test('영토전 UI는 25칸 포화 후 전면전과 본진 탈취 가능 상태를 명확히 안내한다', () => {
  assert.match(app, /battleUnlocked/);
  assert.match(app, /전면전 진행 중 · 본진 탈취 가능/);
  assert.match(app, /16\/25칸이 모두 차면 이후 탈취 가능/);
});

test('돼지·공룡 체형은 assetKey 이미지와 깨짐 방지 fallback을 사용하고 긴 단계명이 모바일에서 넘치지 않는다', () => {
  assert.match(app, /stage\.assetKey \|\| stage\.key/);
  assert.match(app, /onerror=\"this\.onerror=null;this\.src='data:image\/svg\+xml;base64/);
...
test('오목·라이어 관전자는 5종 리액션을 보낼 수 있고 플레이어 화면에도 리액션 표시 영역이 있다', () => {
  for (const text of ['웃겨요', '슬퍼요', '화나요', '졸려요', '짜쳐요']) assert.match(app, new RegExp(text));
  for (const emoji of ['😂', '😢', '😡', '😴', '🥵']) assert.match(app, new RegExp(emoji));
  assert.match(app, /data-action="\$\{scope\}-reaction"/);
  assert.match(app, /action === 'liar-reaction'/);
  assert.match(app, /action === 'omok-reaction'/);
  assert.match(app, /spectatorReactionBar\('liar', null, game\.reactions \|\| \[\], false\)/);
  assert.match(app, /spectatorReactionBar\('omok', room\.id, room\.reactions \|\| \[\], false\)/);
});
