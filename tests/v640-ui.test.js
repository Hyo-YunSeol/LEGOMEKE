...
test('모바일 오목 수정본은 app·CSS·서비스워커 캐시를 v644로 강제 갱신한다', async () => {
  const sw = await readFile(new URL('../public/sw.js', import.meta.url), 'utf8');
  assert.match(html, /styles\.css\?v=644/);
  assert.match(html, /app\.js\?v=644/);
  assert.match(app, /sw\.js\?v=644/);
  assert.match(sw, /lego-life-v644-final/);
  assert.match(sw, /styles\.css\?v=644/);
  assert.match(sw, /app\.js\?v=644/);
});
