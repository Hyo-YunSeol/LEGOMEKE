...
test('요리하기는 생활 행동 1회를 사용해 포만감을 최대 50 회복하고 몸집·체력을 바꾸지 않는다', () => {
  const pet = petAt();
  pet.stats.hunger = 30;
  pet.stats.body = 500;
  ...
  assert.equal(pet.stats.stamina, 77);
...
test('해양 초거대 생물 6단계는 아르헨티노사우루스 뒤에 연속 범위로 배치되고 SVG가 존재한다', async () => {
  const tail = BODY_STAGES.slice(-7);
  assert.deepEqual(tail.map((s) => [s.min, Number.isFinite(s.max) ? s.max : null, s.label]), [
  ...
  for (const stage of tail.slice(1)) {
...
test('요리하기 API와 구버전 exercise 경로 모두 새 요리하기 규칙으로 처리된다', async () => {
  ...
  let response = await room.fetch(authRequest('/api/minigames/start', token, { method: 'POST', body: JSON.stringify({ gameId: 'reaction' }) }));
  ...
  response = await legacyRoom.fetch(authRequest('/api/minigames/start', legacyToken, { method: 'POST', body: JSON.stringify({ gameId: 'reaction' }) }));
  assert.equal(response.status, 200);
...
test('지뢰찾기 클라이언트 큐는 공개되지 않는 challenge.board를 검사하지 않는다', async () => {
  ...
  assert.match(app, /navigator\.vibrate\(25\)/);
});

test('화면 문구는 포만감으로 통일되고 헬스 대신 요리하기를 노출한다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(app, /<strong>요리하기<\/strong><small>포만감 \+50/);
