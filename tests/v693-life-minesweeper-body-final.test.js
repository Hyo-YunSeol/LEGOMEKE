...
import { BODY_STAGES, BODY_ADVANCEMENTS, HUNGER_PENALTY_POINTS_PER_HOUR, HUNGER_PENALTY_MAX_HOURS, ODD_EVEN_PAYOUT_PERCENT } from '../src/game/constants.js';
...
test('요리하기는 생활 행동 1회를 사용해 포만감을 최대 50 회복하고 체력 5를 사용하며 몸집은 바꾸지 않는다', () => {
  const pet = petAt();
  pet.stats.hunger = 30;
  pet.stats.body = 500;
...
test('체력이 5 미만이면 요리하기는 행동 횟수와 포만감을 소비하지 않는다', () => {
  ...
  assert.equal(pet.stats.stamina, 4);
...
test('해양편은 보존되고 레비아탄 이후에는 신화 체형 대신 전직 11종을 사용한다', async () => {
  ...
  for (const advancement of Object.values(BODY_ADVANCEMENTS)) {
    const svg = await readFile(new URL(`../public/pets/${advancement.assetKey}.svg`, import.meta.url), 'utf8');
...
test('화면 문구는 포만감으로 통일되고 헬스 대신 요리하기를 노출한다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(app, /<strong>요리하기<\/strong><small>포만감 \+50 · 체력 -5/);
