...
test('확정 경제 수치·낚시 목록·판돈·랭킹 보상이 서버 상수에 반영된다', () => {
  ...
  assert.equal(SHOP_ITEMS.miniGame10.price, 1_000);
  assert.equal(SHOP_ITEMS.fishing5.price, 300);
...
test('미니게임·낚시 추가 이용권은 게임 하루에 각각 한 번만 구매된다', () => {
  const { state, user, pet } = stateWithPet(5_000);
  const mini = purchaseShopItem(state, user, pet, 'miniGame10', {}, 'ticket-mini-0001', BASE);
  assert.equal(mini.ok, true);
  assert.equal(mini.price, 1_000);
  assert.equal(pet.daily.miniGameBonus, 10);
  assert.equal(pet.daily.miniGameTicketBought, true);
  const miniAgain = purchaseShopItem(state, user, pet, 'miniGame10', {}, 'ticket-mini-0002', BASE);
  assert.equal(miniAgain.ok, false);
  assert.match(miniAgain.message, /1회/);
  ...
  const pointsAfterTickets = pet.stats.points;
  const fishingAgain = purchaseShopItem(state, user, pet, 'fishing5', {}, 'ticket-fish-0002', BASE);
  assert.equal(fishingAgain.ok, false);
  assert.equal(pet.stats.points, pointsAfterTickets, '거절된 중복 구매는 포인트를 쓰지 않는다');

  applyDailyReset(pet, new Date(BASE.getTime() + 6 * 60 * 60_000));
  assert.equal(pet.daily.miniGameTicketBought, false);
  assert.equal(pet.daily.fishingTicketBought, false);
...
test('블록게임은 12x10·5색 판, 상하좌우 그룹, 중력과 빈 열 이동을 적용한다', () => {
  ...
  assert.equal(collapsed[11][0], 1, '빈 첫 열을 없애고 다음 열을 왼쪽으로 붙인다');
  assert.equal(collapsed[11][1], 1, '열 안의 블록은 아래로 내려온다');
  assert.equal(collapsed[10][1], null);
});
