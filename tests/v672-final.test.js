...
test('블록게임 완주형 생성기는 기존 블록별 포인트표와 ALL CLEAR +100P를 그대로 사용한다', () => {
  ...
  for (let seed = 1; seed <= 150; seed += 1) {
    ...
    let points = 0;
    while (blockBoardStats(board).remainingBlocks > 0) {
      const groups = groupsOn(board);
      assert.ok(groups.length > 0, `seed ${seed}: 블록이 남았는데 제거 가능한 그룹이 없어서는 안 된다`);
      const group = groups[Math.floor(random() * groups.length)];
      const reward = blockRewardForSize(group.length);
      assert.equal(reward, blockRewardForSize(group.length), '기존 크기별 보상표를 그대로 사용해야 한다');
      for (const [row, col] of group) board[row][col] = null;
      points += reward;
      board = collapseBlockBoard(board);
    }
    assert.equal(blockBoardStats(board).remainingBlocks, 0);
    points += BLOCK_ALL_CLEAR_BONUS;
    assert.ok(points >= BLOCK_ALL_CLEAR_BONUS, '완주 시 기존 ALL CLEAR +100P가 포함되어야 한다');
  }
});
