...
test('확정 경제 수치·낚시 목록·판돈·랭킹 보상이 서버 상수에 반영된다', () => {
  ...
  assert.equal(SHOP_ITEMS.miniGame20.price, 4_000);
...
test('미니게임·낚시 추가 이용권은 하루 1회 제한 없이 구매할 때마다 누적된다', () => {
  ...
  const mini = purchaseShopItem(state, user, pet, 'miniGame20', {}, 'ticket-mini-0001', BASE);
  assert.equal(mini.ok, true);
  assert.equal(mini.price, 4_000);
  assert.equal(pet.daily.miniGameBonus, 20);
  const miniAgain = purchaseShopItem(state, user, pet, 'miniGame20', {}, 'ticket-mini-0002', BASE);
  assert.equal(miniAgain.ok, true);
  assert.equal(pet.daily.miniGameBonus, 40);
  ...
  assert.equal(pet.stats.points, 1_000);
...
test('블록판은 12×10·5색 일반 랜덤을 그대로 사용하고 완주 보정 생성기를 거치지 않는다', () => {
  for (let seed = 1; seed <= 200; seed += 1) {
    let a = seed >>> 0;
    let b = seed >>> 0;
    const randomA = () => ((a = (a * 1664525 + 1013904223) >>> 0) / 0x1_0000_0000);
    const randomB = () => ((b = (b * 1664525 + 1013904223) >>> 0) / 0x1_0000_0000);
    const playable = createPlayableBlockBoard(randomA);
    const rawRandom = createBlockBoard(randomB);
    assert.deepEqual(playable, rawRandom, `seed ${seed}: playable 판은 일반 랜덤판과 같아야 한다`);
    assert.equal(playable.length, BLOCK_ROWS);
    assert.equal(playable.every((row) => row.length === BLOCK_COLUMNS), true);
    assert.equal(playable.flat().every((cell) => Number.isInteger(cell) && cell >= 0 && cell < 5), true);
  }
  assert.equal(BLOCK_MIN_STARTING_GROUPS, 0);
  assert.equal(BLOCK_MIN_STARTING_REMOVABLE_CELLS, 0);
});

test('블록게임은 대부분 미완주로 끝나며 기존 블록별 포인트표와 희귀 ALL CLEAR +100P만 유지한다', () => {
  ...
  for (let seed = 1; seed <= 300; seed += 1) {
    ...
    if (blockBoardStats(board).remainingBlocks === 0) { allClears += 1; points += BLOCK_ALL_CLEAR_BONUS; }
    totalPoints += points;
  }
  assert.ok(allClears <= 15, `ALL CLEAR는 드물어야 한다. ${allClears}/300`);
  const average = totalPoints / 300;
  assert.ok(average >= 240 && average <= 330, `평균 보상은 대략 250~300P 중심이어야 한다. average=${average}`);
});
...
test('블록 선택은 판 버전·요청 ID를 검사하고 ALL CLEAR를 한 번만 정산한다', () => {
  ...
  assert.equal(duplicate.finished, true);
  assert.equal(duplicate.replayed, true);
  assert.equal(duplicate.reward, cleared.reward);
  assert.equal(pet.stats.points, 105);
  assert.equal(pet.daily.miniGamesPlayed, 1);
});
