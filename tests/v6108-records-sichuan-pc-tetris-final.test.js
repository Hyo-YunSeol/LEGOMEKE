...
test('왕 경력은 시즌 로그를 쌓지 않고 7개 고정 boolean만 유지한다', () => {
...
test('레고의 기록은 생활 기록 중심이며 최고레벨·최대영토·승패 전적을 표시하지 않는다', () => {
  ...
  assert.doesNotMatch(source, /최고 레벨|최대 영토|총 경기|총 게임|승률|승\s*\/\s*패|승패 전적|테트리스 총|다빈치 총|개인게임 총/);
});

test('PC 1대1 테트리스는 GPU translate 낙하 대신 left/top 직접 갱신 경로를 사용한다', () => {
  assert.match(app, /matchMedia\?\.\('\(hover: hover\) and \(pointer: fine\)'\)/);
  assert.match(app, /desktop\s*\?\s*`left:\$\{cell\.col \* 10\}%;top:\$\{cell\.row \* 5\}%;transform:none`/);
  assert.match(app, /const desktop = layer\.dataset\.paintMode === 'desktop';/);
  assert.match(app, /if \(node\.style\.left !== left\) node\.style\.left = left;/);
  assert.match(app, /if \(node\.style\.top !== top\) node\.style\.top = top;/);
  assert.match(css, /\.block-battle-active-layer\.desktop-paint\s*\{[^}]*transform:none;[^}]*backface-visibility:visible/s);
  assert.match(css, /\.block-battle-active-layer\.desktop-paint \.block-battle-active-cell\s*\{[^}]*will-change:auto/s);
});

test('모바일 1대1 테트리스 GPU 경로는 유지하고 싱글 테트리스는 변경 셀만 다시 칠한다', () => {
  assert.match(app, /translate3d\(\$\{cell\.col \* 100\}%,\$\{cell\.row \* 100\}%,0\)/);
  assert.match(css, /\.block-battle-active-layer\s*\{[^}]*transform:translateZ\(0\)/s);
