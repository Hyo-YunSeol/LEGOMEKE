...
test('외부 Worker 상태 확인과 정적 파일 보안 헤더가 v6.4.4로 동작한다', async () => {
  let response = await worker.fetch(new Request('https://game.test/healthz'), {});
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), { ok: true, version: '6.4.4-final', platform: 'Cloudflare Workers Free' });
...
test('이전 버전에서 진행 중이던 홀짝은 업데이트 시 판돈을 한 번 환불하고 종료한다', async () => {
  ...
  assert.equal(persistedState.meta.version, 16, '마이그레이션 결과를 Durable Object 저장소에 즉시 저장해야 한다');
