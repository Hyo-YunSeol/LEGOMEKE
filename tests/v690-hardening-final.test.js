...
test('v6.9.0 경고 시스템은 과거 저장 데이터·알림·감사로그까지 마이그레이션에서 제거된다', async () => {
  ...
  assert.equal(loaded.meta.version, 25);
});
...
test('매칭·교미 신청은 보낸 사람이 취소할 수 있고 24시간 후 자동 만료된다', () => {
  const state = stateWithUsers([['a', 'A'], ['b', 'B']], BASE);
  const [actor, target] = pets(state);
  socialAction(state, actor, target, 'requestMatch', {}, BASE);
  const matchRequest = Object.values(state.requests)[0];
  const cancelled = socialAction(state, actor, undefined, 'cancelRequest', { requestId: matchRequest.id }, new Date(BASE.getTime() + 1000));
  assert.equal(cancelled.ok, true);
  assert.equal(state.requests[matchRequest.id].status, 'cancelled');

  socialAction(state, actor, target, 'requestMating', {}, new Date(BASE.getTime() + 2000));
  const matingRequest = Object.values(state.requests).find((request) => request.type === 'mating');
  const expired = expireSocialRequests(state, new Date(BASE.getTime() + DAY + 3000));
  assert.equal(expired.changed, true);
  assert.equal(state.requests[matingRequest.id].status, 'expired');
});
...
test('개인게임은 정상 시작 순간 1회를 사용하고 만료·포기로 일일 40회 제한을 우회할 수 없다', () => {
  ...
  const blocked = startMiniGame(state, pet, 'number', new Date(BASE.getTime() + 3000));
  assert.equal(blocked.ok, false);
  assert.match(blocked.message, /40회/);
});

test('번개반응 랭킹·보상은 조작 가능한 클라이언트 100ms가 아니라 서버 실제 수신시간을 사용한다', () => {
  const state = stateWithUsers([['u1', '번개']], BASE);
  const pet = pets(state)[0];
  const started = startMiniGame(state, pet, 'reaction', BASE);
  const challenge = state.miniGameChallenges[started.challenge.id];
  challenge.readyAt = BASE.getTime() + 1000;
  const result = finishMiniGame(state, pet, challenge.id, 1, new Date(challenge.readyAt + 2000), {
    clientReactionMs: 100,
    clientClickedAt: challenge.readyAt + 100
  });
  assert.equal(result.ok, true);
  assert.equal(result.reactionMs, 2000);
  assert.equal(result.reward, 5);
  assert.equal(pet.records.bestReactionMs, 2000);
});
...
test('정상 저장 백업이 있으면 현재 JSON 손상 시 빈 게임 초기화 대신 마지막 정상 상태로 복구한다', async () => {
  ...
  assert.equal(recovered.meta.version, 25);
