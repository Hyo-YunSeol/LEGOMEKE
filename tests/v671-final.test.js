...
test('저장 버전 21은 로드할 때 매번 불필요하게 다시 저장하지 않는다', async () => {
...
test('라이어 채팅 전용 저장은 전체 상태 청크를 덮어쓰지 않고 재시작 뒤에도 복구된다', async () => {
  const shared = new Map();
  const storage = new MemoryStorage(shared);
  const store = new DurableJsonStore(storage);
  const state = stateWithUsers([['chat-user', '채팅회원']], BASE);
  const [pet] = petsFor(state);
  state.liarGame = initialLiarGame();
  liarJoin(state, pet, BASE);
  await store.save(state);
  const manifestBefore = structuredClone(shared.get('state-manifest'));
  const chunkBefore = shared.get('state-chunk-0');

  const sent = liarAddChat(state, pet, '입력창을 지키는 채팅', 'chat-durable-001', new Date(BASE.getTime() + 1_000));
  assert.equal(sent.ok, true);
  await store.saveLiarChat(state);
  assert.deepEqual(shared.get('state-manifest'), manifestBefore);
  assert.equal(shared.get('state-chunk-0'), chunkBefore);

  const restored = await new DurableJsonStore(new MemoryStorage(shared)).load();
  assert.equal(restored.liarGame.messages.some((message) => message.id === sent.chat.id), true);
  assert.equal(restored.liarGame.chatOperations['chat-durable-001'].chatId, sent.chat.id);
});

test('라이어 채팅 API는 요청 ID를 멱등 처리하고 전체 bootstrap 없이 전용 상태로 영속화한다', async () => {
  const shared = new Map();
  const first = await createRoom(shared);
  const token = await register(first.room, '채팅API');
  const joined = await responseJson(await first.room.fetch(authRequest('/api/liar/join', token, {
    method: 'POST', body: JSON.stringify({})
  })));
  assert.equal(joined.response.status, 200);
  const body = JSON.stringify({ text: '한 번만 저장', requestId: 'chat-api-once-001' });
  const sent = await responseJson(await first.room.fetch(authRequest('/api/liar/chat', token, { method: 'POST', body })));
  const retried = await responseJson(await first.room.fetch(authRequest('/api/liar/chat', token, { method: 'POST', body })));
  assert.equal(sent.response.status, 200);
  assert.equal(sent.data.ok, true);
  assert.equal('bootstrap' in sent.data, false);
  assert.equal(retried.data.duplicate, true);
  assert.equal(retried.data.chat.id, sent.data.chat.id);

  const second = await createRoom(shared);
  const restored = await second.room.store.load();
  assert.equal(restored.liarGame.messages.filter((message) => message.id === sent.data.chat.id).length, 1);
});
...
test('운영자 종료방 비우기 API는 권한을 확인하고 정산된 방만 제거한다', async () => {
  ...
  const ended = createOmokRoom(state, adminPet, 100, BASE);
  joinOmokRoom(state, opponentPet, ended.roomId, BASE);
  leaveOmokRoom(state, opponentPet, ended.roomId, BASE);
...
test('운영자 상태 초기화는 진행 중 라이어게임을 먼저 종료하고 전원 판돈을 환불한다', () => {
  const state = stateWithUsers([['l1', '초기화1'], ['l2', '초기화2'], ['l3', '초기화3']], BASE);
  const pets = petsFor(state);
  for (const pet of pets) pet.stats.points = 1_000;
  state.liarGame = initialLiarGame();
  ...
  assert.deepEqual(pets.map((pet) => pet.stats.points), [990, 990, 990]);

  const cleanup = resetLiarParticipantForAdmin(state, pets[1].id, BASE);
  assert.equal(cleanup.forceEnded, true);
  assert.equal(state.liarGame.phase, 'waiting');
  assert.equal(state.liarGame.players[pets[1].id], undefined);
  assert.deepEqual(pets.map((pet) => pet.stats.points), [1_000, 1_000, 1_000]);
});
...
test('프런트 회귀 방어는 채팅 부분 갱신·IME 보호·모바일 사과 모달·관리자 부분 새로고침을 포함한다', async () => {
  const [app, css, worker] = await Promise.all([
    readFile(new URL('../public/app.js', import.meta.url), 'utf8'),
    readFile(new URL('../public/styles.css', import.meta.url), 'utf8'),
    readFile(new URL('../src/worker.js', import.meta.url), 'utf8')
  ]);
  assert.match(app, /function refreshLiarChatOnly/);
  assert.match(app, /liarComposerIsActive\(\)/);
  assert.match(app, /compositionstart/);
  assert.match(app, /liar-chat-jump-latest/);
  assert.match(app, /requestId.*liar\/chat|liar\/chat.*requestId/s);
  ...
  assert.match(worker, /broadcastLiarChatEvent/);
