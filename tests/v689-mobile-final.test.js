...
test('테트리스 자동 낙하는 장시간 플레이에도 700ms 고정이며 시간 가속식을 사용하지 않는다', () => {
  ...
  assert.match(app, /const key = active \? `\$\{room\.id\}:\$\{room\.matchId\}` : null;/);
  assert.match(app, /setTimeout\(run, BLOCK_BATTLE_GRAVITY_MS\)/);
...
test('테트리스 로컬 입력은 싱글과 같은 composite board에서 변경된 셀만 갱신한다', () => {
  assert.match(app, /function paintBlockBattleBoard\(player, root = document, \{ activeOnly = false, preserveActiveOnMissing = false \} = \{\}\)/);
