...
test('모바일 테트리스 플레이 중 하단 메뉴만 숨기고 종료 시 클래스 기반으로 복구한다', () => {
  ...
  assert.match(css, /body\.block-battle-playing \.block-battle-controls\s*\{[^}]*position:relative;[^}]*bottom:auto;/);
...
test('테트리스 로컬 입력은 첫 전체 렌더 이후 활성 블록 주변 칸만 다시 그린다', () => {
  ...
  assert.match(app, /paintBlockBattleBoard\(player, document, \{ activeOnly: !boardChanged \}\)/);
});
