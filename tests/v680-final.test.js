...
import {
  BLOCK_BATTLE_HEIGHT, BLOCK_BATTLE_MAX_ROOMS, BLOCK_BATTLE_STAKES, BLOCK_BATTLE_WIDTH,
  TETROMINO_SHAPES, blockBattleGravityMs, blockBattleRankings, blockBattleSetConnected, blockBattleView, createBlockBattleRoom,
...
test('블럭대전은 10×20, 7종 블럭, 3방, 100/500/1000P 구조다', () => {
  ...
  assert.equal(blockBattleGravityMs(firstRoom, KST_1801), 850);
  assert.equal(blockBattleGravityMs(firstRoom, new Date(KST_1801.getTime() + 5 * 60_000)), 180);
});
