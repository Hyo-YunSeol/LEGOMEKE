...
test('기권 승패·판돈 정산은 한 번만 실행되고 재대결은 양쪽 수락 뒤 시작한다', () => {
  ...
  assert.equal(requestBlockBattleRematch(state, pets[0], roomId, new Date(KST_1801.getTime() + 3_000)).ok, false, '기권하고 나간 플레이어는 재대결할 수 없다');
  const cleanup = leaveBlockBattleRoom(state, pets[1], roomId, new Date(KST_1801.getTime() + 4_000));
  assert.equal(cleanup.ok, true);
  assert.equal(cleanup.deleted, true);
  assert.equal(state.blockBattle.rooms[roomId], undefined);
});
...
test('관전자는 전체 판을 보되 조작할 수 없고 시즌 승패 TOP5가 정렬된다', () => {
  ...
  pets[0].records.seasonBlockBattleWins = 2;
  pets[1].records.seasonBlockBattleWins = 2;
  pets[0].records.seasonBlockBattleLosses = 3;
  pets[1].records.seasonBlockBattleLosses = 1;
  assert.equal(blockBattleRankings(state).top[0].petId, pets[1].id);
});

test('랜덤 종료시각 직전 점령은 유지되고 해당 시각에 정확히 한 번 정산·초기화된다', () => {
  const { state, pets } = players(1, KST_1801);
  state.territory = initialTerritory(KST_1801, 7);
  const randomEnd = new Date('2026-08-11T13:31:00.000Z'); // 한국시간 22:31
  state.territory.endsAt = randomEnd.toISOString();
  state.territory.seasonId = gameDayKey(new Date(randomEnd.getTime() - 1));
  assert.equal(claimTerritory(state, pets[0], 2, 2, KST_1801).ok, true);
  const pointsBefore = pets[0].stats.points;
  const before = processTerritorySeason(state, KST_2230);
  assert.equal(before.changed, false);
  assert.equal(Object.keys(state.territory.cells).length, 1);
  const first = processTerritorySeason(state, randomEnd);
  assert.equal(first.changed, true);
  assert.equal(first.reset, true);
  assert.equal(Object.keys(state.territory.cells).length, 0);
  assert.equal(pets[0].stats.points, pointsBefore + 5_000);
  assert.ok(state.publicEvents.some((event) => event.text.includes('랜덤 종료되어 영토가 초기화')));
  const second = processTerritorySeason(state, new Date(randomEnd.getTime() + 1));
  assert.equal(second.changed, false);
  assert.equal(pets[0].stats.points, pointsBefore + 5_000);
});

test('18:01 점령은 저장된 랜덤 종료시각 전의 새로고침·재접속에도 유지된다', () => {
  const { state, pets } = players(1, KST_1801);
  state.territory = initialTerritory(KST_1801, 8);
  const end = new Date('2026-08-11T14:45:00.000Z'); // 한국시간 23:45
...
test('종료 전 seasonId 불일치와 비정상 endsAt은 영토를 보존하고 현재 6시간 구간 안의 새 랜덤 종료시각으로 복구한다', () => {
  const { state, pets } = players(1, KST_1801);
  ...
  assert.ok(new Date(mismatch.endsAt).getTime() > KST_2230.getTime());
  assert.ok(new Date(mismatch.endsAt).getTime() < KST_2400.getTime());
  ...
  assert.equal(recovered.seasonId, gameDayKey(KST_2230));
  assert.ok(new Date(recovered.endsAt).getTime() > KST_2230.getTime());
  assert.ok(new Date(recovered.endsAt).getTime() < KST_2400.getTime());
...
test('정각 경계가 아닌 랜덤 endsAt도 유효하며 회차 정보가 손상된 경우에만 맵을 보존해 복구한다', () => {
  const { state, pets } = players(1, KST_1801);
  state.territory = initialTerritory(KST_1801, 10);
  claimTerritory(state, pets[0], 2, 4, KST_1801);
  const randomEnd = new Date('2026-08-11T14:00:00.000Z'); // 한국시간 23:00
  state.territory.endsAt = randomEnd.toISOString();
  state.territory.seasonId = gameDayKey(new Date(randomEnd.getTime() - 1));
  const validRandomEnd = processTerritorySeason(state, KST_2230);
  assert.equal(validRandomEnd.reset, false);
  assert.equal(state.territory.endsAt, randomEnd.toISOString());
  assert.equal(Object.keys(state.territory.cells).length, 1);

  state.territory.seasonId = 'corrupt-season';
  const recovered = processTerritorySeason(state, KST_2230);
  assert.equal(recovered.changed, true);
  assert.equal(recovered.reset, false);
  assert.equal(state.territory.seasonId, gameDayKey(KST_2230));
  assert.ok(new Date(state.territory.endsAt).getTime() > KST_2230.getTime());
  assert.ok(new Date(state.territory.endsAt).getTime() < KST_2400.getTime());
  assert.equal(Object.keys(state.territory.cells).length, 1);
  assert.ok(state.territory.recoveryLog.some((entry) => entry.reasons.includes('invalid-seasonId')));
});

test('저장소 재로딩·재배포에 해당하는 새 인스턴스에서도 기존 영토 좌표를 6×6 확장 뒤에도 보존한다', async () => {
  ...
  assert.equal(loaded.territory.size, 6);
...
test('영토전 화면은 각 6시간 구간의 랜덤 종료를 안내하고 정확한 종료시각은 공개하지 않는다', async () => {
  const source = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(source, /compactMetric\('남은 시간', '비공개'\)/);
  assert.match(source, /각 6시간 구간 안에서 한 번 랜덤 종료됩니다\./);
  assert.match(source, /00~06시, 06~12시, 12~18시, 18~24시 각 구간에서 정확히 한 번 랜덤한 시각에 종료됩니다\. 정확한 종료 시각은 공개되지 않습니다\./);
  assert.doesNotMatch(source, /다음 초기화: \$\{territoryResetText\(territory\.endsAt\)\}/);
});
