...
test('영토전은 4×4이며 황금칸은 시즌 종료 전까지 서버 뷰에서 숨기고 종료 시각만 알람으로 잡는다', () => {
...
test('4×4 16칸이 모두 차면 전면전이 영구 해제되어 한 칸짜리 본진도 탈취할 수 있다', () => {
...
test('전면전 해제 상태는 이후 빈칸이 생겨도 같은 시즌 동안 다시 잠기지 않는다', () => {
  ...
  assert.equal(result.ok, true, '전면전 이후에는 현재 점유가 16칸 미만이어도 본진 탈취 가능');
});
