...
test('6시간 게임 하루 경계에서는 지뢰찾기 기록과 시즌이 초기화되지 않는다', () => {
  const beforeBoundary = new Date('2026-08-15T08:59:00.000Z'); // KST 17:59
  const afterBoundary = new Date('2026-08-15T09:01:00.000Z'); // KST 18:01, 같은 3일 시즌
  const { state, pet } = stateWithPet('same-season', '같은시즌', beforeBoundary);
  ...
  assert.equal(settled.champions.length, 0);
  assert.equal(pet.records.minesweeperNormalBestMs, 11100);
  assert.equal(state.minesweeperSeason.key, normalizeMinesweeperSeason(null, afterBoundary).key);
});

test('공통 3일 게임 순위 경계에서 보통/어려움 기록을 함께 초기화하고 직전 1위에게 다음 3일 시즌 칭호를 준다', () => {
  const beforeBoundary = new Date('2026-08-16T14:59:00.000Z'); // KST 23:59
  const afterBoundary = new Date('2026-08-16T15:01:00.000Z'); // KST 00:01, 새 3일 시즌
  const { state, pet } = stateWithPet('champ', '챔피언', beforeBoundary);
  state.minesweeperSeason = normalizeMinesweeperSeason(null, beforeBoundary);
  pet.records.minesweeperNormalBestMs = 11100;
  pet.records.minesweeperNormalBestAt = '2026-08-16T12:30:00.000Z';
  pet.records.minesweeperHardBestMs = 55000;
  pet.records.minesweeperHardBestAt = '2026-08-16T12:40:00.000Z';
  const settled = processMinesweeperSeason(state, afterBoundary);
  ...
  assert.equal(pet.records.minesweeperHardBestMs, 0);
  assert.equal(pet.seasonBadges.minesweeperNormal, '2026-08-19T15:00:00.000Z');
  assert.equal(pet.seasonBadges.minesweeperHard, '2026-08-19T15:00:00.000Z');
});

test('기존 6시간 시즌 키는 배포 시 현재 3일 시즌으로 흡수되어 즉시 초기화되지 않는다', () => {
  const date = new Date('2026-08-15T09:10:00.000Z');
  const migrated = normalizeMinesweeperSeason({
    key: '2026-08-15@3',
    startsAt: '2026-08-15T09:00:00.000Z',
    endsAt: '2026-08-15T15:00:00.000Z',
    initializedAt: '2026-08-15T09:00:00.000Z'
  }, date);
  assert.match(migrated.key, /^season-\d+$/u);
  assert.equal(migrated.startsAt, '2026-08-13T15:00:00.000Z');
  assert.equal(migrated.endsAt, '2026-08-16T15:00:00.000Z');
});
...
test('지뢰찾기 API는 난이도·첫 클릭 차감·포기 흐름을 연결하고 숨은 지뢰 배열을 응답에 노출하지 않는다', async () => {
  ...
  const hiddenIndex = revealed.data.challenge.cells.flat().findIndex((value) => value === null);
  assert.ok(hiddenIndex >= 0);
  const hiddenRow = Math.floor(hiddenIndex / 16);
  const hiddenCol = hiddenIndex % 16;
  const batched = await responseJson(await room.fetch(authRequest('/api/minigames/minesweeper/action', token, {
    method: 'POST',
    body: JSON.stringify({
      challengeId: challenge.id,
      actions: [
        { action: 'flag', row: hiddenRow, col: hiddenCol },
        { action: 'flag', row: hiddenRow, col: hiddenCol }
      ]
    })
  })));
  assert.equal(batched.data.ok, true, JSON.stringify(batched.data));
  assert.equal(batched.data.processedCount, 2);
  assert.equal(batched.data.results.length, 2);
  assert.equal('bootstrap' in batched.data, false);
  assert.equal('minesweeperBoard' in batched.data.challenge, false);
...
test('3일 공통 시즌 경계를 넘긴 진행 중 지뢰판은 새 시즌 기록으로 넘어가지 않고 0P 포기 종료된다', () => {
  const beforeBoundary = new Date('2026-08-16T14:58:00.000Z');
  const afterBoundary = new Date('2026-08-16T15:00:01.000Z');
