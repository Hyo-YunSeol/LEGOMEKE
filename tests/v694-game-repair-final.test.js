...
test('사천성은 Grid 자동행을 버리고 10×8 절대좌표 판으로 고정한다', () => {
  assert.match(styles, /\.sichuan-board\{[^}]*position:relative[^}]*display:block[^}]*aspect-ratio:10\/8[^}]*overflow:hidden/);
  assert.doesNotMatch(styles, /\.sichuan-board\{[^}]*grid-template-columns/);
  assert.match(styles, /\.sichuan-cell\{[^}]*position:absolute[^}]*width:calc\(10% - var\(--sichuan-gap\)\)[^}]*height:calc\(12\.5% - var\(--sichuan-gap\)\)/);
...
test('사과 포기는 제목 옆 소형 버튼이고 블록·지뢰찾기는 게임 내용 최상단에 배치된다', () => {
  assert.match(app, /function appleModalHeader[\s\S]*apple-header-abandon[\s\S]*>포기<\/button>/);
...
test('영토전은 남은 시간을 비공개로 고정하고 1위 5,000P·Lv20 7칸·최대 10칸을 사용한다', () => {
  ...
  assert.match(app, /단독 1위 5,000P · 공동 1위는 황금영토로 결정/);
});
