...
test('사천성은 10열 8행 고정 그리드와 셀 높이 제한으로 판 틀을 유지한다', () => {
  assert.match(styles, /\.sichuan-board\{[^}]*grid-template-columns:repeat\(10,minmax\(0,1fr\)\)[^}]*grid-template-rows:repeat\(8,minmax\(0,1fr\)\)[^}]*aspect-ratio:11\/10[^}]*overflow:hidden/);
  assert.match(styles, /\.sichuan-cell\{[^}]*min-height:0[^}]*height:100%[^}]*overflow:hidden/);
