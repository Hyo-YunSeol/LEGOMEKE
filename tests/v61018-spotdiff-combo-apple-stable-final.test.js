...
import {
  ...
  SPOT_DIFFERENCE_RECENT_HISTORY, spotDifferenceHitboxesForPuzzle,
...
function puzzleKey(puzzle) {
  return Number(puzzle.assetVersion) >= 5 ? `v${Number(puzzle.assetVersion)}:${puzzle.themeId}:${Number(puzzle.variant)}:${Number(puzzle.seed)}:${[...puzzle.differenceIds].sort((a, b) => Number(a.slice(1)) - Number(b.slice(1))).join(',')}` : `v${Number(puzzle.assetVersion)}:${puzzle.themeId}:${Number(puzzle.variant)}:${[...puzzle.differenceIds].sort((a, b) => Number(a.slice(1)) - Number(b.slice(1))).join(',')}`;
}

function pointFor(puzzle, differenceId) {
  const hit = spotDifferenceHitboxesForPuzzle(puzzle).find((item) => item.id === differenceId);
