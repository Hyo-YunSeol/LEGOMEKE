...
test('오목·테트리스 생성 모달은 실시간 갱신으로 다시 그려져 100P로 초기화되지 않는다', async () => {
  ...
  assert.match(app, /descriptor\.type === 'omokCreate' \|\| descriptor\.type === 'blockBattleCreate' \|\| descriptor\.type === 'sichuanCreate' \|\| descriptor\.type === 'spotDifferenceCreate' \|\| descriptor\.type === 'davinciCreate'\) return;/);
