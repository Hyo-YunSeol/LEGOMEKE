...
test('사과는 제목 옆 소형 포기 버튼, 블록·지뢰찾기는 공통 포기하기 버튼을 사용한다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  const styles = await readFile(new URL('../public/styles.css', import.meta.url), 'utf8');
  assert.match(app, /apple-header-abandon[^`]*data-action="abandon-mini"[^`]*>포기<\/button>/u);
  ...
  assert.match(styles, /\.apple-header-abandon/u);
