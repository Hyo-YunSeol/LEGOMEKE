...
test('지뢰찾기 모바일 롱프레스는 400ms를 유지하고 진동 호출은 제거한다', () => {
  assert.match(appSource, /MINESWEEPER_LONG_PRESS_MS = 400/u);
  assert.doesNotMatch(appSource, /navigator\.vibrate/u);
});
...
test('지뢰찾기 입력은 in-flight 동안 폐기하지 않고 큐로 순차 처리한다', () => {
  ...
  assert.match(appSource, /actions: sendable\.map/u);
  assert.match(workerSource, /mutateForUserLight/u);
});
