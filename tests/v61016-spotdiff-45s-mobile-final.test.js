...
test('틀린그림찾기 제한시간은 서버 45초·오답 5회이며 로비 TMI는 노출하지 않는다', () => {
  ...
  assert.doesNotMatch(app, /45초 안에 틀린 곳 7개를 먼저 찾으면 승리합니다/);
...
test('모바일 SVG 합성 안정화와 v6.10.20 캐시 버전을 함께 배포한다', () => {
  ...
  assert.match(app, /spot-difference-scene\.js\?v=610120/);
