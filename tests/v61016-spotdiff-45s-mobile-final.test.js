...
test('틀린그림찾기 제한시간은 서버/UI 모두 45초이고 오답 5회 패배 규칙을 노출한다', () => {
  assert.match(server, /SPOT_DIFFERENCE_MATCH_SECONDS = 45/);
  ...
  assert.match(app, /7개 · 45초 · 오답 5회 패배/);
  assert.match(app, /<span>0:45<\/span><span>오답 5회 패배<\/span>/);
  assert.match(app, /45초 안에 틀린 곳 7개를 먼저 찾으면 승리합니다/);
});
...
test('모바일 SVG 합성 안정화와 v6.10.18 캐시 버전을 함께 배포한다', () => {
  assert.match(css, /spot-picture-button[^}]*contain:paint[^}]*translateZ\(0\)[^}]*backface-visibility:hidden/);
  assert.equal(JSON.parse(pkg).version, '6.10.18');
  assert.match(index, /styles\.css\?v=610118/);
  assert.match(index, /app\.js\?v=610118/);
  assert.match(app, /spot-difference-scene\.js\?v=610118/);
  assert.match(app, /sw\.js\?v=610118/);
  assert.match(sw, /const VERSION = '610118'/);
});
