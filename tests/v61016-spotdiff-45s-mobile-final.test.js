...
test('모바일 SVG 합성 안정화와 v6.10.17 캐시 버전을 함께 배포한다', () => {
  assert.match(css, /spot-picture-button[^}]*contain:paint[^}]*translateZ\(0\)[^}]*backface-visibility:hidden/);
  assert.equal(JSON.parse(pkg).version, '6.10.17');
  assert.match(index, /styles\.css\?v=610117/);
  assert.match(index, /app\.js\?v=610117/);
  assert.match(app, /spot-difference-scene\.js\?v=610117/);
  assert.match(app, /sw\.js\?v=610117/);
  assert.match(sw, /const VERSION = '610117'/);
});
