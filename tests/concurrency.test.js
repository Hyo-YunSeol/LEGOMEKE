...
test('게임 랭킹 시즌 경계에서 부트스트랩 요청이 동시에 몰려도 TOP3 보상과 시즌 소식은 정확히 한 번만 정산된다', async () => {
  ...
  a.records.oddEvenBest = 3; a.records.oddEvenBestAt = '2026-08-01T00:00:00.000Z';
  b.records.oddEvenBest = 2; b.records.oddEvenBestAt = '2026-08-01T01:00:00.000Z';
  c.records.oddEvenBest = 1; c.records.oddEvenBestAt = '2026-08-01T02:00:00.000Z';
  ...
  assert.equal(after.pets[a.id].records.oddEvenBest, 0);
  const seasonNews = after.publicEvents.filter((event) => event.type === 'game-season');
  assert.equal(seasonNews.length, 3, '홀짝·사과·오목 소식은 각 한 줄만 생성되어야 한다');
  assert.equal(seasonNews.filter((event) => event.text.includes('홀짝 시즌 종료')).length, 1);
