...
test('게임 랭킹 시즌 경계에서 부트스트랩 요청이 동시에 몰려도 TOP3 보상과 시즌 소식은 정확히 한 번만 정산된다', async () => {
  ...
  assert.equal(seasonNews.length, 6, '사과·싱글 테트리스·오목·테트리스대전·사천성·레고도쿠 소식은 각 한 줄만 생성되어야 한다');
  ...
  assert.equal(seasonNews.filter((event) => event.text.includes('레고도쿠 시즌 종료')).length, 1);
