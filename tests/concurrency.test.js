...
test('게임 랭킹 시즌 경계에서 부트스트랩 요청이 동시에 몰려도 TOP3 보상과 시즌 소식은 정확히 한 번만 정산된다', async () => {
  ...
  a.records.seasonBestReactionMs = 180; a.records.seasonBestReactionAt = '2026-08-01T00:00:00.000Z';
  b.records.seasonBestReactionMs = 220; b.records.seasonBestReactionAt = '2026-08-01T01:00:00.000Z';
  c.records.seasonBestReactionMs = 260; c.records.seasonBestReactionAt = '2026-08-01T02:00:00.000Z';
  ...
  assert.equal(after.pets[a.id].stats.points, 2_000, '번개반응 1위 + 테트리스 1위 보상이 각각 누적되어야 한다');
  assert.equal(after.pets[b.id].stats.points, 500);
  assert.equal(after.pets[c.id].stats.points, 300);
  assert.equal(after.pets[a.id].records.seasonBestReactionMs, 0);
  const seasonNews = after.publicEvents.filter((event) => event.type === 'game-season');
  assert.equal(seasonNews.length, 5, '번개·사과·오목·테트리스·다빈치코드 소식은 각 한 줄만 생성되어야 한다');
  assert.equal(seasonNews.filter((event) => event.text.includes('번개반응 시즌 종료')).length, 1);
