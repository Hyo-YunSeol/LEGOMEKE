...
test('두 사용자가 같은 빈칸을 동시에 첫 본진으로 누르면 한 명만 성공하고 다른 요청은 안전하게 거부된다', async () => {
  ...
  assert.equal(results.filter((result) => result.data.ok === true).length, 1);
  assert.equal(results.filter((result) => result.data.ok === false).length, 1);
...
test('오목 동일 승리 착수가 동시에 반복되어도 판돈 정산과 승패 기록은 정확히 한 번만 반영된다', async () => {
  ...
  assert.equal((await api(room, tokenA, `/api/omok/rooms/${roomId}/rps`, { choice: 'rock' })).response.status, 200);
  assert.equal((await api(room, tokenB, `/api/omok/rooms/${roomId}/rps`, { choice: 'scissors' })).response.status, 200);
  assert.equal((await api(room, tokenA, `/api/omok/rooms/${roomId}/color`, { color: 'black' })).response.status, 200);
...
test('게임 랭킹 시즌 경계에서 부트스트랩 요청이 동시에 몰려도 TOP3 보상과 시즌 소식은 정확히 한 번만 정산된다', async () => {
  const { room } = await createRoom();
  const tokenA = await register(room, '시즌동시A');
  const tokenB = await register(room, '시즌동시B');
  const tokenC = await register(room, '시즌동시C');
  const boots = await Promise.all([bootstrap(room, tokenA), bootstrap(room, tokenB), bootstrap(room, tokenC)]);
  const petIds = boots.map((boot) => boot.dashboard.pet.id);

  const state = await room.store.load();
  const [a, b, c] = petIds.map((id) => state.pets[id]);
  for (const pet of [a, b, c]) pet.stats.points = 0;
  a.records.appleBestScore = 1800; a.records.appleBestAt = '2026-08-01T00:00:00.000Z';
  b.records.appleBestScore = 1500; b.records.appleBestAt = '2026-08-01T01:00:00.000Z';
  c.records.appleBestScore = 1200; c.records.appleBestAt = '2026-08-01T02:00:00.000Z';
  a.records.seasonBlockBattleWins = 5; a.records.seasonBlockBattleLosses = 1;
  // 현재 시즌과 확실히 다른 과거 시즌으로 만들어 다음 서버 처리에서 정산되게 한다.
  state.gameRankingSeason = {
    key: 'season-1',
    startsAt: '1970-01-04T15:00:00.000Z',
    endsAt: '1970-01-07T15:00:00.000Z',
    initializedAt: '1970-01-04T15:00:00.000Z',
    lastSettledAt: null
  };
  await room.store.save(state);

  await Promise.all(Array.from({ length: 12 }, (_, index) => bootstrap(room, [tokenA, tokenB, tokenC][index % 3])));
  const after = await room.store.load();
  assert.equal(after.pets[a.id].stats.points, 2_000, '사과게임 1위 + 테트리스 1위 보상이 각각 누적되어야 한다');
  assert.equal(after.pets[b.id].stats.points, 500);
  assert.equal(after.pets[c.id].stats.points, 300);
  assert.equal(after.pets[a.id].records.appleBestScore, 0);
  const seasonNews = after.publicEvents.filter((event) => event.type === 'game-season');
  assert.equal(seasonNews.length, 7, '사과·싱글 테트리스·오목·테트리스대전·사천성·틀린그림찾기·다빈치코드 소식은 각 한 줄만 생성되어야 한다');
  assert.equal(seasonNews.filter((event) => event.text.includes('사과게임 시즌 종료')).length, 1);
  assert.equal(seasonNews.filter((event) => event.text.includes('싱글 테트리스 시즌 종료')).length, 1);
  assert.equal(seasonNews.filter((event) => event.text.includes('오목 시즌 종료')).length, 1);
  assert.equal(seasonNews.filter((event) => event.text.includes('테트리스대전 시즌 종료')).length, 1);
  assert.equal(seasonNews.filter((event) => event.text.includes('사천성대전 시즌 종료')).length, 1);
  assert.equal(seasonNews.filter((event) => event.text.includes('틀린그림찾기 시즌 종료')).length, 1);
  assert.equal(seasonNews.filter((event) => event.text.includes('다빈치코드 시즌 종료')).length, 1);
  assert.equal(after.pets[a.id].records.seasonBlockBattleWins, 0, '테트리스 시즌 승수도 같은 시즌 경계에서 초기화되어야 한다');
  assert.equal(after.pets[a.id].records.seasonBlockBattleLosses, 0, '테트리스 시즌 패수도 같은 시즌 경계에서 초기화되어야 한다');
});
