...
test('같은 piece의 서버 ACK가 row/col에서 늦어도 로컬 active 객체와 위치를 교체하지 않는다', () => {
  const emptyBoard = () => Array.from({ length: 20 }, () => Array(10).fill(null));
  const server = {
    petId: 'p1', displayName: '나', connected: true, board: emptyBoard(),
    active: { type: 'T', rotation: 0, row: 4, col: 3 }, next: ['I', 'S'],
    lines: 0, score: 0, pieces: 0, attackSent: 0, pendingGarbage: 0, pendingGarbageHoles: []
  };
  const visualPlayer = cloneBlockBattleVisualPlayer(server);
  visualPlayer.active = { type: 'T', rotation: 1, row: 6, col: 2 };
  const activeRef = visualPlayer.active;
  const current = { key: 'r:m:p1', player: visualPlayer };
  const reconciled = reconcileBlockBattleVisual(current, server, 'r:m:p1', { force: true });
  assert.strictEqual(reconciled, current);
  assert.strictEqual(reconciled.player.active, activeRef);
  assert.deepEqual(reconciled.player.active, { type: 'T', rotation: 1, row: 6, col: 2 });
});

test('서버 push의 일반 DOM patch는 내 테트리스 보드를 다시 칠하지 않고 로컬 입력만 self paint한다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(app, /function patchBlockBattleDynamic\(room, \{ paintSelf = false \} = \{\}\)/);
  assert.match(app, /if \(!isSelf \|\| paintSelf\) paintBlockBattleBoard/);
  assert.match(app, /previewBlockBattleLock[\s\S]*patchBlockBattleDynamic\(room, \{ paintSelf: true \}\)/);
  assert.match(app, /scheduleBlockBattleDomUpdate\(needsLayout, \{ paintSelf: serverSelfPaintNeeded \}\)/);
});

test('6시간 대전 정규화 변경은 processTimeState가 변경으로 감지해 Durable Object 저장 대상이 된다', async () => {
  const worker = await readFile(new URL('../src/worker.js', import.meta.url), 'utf8');
  assert.match(worker, /const beforeBattleDaily = JSON\.stringify/);
  assert.match(worker, /const afterBattleDaily = JSON\.stringify/);
  assert.match(worker, /beforeBattleDaily !== afterBattleDaily/);
});

test('이번 배포는 전체 레고의 대전 이용량만 한 번 0으로 미는 migration marker를 저장한다', async () => {
  const worker = await readFile(new URL('../src/worker.js', import.meta.url), 'utf8');
  assert.match(worker, /BATTLE_USAGE_FORCE_RESET_ID = '2026-08-27-battle-usage-reset-v1'/);
  assert.match(worker, /state\.meta\.battleUsageForceResetId !== BATTLE_USAGE_FORCE_RESET_ID/);
  assert.match(worker, /pet\.daily\.battlePlayed = 0;/);
  assert.match(worker, /pet\.daily\.battleBonus = 0;/);
  assert.match(worker, /pet\.daily\.battleDate = currentBattleDate;/);
  assert.match(worker, /state\.meta\.battleUsageForceResetId = BATTLE_USAGE_FORCE_RESET_ID/);
});
