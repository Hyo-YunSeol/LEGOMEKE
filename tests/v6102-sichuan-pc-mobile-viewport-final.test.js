...
test('v6.10.4 정적 자산 캐시 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.23');
  assert.match(index, /styles\.css\?v=6101231/);
  assert.match(index, /app\.js\?v=6101231/);
  assert.match(app, /sw\.js\?v=6101231/);
  assert.match(sw, /const VERSION = '6101231'/);
});
