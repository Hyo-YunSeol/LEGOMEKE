...
test('사과·블록·지뢰찾기는 실제 flex 잔여 높이와 margin을 기준으로 판을 축소한다', () => {
  for (const name of ['syncAppleGameLayout', 'syncBlockGameLayout', 'syncMinesweeperGameLayout']) {
...
test('모바일 전체화면 미니게임 모달은 padding 포함 높이를 넘지 않고 3개 HUD를 한 줄로 유지한다', () => {
  assert.match(css, /\.modal-root\.apple-modal-root \.modal \{[^}]*height:\s*100%;[^}]*min-height:0;[^}]*overflow:\s*hidden;[^}]*box-sizing:border-box;/s);
...
test('v6.10.2 정적 자산 캐시 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.2');
  assert.match(index, /styles\.css\?v=610102/);
  assert.match(index, /app\.js\?v=610102/);
  assert.match(app, /sw\.js\?v=610102/);
  assert.match(sw, /const VERSION = '610102'/);
});
