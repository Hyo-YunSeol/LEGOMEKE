...
test('사천성은 네트워크 왕복 동안 판을 잠그지 않고 낙관적 제거를 순차 전송한다', () => {
  ...
  assert.match(app, /const interactive = Boolean\(isPlayer && room\.status === 'playing' && self && !sichuanPenaltyActive\(room, room\.selfPetId\)\);/);
  assert.match(app, /if \(!patchSichuanBoard\(self, \{ interactive: !sichuanPenaltyActive\(room, room\.selfPetId\) \}\)\) return false;/);
  const tileHandler = app.slice(app.indexOf("if (action === 'sichuan-tile')"), app.indexOf("if (action === 'toggle-relation-rankings')"));
...
test('사과 PC는 CSS 고정, 모바일만 안정 컨테이너 높이로 계산하고 블록·지뢰찾기는 기존 축소를 유지한다', () => {
  const appleSource = extractFunction(app, 'syncAppleGameLayout');
  assert.match(appleSource, /#apple-board-stage/);
  assert.doesNotMatch(appleSource, /stage\.clientHeight - verticalPadding/);
  assert.match(appleSource, /game\.clientHeight - fixedHeight - verticalPadding - 4/);
  assert.match(appleSource, /game\.clientWidth - horizontalPadding/);
...
test('v6.10.4 정적 자산 캐시 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.27');
  assert.match(index, /styles\.css\?v=6101270/);
  assert.match(index, /app\.js\?v=6101270/);
  assert.match(app, /sw\.js\?v=6101270/);
  assert.match(sw, /const VERSION = '6101270'/);
});
