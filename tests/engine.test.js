...
test('몸집 단계는 돼지 이후 코끼리·맘모스·돼룩돼룩레고까지 최신 구간을 사용한다', () => {
  const cases = [[0,'마름레고'],[60,'마름레고'],[69,'마름레고'],[70,'보통레고'],[79,'보통레고'],[80,'짭덥레고'],[89,'짭덥레고'],[90,'통통레고'],[99,'통통레고'],[100,'떱레고'],[109,'떱레고'],[110,'리얼떱레고'],[119,'리얼떱레고'],[120,'돼지레고'],[129,'돼지레고'],[130,'코끼리레고'],[159,'코끼리레고'],[160,'맘모스레고'],[199,'맘모스레고'],[200,'돼룩돼룩레고'],[999,'돼룩돼룩레고']];
  for (const [body, label] of cases) assert.equal(getBodyStage(body).label, label);
  assert.equal(BODY_STAGES.length, 10);
});

test('게임 하루는 한국시간 0·6·12·18시 기준 6시간이다', () => {
  const date = new Date('2026-08-05T15:00:00.000Z');
  assert.equal(gameDayKey(date), '2026-08-06@0');
  assert.equal(nextGameDayAt(date), '2026-08-05T21:00:00.000Z');
});
...
test('4×4 영토전 최대치는 Lv1 1칸, Lv2 2칸, Lv3 3칸, Lv4 이상 4칸이다', () => {
...
test('오늘의 레고력 목표 6개는 각각 한 번만 +1을 지급한다', () => {
  const state = stateWithUsers([['u1','윤설']]);
  const pet = current(state);
  ...
  completeDailyGoal(pet, 'liarWin');
  completeDailyGoal(pet, 'bung');
  assert.equal(pet.stats.legoPower, 7);
  assert.equal(completeDailyGoal(pet, 'bung').awarded, false);
  assert.equal(pet.stats.legoPower, 7);
});
...
test('벙 정상 종료는 참가자 체력 -20, 반복 레고력 +1, 첫 오늘 목표 +1을 지급한다', () => {
  const state = stateWithUsers();
  const host = current(state, 'u1');
  const guest = current(state, 'u2');
  ...
  assert.equal(host.stats.legoPower, 3);
...
test('포인트·레벨·커플·찌르기 랭킹은 살아 있는 현재 레고만 사용한다', () => {
...
test('대시보드는 필요한 포인트·레벨·생활·핵심 기록만 반환한다', () => {
  ...
  assert.equal(dashboard.pet.daily.goals.total, 6);
