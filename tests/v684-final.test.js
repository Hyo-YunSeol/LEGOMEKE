...
test('v6.8.4 배포 버전과 정적 자산 캐시는 함께 올라간다', async () => {
  const { app, worker, index, sw, pkg } = await files();
  assert.match(pkg, /"version": "6\.8\.4"/);
  assert.match(worker, /6\.8\.4-final/);
  assert.match(index, /\?v=684/);
  assert.match(app, /\?v=684/);
  assert.match(sw, /lego-life-v684-final/);
});
