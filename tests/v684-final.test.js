...
test('현재 배포 버전과 정적 자산 캐시는 함께 올라간다', async () => {
  ...
  assert.match(pkg, /"version": "6\.10\.11"/);
  assert.match(worker, /6\.10\.11-battle-tetris-sync-final/);
  assert.match(index, /\?v=610111/);
  assert.match(app, /\?v=610111/);
  assert.match(sw, /lego-life-v610111-battle-tetris-sync-final/);
});
