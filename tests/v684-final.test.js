...
test('v6.10.24 주요 게임 순위 묶음은 모바일에서도 3열이고 커플 D-Day·찌르기는 더보기 전까지 숨긴다', async () => {
...
test('현재 배포 버전과 정적 자산 캐시는 함께 올라간다', async () => {
  ...
  assert.match(pkg, /"version": "6\.10\.24"/);
  assert.match(worker, /6\.10\.24-safe-fixes-final/);
  assert.match(index, /\?v=6101240/);
