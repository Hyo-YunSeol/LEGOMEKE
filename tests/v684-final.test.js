...
test('v6.8.4 테트리스 라이브 갱신은 판 전체 재생성 대신 부분 패치하고 모바일 높이를 반영한다', async () => {
  const { app, css } = await files();
  assert.match(app, /function patchBlockBattleDynamic\(room\)/);
  ...
  assert.match(css, /block-battle-cell\[class\*="piece-"\], \.block-battle-active-cell\[class\*="piece-"\] \{ box-shadow:none/);
});
...
test('현재 배포 버전과 정적 자산 캐시는 함께 올라간다', async () => {
  ...
  assert.match(pkg, /"version": "6\.10\.8"/);
  assert.match(worker, /6\.10\.8-records-sichuan-pc-tetris-final/);
  assert.match(index, /\?v=610108/);
  assert.match(app, /\?v=610108/);
  assert.match(sw, /lego-life-v610108-records-sichuan-pc-tetris-final/);
});
