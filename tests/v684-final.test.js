...
test('현재 배포 버전과 정적 자산 캐시는 함께 올라간다', async () => {
  ...
  assert.match(pkg, /"version": "6\.10\.10"/);
  assert.match(worker, /6\.10\.10-battle-tetris-sync-final/);
  assert.match(index, /\?v=610110/);
  assert.match(app, /\?v=610110/);
  assert.match(sw, /lego-life-v610110-battle-tetris-sync-final/);
});
