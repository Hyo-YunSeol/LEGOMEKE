...
test('신규 틀린그림은 고밀도 WebP 20장면 × 18개 후보 중 7개 조합형으로 운영한다', async () => {
  assert.equal(SPOT_DIFFERENCE_ATLAS_VERSION, 3);
...
test('새 대전은 assetVersion 3의 랜덤 7곳을 만들고 changed 쪽은 같은 고밀도 원본 위에 7개 차이를 합성한다', () => {
  ...
  assert.equal(room.puzzle.assetVersion, 3);
  ...
  assert.match(urls.original, /\/spot-atlas\/.+-base\.webp\?v=610118$/);
  ...
  assert.match(original, /-base\.webp\?v=610118/);
  assert.doesNotMatch(original, /spot-dynamic-difference/);
  assert.equal((changed.match(/class="spot-dynamic-difference"/g) || []).length, 7);
