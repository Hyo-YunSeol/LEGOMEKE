...
test('v6.10.25 정적 버전·서비스워커 모듈 캐시 키가 모두 6101280으로 일치한다', async () => {
  ...
  assert.equal(JSON.parse(pkgText).version, '6.10.28');
  assert.match(index, /styles\.css\?v=6101280/);
  assert.match(index, /app\.js\?v=6101280/);
  assert.match(app, /block-battle-visual-state\.js\?v=6101280/);
  assert.match(app, /sw\.js\?v=6101280/);
  assert.match(app, /\/pets\/\$\{esc\(stage\.assetKey \|\| stage\.key\)\}\.svg\?v=6101280/);
  assert.match(app, /\/flex\/\$\{esc\(flexItem\.assetKey\)\}\.svg\?v=6101280/);
  assert.match(app, /legodoku\/bi-tteop-head\.svg\?v=6101280/);
  assert.match(app, /v=6101280/);
