...
import {
  ...
  SPOT_DIFFERENCE_START_COUNTDOWN_MS, SPOT_DIFFERENCE_WRONG_LOCK_MS, SPOT_DIFFERENCE_MAX_WRONG_CLICKS, SPOT_DIFFERENCE_THEMES,
  ...
  spotDifferenceRanking, validSpotDifferenceStake, spotDifferenceHitboxesForPuzzle
} from '../src/game/spot-difference.js';
import { SPOT_CLIENT_HITBOXES, spotClientHitboxesForPuzzle, spotDifferenceHitIdAt } from '../public/spot-difference-scene.js';
...
function pointFor(room, differenceId) {
  const hit = spotDifferenceHitboxesForPuzzle(room.puzzle).find((item) => item.id === differenceId);
...
test('틀린그림찾기는 7개·45초·3초 카운트다운·오답 5회 패배 사양을 고정한다', () => {
...
test('클라이언트와 서버는 legacy/v4 모두 같은 비율좌표 hitbox를 사용하고 정답 영역끼리 겹치지 않는다', () => {
  assert.deepEqual(SPOT_CLIENT_HITBOXES, SPOT_DIFFERENCE_HITBOXES);
  for (const theme of SPOT_DIFFERENCE_THEMES) for (const variant of [0, 1]) {
    const puzzle = { assetVersion: 4, themeId: theme.id, variant };
    const serverBoxes = spotDifferenceHitboxesForPuzzle(puzzle);
    const clientBoxes = spotClientHitboxesForPuzzle(puzzle);
    assert.deepEqual(clientBoxes, serverBoxes, `${theme.id}:${variant} 좌표 불일치`);
    assert.equal(serverBoxes.length, 30);
    for (let i = 0; i < serverBoxes.length; i += 1) {
      const a = serverBoxes[i];
      for (let j = i + 1; j < serverBoxes.length; j += 1) {
        const b = serverBoxes[j];
        assert.ok(Math.hypot(a.x - b.x, a.y - b.y) > a.r + b.r, `${theme.id}:${variant} ${a.id}/${b.id} 터치 영역이 겹치면 안 됩니다.`);
      }
    }
  }
});
...
test('두 플레이어는 같은 랜덤 문제를 받고 카운트다운 전에는 정답 입력이 처리되지 않는다', () => {
  ...
  assert.equal(new Date(room.deadlineAt).getTime() - new Date(room.revealAt).getTime(), 45000);
...
test('오답 5회째는 즉시 패배하고 이미 찾은 정답을 다시 눌러도 오답 횟수는 늘지 않는다', () => {
  const { state, ps, room } = setup(500);
  const answer = room.puzzle.differenceIds[0];
  const first = clickDifference(state, ps[0], room, answer, 'found-once', at(5));
  assert.equal(first.correct, true);
  const duplicate = clickDifference(state, ps[0], room, answer, 'found-again', at(6));
  assert.equal(duplicate.alreadyFound, true);
  assert.equal(room.players[ps[0].id].wrongClicks, 0);

  for (let count = 1; count <= 5; count += 1) {
    const result = playSpotDifferenceClick(state, ps[0], room.id, {
      matchId: room.matchId, actionId: `wrong-${count}`, x: 0.50, y: 0.96
    }, at(6 + count * 1.1));
    assert.equal(result.correct, false);
    assert.equal(room.players[ps[0].id].wrongClicks, count);
    if (count < 5) assert.equal(room.status, 'playing');
    else {
      assert.equal(result.finished, true);
      assert.equal(room.status, 'ended');
      assert.equal(room.winnerPetId, ps[1].id);
      assert.equal(room.loserPetId, ps[0].id);
      assert.match(room.resultReason, /오답 5회/);
      assert.equal(ps[0].records.spotDifferenceLosses, 1);
      assert.equal(ps[1].records.spotDifferenceWins, 1);
    }
  }
});
...
test('45초 종료 동수는 같은 개수에 먼저 도달한 플레이어가 이기고 0대0은 무승부다', () => {
  {
