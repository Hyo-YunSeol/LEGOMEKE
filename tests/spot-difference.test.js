...
test('틀린그림찾기는 7개·30초·3초 카운트다운·오답 5회 패배 사양을 고정한다', () => {
...
test('두 플레이어는 같은 랜덤 문제를 받고 카운트다운 전에는 정답 입력이 처리되지 않는다', () => {
  ...
  assert.equal(new Date(room.deadlineAt).getTime() - new Date(room.revealAt).getTime(), 30000);
...
test('30초 종료 동수는 같은 개수에 먼저 도달한 플레이어가 이기고 0대0은 무승부다', () => {
  {
