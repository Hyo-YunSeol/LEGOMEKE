...
import {
  SPOT_DIFFERENCE_COUNT, SPOT_DIFFERENCE_HITBOXES, SPOT_DIFFERENCE_MATCH_SECONDS, SPOT_DIFFERENCE_MAX_ROOMS,
...
test('틀린그림찾기는 7개·45초·3개 방·3초 카운트다운·오답 1초 잠금 사양을 고정한다', () => {
...
test('클라이언트와 서버는 같은 비율좌표 hitbox를 사용하고 실제 출제된 7개 정답 영역은 겹치지 않는다', () => {
  ...
  const difficulty = selected.reduce((acc, item) => { acc[item.difficulty] = (acc[item.difficulty] || 0) + 1; return acc; }, {});
  assert.deepEqual(difficulty, { easy: 1, normal: 4, hard: 2 });
  for (let i = 0; i < selected.length; i += 1) {
    const a = selected[i];
    for (let j = i + 1; j < selected.length; j += 1) {
      const b = selected[j];
      assert.ok(Math.hypot(a.x - b.x, a.y - b.y) > a.r + b.r + 0.018, `${a.id}/${b.id} 출제 정답 터치 영역이 겹치면 안 됩니다.`);
    }
  }
});
