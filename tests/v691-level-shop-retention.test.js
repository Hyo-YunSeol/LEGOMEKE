...
test('영토 한도는 레벨마다 +1칸이며 5×5 전체 25칸을 넘지 않는다', () => {
  for (const level of [1, 2, 5, 14, 20, 24, 25]) assert.equal(territoryLimitForLevel(level), level);
  assert.equal(territoryLimitForLevel(26), 25);
  assert.equal(territoryLimitForLevel(500), 25);
});

test('플렉스 상점은 Lv.5~50 매 5레벨마다 정확히 5종이며 가격 구조와 이름표 시작 Lv.15를 지킨다', () => {
  const items = Object.values(FLEX_ITEMS);
  ...
  for (let level = 5; level <= 50; level += 5) {
    ...
    assert.deepEqual(group.slice(0, 3).map((item) => item.price), [500, 500, 500], `Lv.${level} 일반`);
    if (level < 15) {
      assert.deepEqual(group.slice(3).map((item) => item.price), [700, 700], `Lv.${level} 펫`);
      assert.equal(group.some((item) => item.kind === 'nameplate'), false);
    } else {
      assert.equal(group[3].price, 700, `Lv.${level} 펫`);
      assert.equal(group[3].kind, 'pet', `Lv.${level} 펫`);
      assert.equal(group[4].price, 1500, `Lv.${level} 이름표`);
      assert.equal(group[4].kind, 'nameplate', `Lv.${level} 이름표`);
      assert.ok(group[4].nameplateKey, `Lv.${level} 이름표 키`);
    }
    assert.ok(group.every((item) => item.durationHours === 24));
  }
});

test('플렉스 구매 레벨 제한은 서버가 강제하고 Lv.15 이름표 상품은 해금 뒤 정상 장착된다', () => {
  ...
  pet.stats.legoPower = levelUpperBound(14);
  pet.stats.points = 5000;

  const blocked = purchaseShopItem(state, user, pet, 'diamond', {}, 'flex-level-lock-001', now);
  assert.equal(blocked.ok, false);
  assert.match(blocked.message, /Lv\.15/);
  assert.equal(pet.stats.points, 5000);

  pet.stats.legoPower = levelUpperBound(14) + 1;
  assert.equal(levelForPower(pet.stats.legoPower), 15);
  const bought = purchaseShopItem(state, user, pet, 'diamond', {}, 'flex-level-lock-002', now);
  assert.equal(bought.ok, true);
  assert.equal(bought.price, 1500);
  assert.equal(pet.stats.points, 3500);
  assert.equal(flexItemView(pet, now)?.nameplateKey, 'diamond');
});
...
test('테트리스도 동일한 시즌 TOP3 보상과 왕 칭호를 받고 여러 종목 1위 보상은 모두 중복 지급된다', () => {
  ...
  a.records.seasonBestReactionMs = 100; b.records.seasonBestReactionMs = 200;
  ...
  assert.equal(a.stats.points, 4000, '4개 종목 1위 1000P가 각각 누적되어야 한다');
  const labels = seasonBadgeView(a, date).map((item) => item.label);
  for (const label of ['⚡왕', '🍎왕', '⚫오목왕', '🧱테트리스왕']) assert.ok(labels.includes(label), label);
});
...
test('플렉스 UI는 PC 5열·모바일 2열이며 레벨 메달/이름표는 무한 애니메이션 없이 정적으로 표시된다', async () => {
  ...
  assert.match(app, /Lv\.15부터 각 단계의 1,500P 상품은 전용 이름표/);
