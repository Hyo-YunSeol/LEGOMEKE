...
test('25칸이 모두 각자의 본진이어도 전면전에서 신규 참가와 영토 0칸 피해자가 건물을 빼앗아 재진입한다', () => {
  const names = Array.from({ length: 26 }, (_, index) => [`territory-${index}`, `영토${index}`]);
  ...
  for (let index = 0; index < 25; index += 1) {
    ...
    const col = index % 5;
    state.territory.cells[`${row}:${col}`] = { ownerPetId: pets[index].id, claimedAt: new Date(NOW.getTime() + index).toISOString(), home: true };
  }
  state.territory = normalizeTerritory(state.territory, state, NOW);
  assert.equal(state.territory.battleUnlocked, true);
  assert.equal(Object.values(state.territory.cells).every((cell) => cell.home), true);

  const newcomer = pets[25];
  const entered = claimTerritory(state, newcomer, 0, 0, new Date(NOW.getTime() + 100));
  assert.equal(entered.ok, true);
  assert.equal(entered.stolenFromPetId, pets[0].id);
  assert.equal(state.territory.cells['0:0'].home, true);
  assert.equal(newcomer.stats.points, 50);

  const reentered = claimTerritory(state, pets[0], 0, 1, new Date(NOW.getTime() + 200));
  assert.equal(reentered.ok, true);
  assert.equal(state.territory.cells['0:1'].ownerPetId, pets[0].id);
  assert.equal(state.territory.cells['0:1'].home, true);
  assert.equal(pets[0].stats.points, 50);
});
...
test('포화 영토의 같은 건물을 동시에 노려도 오래된 화면의 뒤 요청은 새 소유자까지 연쇄 탈취하지 않는다', async () => {
  ...
  const body = (requestId) => ({
    row: 0, col: 0, requestId, seasonId: state.territory.seasonId, expectedOwnerPetId: owner.id
  });
  ...
  assert.ok([first.id, second.id].includes(saved.territory.cells['0:0'].ownerPetId));
});
...
test('후반 성능·종료 정리·영토 포화 재진입 코드가 배포 소스에 연결되어 있다', async () => {
  ...
  assert.match(app, /25칸이 모두 각자의 본진이어도 예외 없이 서로 탈취/);
