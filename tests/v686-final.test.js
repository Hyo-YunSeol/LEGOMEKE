...
test('v6.9.0 정적 자산·Worker 버전은 690으로 전부 동기화된다', async () => {
  ...
  assert.match(html, /app\.js\?v=610110/);
  assert.match(html, /styles\.css\?v=610110/);
  assert.match(app, /sw\.js\?v=610110/);
  assert.match(app, /\.svg\?v=610110/);
  assert.match(sw, /lego-life-v610110-battle-tetris-sync-final/);
  assert.match(sw, /const VERSION = '610110'/);
  assert.match(worker, /const APP_VERSION = '6\.10\.10-battle-tetris-sync-final'/);
});
