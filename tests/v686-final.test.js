...
test('v6.8.7 정적 자산·Worker 버전은 687으로 전부 동기화된다', async () => {
  ...
  assert.match(app, /\.svg\?v=687/);
  assert.match(sw, /lego-life-v687-final/);
  assert.match(sw, /const VERSION = '687'/);
  assert.match(worker, /const APP_VERSION = '6\.8\.7-final'/);
});
