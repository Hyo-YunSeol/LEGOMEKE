...
test('v6.10.25 정적 자산·Worker 버전은 6101280으로 전부 동기화된다', async () => {
  const [html, app, sw, worker] = await Promise.all([readFile(HTML,'utf8'),readFile(APP,'utf8'),readFile(SW,'utf8'),readFile(WORKER,'utf8')]);
  assert.match(html, /app\.js\?v=6101280/);
  assert.match(html, /styles\.css\?v=6101280/);
  assert.match(app, /sw\.js\?v=6101280/);
  assert.match(app, /\.svg\?v=6101280/);
  assert.match(sw, /lego-life-v6101280-full-fix-final/);
  assert.match(sw, /const VERSION = '6101280'/);
  assert.match(worker, /const APP_VERSION = '6\.10\.28-full-fix-final'/);
});
