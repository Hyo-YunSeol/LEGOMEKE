...
test('v6.10.25 게임 순위 주요 묶음은 3열이고 커플·찌르기는 더보기 전 렌더링하지 않는다', async () => {
  const [app, css] = await Promise.all([readFile(APP, 'utf8'), readFile(CSS, 'utf8')]);
  assert.match(css, /\.ranking-section \.game-rank-grid \{ grid-template-columns:repeat\(3,minmax\(0,1fr\)\) !important;/);
...
test('v6.10.25 정적 자산·Worker 버전은 6101250으로 전부 동기화된다', async () => {
  const [html, app, sw, worker] = await Promise.all([readFile(HTML,'utf8'),readFile(APP,'utf8'),readFile(SW,'utf8'),readFile(WORKER,'utf8')]);
  assert.match(html, /app\.js\?v=6101250/);
  assert.match(html, /styles\.css\?v=6101250/);
  assert.match(app, /sw\.js\?v=6101250/);
  assert.match(app, /\.svg\?v=6101250/);
  assert.match(sw, /lego-life-v6101250-safe-fixes-final/);
  assert.match(sw, /const VERSION = '6101250'/);
  assert.match(worker, /const APP_VERSION = '6\.10\.25-safe-fixes-final'/);
});
