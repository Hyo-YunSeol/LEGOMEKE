...
test('v6.10.23 게임 순위 주요 묶음은 3열이고 커플·찌르기는 더보기 전 렌더링하지 않는다', async () => {
...
test('v6.9.0 정적 자산·Worker 버전은 690으로 전부 동기화된다', async () => {
  ...
  assert.match(html, /app\.js\?v=6101231/);
  assert.match(html, /styles\.css\?v=6101231/);
  assert.match(app, /sw\.js\?v=6101231/);
  assert.match(app, /\.svg\?v=6101231/);
  assert.match(sw, /lego-life-v6101231-legodoku-final/);
  assert.match(sw, /const VERSION = '6101231'/);
  assert.match(worker, /const APP_VERSION = '6\.10\.23-legodoku-final'/);
});
