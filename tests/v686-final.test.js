...
test('v6.9.0 정적 자산·Worker 버전은 690으로 전부 동기화된다', async () => {
  ...
  assert.match(html, /app\.js\?v=6101232/);
  assert.match(html, /styles\.css\?v=6101232/);
  assert.match(app, /sw\.js\?v=6101232/);
  assert.match(app, /\.svg\?v=6101232/);
  assert.match(sw, /lego-life-v6101232-legodoku-sichuan-final/);
  assert.match(sw, /const VERSION = '6101232'/);
  assert.match(worker, /const APP_VERSION = '6\.10\.23-legodoku-sichuan-final'/);
});
