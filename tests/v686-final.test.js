...
test('v6.8.6 테트리스 시작 직후 lastAttack=null은 공격 알림 렌더링을 중단시키지 않는다', async () => {
...
test('v6.8.6 낮은 가로 모바일 화면도 테트리스 판과 조작부를 별도로 축소한다', async () => {
...
test('v6.8.6 게임 순위 4종은 2×2이고 커플·찌르기는 더보기 전 렌더링하지 않는다', async () => {
...
test('v6.8.6 비정상 테트리스 부분 상태도 화면 전체 오류 대신 안전 복구한다', async () => {
  const app = await readFile(APP, 'utf8');
...
test('v6.8.6 정적 자산·Worker 버전은 686으로 전부 동기화된다', async () => {
  const [html, app, sw, worker] = await Promise.all([readFile(HTML,'utf8'),readFile(APP,'utf8'),readFile(SW,'utf8'),readFile(WORKER,'utf8')]);
  assert.match(html, /app\.js\?v=686/);
  assert.match(html, /styles\.css\?v=686/);
  assert.match(app, /sw\.js\?v=686/);
  assert.match(app, /\.svg\?v=686/);
  assert.match(sw, /lego-life-v686-final/);
  assert.match(sw, /const VERSION = '686'/);
  assert.match(worker, /const APP_VERSION = '6\.8\.6-final'/);
});
