...
test('v6.8.9 정적 자산·Worker 버전은 689로 전부 동기화된다', async () => {
  ...
  assert.match(app, /\.svg\?v=68946/);
  assert.match(sw, /lego-life-v68946-final/);
  assert.match(sw, /const VERSION = '68946'/);
  assert.match(worker, /const APP_VERSION = '6\.8\.9-final'/);
});
