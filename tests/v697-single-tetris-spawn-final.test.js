...
test('v6.9.9 정적 자산과 서비스워커 캐시 버전이 일치한다', () => {
  assert.match(index, /styles\.css\?v=69901/);
  assert.match(index, /app\.js\?v=69901/);
  assert.match(app, /sw\.js\?v=69901/);
  assert.match(sw, /const CACHE = 'lego-life-v69901-single-tetris-viewport-final'/);
  assert.match(sw, /const VERSION = '69901'/);
});
