...
test('확정 체형 28단계와 배고픔 완화',()=>{
  assert.equal(BODY_STAGES.length,28);
  assert.deepEqual(BODY_STAGES.map(x=>x.label), ['마름레고','보통레고','육덕레고','짭덥레고','멸떱레고','통통레고','비떱레고','뚱뚱레고','세자리레고','빅빅우먼레고','로얄비떱레고','하마레고','코끼리레고','맘모스레고','매태지레고','돼룩돼룩레고','초대룩레고','돼황레고','괴수돼지레고','침대파괴돼지레고','재난문자돼지레고','국가비상돼지레고','프로토케라톱스레고','트리케라톱스레고','스테고사우루스레고','브라키오사우루스레고','파타고티탄레고','아르헨티노사우루스레고']);
  assert.equal(interactionHungerCostForBody(4520),6);
  assert.deepEqual(lifeHungerCostsForBody(4520),{work:14,rest:7,exercise:20});
});

test('파손 시스템 제거와 영토 5x5/탈취비',()=>{ assert.equal(TERRITORY_SIZE,5); assert.equal(TERRITORY_STEAL_COST,20); });
