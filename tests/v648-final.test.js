...
import { BODY_STAGES, BODY_ADVANCEMENTS, BODY_ADVANCEMENT_BODY, TERRITORY_SIZE, TERRITORY_STEAL_COST } from '../src/game/constants.js';
...
test('확정 체형은 레비아탄까지 34단계이며 이후 전직 11종으로 이어진다',()=>{
  assert.equal(BODY_STAGES.length,34);
  assert.equal(BODY_STAGES.at(-1).label,'레비아탄레고');
  assert.equal(BODY_STAGES.at(-1).min,BODY_ADVANCEMENT_BODY);
  ...
  assert.deepEqual(lifeHungerCostsForBody(4520),{work:6,rest:3,exercise:8});
  assert.deepEqual(lifeHungerCostsForBody(8750),{work:7,rest:4,exercise:9});
});

test('파손 시스템 제거와 영토 6x6/탈취비',()=>{ assert.equal(TERRITORY_SIZE,6); assert.equal(TERRITORY_STEAL_COST,50); });
