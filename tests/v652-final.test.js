...
test('서비스워커와 정적·이미지 자산 캐시버전은 690으로 정확히 동기화된다',async()=>{const sw=await readFile(new URL('../public/sw.js',import.meta.url),'utf8');const html=await readFile(new URL('../public/index.html',import.meta.url),'utf8');const app=await readFile(new URL('../public/app.js',import.meta.url),'utf8');assert.match(sw,/lego-life-v69201-minesweeper-input-final/);assert.match(sw,/const VERSION = '69201'/);assert.match(sw,/versioned\('\/styles\.css'\)/);assert.match(sw,/versioned\(`\/pets\/\$\{name\}\.svg`\)/);assert.match(sw,/versioned\(`\/flex\/\$\{name\}\.svg`\)/);assert.match(html,/app\.js\?v=69201/);assert.match(app,/sw\.js\?v=69201/);assert.match(app,/pets\/\$\{esc\(stage\.assetKey \|\| stage\.key\)\}\.svg\?v=69201/);assert.match(app,/flex\/\$\{esc\(flexItem\.assetKey\)\}\.svg\?v=69201/);});
