...
test('서비스워커와 정적 자산 캐시버전은 661로 동기화된다',async()=>{const sw=await readFile(new URL('../public/sw.js',import.meta.url),'utf8');const html=await readFile(new URL('../public/index.html',import.meta.url),'utf8');const app=await readFile(new URL('../public/app.js',import.meta.url),'utf8');assert.match(sw,/lego-life-v661-final/);assert.match(sw,/styles\.css\?v=661/);assert.match(html,/app\.js\?v=661/);assert.match(app,/sw\.js\?v=661/);});
