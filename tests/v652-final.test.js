...
test('하단 5개 메뉴는 캐시된 탭 pane을 show/hide하고 숨은 탭을 미리 렌더하지 않는다',async()=>{const app=await readFile(new URL('../public/app.js',import.meta.url),'utf8');assert.match(app,/function ensureTabPanes/);assert.match(app,/function showTabPane/);assert.match(app,/function renderTab/);assert.doesNotMatch(app,/function warmInactiveTabs/);assert.match(app,/showTabPane\(tabName\)/);assert.doesNotMatch(app,/if \(app\.tab === 'home'\) view\.innerHTML/);assert.match(app,/bottomNav\?\.addEventListener\('pointerdown'/);});
...
test('서비스워커와 정적 자산 캐시버전은 660으로 동기화된다',async()=>{const sw=await readFile(new URL('../public/sw.js',import.meta.url),'utf8');const html=await readFile(new URL('../public/index.html',import.meta.url),'utf8');const app=await readFile(new URL('../public/app.js',import.meta.url),'utf8');assert.match(sw,/lego-life-v660-final/);assert.match(sw,/styles\.css\?v=660/);assert.match(html,/app\.js\?v=660/);assert.match(app,/sw\.js\?v=660/);});
