...
test('서비스워커와 정적 자산 캐시버전은 670으로 동기화된다',async()=>{const sw=await readFile(new URL('../public/sw.js',import.meta.url),'utf8');const html=await readFile(new URL('../public/index.html',import.meta.url),'utf8');const app=await readFile(new URL('../public/app.js',import.meta.url),'utf8');assert.match(sw,/lego-life-v670-final/);assert.match(sw,/styles\.css\?v=670/);assert.match(html,/app\.js\?v=670/);assert.match(app,/sw\.js\?v=670/);});
