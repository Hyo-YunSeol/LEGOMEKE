...
test('싱글 테트리스는 개인게임 1회를 시작 즉시 쓰고 3분·고정 700ms 사양을 제공한다', () => {
  assert.equal(SINGLE_TETRIS_DURATION_MS, 180_000);
  ...
  assert.equal(new Date(started.challenge.expiresAt).getTime() - BASE.getTime(), 180_000);
...
test('타임아웃은 3분 전에 제출할 수 없고 3분 후 정상 정산한다', () => {
  const state = stateWithUsers([['u1', '레고']], BASE);
  const pet = state.pets[state.users.u1.currentPetId];
  const first = startMiniGame(state, pet, 'tetrisSingle', BASE);
  const early = finishMiniGame(state, pet, first.challenge.id, null, new Date(BASE.getTime() + 170_000), {
    endReason: 'timeout', lineClearCounts: { 4: 1 }
  });
  assert.equal(early.ok, false);
  assert.equal(state.miniGameChallenges[first.challenge.id].completed, false);
  const normal = finishMiniGame(state, pet, first.challenge.id, null, new Date(BASE.getTime() + 180_100), {
    endReason: 'timeout', lineClearCounts: { 4: 1 }
  });
...
test('싱글 테트리스 클라이언트는 기존 7종·10x20·NEXT·고정 속도·무일시정지·결과 오버레이를 구현한다', () => {
  ...
  assert.match(app, /3분 · 속도 고정[^`]*일시정지 없음/);
...
test('사천성 iPhone 레이아웃은 부모 aspect-ratio/1fr 행 의존을 제거하고 각 셀 자체를 정사각형으로 고정한다', () => {
  ...
  assert.match(boardRule, /grid-auto-rows:auto/);
  assert.match(boardRule, /align-content:start/);
  assert.doesNotMatch(boardRule, /grid-template-rows:repeat\(8/);
  assert.doesNotMatch(boardRule, /aspect-ratio:11\/10/);
  assert.match(cellRule, /height:auto/);
  assert.match(cellRule, /aspect-ratio:1\/1/);
  assert.doesNotMatch(cellRule, /height:100%/);
