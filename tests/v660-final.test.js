...
test('v6.5.3 저장 데이터는 계정과 진행상황을 보존한 채 v6.7.1로 마이그레이션된다', async () => {
  ...
  assert.equal(migrated.meta.version, 21);
...
test('횟수권은 현재 게임 하루 한도만 늘리고 동일 요청은 한 번만 차감·지급된다', () => {
  ...
  assert.equal(pet.stats.points, 1_200);
...
test('상점 요청 ID는 새로고침·재접속에 해당하는 새 서버 인스턴스에서도 한 번만 처리된다', async () => {
  ...
  assert.equal(state.pets[user.currentPetId].stats.points, 700);
