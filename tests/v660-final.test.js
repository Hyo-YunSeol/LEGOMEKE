...
test('v6.5.3 저장 데이터는 계정과 진행상황을 보존한 채 최신 버전으로 마이그레이션된다', async () => {
  ...
  assert.equal(migrated.meta.version, 25);
...
test('번개반응은 서버 수신 시각을 기준으로 기록하고 조작 가능한 클라이언트 값은 랭킹에 쓰지 않는다', () => {
  const state = initialState();
  ...
  assert.equal(accepted.reactionMs, 350);
  assert.equal(accepted.verified, true);
  assert.equal(pet.records.bestReactionMs, 350);
  ...
  assert.equal(blocked.invalidReaction, undefined);
  assert.equal(blocked.reactionMs, 180);
  assert.equal(blocked.reward, 100);
  assert.equal(pet.records.bestReactionMs, 180);
});
...
test('상점 구매 API도 동시 동일 요청을 한 번만 처리한다', async () => {
  ...
  state.pets[user.currentPetId].stats.points = 2_000;
