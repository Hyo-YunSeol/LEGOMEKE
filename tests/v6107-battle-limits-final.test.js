...
import { createDavinciRoom, joinDavinciRoom, setDavinciReady, startDavinciRoom } from '../src/game/davinci.js';
...
test('대전은 기본 30회이고 +20회권 6,000P를 제한 없이 누적 구매한다', () => {
  const { state, a } = setup();
  ...
  assert.equal(one.ok, true);
  assert.equal(two.ok, true);
  assert.equal(one.price, 6_000);
  assert.equal(a.pet.daily.battleBonus, 40);
  assert.equal(battleLimit(a.pet), 70);
});

test('오목·테트리스·사천성·다빈치 실제 시작은 같은 합산 대전 횟수를 각각 1회 사용한다', () => {
  ...
  const davinci = createDavinciRoom(state, a.pet, 100, BASE);
  assert.equal(davinci.ok, true);
  assert.equal(joinDavinciRoom(state, b.pet, davinci.roomId, BASE).ok, true);
  assert.equal(setDavinciReady(state, b.pet, davinci.roomId, true, BASE).ok, true);
  assert.equal(startDavinciRoom(state, a.pet, davinci.roomId, BASE).ok, true);

  assert.equal(a.pet.daily.battlePlayed, 4);
  assert.equal(b.pet.daily.battlePlayed, 4);
});

test('합산 한도를 다 쓰면 네 대전 모두 방 생성부터 막고 포인트 횟수권 구매 후 다시 허용한다', () => {
  const { state, a } = setup();
  a.pet.daily.battlePlayed = 30;
  for (const create of [createOmokRoom, createBlockBattleRoom, createSichuanRoom, createDavinciRoom]) {
