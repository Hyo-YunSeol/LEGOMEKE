...
test('레고도쿠 Worker API는 생성→입장→플레이→관전 양쪽 판 공개→재시작 복구까지 실제 저장 경로로 동작한다', async () => {
  ...
  assert.deepEqual(spectatorView.players[hostPetId].confirmed, [correct], '관전자는 플레이어가 현재 맞힌 레고 위치를 볼 수 있어야 한다');
