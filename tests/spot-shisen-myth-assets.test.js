...
test('틀린그림 입력은 실제 좌표가 있는 pointerdown에 연결되고 focusin에는 연결되지 않는다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  const focusStart = app.indexOf("document.addEventListener('focusin'");
  const focusEnd = app.indexOf("document.addEventListener('focusout'", focusStart);
  const pointerStart = app.indexOf("document.addEventListener('pointerdown'", focusEnd);
  const pointerEnd = app.indexOf("const singleControl", pointerStart);
  assert.ok(focusStart >= 0 && focusEnd > focusStart && pointerStart > focusEnd && pointerEnd > pointerStart);
  const focusBlock = app.slice(focusStart, focusEnd);
  const pointerBlock = app.slice(pointerStart, pointerEnd);
  assert.doesNotMatch(focusBlock, /spot-difference-image/);
  assert.match(pointerBlock, /spot-difference-image/);
  assert.match(pointerBlock, /submitSpotDifferenceClick\(spotPictureButton, event\)/);
});
