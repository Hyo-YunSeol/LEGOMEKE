...
test('순위 2/2/2열은 유지하고 닉네임 글씨를 크게 표시한다',async()=>{const css=await readFile(new URL('../public/styles.css',import.meta.url),'utf8');assert.match(css,/\.ranking-section \.game-rank-grid\s*\{\s*grid-template-columns:repeat\(2/);assert.match(css,/\.ranking-section \.rank-tabs-grid\s*\{\s*grid-template-columns:repeat\(2/);assert.match(css,/font-size:\s*\.88rem !important/);assert.match(css,/@media \(max-width: 520px\)[\s\S]*?font-size:\s*\.78rem !important/);});
