...
test('1대1 자동 낙하와 조작은 서버 객체가 아닌 격리 visualSelf에 정확히 한 번만 적용된다', () => {
  ...
  assert.doesNotMatch(queue, /previewBlockBattleLock\(room, \{ paint: true, mark: true \}\)/);
});
...
test('게임 중 generic refresh는 1대1 테트리스 200셀 DOM을 통째로 재생성하지 않는다', () => {
  assert.match(app, /blockBattleLivePatched = Boolean\(liveBlockBattleRoom\?\.status === 'playing' && patchBlockBattleDynamic\(liveBlockBattleRoom\)\)/);
  assert.match(app, /if \(tab === 'games' && pane\.dataset\.rendered === 'true'\)[\s\S]*patchBlockBattleDynamic\(liveBlockBattleRoom\)[\s\S]*return;/);
  assert.match(app, /liveBlockBattleRoom\?\.status === 'playing' && patchBlockBattleDynamic\(liveBlockBattleRoom\)/);
});
