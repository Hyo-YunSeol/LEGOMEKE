...
test('v6.10.0 캐시 버전은 HTML·앱·서비스워커에 완전히 일치한다', () => {
  assert.equal(JSON.parse(pkg).version, '6.10.0');
  assert.match(index, /styles\.css\?v=610001/);
  assert.match(index, /app\.js\?v=610001/);
  assert.match(app, /sw\.js\?v=610001/);
  assert.match(sw, /const CACHE = 'lego-life-v610001-sichuan-room-cleanup-final'/);
