...
test('v6.10.3 캐시 버전은 HTML·앱·서비스워커에 완전히 일치한다', () => {
  assert.equal(JSON.parse(pkg).version, '6.10.3');
  assert.match(index, /styles\.css\?v=610103/);
  assert.match(index, /app\.js\?v=610103/);
  assert.match(app, /sw\.js\?v=610103/);
  assert.match(sw, /const CACHE = 'lego-life-v610103-apple-territory-final'/);
