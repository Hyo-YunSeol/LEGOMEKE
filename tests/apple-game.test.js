...
test('사과게임 보상은 2개 제거 5P, 3개 이상 제거 6P이며 랭킹 점수 계산은 그대로다', () => {
  const two = { completed: false, expiresAt: new Date(BASE.getTime() + 120_000).toISOString(), appleBoard: Array.from({ length: 10 }, () => Array(10).fill(null)), applePendingPoints: 0, appleScore: 0, appleRemovedCount: 0, appleSuccesses: 0, appleProcessedRequestIds: [] };
  two.appleBoard[0][0] = 4; two.appleBoard[0][1] = 6;
  const twoResult = selectAppleRectangle(two, { startRow: 0, startCol: 0, endRow: 0, endCol: 1 }, 'two-cells', BASE);
  assert.equal(twoResult.gainedPoints, 5);
  assert.equal(two.appleScore, 40);

  const three = { completed: false, expiresAt: new Date(BASE.getTime() + 120_000).toISOString(), appleBoard: Array.from({ length: 10 }, () => Array(10).fill(null)), applePendingPoints: 0, appleScore: 0, appleRemovedCount: 0, appleSuccesses: 0, appleProcessedRequestIds: [] };
  three.appleBoard[0][0] = 1; three.appleBoard[0][1] = 2; three.appleBoard[0][2] = 7;
  const threeResult = selectAppleRectangle(three, { startRow: 0, startCol: 0, endRow: 0, endCol: 2 }, 'three-cells', BASE);
  assert.equal(threeResult.gainedPoints, 6);
  assert.equal(three.appleScore, 60);
});
...
test('합 10만 제거하고 중복 요청은 다시 지급하지 않는다', () => {
  ...
  assert.equal(result.gainedPoints, 6);
  assert.equal(challenge.applePendingPoints, 6);
...
test('사과게임 포인트는 서버 상태에서 계산하고 종료 시 한 번만 정산한다', () => {
  ...
  assert.equal(selected.boardRefreshed, true);
  assert.equal(selected.boardsGenerated, 2);
  assert.equal(pet.stats.points, 1000);
  assert.equal(pet.daily.miniGamesPlayed, 0);
  assert.equal(pet.records.appleBestScore, 0);
...
test('게임 랭킹 뷰는 TOP 5 밖 사용자의 사과게임 전체 순위를 내 기록으로 별도 제공한다', () => {
  const state = stateWithUsers([
    ['u1', 'A'], ['u2', 'B'], ['u3', 'C'], ['u4', 'D'], ['u5', 'E'], ['u6', 'F'], ['u7', 'G']
  ], BASE);
  for (let index = 0; index < 7; index += 1) {
    const userId = `u${index + 1}`;
    const pet = state.pets[state.users[userId].currentPetId];
    pet.records.appleBestScore = 700 - index * 100;
    pet.records.appleBestAt = new Date(BASE.getTime() + index * 1000).toISOString();
  }
  const viewerPet = state.pets[state.users.u7.currentPetId];
  const view = rankingsView(state, viewerPet.id);
  assert.equal(view.apple.length, 5);
  assert.equal(view.apple.some((item) => item.petId === viewerPet.id), false);
  assert.equal(view.myGameRanks.apple.petId, viewerPet.id);
  assert.equal(view.myGameRanks.apple.rank, 7);
  assert.equal(view.myGameRanks.apple.score, 100);
});
