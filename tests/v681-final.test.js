...
test('블록게임 클릭은 전역 busy를 거치지 않고 낙관 큐로 즉시 갱신하며 서버는 순차 전송한다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(app, /function queueBlockGameSelection/);
  assert.match(app, /const preview = previewBlockGameSelection/);
  assert.match(app, /app\.blockGameInputQueue\.push\(/);
  assert.match(app, /refreshBlockMiniOnly\(\);\s*drainBlockGameInputQueue\(\)/);
  assert.match(app, /async function drainBlockGameInputQueue/);
  assert.match(app, /if \(app\.blockGameInputInFlight \|\| !app\.blockGameInputQueue\.length\) return;/);
  assert.match(app, /boardVersion: preview\.boardVersion/);
  assert.match(app, /requestId/);
  assert.doesNotMatch(app.slice(app.indexOf('function queueBlockGameSelection'), app.indexOf('function minesweeperTimeText')), /perform\(/);
});
