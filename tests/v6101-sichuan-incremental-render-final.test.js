...
test('v6.10.4 캐시·HTML·Worker 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.8');
  assert.match(index, /styles\.css\?v=610108/);
  assert.match(index, /app\.js\?v=610108/);
  assert.match(app, /sw\.js\?v=610108/);
  assert.match(sw, /lego-life-v610108-records-sichuan-pc-tetris-final/);
  assert.match(sw, /const VERSION = '610108'/);
  assert.match(worker, /APP_VERSION = '6\.10\.8-records-sichuan-pc-tetris-final'/);
});
