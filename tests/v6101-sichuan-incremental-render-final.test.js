...
test('v6.10.4 캐시·HTML·Worker 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.12');
  assert.match(index, /styles\.css\?v=610112/);
  assert.match(index, /app\.js\?v=610112/);
  assert.match(app, /sw\.js\?v=610112/);
  assert.match(sw, /lego-life-v610112-battle-reset-tetris-stable-final/);
  assert.match(sw, /const VERSION = '610112'/);
  assert.match(worker, /APP_VERSION = '6\.10\.12-battle-reset-tetris-stable-final'/);
});
