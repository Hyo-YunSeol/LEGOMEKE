...
test('v6.10.4 캐시·HTML·Worker 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.4');
  assert.match(index, /styles\.css\?v=610104/);
  assert.match(index, /app\.js\?v=610104/);
  assert.match(app, /sw\.js\?v=610104/);
  assert.match(sw, /lego-life-v610104-apple-territory-final/);
  assert.match(sw, /const VERSION = '610104'/);
  assert.match(worker, /APP_VERSION = '6\.10\.4-apple-territory-final'/);
});
