...
test('v6.10.1 캐시·HTML·Worker 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.1');
  assert.match(index, /styles\.css\?v=610101/);
  assert.match(index, /app\.js\?v=610101/);
  assert.match(app, /sw\.js\?v=610101/);
  assert.match(sw, /lego-life-v610101-sichuan-incremental-render-final/);
  assert.match(sw, /const VERSION = '610101'/);
  assert.match(worker, /APP_VERSION = '6\.10\.1-sichuan-incremental-render-final'/);
});
