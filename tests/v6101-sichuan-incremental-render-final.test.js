...
test('v6.10.4 캐시·HTML·Worker 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.16');
  assert.match(index, /styles\.css\?v=610116/);
  assert.match(index, /app\.js\?v=610116/);
  assert.match(app, /sw\.js\?v=610116/);
  assert.match(sw, /lego-life-v61016-mobile-tetris-timer-spot-hard-final/);
  assert.match(sw, /const VERSION = '610116'/);
