...
test('v6.10.3 캐시·HTML·Worker 버전이 모두 일치한다', () => {
  assert.equal(pkg.version, '6.10.3');
  assert.match(index, /styles\.css\?v=610103/);
  assert.match(index, /app\.js\?v=610103/);
  assert.match(app, /sw\.js\?v=610103/);
  assert.match(sw, /lego-life-v610103-apple-territory-final/);
  assert.match(sw, /const VERSION = '610103'/);
  assert.match(worker, /APP_VERSION = '6\.10\.3-apple-territory-final'/);
});
