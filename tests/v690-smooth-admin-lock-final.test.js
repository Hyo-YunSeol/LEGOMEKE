...
test('테트리스 클라이언트는 서버 gravityDueAt에 타이머를 맞추고 미확정 낙하 입력 재생 때 기준시각을 보존한다', async () => {
  ...
  assert.match(appSource, /const actions = awaitingLock \? pendingActions : \[\.\.\.pendingActions, \.\.\.app\.blockBattleInputBuffer\]/);
  assert.match(appSource, /const willLock = action === 'hardDrop'/);
