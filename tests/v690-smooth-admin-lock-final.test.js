...
test('테트리스 클라이언트는 서버 gravityDueAt에 맞추되 자동 tick은 ACK 전 화면에서 재생하지 않는다', async () => {
  ...
  assert.match(appSource, /const BLOCK_BATTLE_VERTICAL_ACTIONS = new Set\(\['softDrop', 'hardDrop', 'tick'\]\)/);
  assert.match(appSource, /function blockBattleHasUnconfirmedVerticalInput\(\)/);
  assert.match(appSource, /function blockBattleAwaitingLock\(\)/);
  assert.match(appSource, /const awaitingLock = roomPendingBatches\.some\(\(batch\) => batch\.expectsLock\)/);
  assert.match(appSource, /const replayablePendingActions = pendingActions\.filter\(\(action\) => action !== 'tick'\)/);
  assert.match(appSource, /app\.blockBattleInputBuffer\.filter\(\(action\) => action !== 'tick'\)/);
  assert.match(appSource, /const willLock = action === 'hardDrop'[\s\S]*?action === 'softDrop'/);
  assert.doesNotMatch(appSource, /else if \(action === 'tick'\) \{\s*previewBlockBattleInput\(action\)/);
  assert.match(appSource, /if \(willLock\) app\.blockBattleLockQueued = true/);
  assert.match(appSource, /if \(BLOCK_BATTLE_VERTICAL_ACTIONS\.has\(action\)[\s\S]*?flushBlockBattleInputs\(\)/);
