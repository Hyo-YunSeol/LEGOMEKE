...
test('운영자 계정 잠금은 기존 토큰과 재로그인을 차단하고 잠금 해제 뒤 새 로그인만 허용한다', async () => {
  ...
  targetPet.stats.points = 987654;
  targetPet.records.appleBestScore = 4321;
  ...
  assert.equal(locked.data.bootstrap.residents.some((profile) => profile.id === targetPet.id), false, '잠금 계정은 주민목록에서 즉시 숨겨야 한다');
  assert.equal(JSON.stringify(locked.data.bootstrap.rankings).includes(targetPet.id), false, '잠금 계정은 모든 공개 랭킹 데이터에서 숨겨야 한다');
  assert.equal(locked.data.bootstrap.admin.members.some((member) => member.userId === targetUser.id && member.accountLocked), true, '관리자 화면에는 잠금 계정을 유지해야 한다');
  ...
  assert.equal(unlocked.data.bootstrap.residents.some((profile) => profile.id === targetPet.id), true, '잠금 해제하면 주민목록에 기존 데이터가 복귀해야 한다');
  assert.equal(JSON.stringify(unlocked.data.bootstrap.rankings).includes(targetPet.id), true, '잠금 해제하면 기존 랭킹 기록이 복귀해야 한다');
...
test('테트리스 클라이언트는 격리 visual state와 단일 700ms timer를 쓰고 서버 fallback은 입력 뒤에만 처리한다', async () => {
  ...
  assert.match(appSource, /서버 왕복을 기다리지 않고 화면용 visualSelf/);
  assert.match(appSource, /if \(willLock\) previewBlockBattleLock\(room, \{ paint: true, mark: true \}\)/);
