...
test('1대1 Space는 새 물리 keydown을 ACK 전에도 보존하고 키 반복·합성 click만 차단한다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  ...
  assert.match(app, /if \(!awaitingHardDrop \|\| predictedLock\)/);
  assert.match(app, /app\.blockBattleInputBuffer\.push\(\{ action, piece: inputPiece, seq: \+\+app\.blockBattleInputSeq \}\)/);
  assert.match(app, /if \(!app\.blockBattleSending && app\.blockBattleInputBuffer\.length && !app\.blockBattleFlushTimer\)/);
