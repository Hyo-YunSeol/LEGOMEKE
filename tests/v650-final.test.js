...
test('영토 최대 보유 수는 2레벨마다 1칸씩 증가하고 5×5 맵 최대 25칸에서 멈춘다',()=>{
  assert.equal(TERRITORY_SIZE,5); assert.equal(TERRITORY_STEAL_COST,50);
  for(const [level,limit] of [[1,1],[2,1],[3,2],[5,3],[10,5],[14,7],[20,10],[24,12],[25,13],[48,24],[49,25],[999,25]]) assert.equal(territoryLimitForLevel(level),limit);
  assert.deepEqual(nextTerritoryUpgrade(5),{level:7,limit:4}); assert.deepEqual(nextTerritoryUpgrade(24),{level:25,limit:13}); assert.deepEqual(nextTerritoryUpgrade(48),{level:49,limit:25}); assert.equal(nextTerritoryUpgrade(49),null);
});
...
test('영토는 상대 땅 탈취 때 50P를 소모하고 기존 전쟁 룰을 유지한다',()=>{
  ...
  a.stats.legoPower=levelUpperBound(8)+1;b.stats.legoPower=levelUpperBound(8)+1;a.stats.points=100;b.stats.points=100;state.territory=initialTerritory(BASE);
