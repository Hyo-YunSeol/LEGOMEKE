...
test('영토는 실제 레벨 한도를 적용하고 상대 땅 탈취 때만 정확히 20P를 소모한다', () => {
  ...
  const blocked=claimTerritory(state,a,1,1,new Date(BASE.getTime()+7000));
  assert.equal(blocked.ok,false,'보유 한도가 꽉 차면 새 점령/탈취를 막아야 한다');
  assert.match(blocked.message,/최대 5칸/);
  assert.equal(a.stats.points,before,'한도 때문에 막힌 탈취에서는 포인트를 차감하지 않는다');
  delete state.territory.cells['3:4']; // 내 땅을 하나 잃어 4/5가 된 상황
  const steal=claimTerritory(state,a,1,1,new Date(BASE.getTime()+8000));
...
test('하단 메뉴는 API 처리 중에도 눌릴 수 있고 직접 포인터/클릭 처리한다', async () => {
  ...
  assert.match(app,/function activateBottomNav/);
  assert.match(app,/bottomNav\?\.addEventListener\('pointerup'[\s\S]*capture:\s*true/);
  assert.match(app,/if \(app\.modal\) closeModal\(\)/);
  const css=await readFile(new URL('../public/styles.css',import.meta.url),'utf8');
  const final=css.slice(css.lastIndexOf('/* v6.5.0 최종'));
  assert.match(final,/\.bottom-nav \{[\s\S]*?z-index:120/,'하단 메뉴는 모달보다 위에 있어 게임창에서도 즉시 탭 전환 가능해야 한다');
});
