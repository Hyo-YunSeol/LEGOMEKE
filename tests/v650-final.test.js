...
test('영토 최대 보유 수는 레벨마다 1칸씩 증가하고 5×5 맵 최대 25칸에서 멈춘다',()=>{
  assert.equal(TERRITORY_SIZE,5); assert.equal(TERRITORY_STEAL_COST,50);
  for(const [level,limit] of [[1,1],[2,2],[5,5],[10,10],[14,14],[20,20],[24,24],[25,25],[31,25],[999,25]]) assert.equal(territoryLimitForLevel(level),limit);
  assert.deepEqual(nextTerritoryUpgrade(5),{level:6,limit:6}); assert.deepEqual(nextTerritoryUpgrade(24),{level:25,limit:25}); assert.equal(nextTerritoryUpgrade(25),null);
});
...
test('영토는 상대 땅 탈취 때 50P를 소모하고 기존 전쟁 룰을 유지한다',()=>{
  ...
  a.stats.legoPower=levelUpperBound(4)+1;b.stats.legoPower=levelUpperBound(4)+1;a.stats.points=100;b.stats.points=100;state.territory=initialTerritory(BASE);
