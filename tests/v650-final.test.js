...
test('영토 최대 보유 수는 확정 레벨 구간대로 증가하고 10칸에서 멈춘다',()=>{
  assert.equal(TERRITORY_SIZE,5); assert.equal(TERRITORY_STEAL_COST,20);
  for(const [level,limit] of [[1,1],[2,2],[3,3],[4,4],[5,4],[6,5],[8,5],[9,6],[12,6],[13,7],[17,7],[18,8],[23,8],[24,9],[30,9],[31,10],[999,10]]) assert.equal(territoryLimitForLevel(level),limit);
  assert.deepEqual(nextTerritoryUpgrade(5),{level:6,limit:5}); assert.equal(nextTerritoryUpgrade(31),null);
});

test('영토는 상대 땅 탈취 때 20P를 소모하고 기존 전쟁 룰을 유지한다',()=>{
  ...
  a.stats.legoPower=levelUpperBound(5)+1;b.stats.legoPower=levelUpperBound(5)+1;a.stats.points=100;b.stats.points=100;state.territory=initialTerritory(BASE);
  ...
  const result=claimTerritory(state,a,1,1,new Date(BASE.getTime()+8000)); assert.equal(result.ok,true);assert.equal(result.stolenFromPetId,b.id);assert.equal(a.stats.points,80);assert.equal(territoryView(state,a.id).my.owned,5);
});
...
test('도감 28단계 자산은 투명 배경 SVG로 연결된다',async()=>{
  assert.equal(BODY_STAGES.length,28); for(const [index,stage] of BODY_STAGES.entries()){const name=stage.assetKey||stage.key;const svg=await readFile(new URL(`../public/pets/${name}.svg`,import.meta.url),'utf8');assert.match(svg,/<svg/);assert.doesNotMatch(svg,/<rect\s+width="(?:220|240)"\s+height="(?:220|240)"/,`stage ${index+1}`);}
});

test('기존 저장 데이터의 파손/상점 필드는 스키마 정리 때 제거된다',()=>{const {pet}=stateWithPet('legacy','레거시');pet.integrity={broken:true};pet.cosmetics={auroraUntil:BASE.toISOString()};pet.cosmeticExpiryNotices={aurora:'x'};ensurePetSchema(pet,BASE);assert.equal('integrity'in pet,false);assert.equal('cosmetics'in pet,false);assert.equal('cosmeticExpiryNotices'in pet,false);});
