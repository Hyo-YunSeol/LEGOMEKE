...
import { BODY_STAGES, BODY_ADVANCEMENTS, TERRITORY_SIZE, TERRITORY_STEAL_COST } from '../src/game/constants.js';
...
test('영토 최대 보유 수는 3레벨마다 1칸씩 증가하고 최대 10칸에서 멈춘다',()=>{
  assert.equal(TERRITORY_SIZE,6); assert.equal(TERRITORY_STEAL_COST,50);
...
test('도감 34단계와 레비아탄 전직 11종 자산은 SVG로 연결된다',async()=>{
  assert.equal(BODY_STAGES.length,34);
  for(const [index,stage] of BODY_STAGES.entries()){const name=stage.assetKey||stage.key;const svg=await readFile(new URL(`../public/pets/${name}.svg`,import.meta.url),'utf8');assert.match(svg,/<svg/);assert.doesNotMatch(svg,/<script/i,`stage ${index+1}`);}
  for(const advancement of Object.values(BODY_ADVANCEMENTS)){const svg=await readFile(new URL(`../public/pets/${advancement.assetKey}.svg`,import.meta.url),'utf8');assert.match(svg,/<svg/);assert.doesNotMatch(svg,/<script/i);}
});
