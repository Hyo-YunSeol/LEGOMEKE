...
test('도감 46단계 자산은 투명 배경 SVG로 연결된다',async()=>{
  assert.equal(BODY_STAGES.length,46); for(const [index,stage] of BODY_STAGES.entries()){const name=stage.assetKey||stage.key;const svg=await readFile(new URL(`../public/pets/${name}.svg`,import.meta.url),'utf8');assert.match(svg,/<svg/);assert.doesNotMatch(svg,/<rect\s+width="(?:220|240)"\s+height="(?:220|240)"/,`stage ${index+1}`);}
});
