...
import { createOmokRoom, joinOmokRoom, playOmokMove, omokView, requestOmokRematch } from '../src/game/omok.js';
...
test('시즌 1위는 다음 시즌 종료까지 왕 뱃지를 받는다', () => {
  ...
  a.records.seasonBestReactionMs = 180; a.records.seasonBestReactionAt = '2026-08-08T00:00:00.000Z';
  b.records.seasonBestReactionMs = 220; b.records.seasonBestReactionAt = '2026-08-08T00:00:01.000Z';
  ...
  assert.deepEqual(badges.sort(), ['⚡왕','⚫오목왕','🍎왕'].sort());
});
...
test('모바일 레이아웃과 5x5 영토 CSS 회귀', async () => {
  ...
  assert.match(css, /v6\.5\.0 최종[\s\S]*?@media \(max-width:520px\)[\s\S]*?\.resident-grid\s*\{\s*grid-template-columns:repeat\(5/);
  assert.match(css, /\.ranking-section \.game-rank-grid\s*\{\s*grid-template-columns:repeat\(3/);
  assert.match(css, /\.ranking-section \.rank-tabs-grid\s*\{\s*grid-template-columns:repeat\(2/);
  assert.match(css, /\.daily-goals-compact \.goal-list\s*\{\s*grid-template-columns:\s*repeat\(5/);
  assert.match(css, /\.cos-aura \.pet-visual img/);
});
...
test('프론트에는 마지막 수, 결과창, 1초 리액션, 터치 네비 보강 코드가 포함된다', async () => {
  ...
  assert.match(app, /bottomNav\?\.addEventListener\('pointerup'/);
