...
test('대전 +20회권은 서로 다른 요청 ID로 세 번 연속 구매하면 매번 4,000P와 +20회가 정확히 누적된다', () => {
...
test('같은 대전권 요청 ID를 재전송하면 중복결제 없이 첫 결과만 돌려준다', () => {
  ...
  assert.equal(pet.stats.points, 96_000);
});
...
test('1대1 테트리스 Space는 같은 물리 입력 중복만 막고 ACK 전 다음 블록의 새 keydown은 보존한다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.doesNotMatch(app, /if \(awaitingLock && predictedLock && willLock\) return false;/);
  assert.match(app, /const importantAction = action === 'hardDrop' \|\| action === 'rotate';[\s\S]*if \(unconfirmedActions >= actionLimit\) return false;/);
  assert.match(app, /blockBattleKeyboardSuppressClickUntil: 0/);
  assert.match(app, /if \(event\.repeat\) return;/);
  assert.match(app, /if \(action === 'hardDrop'\) app\.blockBattleKeyboardSuppressClickUntil = Date\.now\(\) \+ 450;/);
  assert.match(app, /event\?\.detail === 0 && Date\.now\(\) <= Number\(app\.blockBattleKeyboardSuppressClickUntil \|\| 0\)/);
  assert.match(app, /anotherPredictedLock/);
});
