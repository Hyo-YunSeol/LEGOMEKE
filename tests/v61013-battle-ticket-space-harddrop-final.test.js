...
test('1대1 테트리스 Space는 keydown 1회당 현재 블록 hardDrop 1회만 처리하고 다음 블록 예약 입력을 만들지 않는다', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.doesNotMatch(app, /blockBattleDeferredHardDrops|BLOCK_BATTLE_MAX_DEFERRED_HARD_DROPS|drainBlockBattleDeferredHardDrop/);
