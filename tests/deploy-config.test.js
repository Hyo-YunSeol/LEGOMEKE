...
test('패키지, Worker, 서비스워커 버전이 v6.4.5로 통일되어 있다', async () => {
  const pkg = await json('package.json');
  const lock = await json('package-lock.json');
  const worker = await text('src/worker.js');
  const sw = await text('public/sw.js');
  assert.equal(pkg.version, '6.4.5');
  assert.equal(lock.version, '6.4.5');
  assert.equal(lock.packages[''].version, '6.4.5');
  ...
  assert.match(worker, /6\.4\.5-final/);
  assert.match(sw, /lego-life-v645-final/);
});

test('몸집 이미지와 서비스워커 캐시는 32단계 장기 체형의 실제 assetKey를 모두 포함한다', async () => {
  const sw = await text('public/sw.js');
  ...
  assert.equal(BODY_STAGES[12].label, '프로토케라톱스레고');
