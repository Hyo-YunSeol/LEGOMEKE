...
test('패키지, Worker, 서비스워커 버전이 v6.4.0으로 통일되어 있다', async () => {
  const pkg = await json('package.json');
  const lock = await json('package-lock.json');
  const worker = await text('src/worker.js');
  const sw = await text('public/sw.js');
  assert.equal(pkg.version, '6.4.0');
  assert.equal(lock.version, '6.4.0');
  assert.equal(lock.packages[''].version, '6.4.0');
  assert.equal(pkg.engines.node, '>=22');
  assert.equal(pkg.scripts.verify, 'npm run check && npm test');
  assert.match(pkg.scripts.test, /territory\.test\.js/);
  assert.match(pkg.scripts['deploy:cloudflare'], /wrangler@4\.114\.0 deploy/);
  assert.match(worker, /6\.4\.0-final/);
  assert.match(sw, /lego-life-v640-final/);
});

test('몸집 이미지와 서비스워커 캐시가 모든 최종 체형을 포함한다', async () => {
  const sw = await text('public/sw.js');
  for (const stage of BODY_STAGES) {
    await access(new URL(`../public/pets/${stage.key}.svg`, import.meta.url));
    assert.match(sw, new RegExp(`/pets/${stage.key}\\.svg`));
  }
  assert.deepEqual(BODY_STAGES.map((stage) => stage.label), [
    '마름레고', '보통레고', '짭덥레고', '통통레고',
    '떱레고', '리얼떱레고', '돼지레고', '코끼리레고',
    '맘모스레고', '돼룩돼룩레고'
  ]);
});
...
test('벙·라이어게임·영토전 최종 수치가 일치한다', () => {
  ...
  assert.equal(TERRITORY_GOLDEN_REVEAL_HOURS_BEFORE_END, 2);
  assert.deepEqual([1, 2, 3, 4, 5, 6, 7, 8, 20].map(territoryLimitForLevel), [1, 2, 3, 4, 4, 4, 4, 4, 4]);
});

test('최종 모바일 UI에는 확정 기능이 노출되고 삭제 기능은 노출되지 않는다', async () => {
...
test('삭제 기능의 API와 런타임 로직은 Worker에서 제거되어 있다', async () => {
  const worker = await text('src/worker.js');
...
test('삭제된 과거 필드는 마이그레이션에서만 정리되고 현재 상태에는 다시 생성되지 않는다', async () => {
  ...
  assert.match(store, /meta:\s*\{\s*version:\s*14/);
