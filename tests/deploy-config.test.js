...
test('패키지, Worker, 서비스워커 버전이 v6.4.2로 통일되어 있다', async () => {
  const pkg = await json('package.json');
  const lock = await json('package-lock.json');
  const worker = await text('src/worker.js');
  const sw = await text('public/sw.js');
  assert.equal(pkg.version, '6.4.2');
  assert.equal(lock.version, '6.4.2');
  assert.equal(lock.packages[''].version, '6.4.2');
  ...
  assert.match(worker, /6\.4\.2-final/);
  assert.match(sw, /lego-life-v642-final/);
});
...
test('삭제된 과거 필드는 마이그레이션에서만 정리되고 현재 상태에는 다시 생성되지 않는다', async () => {
  ...
  assert.match(store, /meta:\s*\{\s*version:\s*15/);
