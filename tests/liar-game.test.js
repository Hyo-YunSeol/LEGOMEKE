...
test('라이어게임은 1라운드이며 최소 3명·최대 12명, 토론 60/120초, 판돈 100/500/1000P를 사용한다', () => {
  ...
  assert.deepEqual(LIAR_BET_OPTIONS, [100, 500, 1000]);
  ...
  assert.equal(LIAR_RECONNECT_GRACE_SECONDS, 30);
});
...
test('시민 승리 시 라운드 판돈을 연결된 시민 승자끼리 균등 분배하고 나머지는 버린다', () => {
  ...
  assert.equal(payout.pot, 400);
  assert.equal(payout.each, 133);
  assert.equal(payout.discarded, 1);
  assert.equal(pets[0].stats.points, 900);
  assert.deepEqual(pets.slice(1).map((pet) => pet.stats.points), [1033, 1033, 1033]);
});

test('1라운드 정상 종료 시 참가 목표만 한 번 반영하고 승리 기록은 별도로 남긴다', () => {
  const { state, pets } = setup(3, 1_000);
  joinReady(state, pets);
  const original = Math.random;
  try {
  ...
  assert.deepEqual(pets.map((pet) => pet.stats.legoPower), [2, 2, 2], '라이어게임 완료 목표 +1만 지급');
  assert.deepEqual(pets.map((pet) => pet.daily.legoGoals.liarPlay), [true, true, true]);
  assert.equal(pets.every((pet) => pet.daily.legoGoals.liarWin === undefined), true);
...
test('일시 연결 종료는 30초 동안 게임을 멈추고 같은 자리·역할·판돈으로 복귀시킨다', () => {
  const { state, pets } = setup(3, 1_000);
  joinReady(state, pets);
  liarUpdateSettings(state, pets[0], { betPoints: 100 }, BASE);
  withFixedRandom(() => liarStart(state, pets[0], BASE));
  const roleBefore = state.liarGame.liarPetId;
  const stakeBefore = state.liarGame.players[pets[0].id].currentRoundStake;
  const disconnected = liarSetConnected(state, pets[0].id, false, BASE);
  assert.equal(disconnected.changed, true);
  assert.equal(state.liarGame.phase, 'discussion');
  assert.equal(state.liarGame.phaseEndsAt, null);
  assert.equal(state.liarGame.pausedPhaseRemainingMs, 60_000);
  assert.equal(liarGameView(state, pets[1].id).pausedForReconnect, true);
  assert.equal(liarNextAlarmAt(state), new Date(BASE.getTime() + 30_000).toISOString());

  const reconnectedAt = new Date(BASE.getTime() + 20_000);
  const reconnected = liarSetConnected(state, pets[0].id, true, reconnectedAt);
  assert.equal(reconnected.changed, true);
  assert.equal(state.liarGame.players[pets[0].id].connected, true);
  assert.equal(state.liarGame.players[pets[0].id].forfeited, false);
  assert.equal(state.liarGame.players[pets[0].id].currentRoundStake, stakeBefore);
  assert.equal(state.liarGame.liarPetId, roleBefore);
  assert.equal(state.liarGame.phaseEndsAt, new Date(reconnectedAt.getTime() + 60_000).toISOString());
  assert.equal(liarGameView(state, pets[0].id).isLiar, true);
});

test('재접속 유예가 끝난 시민은 판돈을 포기하고 3명 미만이면 나머지만 환불받는다', () => {
  const { state, pets } = setup(3, 1_000);
  joinReady(state, pets);
  liarUpdateSettings(state, pets[0], { betPoints: 100 }, BASE);
  withFixedRandom(() => liarStart(state, pets[0], BASE));
  const pending = liarSetConnected(state, pets[2].id, false, BASE);
  assert.equal(pending.autoEnded, undefined);
  assert.equal(state.liarGame.phase, 'discussion');
  advanceLiarGame(state, new Date(BASE.getTime() + 30_001));
...
test('라이어가 최종 이탈하면 시민 승리로 판돈을 정확히 한 번 정산한다', () => {
  const { state, pets } = setup(4, 1_000);
  joinReady(state, pets);
  withFixedRandom(() => liarStart(state, pets[0], BASE));
  ...
  assert.equal(result.ok, true);
  assert.equal(state.liarGame.phase, 'result');
  assert.equal(state.liarGame.roundResult.liarWon, false);
  assert.match(state.liarGame.roundResult.reason, /라이어가 최종 이탈/);
  assert.deepEqual(state.liarGame.roundResult.payout, {
    pot: 400,
    each: 133,
    winnerPetIds: pets.slice(1).map((pet) => pet.id),
    discarded: 1
  });
  assert.deepEqual(pets.map((pet) => pet.stats.points), [900, 1033, 1033, 1033]);
});

test('4명 게임에서 시민이 최종 이탈하면 게임은 유지되고 이탈자만 판돈을 포기한다', () => {
  const { state, pets } = setup(4, 1_000);
  joinReady(state, pets);
  withFixedRandom(() => liarStart(state, pets[0], BASE));
  const result = liarLeave(state, pets[1], BASE);
  assert.equal(result.ok, true);
  assert.equal(state.liarGame.phase, 'discussion');
  assert.equal(state.liarGame.players[pets[1].id].forfeited, true);
  assert.equal(state.liarGame.hostPetId, pets[0].id);
  assert.equal(pets[1].stats.points, 900);
});
...
test('라이어 채팅은 빈 입력·초고속 중복을 막고 운영자 삭제는 일반 채팅만 지운다', () => {
  const { state, pets } = setup();
  liarJoin(state, pets[0], BASE);
  assert.equal(liarAddChat(state, pets[0], '   ', 'chat-empty-0001', BASE).ok, false);
  const sent = liarAddChat(state, pets[0], '안녕하세요', 'chat-send-0001', BASE);
  assert.equal(sent.ok, true);
  const duplicate = liarAddChat(state, pets[0], '다른 내용', 'chat-send-0001', new Date(BASE.getTime() + 100));
  assert.equal(duplicate.ok, true);
  assert.equal(duplicate.duplicate, true);
  assert.equal(duplicate.chat.id, sent.chat.id);
  assert.equal(liarAddChat(state, pets[0], '연속', 'chat-rate-0001', new Date(BASE.getTime() + 100)).ok, false);
  assert.equal(state.liarGame.messages.filter((message) => message.type === 'chat').length, 1);
...
test('결과 단계에서 연결이 끊겨도 게임 완료·승패 기록과 종료 소식을 보존한다', () => {
  const { state, pets } = setup(3, 1_000);
  joinReady(state, pets);
  withFixedRandom(() => liarStart(state, pets[0], BASE));
  expire(state);
  ...
  liarGuess(state, pets[0], '오답', BASE);
  assert.equal(state.liarGame.phase, 'result');
  liarSetConnected(state, pets[2].id, false, BASE);
  expire(state);
  assert.equal(state.liarGame.phase, 'game_over');
  assert.deepEqual(pets.map((pet) => pet.records.liarGames), [1, 1, 1]);
  assert.deepEqual(pets.map((pet) => pet.records.liarWins), [0, 1, 1]);
  assert.match(consumeLiarPublicEvent(state).text, /라이어게임 종료/);
});
