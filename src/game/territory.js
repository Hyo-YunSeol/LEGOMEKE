...
export function normalizeTerritory(raw, state, date = new Date()) {
  ...
  // 4×4 맵이 한 번이라도 모두 찬 뒤에는 시즌이 끝날 때까지 전면전을 유지한다.
  // 기존 저장 데이터에 필드가 없어도 현재 16칸이 모두 차 있다면 자동으로 전면전 상태로 승격한다.
...
export function claimTerritory(state, pet, rowValue, colValue, date = new Date()) {
  ...
  if (existing?.home && !battleUnlocked) {
    return { ok: false, message: '상대의 본진은 맵 16칸이 모두 차기 전에는 빼앗을 수 없습니다.', hungerCost: hungerUse.cost };
  }
  ...
  if (ownedBefore >= limit) {
    ...
    // 전면전에서는 Lv1처럼 일반 영토가 없는 1칸 보유자도 본진을 옮기며 공격할 수 있다.
  ...
  const moveText = released ? ` 내 가장 오래된 ${releasedWasHome ? '본진' : '일반 영토'} (${released.row + 1}, ${released.col + 1})은 자동으로 비워졌습니다.` : '';
  const battleText = territory.battleUnlocked && !battleUnlocked ? ' 맵 16칸이 모두 차서 ⚔️ 전면전이 시작되었습니다.' : '';
  return {
    ok: true,
