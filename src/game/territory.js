...
import { gameDayKey, gameDayWindow, gameDaysBetweenKeys } from '../lib/time.js';
import {
...
function validFixedBoundary(endMs) {
  if (!Number.isFinite(endMs)) return false;
  const shifted = new Date(endMs + 9 * 60 * 60 * 1000);
  return shifted.getUTCMinutes() === 0
    && shifted.getUTCSeconds() === 0
    && shifted.getUTCMilliseconds() === 0
    && shifted.getUTCHours() % 6 === 0;
}
...
export function normalizeTerritory(raw, state, date = new Date()) {
  ...
  // 현재 5×5 좌표가 정상인 저장 데이터는 영토를 지우지 않고 그대로 복구한다.
  ...
  const rawSeasonId = source ? source.seasonId : base.seasonId;
  territory.version = 6;
  ...
  territory.startedAt = Number.isFinite(startedAt) ? new Date(startedAt).toISOString() : window.startsAt;
  const parsedEndMs = new Date(territory.endsAt ?? 0).getTime();
  const recoveryReasons = [];
  const storedSeasonId = rawSeasonId;
  ...
  const invalidEnd = !Number.isFinite(parsedEndMs) || !validFixedBoundary(parsedEndMs);
  const inconsistentSeason = !storedSeasonIdValid || (!invalidEnd && storedSeasonId !== expectedStoredSeasonId);
  if (invalidEnd || inconsistentSeason) {
    territory.endsAt = window.endsAt;
    territory.startedAt = window.startsAt;
    territory.seasonId = window.seasonId;
    if (!Number.isFinite(parsedEndMs)) recoveryReasons.push('invalid-endsAt');
    else if (!validFixedBoundary(parsedEndMs)) recoveryReasons.push('invalid-endsAt-boundary');
    if (!storedSeasonIdValid) recoveryReasons.push('invalid-seasonId');
    else if (!invalidEnd && storedSeasonId !== expectedStoredSeasonId) {
      recoveryReasons.push(parsedEndMs > date.getTime() ? 'seasonId-mismatch-before-end' : 'seasonId-endsAt-inconsistent');
    }
  } else {
    territory.endsAt = new Date(parsedEndMs).toISOString();
    // 종료 전 seasonId 불일치는 정산/초기화 사유가 아니다. 현재 회차의 고정 경계로
    // 메타데이터만 복구하며 cells는 아래에서 그대로 보존한다.
    if (parsedEndMs > date.getTime() && (territory.seasonId !== window.seasonId || parsedEndMs !== new Date(window.endsAt).getTime())) {
      const seasonMismatch = territory.seasonId !== window.seasonId;
      territory.seasonId = window.seasonId;
      territory.startedAt = window.startsAt;
      territory.endsAt = window.endsAt;
      if (seasonMismatch) recoveryReasons.push('seasonId-mismatch-before-end');
      if (parsedEndMs !== new Date(window.endsAt).getTime()) recoveryReasons.push('endsAt-mismatch-before-end');
    }
  }
...
export function processTerritorySeason(state, date = new Date()) {
  ...
  // seasonId, 배포, 재접속, 알람 지연 여부와 관계없이 저장된 endsAt 전에는
  // 절대로 정산하거나 맵을 교체하지 않는다.
  ...
  // normalizeTerritory가 비정상 endsAt을 현재 회차 경계로 복구하므로 이 경로에는
  // 실제로 만료된 정상 ISO 시각만 도달한다.
  ...
  const elapsedSeasons = Math.max(1, gameDaysBetweenKeys(previousSeasonId, currentId));
  const next = initialTerritory(date, previousSeasonNumber + elapsedSeasons);
  ...
  const scheduledKstHour = new Date(endMs + 9 * 60 * 60 * 1000).getUTCHours();
  const scheduledLabel = `${String(scheduledKstHour).padStart(2, '0')}:00`;
  addTerritoryNews(state, `제${next.seasonNumber}회 레고 영토전이 시작되었습니다.`, [], scheduledEnd);
  addTerritoryNews(state, `${scheduledLabel}에 이전 영토전이 종료되어 영토가 초기화되고 새 회차가 시작되었습니다.`, [], scheduledEnd);
  const resetLog = {
    reason: 'stored-endsAt-expired',
    processedAt: nowIso(date),
    scheduledEndsAt: new Date(endMs).toISOString(),
    previousSeasonId,
...
function oldestOwnedCell(territory, petId, exceptKey = null) {
  return ownedEntries(territory, petId)
...
export function claimTerritory(state, pet, rowValue, colValue, date = new Date()) {
  ...
  // 전면전이 열린 뒤에는 일반 영토가 하나도 없는 경우에만 기존 본진을 옮길 수 있다.
  let released = null;
  let releasedWasHome = false;
  if (ownedBefore >= limit) {
    ...
    let releaseKey = oldestMovableOwnedCell(territory, pet.id, key);
    if (!releaseKey && battleUnlocked) releaseKey = oldestOwnedCell(territory, pet.id, key);
    if (!releaseKey) {
      return {
        ok: false,
        message: '지금은 옮길 수 있는 일반 영토가 없습니다. 전면전이 열리면 본진까지 옮기며 탈취할 수 있습니다.',
        hungerCost: hungerUse.cost,
        myOwned: ownedBefore,
        myLimit: limit
      };
    }
    releasedWasHome = Boolean(territory.cells[releaseKey]?.home);
  ...
  territory.cells[key] = { ownerPetId: pet.id, claimedAt: nowIso(date), home: releasedWasHome };
  if (victimPetId) pet.records.territorySteals = Math.max(0, Number(pet.records.territorySteals) || 0) + 1;
  else pet.records.territoryClaims = Math.max(0, Number(pet.records.territoryClaims) || 0) + 1;
  if (!territory.battleUnlocked && Object.keys(territory.cells).length >= capacity) territory.battleUnlocked = true;
  ...
  const moveText = released
    ? ` 보유 한도 ${limit}칸을 유지하기 위해 가장 오래된 ${releasedWasHome ? '본진' : '일반 영토'} (${released.row + 1}, ${released.col + 1})을 비우고 이동했습니다.`
    : '';
