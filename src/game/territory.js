...
function playerRows(state, territory, excludedPetIds = null) {
  const counts = {};
  ...
  return Object.entries(counts)
    .filter(([petId]) => Boolean(activePet(state, petId)) && !excludedPetIds?.has?.(petId))
    .map(([petId, count]) => ({
      petId,
...
export function territoryRanking(state, { settlement = false, excludedPetIds = null } = {}) {
  const territory = state.territory = normalizeTerritory(state.territory, state);
  const players = playerRows(state, territory, excludedPetIds);
...
export function claimTerritory(state, pet, rowValue, colValue, date = new Date()) {
  ...
  // 유효한 영토 행동을 눌렀다면 성공/실패와 무관하게 반복 콘텐츠 공통 포만감 1을 소비한다.
...
export function territoryView(state, viewerPetId, { excludedPetIds = null } = {}) {
  const territory = state.territory = normalizeTerritory(state.territory, state);
  const counts = territoryCounts(state);
  const pet = state.pets[viewerPetId];
  const level = pet ? levelForPower(pet.stats.legoPower) : 1;
  const limit = territoryLimitForLevel(level);
  const ranking = territoryRanking(state, { excludedPetIds });
