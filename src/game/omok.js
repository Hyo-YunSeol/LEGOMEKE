...
export function createOmokRoom(state, pet, stakeValue, date = new Date()) {
  ...
  const room = {
    ...
    settled: false, settlementId: null, rematchRequests: [], processedMoveIds: [], moveCount: 0, lastMove: null,
...
export function omokNextAlarmAt(state, date = new Date()) {
  ...
  for (const room of Object.values(omok.rooms)) {
    if (room.status === 'playing' && room.turnStartedAt) times.push(new Date(room.turnStartedAt).getTime() + OMOK_TURN_SECONDS * 1000);
