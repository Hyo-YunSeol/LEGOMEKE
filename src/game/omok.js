...
export function leaveOmokRoom(state, pet, roomId, date = new Date()) {
  ...
  if (room.status === 'playing') {
    ...
    finishRoom(state, room, { result: 'forfeit', winnerPetId: opponent, loserPetId: pet.id, reason: `${withJosa(pet.displayName, '이/가')} 게임 중 나가 기권패했습니다.` }, date);
...
export function playOmokMove(state, pet, roomId, rowValue, colValue, requestIdValue, date = new Date()) {
  ...
  if (won) {
    const opponent = opponentPetId(room, pet.id);
    finishRoom(state, room, { result: 'win', winnerPetId: pet.id, loserPetId: opponent, reason: `${withJosa(pet.displayName, '이/가')} 오목을 완성했습니다.` }, date);
...
function advanceRoomTimeout(state, room, date = new Date()) {
  ...
  for (let guard = 0; guard < 12 && room.status === 'playing' && room.phase === 'turn'; guard += 1) {
    ...
    if (room.consecutiveTimeouts[timedOutPetId] >= OMOK_MAX_CONSECUTIVE_TIMEOUTS) {
      ...
      finishRoom(state, room, { result: 'timeout', winnerPetId, loserPetId: timedOutPetId, reason: `${withJosa(loser?.displayName ?? '플레이어', '이/가')} 연속 3회 시간초과로 기권패했습니다.` }, new Date(deadline));
      break;
    }
