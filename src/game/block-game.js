...
// 하위 테스트/호환용 상수다. v3부터는 시작판을 인위적으로 쉬운 판으로 보정하지 않는다.
export const BLOCK_MIN_STARTING_GROUPS = 0;
export const BLOCK_MIN_STARTING_REMOVABLE_CELLS = 0;
export const BLOCK_BOARD_GENERATOR_VERSION = 3;
...
export function createPlayableBlockBoard(random = Math.random) {
  // v3: 12×10 / 5색을 그대로 독립 난수로 생성한다.
  // 완주를 보장하는 묶음 생성은 포인트 밸런스를 무너뜨리므로 사용하지 않는다.
  return createBlockBoard(random);
}
