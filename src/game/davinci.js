...
function resetWaitingReady(room) {
  if (!room || room.status !== 'waiting') return;
  for (const player of room.players) player.ready = false;
}

function occupiedRoomNumbers(davinci) { return new Set(Object.values(davinci.rooms).map((room) => room.roomNumber)); }
...
export function createDavinciRoom(state, pet, stakeValue, date = new Date()) {
  ...
  if (Object.keys(davinci.rooms).length >= DAVINCI_MAX_ROOMS) return { ok: false, message: '다빈치코드 방은 최대 2개까지 만들 수 있습니다.' };
...
export function joinDavinciRoom(state, pet, roomId, date = new Date()) {
  ...
  room.rematchRequests = [];
  // 참가 인원이 바뀌면 기존 준비 의사를 다시 확인한다.
  resetWaitingReady(room);
  touch(room, date);
...
export function kickDavinciPlayer(state, pet, roomId, targetPetId, date = new Date()) {
  const davinci = state.davinci = normalizeDavinci(state.davinci, state);
  const room = davinci.rooms[roomId];
  if (!room || room.status !== 'waiting') return { ok: false, message: '대기방에서만 강퇴할 수 있습니다.' };
  if (room.hostPetId !== pet.id) return { ok: false, message: '방장만 참가자를 강퇴할 수 있습니다.' };
  const targetId = String(targetPetId ?? '');
  if (!targetId || targetId === pet.id) return { ok: false, message: '방장 본인은 강퇴할 수 없습니다.' };
  const index = room.players.findIndex((player) => player.petId === targetId && !player.leftRoom);
  if (index < 0) return { ok: false, message: '강퇴할 참가자를 찾을 수 없습니다.' };
  const [removed] = room.players.splice(index, 1);
  delete room.spectators?.[targetId];
  room.rematchRequests = room.rematchRequests.filter((petId) => petId !== targetId);
  resetWaitingReady(room);
  touch(room, date);
  return { ok: true, roomId, kickedPetId: targetId, message: `${removed.displayName}님을 방에서 내보냈습니다.` };
}
...
export function leaveDavinciRoom(state, pet, roomId, date = new Date()) {
  ...
  if (index < 0) return { ok: false, message: '이 방의 참가자가 아닙니다.' };
  ...
  else { resetWaitingReady(room); touch(room, date); }
  return { ok: true, roomId, message: '방을 나갔습니다.' };
}
...
function publicTile(tile, ownerPetId, viewerPetId, viewerIsOwner, drawn = false, forceVisible = false) {
  const visible = forceVisible || viewerIsOwner || tile.revealed;
  return {
    id: tile.id,
    color: tile.color,
...
function playerView(room, player, viewerPetId) {
  ...
  return {
    ...
    hand: player.hand.map((tile) => publicTile(tile, player.petId, viewerPetId, mine, false, revealAll)),
    drawnTile: (mine || revealAll) && player.drawnTile ? publicTile(player.drawnTile, player.petId, viewerPetId, mine, true, revealAll) : null,
...
export function davinciRoomView(state, roomId, viewerPetId, date = new Date()) {
  ...
  return {
    ...
    deckColors: {
      black: room.deck.filter((tile) => tile.color === 'black').length,
      white: room.deck.filter((tile) => tile.color === 'white').length
    },
    endedDeck: room.status === 'ended' ? room.deck.map((tile) => publicTile(tile, '', viewerPetId, false, false, true)) : [],
