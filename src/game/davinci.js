...
function initializeMatch(state, room, date = new Date()) {
  ...
  const battlePets = room.players.map((player) => playerPet(state, player.petId));
  const battleCheck = canStartBattleForPets(battlePets, date);
  ...
  const battleConsumed = consumeBattleForPets(battlePets, date);
  if (!battleConsumed.ok) { for (const petId of deducted) refundStake(playerPet(state, petId), room.stakePoints); return battleConsumed; }
