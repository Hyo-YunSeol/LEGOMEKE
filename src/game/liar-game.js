...
export const LIAR_BET_OPTIONS = [100, 500, 1000];
...
export const LIAR_RECONNECT_GRACE_SECONDS = 30;
...
export function initialLiarGame() {
  return {
    ...
    players: {},
    spectators: {},
    settings: { totalRounds: LIAR_TOTAL_ROUNDS, discussionSeconds: 60, maxPlayers: LIAR_MAX_PLAYERS, betPoints: 100 },
    roundNo: 0,
    roundPlayerIds: [],
    category: null,
    word: null,
...
export function normalizeLiarGame(raw, state) {
  ...
  game.settings.betPoints = LIAR_BET_OPTIONS.includes(Number(game.settings.betPoints)) ? Number(game.settings.betPoints) : 100;
  ...
  game.chatOperations = game.chatOperations && typeof game.chatOperations === 'object' && !Array.isArray(game.chatOperations) ? game.chatOperations : {};
  game.phaseEndsAt = parseTime(game.phaseEndsAt) == null ? null : new Date(parseTime(game.phaseEndsAt)).toISOString();
  game.pausedPhaseRemainingMs = game.pausedPhaseRemainingMs !== null && game.pausedPhaseRemainingMs !== undefined && game.pausedPhaseRemainingMs !== '' && Number.isFinite(Number(game.pausedPhaseRemainingMs))
    ? Math.max(0, Math.floor(Number(game.pausedPhaseRemainingMs)))
    : null;
  game.pauseStartedAt = parseTime(game.pauseStartedAt) == null ? null : new Date(parseTime(game.pauseStartedAt)).toISOString();
  game.finishedAt = parseTime(game.finishedAt) == null ? null : new Date(parseTime(game.finishedAt)).toISOString();
  ...
  for (const [petId, player] of Object.entries(game.players)) {
    ...
    player.disconnectDeadlineAt = parseTime(player.disconnectDeadlineAt) == null ? null : new Date(parseTime(player.disconnectDeadlineAt)).toISOString();
    ...
    if (player.connected || player.forfeited) player.disconnectDeadlineAt = null;
  }
  ...
  for (const [requestId, operation] of Object.entries(game.chatOperations)) {
    if (!/^[A-Za-z0-9._:-]{8,120}$/.test(requestId) || !operation || typeof operation !== 'object') {
      delete game.chatOperations[requestId];
      continue;
    }
    operation.petId = String(operation.petId ?? '');
    operation.chatId = String(operation.chatId ?? '');
    operation.createdAt = parseTime(operation.createdAt) == null ? nowIso() : new Date(parseTime(operation.createdAt)).toISOString();
  }
  game.chatOperations = Object.fromEntries(Object.entries(game.chatOperations)
    .sort(([, a], [, b]) => (parseTime(a.createdAt) ?? 0) - (parseTime(b.createdAt) ?? 0))
    .slice(-240));
  if (!game.players[game.hostPetId] || game.players[game.hostPetId].forfeited) game.hostPetId = oldestConnectedPlayerId(game);
  ...
  const interactive = ['discussion', 'voting', 'liar_guess'].includes(game.phase);
  if (!interactive || !Object.values(game.players).some((player) => !player.connected && !player.forfeited && player.disconnectDeadlineAt)) {
    if (!interactive) {
      game.pausedPhaseRemainingMs = null;
      game.pauseStartedAt = null;
    }
  } else if (game.pausedPhaseRemainingMs == null) {
    game.pausedPhaseRemainingMs = 0;
  }
  game.version = 4;
  return game;
}
...
export function liarJoin(state, pet, date = new Date()) {
  ...
  if (existing) {
    if (existing.forfeited && game.phase !== 'waiting') return { ok: false, message: '이미 진행 중 게임에서 최종 이탈 처리되었습니다.' };
    existing.connected = true;
    existing.forfeited = false;
    existing.disconnectDeadlineAt = null;
    existing.displayName = pet.displayName;
    resumeAfterReconnect(game, date);
    game.updatedAt = nowIso(date);
  ...
  game.players[pet.id] = {
    ...
    disconnectDeadlineAt: null, escrowRemaining: 0, currentRoundStake: 0, forfeited: false
  };
  if (!game.hostPetId) {
...
function interactivePhase(game) {
  return ['discussion', 'voting', 'liar_guess'].includes(game.phase);
}

function pendingReconnectPlayers(game) {
  return Object.values(game.players).filter((player) => !player.connected && !player.forfeited && parseTime(player.disconnectDeadlineAt) != null);
}

function pauseForReconnect(game, date = new Date()) {
  if (!interactivePhase(game) || game.pausedPhaseRemainingMs != null) return;
  const phaseEnd = parseTime(game.phaseEndsAt);
  game.pausedPhaseRemainingMs = Math.max(0, (phaseEnd ?? date.getTime()) - date.getTime());
  game.pauseStartedAt = nowIso(date);
  game.phaseEndsAt = null;
}

function resumeAfterReconnect(game, date = new Date()) {
  if (!interactivePhase(game) || pendingReconnectPlayers(game).length || game.pausedPhaseRemainingMs == null) return false;
  const remaining = Math.max(1_000, nonNegativeInt(game.pausedPhaseRemainingMs));
  game.phaseEndsAt = new Date(date.getTime() + remaining).toISOString();
  game.pausedPhaseRemainingMs = null;
  game.pauseStartedAt = null;
  game.updatedAt = nowIso(date);
  return true;
...
function finalizeDisconnectedPlayer(state, game, player, date = new Date()) {
  if (!player || player.connected || player.forfeited) return { changed: false };
  player.disconnectDeadlineAt = null;
  if (game.phase === 'waiting') {
    delete game.players[player.petId];
    return { changed: true, removed: true };
  }
  if (game.phase === 'game_over') {
    delete game.players[player.petId];
    if (game.hostPetId === player.petId) game.hostPetId = oldestConnectedPlayerId(game);
    if (!Object.keys(game.players).length) state.liarGame = initialLiarGame();
    return { changed: true, removed: true };
  }
  if (game.phase === 'result') {
    // 이미 정산된 결과 단계에서는 참가 기록을 보존한다. finishGame이 연결 상태와 무관하게 한 번 반영한다.
    addSystem(game, `${player.displayName}의 재접속 대기시간이 끝났지만 완료 기록은 유지됩니다.`, date);
    return { changed: true, resultPreserved: true };
  }

  forfeitPlayer(game, player, date);
  if (game.hostPetId === player.petId) {
  ...
  if (player.petId === game.liarPetId && game.roundPlayerIds.includes(player.petId)) {
    finishRound(state, game, { liarWon: false, reason: '라이어가 최종 이탈해 시민이 승리했습니다.' }, date);
    return { changed: true, liarForfeit: true };
  }
  if (connectedPlayers(game).length < LIAR_MIN_PLAYERS) {
    returnToWaitingForTooFewPlayers(state, game, date);
    return { changed: true, autoEnded: true };
  }
  resumeAfterReconnect(game, date);
  return { changed: true, forfeited: true };
}

function processExpiredReconnects(state, game, date = new Date()) {
  let changed = false;
  const expired = pendingReconnectPlayers(game)
    .filter((player) => (parseTime(player.disconnectDeadlineAt) ?? Infinity) <= date.getTime())
    .sort((a, b) => (parseTime(a.disconnectDeadlineAt) ?? 0) - (parseTime(b.disconnectDeadlineAt) ?? 0));
  for (const player of expired) {
    const currentGame = state.liarGame;
    const current = currentGame?.players?.[player.petId];
    if (!current || current.connected || current.forfeited) continue;
    const result = finalizeDisconnectedPlayer(state, currentGame, current, date);
    if (result.changed) changed = true;
    if (state.liarGame !== game) break;
  }
  if (state.liarGame === game && resumeAfterReconnect(game, date)) changed = true;
  return changed;
}
...
export function liarSetConnected(state, petId, connected, date = new Date()) {
  ...
  if (connected) {
    ...
    if (player.forfeited) return { changed: false, forfeited: true };
    const deadline = parseTime(player.disconnectDeadlineAt);
    if (deadline != null && deadline <= date.getTime()) {
      const finalized = finalizeDisconnectedPlayer(state, game, player, date);
      return { ...finalized, reconnectExpired: true };
    }
    ...
    if (game.phase === 'waiting') {
      ...
      player.connected = false;
      player.disconnectDeadlineAt = new Date(date.getTime() + LIAR_RECONNECT_GRACE_SECONDS * 1000).toISOString();
      pauseForReconnect(game, date);
      addSystem(game, `${player.displayName}의 연결이 끊겨 ${LIAR_RECONNECT_GRACE_SECONDS}초간 재접속을 기다립니다.`, date);
    }
  }
  if (!connected && game.phase === 'waiting' && game.hostPetId === petId) game.hostPetId = oldestConnectedPlayerId(game);
  if (!connected && game.phase === 'waiting' && connectedPlayers(game).length === 0) state.liarGame = initialLiarGame();
  return { changed: true, reconnectDeadlineAt: player.disconnectDeadlineAt ?? null };
}
...
export function liarLeave(state, pet, date = new Date()) {
  ...
  if (['waiting', 'game_over'].includes(game.phase)) {
    ...
    player.connected = false;
    player.disconnectDeadlineAt = null;
    addSystem(game, `${player.displayName}이 결과 화면에서 나갔습니다. 완료 기록은 유지됩니다.`, date);
  } else {
    player.connected = false;
    player.disconnectDeadlineAt = null;
    const finalized = finalizeDisconnectedPlayer(state, game, player, date);
    return { ok: true, ...finalized, message: player.petId === game.liarPetId ? '라이어 이탈로 시민 승리 처리되었습니다.' : '라이어게임에서 나갔습니다. 진행 중 이탈로 판돈을 포기했습니다.' };
  }
...
export function liarVote(state, pet, targetPetId, date = new Date()) {
  ...
  if (pendingReconnectPlayers(game).length) return { ok: false, message: '참가자의 재접속을 기다리는 동안 투표가 일시 정지되었습니다.' };
...
export function liarGuess(state, pet, guessValue, date = new Date()) {
  ...
  if (pendingReconnectPlayers(game).length) return { ok: false, message: '참가자의 재접속을 기다리는 동안 게임이 일시 정지되었습니다.' };
...
function finishGame(state, game, date = new Date()) {
  if (game.phase === 'game_over' && game.finishedAt) return;
  refundAll(state, game, { includeCurrent: true });
  game.phase = 'game_over';
  game.phaseEndsAt = null;
  game.pausedPhaseRemainingMs = null;
  game.pauseStartedAt = null;
  game.finishedAt = nowIso(date);
...
export function advanceLiarGame(state, date = new Date()) {
  let game = state.liarGame = normalizeLiarGame(state.liarGame, state);
  let changed = processExpiredReconnects(state, game, date);
  game = state.liarGame;
  for (let guard = 0; guard < 12; guard += 1) {
    if (pendingReconnectPlayers(game).length && interactivePhase(game)) break;
...
export function liarAddChat(state, pet, textValue, requestIdValue = '', date = new Date()) {
  const game = state.liarGame = normalizeLiarGame(state.liarGame, state);
  const player = game.players[pet.id];
  if (!player?.connected) return { ok: false, message: '먼저 라이어게임에 참가해주세요.' };
  const requestId = String(requestIdValue ?? '').trim();
  if (!/^[A-Za-z0-9._:-]{8,120}$/.test(requestId)) return { ok: false, message: '유효한 채팅 요청 ID가 필요합니다.' };
  const previous = game.chatOperations[requestId];
  if (previous) {
    if (previous.petId !== pet.id) return { ok: false, message: '이미 다른 채팅에 사용된 요청 ID입니다.' };
    const existingChat = game.messages.find((message) => message.id === previous.chatId) ?? null;
    return { ok: true, duplicate: true, message: '이미 전송된 채팅입니다.', chat: existingChat };
  }
  ...
  game.messages = game.messages.slice(-LIAR_MAX_MESSAGES);
  game.chatOperations[requestId] = { petId: pet.id, chatId: message.id, createdAt: nowIso(date) };
  game.chatOperations = Object.fromEntries(Object.entries(game.chatOperations).slice(-240));
  game.updatedAt = nowIso(date);
...
export function resetLiarParticipantForAdmin(state, petId, date = new Date()) {
  let game = state.liarGame = normalizeLiarGame(state.liarGame, state);
  let forceEnded = false;
  if (game.players[petId] && !['waiting', 'game_over'].includes(game.phase)) {
    forceEndLiarGame(state, date);
    forceEnded = true;
    game = state.liarGame;
  }
  const removedPlayer = Boolean(game.players[petId]);
  delete game.players[petId];
  const removedSpectator = Boolean(game.spectators[petId]);
  delete game.spectators[petId];
  if (game.hostPetId === petId) game.hostPetId = oldestConnectedPlayerId(game);
  game.updatedAt = nowIso(date);
  return { changed: forceEnded || removedPlayer || removedSpectator, forceEnded, removedPlayer, removedSpectator };
}
...
export function liarGameView(state, viewerPetId) {
  ...
  return {
    ...
    pausedForReconnect: interactivePhase(game) && pendingReconnectPlayers(game).length > 0,
    reconnectDeadlineAt: pendingReconnectPlayers(game)
      .map((player) => player.disconnectDeadlineAt)
      .filter(Boolean)
      .sort()[0] ?? null,
...
export function liarNextAlarmAt(state) {
  const game = normalizeLiarGame(state.liarGame, state);
  const times = [parseTime(game.phaseEndsAt), ...pendingReconnectPlayers(game).map((player) => parseTime(player.disconnectDeadlineAt))]
