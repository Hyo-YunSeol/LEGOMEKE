...
function occupiedNumbers(game) { return new Set(Object.values(game.rooms).map((room) => room.roomNumber)); }
...
export function createSichuanRoom(state, pet, stakeValue, date = new Date()) {
  ...
  if (Object.keys(game.rooms).length >= SICHUAN_MAX_ROOMS) return { ok: false, message: '동시에 운영할 수 있는 사천성 대전방 3개가 모두 사용 중입니다.' };
