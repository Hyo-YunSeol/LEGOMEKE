import { gameDayWindow, gameDaysBetweenKeys } from '../lib/time.js';
...
export function normalizeMinesweeperSeason(raw, date = new Date()) {
  ...
  const startsAt = finiteIso(raw?.startsAt);
  const endsAt = finiteIso(raw?.endsAt);
  return key
    ? { key, startsAt: startsAt ?? current.startsAt, endsAt: endsAt ?? current.endsAt, initializedAt: finiteIso(raw?.initializedAt) ?? date.toISOString(), lastSettledAt: finiteIso(raw?.lastSettledAt) }
    : { key: current.seasonId, startsAt: current.startsAt, endsAt: current.endsAt, initializedAt: date.toISOString(), lastSettledAt: null };
}
...
export function processMinesweeperSeason(state, date = new Date()) {
  ...
  if (state.minesweeperSeason.key === current.seasonId) {
  ...
  const elapsedGameDays = gameDaysBetweenKeys(state.minesweeperSeason.key, current.seasonId);
  const normalWinner = rankingRows(state, 'normal')[0] ?? null;
  const hardWinner = rankingRows(state, 'hard')[0] ?? null;
  const champions = [];
  // 정확히 직전 게임 하루를 정산할 때만 다음 6시간 동안 칭호를 부여한다.
  // 장시간 서비스가 멈춰 여러 시즌이 건너뛴 경우 오래된 1위 칭호가 현재 시즌에 되살아나지 않는다.
  if (elapsedGameDays === 1) {
  ...
  // 게임 하루가 바뀌면 이전 시즌에서 시작한 진행판은 새 시즌 기록으로 넘어가지 않게 종료한다.
  ...
  state.minesweeperSeason = { key: current.seasonId, startsAt: current.startsAt, endsAt: current.endsAt, initializedAt: state.minesweeperSeason.initializedAt, lastSettledAt: date.toISOString() };
