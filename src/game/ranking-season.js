...
function legodokuRows(state) {
  return alivePets(state).map((pet) => ({
    pet, wins: int(pet.records?.seasonLegodokuWins), draws: int(pet.records?.seasonLegodokuDraws), losses: int(pet.records?.seasonLegodokuLosses)
  })).filter((row) => row.wins || row.draws || row.losses)
    .sort((a, b) => b.wins - a.wins || a.losses - b.losses || b.draws - a.draws || a.pet.displayName.localeCompare(b.pet.displayName, 'ko'));
}
...
function resetSeasonRecords(state) {
  for (const pet of Object.values(state.pets ?? {})) {
    ...
    pet.records.seasonLegodokuWins = 0;
    pet.records.seasonLegodokuDraws = 0;
    pet.records.seasonLegodokuLosses = 0;
    delete pet.records.seasonSpotDifferenceWins;
    delete pet.records.seasonSpotDifferenceDraws;
    delete pet.records.seasonSpotDifferenceLosses;
    delete pet.records.davinciWins;
    delete pet.records.davinciCorrect;
  }
}
...
export function processGameRankingSeason(state, date = new Date()) {
  ...
  const legodoku = awardRows(legodokuRows(state));
  const champions = [
    ['apple', apple[0]],
  ...
  const events = [
    ...
    { text: newsLine('🧩', '레고도쿠', legodoku), type: 'game-season', petIds: legodoku.map((item) => item.petId) }
  ];
  ...
  return { changed: true, events, awards: { apple, singleTetris, omok, blockBattle, sichuan, legodoku } };
}
