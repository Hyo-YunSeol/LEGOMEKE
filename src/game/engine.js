...
import { BLOCK_BATTLE_GRAVITY_MS } from './block-battle.js';
import { BATTLE_PLAYS_PER_DAY, BATTLE_TICKET_BONUS, battleLimit, normalizeBattleDaily } from './battle-limit.js';
import {
  GAME_DAY_HOURS,
  ACTIONS_PER_DAY,
  ACTION_COOLDOWN_MINUTES,
  MINI_GAMES_PER_DAY,
  SINGLE_TETRIS_DURATION_MS,
  FISHING_PER_DAY,
  FISHING_WAIT_MS,
  ...
  STARTING_LEGO_POWER,
  WORK_POINTS,
  HUNGER_PENALTY_POINTS_PER_HOUR, HUNGER_PENALTY_MAX_HOURS,
  BUNG_MIN_STAKE,
  BUNG_MIN_PLAYERS,
  BUNG_MAX_PLAYERS,
  BODY_STAGES, BODY_ADVANCEMENTS, BODY_ADVANCEMENT_BODY,
  FOODS,
  MINI_GAMES,
  ODD_EVEN_MIN_STAKE,
...
import {
  applyMinesweeperAction, minesweeperChallengeFields, minesweeperChallengeView,
  minesweeperDifficulty, minesweeperRankingsView, normalizeMinesweeperChallenge
} from './minesweeper.js';
import { omokRanking, removePetFromOmok } from './omok.js';

const nowIso = (date = new Date()) => date.toISOString();
export const BUNG_OPEN_TTL_MS = 24 * 60 * 60_000;
export const BUNG_LIVE_TTL_MS = 24 * 60 * 60_000;
...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  delete pet.hidden;
  pet.statusMessage = truncateCodePoints(normalizedStatusText(pet.statusMessage), STATUS_MESSAGE_ADMIN_MAX_LENGTH);

  const rawAdvancement = pet.bodyAdvancement && typeof pet.bodyAdvancement === 'object' && !Array.isArray(pet.bodyAdvancement) ? pet.bodyAdvancement : null;
  const rawAdvancementKey = String(rawAdvancement?.key || '');
  const advancementDefinition = BODY_ADVANCEMENTS[rawAdvancementKey] || null;
  const previousMaxBody = Math.max(pet.stats.body, nonNegativeInt(pet.records?.maxBody));
  const rawUnlockedAt = new Date(pet.bodyAdvancementUnlockedAt ?? '').getTime();
  const advancementUnlocked = Number.isFinite(rawUnlockedAt) || previousMaxBody >= BODY_ADVANCEMENT_BODY;
  pet.bodyAdvancementUnlockedAt = advancementUnlocked ? (Number.isFinite(rawUnlockedAt) ? new Date(rawUnlockedAt).toISOString() : date.toISOString()) : null;
  const selectedAt = new Date(rawAdvancement?.selectedAt ?? '').getTime();
  pet.bodyAdvancement = advancementUnlocked && advancementDefinition ? { key: advancementDefinition.key, selectedAt: Number.isFinite(selectedAt) ? new Date(selectedAt).toISOString() : date.toISOString() } : null;
  ...
  pet.daily.balanceCounts = Object.fromEntries(['work', 'rest', 'exercise', 'cook', 'eat', 'bung', 'mini', 'fishing'].map((key) => [key, nonNegativeInt(rawBalance[key])]));
  ...
  const staminaZeroAt = new Date(pet.survival.staminaZeroAt ?? '').getTime();
  pet.survival.staminaZeroAt = pet.stats.stamina === 0 ? (Number.isFinite(staminaZeroAt) ? new Date(staminaZeroAt).toISOString() : date.toISOString()) : null;
  const zeroAt = new Date(pet.survival.hungerZeroAt ?? '').getTime();
  pet.survival.hungerZeroAt = pet.stats.hunger === 0 ? (Number.isFinite(zeroAt) ? new Date(zeroAt).toISOString() : date.toISOString()) : null;
  ...
  const defaults = {
    ...
    territoryWins: 0,
    appleBestScore: 0,
    singleTetrisBestScore: 0,
    minesweeperNormalBestMs: 0,
    minesweeperHardBestMs: 0,
    omokWins: 0,
    omokDraws: 0,
    omokLosses: 0,
    blockBattleGames: 0,
    blockBattleWins: 0,
    blockBattleLosses: 0,
    seasonBlockBattleWins: 0,
    seasonBlockBattleLosses: 0,
    ...
    sichuanLosses: 0,
    seasonSichuanWins: 0,
    seasonSichuanDraws: 0,
    seasonSichuanLosses: 0,

    legodokuGames: 0,
    legodokuWins: 0,
    legodokuDraws: 0,
    legodokuLosses: 0,
    seasonLegodokuWins: 0,
    seasonLegodokuDraws: 0,
    seasonLegodokuLosses: 0,
  ...
  pet.records.singleTetrisBestAt = Number.isFinite(new Date(pet.records.singleTetrisBestAt ?? '').getTime()) ? new Date(pet.records.singleTetrisBestAt).toISOString() : null;
  pet.records.minesweeperNormalBestAt = Number.isFinite(new Date(pet.records.minesweeperNormalBestAt ?? '').getTime()) ? new Date(pet.records.minesweeperNormalBestAt).toISOString() : null;
  pet.records.minesweeperHardBestAt = Number.isFinite(new Date(pet.records.minesweeperHardBestAt ?? '').getTime()) ? new Date(pet.records.minesweeperHardBestAt).toISOString() : null;
  ...
  for (const key of ['liarGames', 'liarWins', 'liarPointsWon', 'davinciGames', 'davinciTotalWins', 'davinciTotalCorrect', 'davinciWins', 'davinciCorrect', 'spotDifferenceGames', 'spotDifferenceWins', 'spotDifferenceDraws', 'spotDifferenceLosses', 'seasonSpotDifferenceWins', 'seasonSpotDifferenceDraws', 'seasonSpotDifferenceLosses']) delete pet.records[key];

  // v6.9.0부터 경고 시스템은 완전히 제거한다. 예전 저장 데이터에 남아 있어도 폐기한다.
  delete pet.warnings;
  delete pet.records.warnings;
  ...
  delete pet.flags.removedFromLiar;
  ...
  for (const key of ['reaction','apple','singleTetris','omok','blockBattle','sichuan','legodoku','minesweeperNormal','minesweeperHard']) { const t = new Date(pet.seasonBadges[key] ?? '').getTime(); pet.seasonBadges[key] = Number.isFinite(t) ? new Date(t).toISOString() : null; }
  // 왕 경력은 시즌별 로그를 쌓지 않고 게임별 고정 boolean만 유지한다.
  // 기존 저장본은 현재 활성 왕 칭호가 있으면 해당 경력을 자동 승계한다.
  delete pet.seasonBadges.spotDifference;
  const rawKingHistory = pet.kingHistory && typeof pet.kingHistory === 'object' && !Array.isArray(pet.kingHistory) ? pet.kingHistory : {};
  delete rawKingHistory.spotDifference;
  pet.kingHistory = {};
  for (const key of ['apple','singleTetris','omok','blockBattle','sichuan','legodoku','minesweeperNormal','minesweeperHard']) {
    pet.kingHistory[key] = Boolean(rawKingHistory[key] || pet.seasonBadges[key]);
  }
...
export function normalizePet(pet, date = new Date()) {
  ...
  if (!pet.bodyAdvancementUnlockedAt && pet.records.maxBody >= BODY_ADVANCEMENT_BODY) pet.bodyAdvancementUnlockedAt = date.toISOString();
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    daily: {
      ...
      interactionKeys: [],
      balanceCounts: { work: 0, rest: 0, exercise: 0, cook: 0, eat: 0, bung: 0, mini: 0, fishing: 0 },
    ...
    survival: { staminaZeroAt: null, hungerZeroAt: null, hungerPenaltyHoursApplied: 0 },
    effects: { staminaFullUntil: null, hungerFullUntil: null },
    bodyAdvancementUnlockedAt: null,
    bodyAdvancement: null,
    flexItem: null,
    records: {
      days: 1, works: 0, rests: 0, workouts: 0, cooks: 0, foods: 0, bungs: 0,
      relationships: 0, breakups: 0,
    ...
    seasonBestReactionMs: 0, seasonBestReactionAt: null, numberBestAttempts: 0,
      ...
      matches: 0, matingRequests: 0,
      territoryClaims: 0, territorySteals: 0, territoryWins: 0,
      appleBestScore: 0, appleBestAt: null,
      singleTetrisBestScore: 0, singleTetrisBestAt: null,
      minesweeperNormalBestMs: 0, minesweeperNormalBestAt: null,
      minesweeperHardBestMs: 0, minesweeperHardBestAt: null,
      omokWins: 0, omokDraws: 0, omokLosses: 0,
      blockBattleGames: 0, blockBattleWins: 0, blockBattleLosses: 0,
      seasonBlockBattleWins: 0, seasonBlockBattleLosses: 0,

      sichuanGames: 0, sichuanWins: 0, sichuanDraws: 0, sichuanLosses: 0,
      seasonSichuanWins: 0, seasonSichuanDraws: 0, seasonSichuanLosses: 0,

      legodokuGames: 0, legodokuWins: 0, legodokuDraws: 0, legodokuLosses: 0,
      seasonLegodokuWins: 0, seasonLegodokuDraws: 0, seasonLegodokuLosses: 0,
    ...
    seasonBadges: { reaction:null, apple:null, singleTetris:null, omok:null, blockBattle:null, sichuan:null, legodoku:null, minesweeperNormal:null, minesweeperHard:null },
    kingHistory: { apple:false, singleTetris:false, omok:false, blockBattle:false, sichuan:false, legodoku:false, minesweeperNormal:false, minesweeperHard:false },
    flags: {}
  };
...
export function pokePet(state, actorPet, targetPet, date = new Date()) {
  ...
  return { ok: true, message: `${withJosa(targetPet.displayName, '을/를')} 찔렀습니다. 서로 총 ${pair.total}회${pokeGoal.awarded ? ' · 찌르기 목표 달성, 레고력 +1' : ''}`, goalAwarded: pokeGoal.awarded, poke: pokeStatus(state, actorPet.id, targetPet.id) };
}
...
export function refreshTopPokeNews(state, date = new Date()) {
  ...
  const text = `${withJosa(top.members[0].displayName, '과/와')} ${withJosa(top.members[1].displayName, '이/가')} 서로 ${top.total}회 찌르고 있습니다.`;
  if (index >= 0) {
...
export function applyDailyReset(pet, date = new Date(), state = null) {
  ...
  pet.daily.battleDate = current;
  pet.daily.lotteryPlays = 0;
  pet.daily.lastLotteryResult = null;
  pet.daily.interactionKeys = [];
  pet.daily.balanceCounts = { work: 0, rest: 0, exercise: 0, cook: 0, eat: 0, bung: 0, mini: 0, fishing: 0 };
...
export function applyHungerPenalty(pet, date = new Date()) {
  ...
  const cappedTotalHours = Math.min(HUNGER_PENALTY_MAX_HOURS, totalHours);
  const appliedHours = Math.min(HUNGER_PENALTY_MAX_HOURS, nonNegativeInt(pet.survival.hungerPenaltyHoursApplied));
  const pendingHours = Math.max(0, cappedTotalHours - appliedHours);
  if (!pendingHours) {
    if (pet.survival.hungerPenaltyHoursApplied !== appliedHours) {
      pet.survival.hungerPenaltyHoursApplied = appliedHours;
      return { changed: true, hours: 0, pointsLost: 0 };
    }
    return { changed: false, hours: 0, pointsLost: 0 };
  }
  ...
  pet.survival.hungerPenaltyHoursApplied = cappedTotalHours;
  normalizePet(pet, date);
...
export function nextHungerPenaltyAt(pet) {
  ...
  const appliedHours = Math.min(HUNGER_PENALTY_MAX_HOURS, nonNegativeInt(pet.survival.hungerPenaltyHoursApplied));
  if (appliedHours >= HUNGER_PENALTY_MAX_HOURS) return null;
  return new Date(zeroAt + (appliedHours + 1) * 3_600_000).toISOString();
}
...
function blockedStaminaAction(pet, activity, date = new Date()) {
  const lock = staminaActionLock(pet, date);
  return lock.locked ? { ok: false, message: staminaLockMessage(activity), resourceLock: 'stamina' } : null;
}

function blockedHungerAction(pet, activity, date = new Date()) {
  const lock = hungerActionLock(pet, date);
  return lock.locked ? { ok: false, message: hungerLockMessage(activity), resourceLock: 'hunger' } : null;
}

export function workAction(pet, date = new Date()) {
  applyDailyReset(pet, date);
  applyHungerPenalty(pet, date);
  const staminaBlocked = blockedStaminaAction(pet, '일하기', date);
  if (staminaBlocked) return staminaBlocked;
  const hungerBlocked = blockedHungerAction(pet, '일하기', date);
  if (hungerBlocked) return hungerBlocked;
  if (pet.stats.stamina < 8) return { ok: false, message: '체력이 부족합니다. 먼저 쉬거나 에너지드링크를 이용해주세요.' };
  const action = consumeAction(pet, date);
  if (!action.ok) return action;
  pet.stats.points += WORK_POINTS;
  const staminaUse = consumeStamina(pet, 8, date);
  ...
  const hungerText = hungerUse.maintained ? '포만감 100 유지' : `포만감 -${hungerUse.deducted}`;
...
export function restAction(pet, date = new Date()) {
  applyDailyReset(pet, date);
  applyHungerPenalty(pet, date);
  const hungerBlocked = blockedHungerAction(pet, '쉬기', date);
  if (hungerBlocked) return hungerBlocked;
...
export function exerciseAction(pet, date = new Date()) {
  applyDailyReset(pet, date);
  applyHungerPenalty(pet, date);
  const staminaBlocked = blockedStaminaAction(pet, '헬스', date);
  if (staminaBlocked) return staminaBlocked;
  const hungerBlocked = blockedHungerAction(pet, '헬스', date);
  if (hungerBlocked) return hungerBlocked;
  if (pet.stats.stamina < 12) return { ok: false, message: '체력이 부족해 헬스할 수 없습니다. 먼저 쉬거나 에너지드링크를 이용해주세요.' };
  const action = consumeAction(pet, date);
  if (!action.ok) return action;
  const staminaUse = consumeStamina(pet, 12, date);
  const hungerCosts = lifeHungerCostsForBody(pet.stats.body);
...
export function cookAction(pet, date = new Date()) {
  applyDailyReset(pet, date);
  applyHungerPenalty(pet, date);
  applyTimedEffects(pet, date);
  const staminaBlocked = blockedStaminaAction(pet, '요리', date);
  if (staminaBlocked) return staminaBlocked;
  if (pet.stats.hunger >= 100) return { ok: false, message: '포만감이 이미 가득 차서 지금은 요리할 필요가 없습니다.' };
  if (pet.stats.stamina < 5) return { ok: false, message: '체력이 부족해 요리할 수 없습니다. 먼저 쉬거나 에너지드링크를 이용해주세요.' };
  const action = consumeAction(pet, date);
  if (!action.ok) return action;
  const staminaUse = consumeStamina(pet, 5, date);
  const beforeHunger = pet.stats.hunger;
  pet.stats.hunger = Math.min(100, pet.stats.hunger + 50);
  const actualRecovery = pet.stats.hunger - beforeHunger;
  pet.records.cooks = nonNegativeInt(pet.records.cooks) + 1;
  pet.daily.balanceCounts.cook = nonNegativeInt(pet.daily.balanceCounts.cook) + 1;
  normalizePet(pet, date);
  const staminaText = staminaUse.maintained ? '체력 100 유지' : `체력 -${staminaUse.deducted}`;
  return lifeActionResult(pet, `요리해서 먹었습니다. 포만감 +${actualRecovery} · ${staminaText}`, date);
}
...
export function eatAction(pet, foodId, date = new Date()) {
  ...
  if (currentLevel < food.minLevel) return { ok: false, message: `Lv.${food.minLevel}부터 이용할 수 있습니다.` };
  if (pet.stats.points < food.price) return { ok: false, message: '포인트가 부족합니다.' };

  if (food.category === 'energy') {
    applyTimedEffects(pet, date);
    if (pet.stats.stamina >= 100) return { ok: false, message: '체력이 이미 가득 찼습니다.' };
    pet.stats.points -= food.price;
    pet.records.pointsSpent += food.price;
    const restored = restoreStamina(pet, Number(food.stamina) || 0, date);
    pet.records.foods += 1;
    ...
    return {
      ok: true,
      foodId: food.id,
      staminaRecovery: restored.restored,
      hungerRecovery: 0,
      bodyDelta: 0,
      bodyStage: getBodyStage(pet.stats.body),
      message: `${food.name}를 마셨습니다. ${food.price}P 사용 · 체력 +${restored.restored}`
    };
  }

  if (pet.stats.hunger >= 100) return { ok: false, message: '포만감이 이미 가득 찼습니다.' };
  pet.stats.points -= food.price;
  pet.records.pointsSpent += food.price;
  const beforeHunger = pet.stats.hunger;
  const beforeBody = pet.stats.body;
  ...
  return {
    ...
    bodyDelta,
    bodyStage: getBodyStage(pet.stats.body),
    message: `${withJosa(food.name, '을/를')} 먹었습니다. ${food.price}P 사용 · 포만감 +${actualRecovery} · ${bodyText}`
  };
}
...
export function loudspeakerView(state, date = new Date()) {
  const raw = state?.loudspeaker;
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  const startedAt = new Date(raw.startedAt ?? '').getTime();
  const expiresAt = new Date(raw.expiresAt ?? '').getTime();
  const message = normalizedStatusText(raw.message);
  const displayName = String(raw.displayName ?? '').trim();
  if (!Number.isFinite(startedAt) || !Number.isFinite(expiresAt) || expiresAt <= date.getTime() || !message || !displayName) return null;
  return {
    id: String(raw.id ?? ''),
    userId: String(raw.userId ?? ''),
    petId: String(raw.petId ?? ''),
    displayName,
    message: truncateCodePoints(message, LOUDSPEAKER_MAX_LENGTH),
    startedAt: new Date(startedAt).toISOString(),
...
export function shopStateView(user, pet, date = new Date()) {
  ...
  return {
    ...
    battlePlayed: nonNegativeInt(pet.daily.battlePlayed),
    battleBonus: nonNegativeInt(pet.daily.battleBonus),
    battleLimit: battleLimit(pet),
    battleBaseLimit: BATTLE_PLAYS_PER_DAY,
    battleTicketBonus: BATTLE_TICKET_BONUS,
...
export function purchaseShopItem(state, user, pet, itemIdValue, input = {}, requestIdValue = '', date = new Date(), random = secureRandomUnit) {
  ...
  let loudspeakerMessage = '';
  if (itemId === 'loudspeaker') {
    loudspeakerMessage = normalizedStatusText(input?.message);
    if (!loudspeakerMessage) return { ok: false, message: '확성기 메시지를 입력해주세요.' };
    if ([...loudspeakerMessage].length > LOUDSPEAKER_MAX_LENGTH) return { ok: false, message: `확성기 메시지는 ${LOUDSPEAKER_MAX_LENGTH}자 이내로 입력해주세요.` };
    const activeLoudspeaker = loudspeakerView(state, date);
    if (activeLoudspeaker) return { ok: false, message: '현재 다른 레고의 확성기가 표시 중입니다. 끝난 뒤 다시 이용해주세요.' };
  }
  ...
  if (FLEX_ITEMS[itemId]) {
    if (item.retired) return { ok: false, message: '현재 판매가 종료된 플렉스 아이템입니다.' };
    const currentLevel = levelForPower(pet.stats.legoPower);
    if (currentLevel < item.requiredLevel) return { ok: false, message: `Lv.${item.requiredLevel}부터 구매할 수 있는 플렉스 아이템입니다.` };
  }
  if (itemId === 'staminaHour' && timedEffectActive(pet, 'staminaFullUntil', date)) return { ok: false, message: '이미 체력 100% 유지권을 사용 중입니다.' };
  if (itemId === 'hungerHour' && timedEffectActive(pet, 'hungerFullUntil', date)) return { ok: false, message: '이미 포만감 100% 유지권을 사용 중입니다.' };
  ...
  if (itemId === 'miniGame20') {
    pet.daily.miniGameBonus = Math.min(10_000, pet.daily.miniGameBonus + 20);
    message = `미니게임 이용 가능 횟수가 이번 게임 하루에 +20회 늘었습니다. 현재 한도 ${dailyMiniGameLimit(pet)}회`;
  } else if (itemId === 'battle20') {
    normalizeBattleDaily(pet, date);
    pet.daily.battleBonus = Math.min(100_000, pet.daily.battleBonus + BATTLE_TICKET_BONUS);
    message = `대전 이용 가능 횟수가 이번 게임 하루에 +${BATTLE_TICKET_BONUS}회 늘었습니다. 현재 합산 한도 ${battleLimit(pet)}회`;
    ...
    if (prize > 0) {
      ...
      if (prize >= 1000) {
        addPublicEvent(state, `${withJosa(pet.displayName, '이/가')} 하루 복권에서 ${prize.toLocaleString('ko-KR')}P에 당첨됐습니다.`, 'lottery', [pet.id], date);
        globalRefresh = true;
      }
    ...
    pet.survival.hungerZeroAt = null;
    pet.survival.hungerPenaltyHoursApplied = 0;
    message = '포만감이 100%가 되었고 1시간 동안 100%로 유지됩니다.';
  } else if (itemId === 'loudspeaker') {
    const startedAt = date.toISOString();
    const expiresAt = new Date(date.getTime() + LOUDSPEAKER_DURATION_SECONDS * 1000).toISOString();
    state.loudspeaker = {
      id: id('loudspeaker'),
      userId: user.id,
      petId: pet.id,
      displayName: pet.displayName,
      message: loudspeakerMessage,
      startedAt,
      expiresAt
    };
    loudspeaker = loudspeakerView(state, date);
    message = `확성기를 ${LOUDSPEAKER_DURATION_SECONDS}초 동안 사용합니다.`;
    ...
    message = `${withJosa(item.name, '을/를')} 24시간 동안 장착합니다.`;
    globalRefresh = true;
    if (item.kind === 'nameplate') addPublicEvent(state, `${withJosa(pet.displayName, '이/가')} ${withJosa(item.name, '을/를')} 장착했습니다.`, 'flex-item', [pet.id], date);
  }

  normalizePet(pet, date);
  const result = { ok: true, itemId, price, prize, loudspeaker, message, globalRefresh, shop: shopStateView(user, pet, date) };
...
export function breakup(state, actorPet, targetPet, date = new Date()) {
  ...
  addNotification(state, targetPet.userId, `${withJosa(actorPet.displayName, '과/와')}의 커플 관계가 종료되었습니다.`, 'relationship', { petId: actorPet.id }, date);
  addPublicEvent(state, `${withJosa(actorPet.displayName, '과/와')} ${withJosa(targetPet.displayName, '이/가')} 헤어졌습니다.`, 'relationship', [actorPet.id, targetPet.id], date);
  return { ok: true, message: `${withJosa(targetPet.displayName, '과/와')} 헤어졌습니다.` };
}

function matingResultText(first, second, compatibility) {
  const groups = compatibility <= 10 ? [
    `${first}와 ${second}가 M텔에 들어간 지 12분 만에 퇴실했습니다. 프런트도 이유를 묻지 않았습니다.`,
    `${second}가 끝나고 담배를 피우려 했는데 ${first}가 “나도 하나 줘”라고 했습니다. ${second}가 “넌 뭘 했다고 피워?”라고 물었습니다.`,
    '둘이 가장 오래 한 것은 배달 메뉴 고르기였습니다.ㅋ',
    `${first}와 ${second}가 대실 8시간을 끊었는데 실제 사용시간은 배달 기다린 시간보다 짧았습니다.`,
    `${first}가 “나 원래 안 이래”라고 하자 ${second}가 “그 말 하는 사람 다 그러더라”라고 했습니다.`
  ] : compatibility <= 20 ? [
    `${first}가 끝난 뒤 괜찮았냐고 묻자 ${second}가 “응 뭐… 잘 자긴 하겠다”라고 했습니다.`,
    `${second}가 “잠깐만”이라고 했는데 ${first}가 “이미 끝났는데?”라고 대답했습니다.`,
    `${first}가 “좋았지?”라고 세 번 물었고 ${second}는 세 번 다 물을 마셨습니다.`,
    `${first}가 다음 날 “어제 괜찮았지?”라고 보냈는데 ${second}가 계좌번호를 보냈습니다.`,
    `${first}가 “이번에는 열심히 할게”라고 했는데 ${second}가 “이번이라는 단어부터 마음에 안 들어”라고 했습니다.`
  ] : compatibility <= 30 ? [
    `둘이 모텔비를 반반 냈는데 ${second}가 자기는 이용한 게 없다며 환불을 요구했습니다.`,
    `${first}가 다시 분위기를 잡으려 하자 ${second}가 “우리 그냥 술이나 마실까?”라고 했습니다.`,
    `${first}가 끝난 뒤 ${second}가 휴대폰 타이머를 확인했습니다. 잘못 눌러서 멈춘 줄 알았습니다.`,
    `${first}가 끝나고 담배를 달라고 하자 ${second}가 “넌 뭘 했다고 피워?”라고 물었습니다.`,
    '둘이 관계를 마치고 가장 먼저 한 일은 네이버에 ‘대실 환불’ 검색하기였습니다.'
  ] : compatibility <= 40 ? [
    `${first}와 ${second}가 편의점에서 비타민을 고르다가 레고를 만났습니다. 셋 다 동시에 다른 코너로 이동했습니다.`,
    `${first}와 ${second}가 한 번 더 할지 그냥 떡볶이를 시킬지 14분째 회의 중입니다.`,
    `${second}가 관계 후 “다시는 술 먹고 결정 안 한다”는 말을 남겼습니다. ${first}는 조용히 물만 마셨습니다.`,
    `세윤이 흡연실 문을 열었다가 ${first}와 ${second}를 발견했습니다. 잠깐 보더니 “아 계속해. 몇 분 안 남은 거 같은데”라며 문을 닫았습니다.`,
    `${first}와 ${second}가 다음 벙에서 마주치면 일단 모르는 사람은 아닌 척하기로 했습니다.`
  ] : compatibility <= 50 ? [
    `킄킄이 ${first}와 ${second}가 동시에 사라지는 걸 보고 아무 말 없이 타이머를 켰습니다.`,
    '둘이 24분 만에 돌아오자 레고가 “어디 갔다 왔어?”가 아니라 “벌써?”라고 물었습니다.',
    '킄킄이 문 앞에 놓인 신발 두 켤레를 보고 “아 여기구나”라고 했습니다. 굳이 문은 열지 않았습니다.',
    '다매가 비상계단에서 둘을 발견하고 “여기 CCTV 있어”라고 알려줬습니다. 둘은 그제야 위를 봤습니다.',
    '상어가 흡연실 문을 열었다가 바로 닫았습니다. 밖에 나와서는 “저긴 지금 금연구역이다”라고 했습니다.'
  ] : compatibility <= 60 ? [
    `${second}가 갑자기 “우리 이거 내일 후회하는 거 아니야?”라고 물었고 ${first}가 “난 이미 좀 하는 중”이라고 했습니다.`,
    '호야 비상계단에서 콩이가 둘을 목격한 뒤 “나도 저런 일 좀 생겨봤으면 좋겠다”고 부러움을 호소했습니다.',
    `둘이 분위기를 잡으려 했는데 ${second}가 갑자기 “근데 너 나 몇 살인지 알긴 해?”라고 물었습니다.`,
    `${second}가 ${first}에게 “아까 얘기 절대 하지 마”라고 보내려다 공챗에 올렸습니다. 삭제 전 17명이 읽었습니다.`,
    `${first}가 멋있게 옷을 벗으려다 안 빠져서 ${second}에게 “잠깐만 잡아봐”라고 부탁했습니다. 옷이 작았나 ㅋ`
  ] : compatibility <= 70 ? [
    '둘이 동시에 사라졌다가 따로 돌아왔는데 둘 다 옷을 거꾸로 입고 있었습니다.',
    `${first}와 ${second}가 보이스룸을 켜둔 걸 한 시간 뒤에 알았습니다. 그날 방에 있던 사람들은 아무도 내용을 말하지 않습니다.`,
    `세윤이 문을 잘못 열었다가 굳었고 안에서 ${first}가 “닫아줘”라고 했습니다. 세윤은 문뿐 아니라 눈도 닫았습니다.`,
    `${first}와 ${second}를 찾으러 간 레고가 20분 뒤 혼자 돌아와 “찾긴 찾았는데 못 찾은 걸로 하자”고 했습니다.`,
    '킄킄이 흡연실 문을 열었다가 닫고 나와 “오늘 담배 끊는다”고 선언했습니다.'
  ] : compatibility <= 80 ? [
    '잔치집 흡연실에서 일정한 박자가 들렸지만 아무도 음악을 튼 적이 없습니다.',
    `${first}가 분위기 잡으며 “오늘 밤 길다”라고 했는데 ${second}가 끝나고 시간을 보더니 “진짜 기네. 이제 뭐 하지?”라고 물었습니다.`,
    `${first}는 지퍼를 올리며 들어왔고 ${second}는 머리를 묶으며 들어왔습니다. 둘 다 편의점 다녀왔다고 합니다.`,
    `${second} 목에 난 흔적을 본 킄킄이 “요즘 모기는 닉네임도 있냐?”라고 물었습니다.`,
    '비상계단에서 둘이 황급히 내려왔는데 3층에는 정체불명의 빅사이즈 팬티 한 장이 남아 있었습니다.'
  ] : compatibility <= 90 ? [
    '잔치집 흡연실 문을 연 레고가 3초 동안 정지하더니 “제가 나갈게요. 아니 근데 왜 제가 나가죠?”라고 했습니다.',
    `${first}가 끝나고 만족스럽게 누웠는데 ${second}가 “근데 본게임은 언제 해?”라고 물었습니다.`,
    `${second}가 “나 오늘 좀 기대했는데”라고 하자 ${first}가 “나도 내가 이럴 줄 몰랐어”라고 했습니다. 피해자가 두 명입니다.`,
    `${first}가 “어땠어?”라고 물었습니다. ${second}가 한참 고민하더니 “너 사람은 진짜 좋아”라고 대답했습니다.`,
    `${first}와 ${second}가 대실 시간을 5번 연장했습니다.`
  ] : [
    `${first}와 ${second}가 나올 때 둘 다 계단을 난간 잡고 내려왔습니다. 하앙하앙`,
    `${first}와 ${second}가 다음 날 서로 연락하지 않기로 했습니다. 오후가 되기 전에 둘 다 먼저 연락했습니다.`,
    '호야 비상계단에서 콩이가 그 현장을 목격한 뒤 본인이 왜 혼자인지 진지하게 고민하기 시작했습니다.',
    '다음 날 둘이 “아무 일도 없었다”고 주장했지만 레고방에서는 이미 둘을 ‘호야키스남녀’라고 부르고 있습니다.',
    `${first}와 ${second}가 어젯밤은 실수였다고 합의했습니다. 그러나 그날 저녁 둘이 다시 만났습니다.`
  ];
...
export function socialAction(state, actorPet, targetPet, action, extra = {}, date = new Date()) {
  state.requests ??= {};
  if (!actorPet?.alive) return { ok: false, message: '내 레고를 찾을 수 없습니다.' };
  if (!targetPet?.alive || actorPet.id === targetPet.id) return { ok: false, message: '상대 레고를 찾을 수 없습니다.' };
  if (action === 'requestMatch') {
    ...
    const request = {
      id: id('req'), type: 'match', fromPetId: actorPet.id, toPetId: targetPet.id, status: 'pending',
  ...
  if (action === 'acceptMatch' || action === 'rejectMatch') {
    ...
    if (action === 'rejectMatch') {
      ...
      addNotification(state, targetPet.userId, `${withJosa(actorPet.displayName, '이/가')} 매칭 신청을 거절했습니다.`, 'match', { petId: actorPet.id }, date);
    ...
    addNotification(state, targetPet.userId, `${withJosa(actorPet.displayName, '이/가')} 매칭을 수락했습니다. 커플이 되었습니다.`, 'match', { petId: actorPet.id }, date);
    addPublicEvent(state, `${withJosa(actorPet.displayName, '과/와')} ${withJosa(targetPet.displayName, '이/가')} 커플이 되었습니다.`, 'relationship', [actorPet.id, targetPet.id], date);
    return { ok: true, message: `${withJosa(targetPet.displayName, '과/와')} 커플이 되었습니다.` };
  }
  if (action === 'breakup') return breakup(state, actorPet, targetPet, date);
  if (action === 'requestMating') {
    const duplicate = Object.values(state.requests).some((request) => request.type === 'mating' && request.status === 'pending'
      && new Set([request.fromPetId, request.toPetId]).has(actorPet.id)
      && new Set([request.fromPetId, request.toPetId]).has(targetPet.id));
    if (duplicate) return { ok: false, message: '이미 두 레고 사이에 처리 중인 교미 신청이 있습니다.' };
    const hungerBlocked = blockedHungerAction(actorPet, '교미 신청', date);
    if (hungerBlocked) return hungerBlocked;
    const hungerUse = consumeInteractionHunger(actorPet, date);
    actorPet.records.matingRequests += 1;
    const request = {
      id: id('req'), type: 'mating', fromPetId: actorPet.id, toPetId: targetPet.id, status: 'pending',
      createdAt: nowIso(date)
    };
    state.requests[request.id] = request;
    addNotification(state, targetPet.userId, `${withJosa(actorPet.displayName, '이/가')} 교미 신청을 걸었습니다.`, 'mating', { requestId: request.id, petId: actorPet.id }, date);
  ...
  if (action === 'acceptMating' || action === 'rejectMating') {
    ...
    if (action === 'acceptMating') {
      ...
      addNotification(state, targetPet.userId, `${withJosa(actorPet.displayName, '이/가')} 교미 신청을 수락했습니다. 궁합도 ${compatibility}%`, 'mating', { compatibility }, date);
    ...
    addNotification(state, targetPet.userId, `${withJosa(actorPet.displayName, '이/가')} 교미 신청을 거절했습니다.`, 'mating', { petId: actorPet.id }, date);
    return { ok: true, accepted: false, message: '교미 신청을 거절했습니다.' };
  }
...
export function createBung(state, hostPet, input = {}, date = new Date()) {
  ...
  const staminaBlocked = blockedStaminaAction(hostPet, '벙 참가', date);
  if (staminaBlocked) return staminaBlocked;
  const hungerBlocked = blockedHungerAction(hostPet, '벙 참가', date);
  if (hungerBlocked) return hungerBlocked;
  state.bungs ??= {};
  state.bungOperations ??= {};
  const title = String(input.title ?? '').replace(/\s+/gu, ' ').trim().slice(0, 40);
  const stakePoints = Math.floor(Number(input.stakePoints));
  const requestId = String(input.requestId ?? '').trim();
  if (requestId && !/^[A-Za-z0-9:_-]{8,160}$/.test(requestId)) return { ok: false, message: '잘못된 요청입니다.' };
  if (requestId) {
    const previous = state.bungOperations[requestId];
    if (previous) {
      if (previous.petId !== hostPet.id) return { ok: false, message: '잘못된 중복 요청입니다.' };
  ...
  const activeBung = Object.values(state.bungs).find((bung) => bung?.hostPetId === hostPet.id && ['open', 'live'].includes(bung.status));
  if (activeBung) return { ok: false, message: '한 번에 하나의 벙만 열 수 있습니다. 기존 벙을 먼저 종료하거나 취소해주세요.' };
  ...
  const result = { ok: true, message: `${stakePoints}P를 사용해 벙을 열었습니다.`, bung: structuredClone(bung) };
  if (requestId) state.bungOperations[requestId] = { id: requestId, userId: hostPet.userId, petId: hostPet.id, createdAt: nowIso(date), result: structuredClone(result) };
  return result;
}
...
export function joinBung(state, pet, bung, date = new Date()) {
  ensurePetSchema(pet, date);
  const staminaBlocked = blockedStaminaAction(pet, '벙 참가', date);
  if (staminaBlocked) return staminaBlocked;
  const hungerBlocked = blockedHungerAction(pet, '벙 참가', date);
  if (hungerBlocked) return hungerBlocked;
...
export function finishBung(state, hostPet, bung, date = new Date()) {
  ...
  bung.endedAt = nowIso(date);
  const rewards = [];
  if (participants.length >= BUNG_MIN_PLAYERS) {
    for (const entry of participants) {
      if (entry.rewarded) continue;
      const pet = state.pets[entry.petId];
      if (!pet?.alive) continue;
      ensurePetSchema(pet, date);
      const staminaUse = consumeStamina(pet, 10, date);
    ...
    for (const entry of participants) entry.status = 'completed';
    addPublicEvent(state, `${bung.title} 벙이 종료되었습니다. 끝까지 참가한 인원이 2명 미만이라 보상은 지급되지 않았습니다.`, 'bung', participants.map((item) => item.petId), date);
  }
  ...
  const endedIds = Object.values(state.bungs ?? {})
    ...
    .sort((a, b) => new Date(b.endedAt) - new Date(a.endedAt))
    .map((item) => item.id);
  for (const oldId of endedIds.slice(50)) delete state.bungs[oldId];

  if (participants.length < BUNG_MIN_PLAYERS) return { ok: true, rewarded: false, message: '벙을 종료했습니다. 끝까지 참가한 인원이 2명 미만이라 보상은 없습니다.', rewards: [] };
  return { ok: true, rewarded: true, message: `벙을 끝냈습니다. 참가자 체력 -10, 레고력 +1${rewards.some((item) => item.legoPower > 1) ? ' (오늘의 레고력 보너스 포함)' : ''}`, rewards };
}

export function processBungTimers(state, date = new Date()) {
  state.bungs ??= {};
  let changed = false;
  for (const bung of Object.values(state.bungs)) {
    if (!bung || !['open', 'live'].includes(bung.status)) continue;
    const base = new Date((bung.status === 'live' ? bung.startedAt : bung.createdAt) ?? '').getTime();
    const ttl = bung.status === 'live' ? BUNG_LIVE_TTL_MS : BUNG_OPEN_TTL_MS;
    if (!Number.isFinite(base) || base + ttl > date.getTime()) continue;
    bung.status = 'cancelled';
    bung.endedAt = nowIso(date);
    bung.autoCancelled = true;
    bung.autoCancelReason = 'inactive-timeout';
    changed = true;
  }
  return { changed };
}

export function bungNextAlarmAt(state, date = new Date()) {
  const now = date.getTime();
  const times = Object.values(state.bungs ?? {}).filter((bung) => bung && ['open', 'live'].includes(bung.status)).map((bung) => {
    const base = new Date((bung.status === 'live' ? bung.startedAt : bung.createdAt) ?? '').getTime();
    const ttl = bung.status === 'live' ? BUNG_LIVE_TTL_MS : BUNG_OPEN_TTL_MS;
    return Number.isFinite(base) ? base + ttl : NaN;
...
export function miniChallengeView(challenge) {
  if (!challenge) return null;
  if (challenge.gameId === 'minesweeper') return minesweeperChallengeView(challenge);
...
function miniChallengeIsActive(challenge, date = new Date()) {
  ...
  if (challenge.gameId === 'block' || challenge.gameId === 'minesweeper' || challenge.gameId === 'tetrisSingle') return true;
...
export function startMiniGame(state, pet, gameId = 'oddEven', date = new Date(), input = {}) {
  ...
  const hungerBlocked = blockedHungerAction(pet, game.name, date);
  if (hungerBlocked) return hungerBlocked;
  ...
  let stake = 0;
  const minesweeperConfig = gameId === 'minesweeper' ? minesweeperDifficulty(input?.difficulty) : null;
  if (gameId === 'oddEven') {
  ...
  const hungerUse = gameId === 'minesweeper' ? { cost: 0 } : consumeInteractionHunger(pet, date);
  if (gameId === 'oddEven') {
  ...
  const challenge = {
    ...
    expiresAt: (gameId === 'block' || gameId === 'minesweeper') ? null : gameId === 'apple' ? new Date(date.getTime() + 120_000).toISOString() : gameId === 'tetrisSingle' ? new Date(date.getTime() + SINGLE_TETRIS_DURATION_MS).toISOString() : new Date(date.getTime() + 10 * 60_000).toISOString(), completed: false,
    ...
    usageCounted: gameId !== 'minesweeper',
  ...
  if (gameId === 'minesweeper') Object.assign(challenge, minesweeperChallengeFields(minesweeperConfig.id));
  if (gameId === 'tetrisSingle') Object.assign(challenge, { tetrisDurationMs: SINGLE_TETRIS_DURATION_MS, tetrisGravityMs: BLOCK_BATTLE_GRAVITY_MS });
  state.miniGameChallenges ??= {};
  if (gameId !== 'minesweeper') pet.daily.miniGamesPlayed += 1;
  state.miniGameChallenges[challenge.id] = challenge;
  normalizePet(pet, date);
  const startMessage = gameId === 'oddEven'
    ? `${stake}P를 걸고 홀짝을 시작했습니다.`
    : gameId === 'minesweeper'
      ? `지뢰찾기 ${minesweeperConfig.label} 준비 완료. 첫 칸을 열면 1회가 차감되고 시간이 시작됩니다.`
      : `${game.name} 시작!`;
  return { ok: true, hungerCost: hungerUse.cost, message: startMessage, challenge: miniChallengeView(challenge) };
}
...
function completeChallenge(pet, challenge, reward, detail, date = new Date()) {
  ...
  const settledReward = nonNegativeInt(reward);
  challenge.completed = true;
  challenge.completedAt = nowIso(date);
  challenge.reward = settledReward;
  challenge.resultDetail = String(detail ?? '').slice(0, 240);
  if (settledReward > 0) {
    pet.stats.points += settledReward;
    pet.records.pointsEarned += settledReward;
  }
  if (!challenge.usageCounted) {
    pet.daily.miniGamesPlayed += 1;
    challenge.usageCounted = true;
  }
  ...
  normalizePet(pet, date);
  return { ok: true, finished: true, reward: settledReward, detail, goalAwarded: goal.awarded, message: `${detail}${settledReward > 0 ? ` · ${settledReward}P 획득` : ''}${goal.awarded ? ' · 개인게임 목표 달성, 레고력 +1' : ''}` };
}
...
export function finishMiniGame(state, pet, challengeId, value, date = new Date(), input = {}) {
  ...
  if (challenge.gameId === 'tetrisSingle') {
    const expiresAt = new Date(challenge.expiresAt ?? '').getTime();
    const createdAt = new Date(challenge.createdAt ?? '').getTime();
    const elapsedMs = Number.isFinite(createdAt) ? Math.max(0, date.getTime() - createdAt) : 0;
    // 타임아웃 직후 네트워크 지연/모바일 백그라운드 복귀를 고려해 60초 정산 유예를 둔다.
    if (!Number.isFinite(expiresAt) || date.getTime() > expiresAt + 60_000) return { ok: false, message: '싱글 테트리스 정산 시간이 지나 기록할 수 없습니다.' };
    const counts = input?.lineClearCounts && typeof input.lineClearCounts === 'object' ? input.lineClearCounts : {};
    const one = Math.max(0, Math.min(500, Math.floor(Number(counts[1] ?? counts.one) || 0)));
    const two = Math.max(0, Math.min(250, Math.floor(Number(counts[2] ?? counts.two) || 0)));
    const three = Math.max(0, Math.min(166, Math.floor(Number(counts[3] ?? counts.three) || 0)));
    const four = Math.max(0, Math.min(125, Math.floor(Number(counts[4] ?? counts.four) || 0)));
    const score = one + two * 3 + three * 5 + four * 8;
    const lines = one + two * 2 + three * 3 + four * 4;
    const reason = String(input?.endReason || '').slice(0, 30);
    if (!['timeout', 'gameover'].includes(reason)) return { ok: false, message: '싱글 테트리스 종료 사유가 올바르지 않습니다.' };
    if (reason === 'timeout' && Number.isFinite(expiresAt) && date.getTime() + 1500 < expiresAt) return { ok: false, message: '아직 싱글 테트리스 제한시간이 남아 있습니다.' };
    // 비정상 조작/변조로 현실적으로 불가능한 수준의 기록이 들어오는 것을 최소한으로 방어한다.
    const maxEvents = Math.floor(Math.max(1, elapsedMs) / 250) + 8;
    if (one + two + three + four > maxEvents) return { ok: false, message: '싱글 테트리스 기록 검증에 실패했습니다.' };
    const previousBest = Math.max(0, nonNegativeInt(pet.records.singleTetrisBestScore));
    const result = completeChallenge(pet, challenge, score, `싱글 테트리스 종료 · ${score}점 · ${lines}줄 제거`, date);
    challenge.tetrisScore = score;
    challenge.tetrisLines = lines;
    challenge.tetrisEndReason = reason;
    challenge.tetrisLineClearCounts = { 1: one, 2: two, 3: three, 4: four };
    if (score > previousBest) {
      pet.records.singleTetrisBestScore = score;
      pet.records.singleTetrisBestAt = nowIso(date);
    }
    normalizePet(pet, date);
    return { ...result, score, lines, previousBest, bestScore: Math.max(previousBest, score), endReason: reason, challenge: miniChallengeView(challenge) };
  }
  if (challenge.gameId === 'apple') {
    normalizeAppleChallenge(challenge);
  ...
  if (new Date(challenge.expiresAt).getTime() < date.getTime()) {
    const expired = completeChallenge(pet, challenge, 0, '미니게임 제한 시간이 지나 종료되었습니다.', date);
    return { ...expired, expired: true, message: '미니게임 제한 시간이 지나 종료되었습니다.' };
  }
  if (challenge.gameId === 'oddEven') {
    ...
    if (challenge.streak >= 3) {
      ...
      addPublicEvent(state, `${withJosa(pet.displayName, '이/가')} 홀짝 3연승으로 순이익 ${netProfit}P를 얻었습니다.`, 'game', [pet.id], date);
  ...
  if (challenge.gameId === 'reaction') {
    const serverObservedMs = Math.round(date.getTime() - Number(challenge.readyAt || 0));
    ...
    if (serverObservedMs < REACTION_MIN_VALID_MS || serverObservedMs > 10_000) {
      return {
    ...
    if (hasClientMeasurement) {
      ...
      if (roundedClientMs < 0 || roundedClientMs > 10_000 || clockMismatch > REACTION_CLOCK_TOLERANCE_MS) {
        return {
          ...completeChallenge(pet, challenge, 0, '비정상적인 클라이언트 측정값이 감지되어 기록과 보상이 반영되지 않았습니다.', date),
          reactionMs: 0,
          invalidReaction: true
        };
      }
    }
    // 랭킹과 보상은 조작 가능한 클라이언트 측정값이 아니라 서버가 실제로 수신한 시간을 사용한다.
    const reactionMs = serverObservedMs;
    ...
    return { ...completeChallenge(pet, challenge, reward, `반응 속도 ${reactionMs}ms`, date), reactionMs, verified: true };
  }
...
export function selectBlockGame(state, pet, challengeId, input = {}, requestId = '', date = new Date()) {
  ...
  applyHungerPenalty(pet, date);
  const challenge = state.miniGameChallenges?.[challengeId];
  if (!challenge || challenge.petId !== pet.id || challenge.gameId !== 'block') return { ok: false, message: '유효한 블록게임이 없습니다.' };
  normalizeBlockChallenge(challenge);
  // 마지막 선택은 서버에서 정산됐지만 응답이 끊긴 경우 같은 요청을 다시 보내도
  // 저장된 확정 보상만 재전송한다. 포인트를 두 번 더하지 않는 idempotent 복구 경로다.
  if (challenge.completed) {
    const reward = Math.max(0, nonNegativeInt(challenge.reward));
    const detail = String(challenge.resultDetail || (challenge.blockAllClear
    ...
    return {
      ok: true, finished: true, replayed: true, reward, finalPoints: reward, detail,
      allClear: challenge.blockAllClear, removedCount: challenge.blockRemovedCount,
      remainingCount: challenge.blockRemainingCount, moveCount: challenge.blockMoveCount,
      pendingPoints: challenge.blockPendingPoints, boardVersion: challenge.blockBoardVersion,
      availableGroups: challenge.blockAvailableGroups, board: challenge.blockBoard.map((row) => [...row]),
      message: `${detail}${reward > 0 ? ` · ${reward}P 획득` : ''}`
    };
  }
...
export function playMinesweeperGame(state, pet, challengeId, input = {}, date = new Date()) {
  applyDailyReset(pet, date);
  applyHungerPenalty(pet, date);
  const challenge = state.miniGameChallenges?.[challengeId];
  if (!challenge || challenge.petId !== pet.id || challenge.completed || challenge.gameId !== 'minesweeper') return { ok: false, message: '유효한 지뢰찾기 게임이 없습니다.' };
  normalizeMinesweeperChallenge(challenge);
  const firstRevealPending = String(input?.action ?? 'reveal') === 'reveal' && !challenge.minesweeperStartedAt;
  if (firstRevealPending && !challenge.usageCounted) {
    const hungerBlocked = blockedHungerAction(pet, '지뢰찾기', date);
    if (hungerBlocked) return hungerBlocked;
    const miniLimit = dailyMiniGameLimit(pet);
    if (pet.daily.miniGamesPlayed >= miniLimit) return { ok: false, message: `이번 게임 하루의 미니게임 ${miniLimit}회를 모두 사용했습니다.` };
  }

  const result = applyMinesweeperAction(challenge, input, date);
  if (!result.ok) return result;
  let hungerCost = 0;
  if (result.firstReveal && !challenge.usageCounted) {
    const hungerUse = consumeInteractionHunger(pet, date);
  ...
  if (!result.finished) {
    normalizePet(pet, date);
    return { ...result, hungerCost, challenge: minesweeperChallengeView(challenge) };
  }

  const reward = result.cleared ? difficulty.successReward : difficulty.failReward;
  if (result.cleared) {
    const recordKey = difficulty.id === 'hard' ? 'minesweeperHardBestMs' : 'minesweeperNormalBestMs';
    const recordAtKey = difficulty.id === 'hard' ? 'minesweeperHardBestAt' : 'minesweeperNormalBestAt';
    const elapsedMs = Math.max(1, Math.round(Number(result.elapsedMs) || 1));
    if (!pet.records[recordKey] || elapsedMs < pet.records[recordKey]) {
      pet.records[recordKey] = elapsedMs;
      pet.records[recordAtKey] = nowIso(date);
    }
  }
  const elapsedSeconds = (Math.max(1, Number(result.elapsedMs) || 1) / 1000).toFixed(1);
  const detail = result.cleared
    ? `지뢰찾기 ${difficulty.label} 클리어 · ${elapsedSeconds}초`
    : `지뢰찾기 ${difficulty.label} 실패 · 지뢰를 밟았습니다.`;
  const completion = completeChallenge(pet, challenge, reward, detail, date);
  return {
    ...result,
    ...completion,
    hungerCost,
    ...
    challenge: minesweeperChallengeView(challenge, { revealMines: true })
  };
}

export function abandonMinesweeperGame(state, pet, challengeId, date = new Date()) {
  applyDailyReset(pet, date);
  applyHungerPenalty(pet, date);
  const challenge = state.miniGameChallenges?.[challengeId];
  if (!challenge || challenge.petId !== pet.id || challenge.completed || challenge.gameId !== 'minesweeper') return { ok: false, message: '포기할 진행 중 지뢰찾기가 없습니다.' };
  normalizeMinesweeperChallenge(challenge);
  challenge.completed = true;
  challenge.completedAt = nowIso(date);
  challenge.reward = 0;
  ...
  return {
    ...
    reward: 0,
    detail: '지뢰찾기를 포기했습니다.',
    message: '지뢰찾기를 포기했습니다. 포인트 보상은 없습니다.',
    challenge: minesweeperChallengeView(challenge, { revealMines: true })
  };
}
...
export function settleExpiredMiniGames(state, date = new Date()) {
  ...
  for (const challenge of Object.values(state.miniGameChallenges ?? {})) {
    ...
    // v6.9.3에서 삭제된 번개반응/숫자맞히기의 구버전 진행판은 보상 없이 즉시 종료해
    // 새 UI에 다시 나타나거나 다른 미니게임 시작을 막지 않게 한다.
    if (challenge.gameId === 'reaction' || challenge.gameId === 'number') {
      challenge.completed = true;
      challenge.completedAt = nowIso(date);
      challenge.reward = 0;
      challenge.removedGame = true;
      changed = true;
      continue;
    }
    if (challenge.gameId === 'block') continue;
    if (challenge.gameId === 'tetrisSingle') {
      const expiresAt = new Date(challenge.expiresAt ?? '').getTime();
      if (!Number.isFinite(expiresAt) || date.getTime() <= expiresAt + 60_000) continue;
      challenge.completed = true;
      challenge.completedAt = nowIso(date);
      challenge.reward = 0;
      challenge.tetrisEndReason = 'expired';
      challenge.abandoned = true;
      changed = true;
      continue;
    }
    const expiresAt = new Date(challenge.expiresAt ?? '').getTime();
    ...
    if (!pet?.alive) { challenge.completed = true; challenge.completedAt = nowIso(date); changed = true; continue; }
    ensurePetSchema(pet, date);
    if (challenge.gameId === 'apple') {
      normalizeAppleChallenge(challenge);
      ...
      completeChallenge(pet, challenge, 0, `${MINI_GAMES[challenge.gameId]?.name ?? '미니게임'} 제한 시간 종료`, date);
    }
    changed = true;
  }
  return { changed };
...
export function stopMiniGame(state, pet, challengeId, date = new Date()) {
  const challenge = state.miniGameChallenges?.[challengeId];
  if (!challenge || challenge.petId !== pet.id || challenge.completed) return { ok: false, message: '포기할 수 있는 진행 중 게임이 없습니다.' };
  if (challenge.gameId === 'tetrisSingle') {
    applyDailyReset(pet, date);
    applyHungerPenalty(pet, date);
    challenge.completed = true;
    ...
    challenge.reward = 0;
    challenge.abandoned = true;
    challenge.tetrisEndReason = 'abandoned';
    // 시작 시 이미 개인게임 1회를 차감하므로 포기해도 사용횟수는 복구하지 않는다.
    pet.daily.balanceCounts.mini += 1;
    pet.records.miniGames += 1;
    incrementGoalCounter(pet, 'mini', 1, date);
    normalizePet(pet, date);
    return { ok: true, finished: true, stopped: true, abandoned: true, reward: 0, detail: '싱글 테트리스를 포기했습니다.', message: '싱글 테트리스를 포기했습니다. 점수·랭킹·포인트는 기록되지 않습니다.' };
  }
  if (challenge.gameId === 'apple') {
    applyDailyReset(pet, date);
    applyHungerPenalty(pet, date);
    normalizeAppleChallenge(challenge);
    const reward = Math.max(0, nonNegativeInt(challenge.applePendingPoints));
    const detail = `사과게임 포기 · ${challenge.appleScore}점 · 숫자 ${challenge.appleRemovedCount}개 제거`;
    return {
      ...completeChallenge(pet, challenge, reward, detail, date),
      stopped: true,
      abandoned: true,
  ...
  if (challenge.gameId === 'block') {
    ...
    const detail = `블록게임 포기 · 블록 ${challenge.blockRemovedCount}개 제거 · ${challenge.blockRemainingCount}개 남음`;
    return {
      ...completeChallenge(pet, challenge, reward, detail, date),
      stopped: true,
      abandoned: true,
  ...
  if (challenge.gameId !== 'oddEven') return { ok: false, message: '이 게임은 중간에 포기할 수 없습니다.' };
...
function fishingView(fishing) {
  if (!fishing) return null;
  return { id: fishing.id, startedAt: fishing.startedAt, readyAt: fishing.readyAt, counted: Boolean(fishing.counted) };
}
...
function settleReadyFishing(pet, date = new Date()) {
  ...
  normalizePet(pet, date);
  return { ok: true, autoCompleted: true, result: completed, goalAwarded: goal.awarded, message: reward.reward > 0 ? `${withJosa(reward.label, '을/를')} 낚아 ${reward.reward}P 획득${goal.awarded ? ' · 낚시 목표 달성, 레고력 +1' : ''}` : `${withJosa(reward.label, '을/를')} 낚았습니다. 꽝${goal.awarded ? ' · 낚시 목표 달성, 레고력 +1' : ''}` };
}
...
export function startFishing(pet, date = new Date()) {
  applyDailyReset(pet, date);
  applyHungerPenalty(pet, date);
  const hungerBlocked = blockedHungerAction(pet, '낚시', date);
  if (hungerBlocked) return hungerBlocked;
  ...
  return { ok: true, hungerCost: hungerUse.cost, fishing: fishingView(pet.daily.fishing), message: '낚시를 시작했습니다. 30초 뒤 자동으로 결과가 나옵니다.' };
}
...
export function publishFishingNews(state, pet, date = new Date()) {
  ...
  return addPublicEvent(state, `${withJosa(pet.displayName, '이/가')} 낚시에서 ${withJosa(result.label, '을/를')} 낚아 ${result.reward}P를 얻었습니다.`, 'fishing', [pet.id], date);
}

export function bodyAdvancementView(pet, date = new Date()) {
  ensurePetSchema(pet, date);
  const unlocked = Boolean(pet.bodyAdvancementUnlockedAt) || nonNegativeInt(pet.records?.maxBody) >= BODY_ADVANCEMENT_BODY;
  const selected = pet.bodyAdvancement ? BODY_ADVANCEMENTS[pet.bodyAdvancement.key] : null;
  return {
    unlocked,
    requiredBody: BODY_ADVANCEMENT_BODY,
    unlockedAt: unlocked ? pet.bodyAdvancementUnlockedAt : null,
    selected: selected ? { ...selected, selectedAt: pet.bodyAdvancement.selectedAt } : null
  };
}

export function selectBodyAdvancement(pet, keyValue, date = new Date()) {
  ensurePetSchema(pet, date);
  const unlocked = Boolean(pet.bodyAdvancementUnlockedAt) || nonNegativeInt(pet.records?.maxBody) >= BODY_ADVANCEMENT_BODY;
  if (!unlocked) return { ok:false, message:`레비아탄레고(몸집 ${BODY_ADVANCEMENT_BODY})에 도달하면 전직이 해금됩니다.` };
  const key = String(keyValue ?? '').trim();
  if (!key || key === 'leviathan' || key === 'none') {
    pet.bodyAdvancement = null;
    return { ok:true, message:'레비아탄 기본 체형으로 돌아왔습니다.', bodyAdvancement: bodyAdvancementView(pet, date) };
  }
  const definition = BODY_ADVANCEMENTS[key];
  if (!definition) return { ok:false, message:'선택할 수 없는 전직 체형입니다.' };
  pet.bodyAdvancement = { key:definition.key, selectedAt:date.toISOString() };
  return { ok:true, message:`${definition.label}(으)로 전직했습니다.`, bodyAdvancement: bodyAdvancementView(pet, date) };
}

export function updateStatusMessage(pet, value, maxLengthValue = STATUS_MESSAGE_MAX_LENGTH) {
  ensurePetSchema(pet);
  const maxLength = Math.max(1, Math.min(STATUS_MESSAGE_ADMIN_MAX_LENGTH, Math.floor(Number(maxLengthValue) || STATUS_MESSAGE_MAX_LENGTH)));
  const normalized = normalizedStatusText(value);
  if ([...normalized].length > maxLength) {
    return { ok: false, message: `상태메시지는 ${maxLength}자 이내로 입력해주세요.` };
  }
...
export function statusSentence(pet) {
  ...
  if (hungerActionLock(pet).locked) return '배고픔이 10분 이상 지속되어 포만감을 쓰는 활동이 잠겼습니다.';
  if (staminaActionLock(pet).locked) return '체력 0이 10분 이상 지속되어 체력을 쓰는 활동이 잠겼습니다.';
  if (pet.stats.hunger === 0) return '포만감이 0이라 굶주림 상태입니다.';
...
export function seasonBadgeView(pet, date = new Date()) {
  ensurePetSchema(pet, date);
  const now = date.getTime();
  const defs = { apple: '🍎왕', singleTetris: '🧱싱글테트리스왕', omok: '⚫오목왕', blockBattle: '🧱테트리스왕', sichuan: '🀄사천성왕', legodoku: '🧩레고도쿠왕', minesweeperNormal: '💥 지뢰왕', minesweeperHard: '💣 지뢰왕고수' };
...
export function flexItemView(pet, date = new Date()) {
  ...
  return {
    ...
    kind: item.kind,
    nameplateKey: item.nameplateKey,
...
export function publicProfile(state, targetPetId, viewerPetId, isOnline = false) {
  ...
  return {
    ...
    statusMessage: pet.statusMessage, bodyAdvancement: bodyAdvancementView(pet), levelBadge: levelBadgeForLevel(progress.level), seasonBadges: seasonBadgeView(pet), flexItem: flexItemView(pet),
...
export function bodyStageRanking(state, limit = 5) {
  return Object.values(state.pets ?? {})
    .filter((pet) => pet?.alive)
    .map((pet) => {
      ensurePetSchema(pet);
      const stage = getBodyStage(pet.stats.body);
      const stageIndex = Math.max(0, BODY_STAGES.findIndex((item) => item.key === stage.key));
      const advancement = pet.bodyAdvancement?.key ? BODY_ADVANCEMENTS[pet.bodyAdvancement.key] : null;
      return {
        pet,
        stageLevel: stageIndex + 1,
        stageLabel: advancement?.label ? `${advancement.label} 체형` : stage.label
      };
    })
    .sort((a, b) => b.stageLevel - a.stageLevel || b.pet.stats.body - a.pet.stats.body || new Date(a.pet.createdAt) - new Date(b.pet.createdAt))
    .slice(0, limit)
    .map((item, index) => ({
      rank: index + 1,
      petId: item.pet.id,
      displayName: item.pet.displayName,
      body: item.pet.stats.body,
...
function singleTetrisRanking(state, limit = 5) {
  return Object.values(state.pets ?? {}).filter((pet) => pet?.alive).map((pet) => {
    ensurePetSchema(pet);
    return { petId: pet.id, displayName: pet.displayName, score: pet.records.singleTetrisBestScore, achievedAt: pet.records.singleTetrisBestAt };
  }).filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score || new Date(a.achievedAt ?? 8640000000000000) - new Date(b.achievedAt ?? 8640000000000000) || a.displayName.localeCompare(b.displayName, 'ko'))
    .slice(0, limit).map((item, index) => ({ ...item, rank: index + 1 }));
}
...
export function rankingsView(state, viewerPetId = null) {
  ...
  const singleTetrisAll = singleTetrisRanking(state, Number.MAX_SAFE_INTEGER);
  const omokAll = omokRanking(state, Number.MAX_SAFE_INTEGER);
  const minesweeper = minesweeperRankingsView(state, viewerPetId);
  ...
  return {
    ...
    bodyStages: bodyStageRanking(state, 5),
    couples: coupleRanking(state),
    pokes: pokeRanking(state, 5),
    apple: appleAll.slice(0, 5),
    singleTetris: singleTetrisAll.slice(0, 5),
    minesweeper,
    omok: omokAll.slice(0, 5),
    myGameRanks: { apple: mine(appleAll), singleTetris: mine(singleTetrisAll), omok: mine(omokAll) },
...
export function privateDashboard(state, userId) {
  ...
  return {
    ...
    pet: {
      ...
      levelBadge: levelBadgeForLevel(progress.level),
      workoutBadge: Boolean(user.workoutBadge),
      seasonBadges: seasonBadgeView(pet),
      kingHistory: { ...pet.kingHistory },
      flexItem: flexItemView(pet),
      bodyAdvancement: bodyAdvancementView(pet),
      ...
      daily: {
        ...pet.daily,
        fishing: fishingView(pet.daily.fishing),
        miniGamesLimit: dailyMiniGameLimit(pet),
        fishingLimit: dailyFishingLimit(pet),
    battlePlayed: nonNegativeInt(pet.daily.battlePlayed),
    battleBonus: nonNegativeInt(pet.daily.battleBonus),
...
export function endLifeAndRestart(state, userId, reason, detail = '', date = new Date()) {
  ...
  refreshTopPokeNews(state, date);
  addPublicEvent(state, `${withJosa(oldPet.displayName, '이/가')} ${reason} 처리되었습니다. 모든 것을 잃고 ${newPet.displayName}으로 다시 시작합니다.`, 'restart', [oldPet.id, newPet.id], date);
