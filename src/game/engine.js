...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  // 파손 시스템은 v6.4.8부터 제거. 구버전 파손 상태도 정상 레고로 자동 복구한다.
  pet.integrity.breakWarnings = 0;
  pet.integrity.broken = false;
  pet.integrity.brokenAt = null;
  pet.integrity.cause = null;
  pet.integrity.stageMessage = null;
  ...
  for (const key of ['nameSparkleUntil','statusGlowUntil','goldBorderUntil','auroraUntil','royalCardUntil']) { const t = new Date(pet.cosmetics[key] ?? '').getTime(); pet.cosmetics[key] = Number.isFinite(t) ? new Date(t).toISOString() : null; }
  ...
  for (const key of ['nameSparkle','statusGlow','goldBorder','aurora','royalCard']) pet.cosmeticExpiryNotices[key] = typeof pet.cosmeticExpiryNotices[key] === 'string' ? pet.cosmeticExpiryNotices[key] : null;
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    cosmetics: { nameSparkleUntil:null, statusGlowUntil:null, goldBorderUntil:null, auroraUntil:null, royalCardUntil:null, title:null },
    cosmeticExpiryNotices: { nameSparkle:null, statusGlow:null, goldBorder:null, aurora:null, royalCard:null },
...
export function applyInactivityConsequence(_state, _pet, _date = new Date()) {
...
export function nextInactivityCheckAt(_pet) { return null; }
...
export function cosmeticView(pet, date = new Date()) {
  ensurePetSchema(pet, date); const now = date.getTime();
  const active = (key) => Number.isFinite(new Date(pet.cosmetics?.[key] ?? '').getTime()) && new Date(pet.cosmetics[key]).getTime() > now;
  return { nameSparkle: active('nameSparkleUntil'), statusGlow: active('statusGlowUntil'), goldBorder: active('goldBorderUntil'), aurora: active('auroraUntil'), royalCard: active('royalCardUntil'), title: pet.cosmetics?.title || null };
}
...
export const COSMETIC_SHOP = Object.freeze([
  { id:'statusGlow', name:'상태메시지 네온', price:500, hours:24, description:'상태메시지가 하루 동안 네온처럼 강조됩니다.' },
  { id:'nameSparkle', name:'닉네임 반짝반짝', price:1000, hours:24, description:'닉네임에 반짝 효과가 하루 동안 붙습니다.' },
  { id:'goldBorder', name:'황금 레고 테두리', price:1800, hours:72, description:'주민 카드가 3일 동안 황금 테두리로 빛납니다.' },
  { id:'aurora', name:'레고 오라', price:2800, hours:72, description:'레고 주변에 3일 동안 오라 효과가 생깁니다.' },
  { id:'royalCard', name:'로얄 주민카드', price:4500, hours:168, description:'주민 카드가 7일 동안 로얄 스타일이 됩니다.' },
  { id:'title-bigshot', name:'칭호 [큰손]', price:8000, title:'큰손', description:'닉네임 위에 큰손 칭호를 영구 장착합니다.' },
  { id:'title-legorich', name:'칭호 [레고재벌]', price:15000, title:'레고재벌', description:'레고재벌 칭호를 영구 장착합니다.' },
  { id:'title-pointmonster', name:'칭호 [포인트괴물]', price:30000, title:'포인트괴물', description:'포인트괴물 칭호를 영구 장착합니다.' }
]);

export function buyCosmetic(pet, itemId, date = new Date()) {
  ensurePetSchema(pet, date); const item = COSMETIC_SHOP.find((entry) => entry.id === String(itemId));
  ...
  pet.stats.points -= item.price; pet.records.pointsSpent += item.price;
  if (item.title) pet.cosmetics.title = item.title; else { const key = `${item.id}Until`; const current = new Date(pet.cosmetics[key] ?? '').getTime(); const base = Number.isFinite(current) && current > date.getTime() ? current : date.getTime(); pet.cosmetics[key] = new Date(base + item.hours * 3600000).toISOString(); }
  return { ok:true, message:`${item.name} 구매 완료!`, itemId:item.id };
}
