...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  pet.daily.miniGameTicketBought = Boolean(pet.daily.miniGameTicketBought) || pet.daily.miniGameBonus >= 10;
  pet.daily.fishingTicketBought = Boolean(pet.daily.fishingTicketBought) || pet.daily.fishingBonus >= 5;
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    daily: {
      ...
      miniGameBonus: 0,
      fishingBonus: 0,
      miniGameTicketBought: false,
      fishingTicketBought: false,
      lotteryPlays: 0,
...
export function applyDailyReset(pet, date = new Date(), state = null) {
  ...
  pet.daily.miniGameTicketBought = false;
  pet.daily.fishingTicketBought = false;
...
export function shopStateView(user, pet, date = new Date()) {
  ...
  return {
    ...
    miniGameTicketBought: Boolean(pet.daily.miniGameTicketBought),
    fishingTicketBought: Boolean(pet.daily.fishingTicketBought),
...
export function purchaseShopItem(state, user, pet, itemIdValue, input = {}, requestIdValue = '', date = new Date(), random = secureRandomUnit) {
  ...
  if (itemId === 'miniGame10' && pet.daily.miniGameTicketBought) return { ok: false, message: '미니게임 +10회권은 게임 하루에 1회만 구매할 수 있습니다.' };
  if (itemId === 'fishing5' && pet.daily.fishingTicketBought) return { ok: false, message: '낚시 +5회권은 게임 하루에 1회만 구매할 수 있습니다.' };
  ...
  if (itemId === 'miniGame10') {
    ...
    pet.daily.miniGameTicketBought = true;
    ...
    pet.daily.fishingTicketBought = true;
