...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  for (const key of ['nameSparkleUntil','statusGlowUntil','goldBorderUntil','auroraUntil','royalCardUntil','backgroundUntil']) {
  ...
  pet.cosmetics.title = typeof pet.cosmetics.title === 'string' ? pet.cosmetics.title.slice(0, 20) : null;
  ...
  for (const key of ['nameSparkle','statusGlow','goldBorder','aurora','royalCard','background']) pet.cosmeticExpiryNotices[key] = typeof pet.cosmeticExpiryNotices[key] === 'string' ? pet.cosmeticExpiryNotices[key] : null;
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    cosmetics: { nameSparkleUntil:null, statusGlowUntil:null, goldBorderUntil:null, auroraUntil:null, royalCardUntil:null, backgroundId:null, backgroundUntil:null, title:null },
    cosmeticExpiryNotices: { nameSparkle:null, statusGlow:null, goldBorder:null, aurora:null, royalCard:null, background:null },
...
export function cosmeticView(pet, date = new Date()) {
  ...
  return {
    nameSparkle: Boolean(nameSparkleUntil),
    statusGlow: Boolean(statusGlowUntil),
    ...
    title: pet.cosmetics?.title || null,
    expiresAt: {
...
export const COSMETIC_SHOP = Object.freeze([
  { id:'statusGlow', kind:'effect', name:'상태메시지 네온', price:600, hours:24, storageKey:'statusGlowUntil', description:'상태메시지가 하루 동안 네온처럼 강조됩니다.' },
  { id:'nameSparkle', kind:'effect', name:'닉네임 반짝반짝', price:1200, hours:24, storageKey:'nameSparkleUntil', description:'닉네임에 하루 동안 반짝 효과가 붙습니다.' },
  ...
  { id:'title-bigshot', kind:'title', name:'칭호 [큰손]', price:10000, title:'큰손', description:'닉네임 위에 큰손 칭호를 영구 장착합니다.' },
  { id:'title-bunggod', kind:'title', name:'칭호 [벙신]', price:12000, title:'벙신', description:'벙에 진심인 레고를 위한 영구 칭호입니다.' },
  { id:'title-terror', kind:'title', name:'칭호 [영토깡패]', price:15000, title:'영토깡패', description:'공용맵에서 눈에 띄는 영구 칭호입니다.' },
  { id:'title-applefarmer', kind:'title', name:'칭호 [사과농장주]', price:18000, title:'사과농장주', description:'사과게임에 미친 레고용 영구 칭호입니다.' },
  { id:'title-legorich', kind:'title', name:'칭호 [레고재벌]', price:30000, title:'레고재벌', description:'돈이 남는 레고를 위한 영구 칭호입니다.' },
  { id:'title-pointmonster', kind:'title', name:'칭호 [포인트괴물]', price:50000, title:'포인트괴물', description:'5만P를 태워야 얻는 최종 과시용 영구 칭호입니다.' }
]);
...
export function buyCosmetic(pet, itemId, date = new Date()) {
  ...
  if (item.title && pet.cosmetics.title === item.title) return { ok:false, message:`이미 [${item.title}] 칭호를 장착하고 있습니다.` };
  ...
  if (item.title) {
    pet.cosmetics.title = item.title;
