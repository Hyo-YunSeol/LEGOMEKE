...
import { removePetFromOmok } from './omok.js';
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    records: {
      ...
      miniGames: 0, oddEvenBest: 0, bestReactionMs: 0, numberBestAttempts: 0,
...
export function finishMiniGame(state, pet, challengeId, value, date = new Date()) {
  ...
  if (challenge.gameId === 'oddEven') {
    ...
    pet.records.oddEvenBest = Math.max(pet.records.oddEvenBest, challenge.streak);
...
export function rankingsView(state) {
  return { points: pointRanking(state, 5), levels: levelRanking(state, 5), couples: coupleRanking(state), pokes: pokeRanking(state, 5), apple: appleRanking(state, 5) };
}
