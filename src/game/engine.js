...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  for (const key of ['reaction','apple','singleTetris','omok','blockBattle','minesweeperNormal','minesweeperHard']) { const t = new Date(pet.seasonBadges[key] ?? '').getTime(); pet.seasonBadges[key] = Number.isFinite(t) ? new Date(t).toISOString() : null; }
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    seasonBadges: { reaction:null, apple:null, singleTetris:null, omok:null, blockBattle:null, minesweeperNormal:null, minesweeperHard:null },
    flags: {}
  };
...
export function seasonBadgeView(pet, date = new Date()) {
  ensurePetSchema(pet, date);
  const now = date.getTime();
  const defs = { apple: '🍎왕', singleTetris: '🧱솔로테트리스왕', omok: '⚫오목왕', blockBattle: '🧱테트리스왕', minesweeperNormal: '💥 지뢰왕', minesweeperHard: '💣 지뢰왕고수' };
