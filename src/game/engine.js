...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  for (const key of ['reaction','apple','omok','blockBattle']) { const t = new Date(pet.seasonBadges[key] ?? '').getTime(); pet.seasonBadges[key] = Number.isFinite(t) ? new Date(t).toISOString() : null; }
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    seasonBadges: { reaction:null, apple:null, omok:null, blockBattle:null },
    flags: {}
  };
...
export function seasonBadgeView(pet, date = new Date()) {
  ensurePetSchema(pet, date);
  const now = date.getTime();
  const defs = { reaction: '⚡왕', apple: '🍎왕', omok: '⚫오목왕', blockBattle: '🧱테트리스왕' };
