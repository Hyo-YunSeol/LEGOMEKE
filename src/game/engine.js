...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  const defaults = {
    ...
    spotDifferenceGames: 0,
    spotDifferenceWins: 0,
    spotDifferenceDraws: 0,
    spotDifferenceLosses: 0,
    seasonSpotDifferenceWins: 0,
    seasonSpotDifferenceDraws: 0,
    seasonSpotDifferenceLosses: 0,
    davinciTotalWins: 0,
    davinciTotalCorrect: 0,
    davinciWins: 0,
    davinciCorrect: 0,
    shopPurchases: 0,
    lotteryPlays: 0,
  ...
  for (const key of ['liarGames', 'liarWins', 'liarPointsWon']) delete pet.records[key];
  ...
  for (const key of ['reaction','apple','singleTetris','omok','blockBattle','sichuan','spotDifference','minesweeperNormal','minesweeperHard']) { const t = new Date(pet.seasonBadges[key] ?? '').getTime(); pet.seasonBadges[key] = Number.isFinite(t) ? new Date(t).toISOString() : null; }
  ...
  pet.kingHistory = {};
  for (const key of ['apple','singleTetris','omok','blockBattle','sichuan','spotDifference','minesweeperNormal','minesweeperHard']) {
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    seasonBestReactionMs: 0, seasonBestReactionAt: null, numberBestAttempts: 0,
      ...
      spotDifferenceGames: 0, spotDifferenceWins: 0, spotDifferenceDraws: 0, spotDifferenceLosses: 0,
      seasonSpotDifferenceWins: 0, seasonSpotDifferenceDraws: 0, seasonSpotDifferenceLosses: 0,
      davinciTotalWins: 0, davinciTotalCorrect: 0, davinciWins: 0, davinciCorrect: 0,
      shopPurchases: 0, lotteryPlays: 0, lotteryEarned: 0
    },
    partnerPetId: null,
    coupleStartedAt: null,
    levelReachedAt: date.toISOString(),
    seasonBadges: { reaction:null, apple:null, singleTetris:null, omok:null, blockBattle:null, sichuan:null, spotDifference:null, minesweeperNormal:null, minesweeperHard:null },
    kingHistory: { apple:false, singleTetris:false, omok:false, blockBattle:false, sichuan:false, spotDifference:false, minesweeperNormal:false, minesweeperHard:false },
    flags: {}
  };
...
export function seasonBadgeView(pet, date = new Date()) {
  ensurePetSchema(pet, date);
  const now = date.getTime();
  const defs = { apple: '🍎왕', singleTetris: '🧱싱글테트리스왕', omok: '⚫오목왕', blockBattle: '🧱테트리스왕', sichuan: '🀄사천성왕', spotDifference: '🔎틀린그림왕', minesweeperNormal: '💥 지뢰왕', minesweeperHard: '💣 지뢰왕고수' };
