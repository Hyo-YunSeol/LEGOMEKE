...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  for (const key of ['reaction','apple','omok','blockBattle','minesweeperNormal','minesweeperHard']) { const t = new Date(pet.seasonBadges[key] ?? '').getTime(); pet.seasonBadges[key] = Number.isFinite(t) ? new Date(t).toISOString() : null; }
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    seasonBadges: { reaction:null, apple:null, omok:null, blockBattle:null, minesweeperNormal:null, minesweeperHard:null },
    flags: {}
  };
...
function miniChallengeIsActive(challenge, date = new Date()) {
  ...
  if (challenge.gameId === 'block' || challenge.gameId === 'minesweeper') return true;
...
export function startMiniGame(state, pet, gameId = 'oddEven', date = new Date(), input = {}) {
  ...
  const challenge = {
    ...
    expiresAt: (gameId === 'block' || gameId === 'minesweeper') ? null : gameId === 'apple' ? new Date(date.getTime() + 120_000).toISOString() : new Date(date.getTime() + 10 * 60_000).toISOString(), completed: false,
...
export function seasonBadgeView(pet, date = new Date()) {
  ensurePetSchema(pet, date);
  const now = date.getTime();
  const defs = { apple: '🍎왕', omok: '⚫오목왕', blockBattle: '🧱테트리스왕', minesweeperNormal: '💥 지뢰왕', minesweeperHard: '💣 지뢰왕고수' };
...
export function rankingsView(state, viewerPetId = null) {
  ...
  return {
    ...
    myGameRanks: { apple: mine(appleAll), omok: mine(omokAll) },
