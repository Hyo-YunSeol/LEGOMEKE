...
export const SOCIAL_REQUEST_TTL_MS = 24 * 60 * 60_000;
export const SOCIAL_REQUEST_HISTORY_MS = 7 * 24 * 60 * 60_000;
...
function socialRequestExpiresAt(request, fallbackDate = new Date()) {
  const explicit = new Date(request?.expiresAt ?? '').getTime();
  if (Number.isFinite(explicit)) return explicit;
  const createdAt = new Date(request?.createdAt ?? '').getTime();
  return (Number.isFinite(createdAt) ? createdAt : fallbackDate.getTime()) + SOCIAL_REQUEST_TTL_MS;
}

export function expireSocialRequests(state, date = new Date()) {
  state.requests ??= {};
  let changed = false;
  for (const request of Object.values(state.requests)) {
    if (!request || request.status !== 'pending' || !['match', 'mating'].includes(request.type)) continue;
    const expiresAt = socialRequestExpiresAt(request, date);
    request.expiresAt = new Date(expiresAt).toISOString();
    if (expiresAt > date.getTime()) continue;
    request.status = 'expired';
    request.respondedAt = nowIso(date);
    request.expiredAt = nowIso(date);
    changed = true;
  }
  return { changed };
}

export function socialRequestNextAlarmAt(state, date = new Date()) {
  const times = Object.values(state.requests ?? {})
    .filter((request) => request?.status === 'pending' && ['match', 'mating'].includes(request.type))
    .map((request) => socialRequestExpiresAt(request, date))
    .filter((time) => Number.isFinite(time) && time > date.getTime());
  return times.length ? new Date(Math.min(...times)).toISOString() : null;
}

export function socialAction(state, actorPet, targetPet, action, extra = {}, date = new Date()) {
  state.requests ??= {};
  expireSocialRequests(state, date);
  if (!actorPet?.alive) return { ok: false, message: '내 레고를 찾을 수 없습니다.' };
  if (action === 'cancelRequest') {
    const request = state.requests[String(extra.requestId ?? '')];
    if (!request || request.status !== 'pending' || request.fromPetId !== actorPet.id || !['match', 'mating'].includes(request.type)) {
      return { ok: false, message: '취소할 신청을 찾을 수 없습니다.' };
    }
    request.status = 'cancelled';
    request.respondedAt = nowIso(date);
    const recipient = state.pets?.[request.toPetId];
    if (recipient?.alive) addNotification(state, recipient.userId, `${actorPet.displayName}이 ${request.type === 'match' ? '매칭' : '교미'} 신청을 취소했습니다.`, request.type, { petId: actorPet.id }, date);
    return { ok: true, cancelled: true, requestId: request.id, message: `${request.type === 'match' ? '매칭' : '교미'} 신청을 취소했습니다.` };
  }
  ...
  if (action === 'requestMatch') {
    ...
    const request = {
      ...
      createdAt: nowIso(date), expiresAt: new Date(date.getTime() + SOCIAL_REQUEST_TTL_MS).toISOString()
    };
...
export function sentRequestsFor(state, petId) {
  return Object.values(state.requests ?? {})
    .filter((request) => ['match', 'mating'].includes(request.type) && request.fromPetId === petId && request.status === 'pending')
    .map((request) => ({ ...request, toDisplayName: state.pets[request.toPetId]?.displayName ?? '알 수 없는 레고' }));
}
