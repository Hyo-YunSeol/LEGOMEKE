...
export function workAction(pet, date = new Date()) {
  ...
  if (pet.stats.stamina < 15) return { ok: false, message: '체력이 부족합니다. 먼저 쉬거나 에너지드링크를 이용해주세요.' };
...
export function exerciseAction(pet, date = new Date()) {
  ...
  if (hungerBlocked) return hungerBlocked;
  if (pet.stats.stamina < 20) return { ok: false, message: '체력이 부족해 헬스할 수 없습니다. 먼저 쉬거나 에너지드링크를 이용해주세요.' };
  const action = consumeAction(pet, date);
  if (!action.ok) return action;
...
export function cookAction(pet, date = new Date()) {
  ...
  if (pet.stats.stamina < 10) return { ok: false, message: '체력이 부족해 요리할 수 없습니다. 먼저 쉬거나 에너지드링크를 이용해주세요.' };
  const action = consumeAction(pet, date);
  if (!action.ok) return action;
...
export function purchaseShopItem(state, user, pet, itemIdValue, input = {}, requestIdValue = '', date = new Date(), random = secureRandomUnit) {
  ...
  if (itemId === 'miniGame20') {
    ...
    globalRefresh = true;
    if (item.kind === 'nameplate') addPublicEvent(state, `${pet.displayName}이 ${item.name}을 장착했습니다.`, 'flex-item', [pet.id], date);
  }
...
export function finishBung(state, hostPet, bung, date = new Date()) {
  ...
  return { ok: true, rewarded: true, message: `벙을 끝냈습니다. 참가자 체력 -20, 레고력 +1${rewards.some((item) => item.legoPower > 1) ? ' (오늘의 레고력 보너스 포함)' : ''}`, rewards };
}
