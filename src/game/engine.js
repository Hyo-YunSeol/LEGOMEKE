...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  pet.daily.lotteryPlays = clamp(pet.daily.lotteryPlays, 0, 2);
...
export function shopStateView(user, pet, date = new Date()) {
  ...
  return {
    ...
    lotteryPlays: clamp(pet.daily.lotteryPlays, 0, 2),
    lotteryNextPrice: pet.daily.lotteryPlays === 0 ? SHOP_ITEMS.lottery.price : pet.daily.lotteryPlays === 1 ? SHOP_ITEMS.lottery.retryPrice : null,
...
export function purchaseShopItem(state, user, pet, itemIdValue, input = {}, requestIdValue = '', date = new Date(), random = secureRandomUnit) {
  ...
  let nickname = null;
  if (itemId === 'lottery') {
    if (pet.daily.lotteryPlays >= 2) return { ok: false, message: '하루 복권은 이번 게임 하루에 최대 2회만 구매할 수 있습니다.' };
