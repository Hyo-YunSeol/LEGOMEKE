...
function matingResultText(first, second, compatibility) {
  const groups = compatibility <= 30
    ? [
      `${first}와 ${second}의 뜨거운 밤이 내용 없이 끝났습니다.`,
      '둘이 가까워졌지만 레고 결합부의 크기가 서로 맞지 않았습니다.',
      `어라..? ${second}가 눈을 뜨니 ${first}는 이미 퇴실했습니다.`,
      '둘이 M텔비를 누가 더 손해 봤는지 계산 중입니다.',
      `${first}와 ${second}가 서로 연락처를 삭제할지 고민 중입니다.`,
      '둘 다 집에 가는 길에 오늘 일을 없던 일로 하기로 했습니다.'
    ]
    : compatibility <= 50
      ? [
        `${first}만 체력을 모두 소진했습니다. ${second}는 아직 시작도 안 했는데..`,
        `${first}와 ${second}가 어색한 침묵과 함께 돌아왔습니다.`,
        '결과는 평범했지만 둘 다 만족한 척하고 있습니다.',
        '다음날 둘의 카톡 답장 속도가 눈에 띄게 느려졌습니다.',
        `${first}가 ${second}에게 끝나고 “괜찮았지?”만 세 번 물었습니다.`,
        `${second}가 집에 도착하자마자 ${first}의 카톡 알림을 껐습니다.`
      ]
      : compatibility <= 60
        ? [
          'CF에서 알 수 없는 규칙적인 소리가 났습니다.',
          `엘리베이터에서 ${first}와 ${second}를 마주쳤습니다!`,
          `${first}와 ${second}의 은밀한 관계가 발각되었습니다.`,
          `${first}와 ${second}가 같은 시간에 사라졌다가 같이 돌아왔습니다.`,
        ...
        : compatibility <= 70
          ...
          : compatibility <= 80
            ? [
              `${first}와 ${second}의 호흡이 제법 잘 맞았습니다.`,
              `${first}와 ${second}가 손잡고 가는 사진이 유포되었습니다.`,
              '잔치집 흡연실에서 알 수 없는 규칙적인 소리가..',
              '두 레고의 결합력이 설명서 허용 범위입니다. 흐읏..!',
              `둘이 아무 일 없었다고 주장했지만 ${second}가 ${first} 옷을 입고 있었습니다.`,
              `둘이 나왔는데 ${first}는 땀을 닦고 ${second}는 머리를 묶고 있었습니다.`
            ]
            : compatibility <= 90
              ? [
                `${first}와 ${second}가 각자 집에 갔다고 했는데 목적지가 같았습니다.`,
                `잔치집 흡연실에서 ${first}와 ${second}가 관계에 집중한 나머지 문 잠그는 걸 잊었습니다.`,
                '현장을 본 레고가 조용히 문을 다시 닫았습니다.',
                `${first}가 이제 그만하자고 했는데 ${second}가 “벌써?”라고 물었습니다.`
              ]
              : [
                `${first}가 다음날 연장을 신청했습니다.`,
                '호야 비상계단에서 하다가 걸려 현재 목격자 입단속 중입니다.',
                '한국밥에서 해장하다 킄킄을 마주쳐 세 명 모두 아무 말도 하지 못했습니다.',
                '둘이 다음날에도 같이 있다는 제보가 들어와 관리자가 사실관계를 확인 중입니다.',
                `${first}와 ${second}를 발견한 상어가 “하던 거 마저하세요” 한마디만 남기고 사라졌습니다.`
              ];
