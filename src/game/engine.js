...
import { consumeInteractionHunger, lifeHungerCostsForBody } from './activity.js';
import { appleBoardEmpty, refreshAppleBoardIfStuck, appleChallengeFields, normalizeAppleChallenge, selectAppleRectangle } from './apple-game.js';
...
export function ensurePetSchema(pet, date = new Date()) {
  ...
  pet.cosmetics = pet.cosmetics && typeof pet.cosmetics === 'object' && !Array.isArray(pet.cosmetics) ? pet.cosmetics : {};
  for (const key of ['nameSparkleUntil','nameRainbowUntil','statusGlowUntil','statusRainbowUntil','goldBorderUntil','auroraUntil','royalCardUntil','backgroundUntil']) {
    const t = new Date(pet.cosmetics[key] ?? '').getTime();
    pet.cosmetics[key] = Number.isFinite(t) ? new Date(t).toISOString() : null;
  }
  pet.cosmetics.backgroundId = typeof pet.cosmetics.backgroundId === 'string' ? pet.cosmetics.backgroundId.slice(0, 30) : null;
  // 영구 칭호 상점은 제거되었습니다. 과거 저장값은 표시/판매하지 않습니다.
  pet.cosmetics.title = null;
  pet.cosmeticExpiryNotices = pet.cosmeticExpiryNotices && typeof pet.cosmeticExpiryNotices === 'object' && !Array.isArray(pet.cosmeticExpiryNotices) ? pet.cosmeticExpiryNotices : {};
  for (const key of ['nameSparkle','nameRainbow','statusGlow','statusRainbow','goldBorder','aurora','royalCard','background']) pet.cosmeticExpiryNotices[key] = typeof pet.cosmeticExpiryNotices[key] === 'string' ? pet.cosmeticExpiryNotices[key] : null;
...
export function createPet(user, generation = 1, date = new Date()) {
  const pet = {
    ...
    cosmetics: { nameSparkleUntil:null, nameRainbowUntil:null, statusGlowUntil:null, statusRainbowUntil:null, goldBorderUntil:null, auroraUntil:null, royalCardUntil:null, backgroundId:null, backgroundUntil:null, title:null },
    cosmeticExpiryNotices: { nameSparkle:null, nameRainbow:null, statusGlow:null, statusRainbow:null, goldBorder:null, aurora:null, royalCard:null, background:null },
...
export function workAction(pet, date = new Date()) {
  ...
  pet.stats.hunger -= hungerCosts.work;
  pet.records.works += 1;
  ...
  return lifeActionResult(pet, `일을 해서 ${WORK_POINTS}P를 벌었습니다. 체력 -15 · 배고픔 -${hungerCosts.work}`, date);
}
...
export function restAction(pet, date = new Date()) {
  ...
  const hungerCosts = lifeHungerCostsForBody(pet.stats.body);
  pet.stats.hunger -= hungerCosts.rest;
  pet.records.rests += 1;
  pet.daily.balanceCounts.rest += 1;
  if (pet.stats.hunger <= 0 && !pet.survival.hungerZeroAt) pet.survival.hungerZeroAt = date.toISOString();
  return lifeActionResult(pet, `쉬어서 체력 +40, 배고픔 -${hungerCosts.rest}가 적용되었습니다.`, date);
}
...
export function exerciseAction(pet, date = new Date()) {
  ...
  const hungerCosts = lifeHungerCostsForBody(pet.stats.body);
  pet.stats.hunger -= hungerCosts.exercise;
  pet.stats.body -= 2;
  pet.records.workouts += 1;
  pet.daily.balanceCounts.exercise += 1;
  if (pet.stats.hunger <= 0 && !pet.survival.hungerZeroAt) pet.survival.hungerZeroAt = date.toISOString();
  return lifeActionResult(pet, `헬스를 마쳤습니다. 몸집 -2, 체력 -20, 배고픔 -${hungerCosts.exercise}가 적용되었습니다.`, date);
}
...
export function eatAction(pet, foodId, date = new Date()) {
  ...
  if (food.minBody && pet.stats.body < food.minBody) return { ok: false, message: `몸집 ${food.minBody} 이상부터 먹을 수 있습니다.` };
...
export function finishBung(state, hostPet, bung, date = new Date()) {
  ...
  for (const entry of participants) {
    ...
    normalizePet(pet);
    rewards.push({ petId: pet.id, displayName: pet.displayName, legoPower: repeatAward.amount + (levelGoal.awarded ? 1 : 0) + (dailyAward.awarded ? 1 : 0) + (hostAward.awarded ? 1 : 0), stamina: -20, hungerCost: hungerUse.cost });
  }
...
export function selectAppleGame(state, pet, challengeId, input = {}, requestId = '', date = new Date()) {
  ...
  if (result.ok && result.removed) {
    const refreshed = refreshAppleBoardIfStuck(challenge);
    if (refreshed) return { ...result, board: challenge.appleBoard.map((row) => [...row]), boardRefreshed: true, boardsGenerated: challenge.appleBoardsGenerated, message: `${result.message} · 가능한 합10 영역이 없어 새 판이 생성되었습니다.` };
  }
  return result;
}
...
export function cosmeticView(pet, date = new Date()) {
  ensurePetSchema(pet, date);
  const now = date.getTime();
  const expiry = (key) => {
    const value = new Date(pet.cosmetics?.[key] ?? '').getTime();
    return Number.isFinite(value) && value > now ? new Date(value).toISOString() : null;
  };
  const nameSparkleUntil = expiry('nameSparkleUntil');
  const nameRainbowUntil = expiry('nameRainbowUntil');
  const statusGlowUntil = expiry('statusGlowUntil');
  const statusRainbowUntil = expiry('statusRainbowUntil');
  const goldBorderUntil = expiry('goldBorderUntil');
  const auroraUntil = expiry('auroraUntil');
  const royalCardUntil = expiry('royalCardUntil');
  const backgroundUntil = expiry('backgroundUntil');
  return {
    nameSparkle: Boolean(nameSparkleUntil) && !nameRainbowUntil,
    nameRainbow: Boolean(nameRainbowUntil),
    statusGlow: Boolean(statusGlowUntil) && !statusRainbowUntil,
    statusRainbow: Boolean(statusRainbowUntil),
    goldBorder: Boolean(goldBorderUntil),
    aurora: Boolean(auroraUntil),
    royalCard: Boolean(royalCardUntil),
    background: backgroundUntil ? pet.cosmetics?.backgroundId || null : null,
    title: null,
    expiresAt: {
      nameSparkle: nameSparkleUntil,
      nameRainbow: nameRainbowUntil,
      statusGlow: statusGlowUntil,
      statusRainbow: statusRainbowUntil,
      goldBorder: goldBorderUntil,
      aurora: auroraUntil,
      royalCard: royalCardUntil,
      background: backgroundUntil
    }
  };
}
...
export const COSMETIC_SHOP = Object.freeze([
  { id:'statusGlow', kind:'effect', effectGroup:'statusText', name:'상태메시지 네온', price:600, hours:24, storageKey:'statusGlowUntil', description:'상태메시지가 하루 동안 네온처럼 강조됩니다.' },
  { id:'statusRainbow', kind:'effect', effectGroup:'statusText', name:'🌈 상태메시지 무지개', price:1200, hours:72, storageKey:'statusRainbowUntil', description:'상태메시지가 3일 동안 천천히 흐르는 무지개 글씨로 바뀝니다.' },
  { id:'nameSparkle', kind:'effect', effectGroup:'nameText', name:'닉네임 반짝반짝', price:1200, hours:24, storageKey:'nameSparkleUntil', description:'닉네임에 하루 동안 반짝 효과가 붙습니다.' },
  { id:'nameRainbow', kind:'effect', effectGroup:'nameText', name:'🌈 닉네임 무지개', price:2000, hours:72, storageKey:'nameRainbowUntil', description:'닉네임이 3일 동안 천천히 흐르는 무지개 글씨로 바뀝니다.' },
  { id:'goldBorder', kind:'effect', name:'황금 레고 테두리', price:2000, hours:72, storageKey:'goldBorderUntil', description:'내 카드가 3일 동안 황금 테두리로 빛납니다.' },
  { id:'aurora', kind:'effect', name:'레고 오라', price:3200, hours:72, storageKey:'auroraUntil', description:'레고 본체 주변에 3일 동안 확실한 오라가 생깁니다.' },
  { id:'royalCard', kind:'effect', name:'로얄 주민카드', price:5200, hours:168, storageKey:'royalCardUntil', description:'내 카드가 7일 동안 로얄 장식으로 바뀝니다.' },

  { id:'bg-cherry', kind:'background', background:'cherry', name:'배경 · 벚꽃난사', price:1800, hours:72, storageKey:'backgroundUntil', description:'3일 동안 분홍 벚꽃 패턴이 카드 뒤에 깔립니다.' },
  { id:'bg-space', kind:'background', background:'space', name:'배경 · 우주미아', price:2200, hours:72, storageKey:'backgroundUntil', description:'3일 동안 별과 우주가 레고 뒤에서 반짝입니다.' },
  { id:'bg-money', kind:'background', background:'money', name:'배경 · 돈비내림', price:2800, hours:72, storageKey:'backgroundUntil', description:'3일 동안 돈이 쏟아지는 부자 배경을 사용합니다.' },
  { id:'bg-club', kind:'background', background:'club', name:'배경 · CF클럽', price:3000, hours:72, storageKey:'backgroundUntil', description:'3일 동안 네온 클럽 조명이 카드 뒤를 채웁니다.' },
  { id:'bg-fire', kind:'background', background:'fire', name:'배경 · 불타는레고', price:3400, hours:72, storageKey:'backgroundUntil', description:'3일 동안 불꽃 패턴이 뒤에서 타오릅니다.' },
  { id:'bg-throne', kind:'background', background:'throne', name:'배경 · 왕좌점거', price:5200, hours:168, storageKey:'backgroundUntil', description:'7일 동안 왕좌와 금장 장식 배경을 사용합니다.' },
  { id:'bg-jurassic', kind:'background', background:'jurassic', name:'배경 · 쥬라기탈출', price:5600, hours:168, storageKey:'backgroundUntil', description:'7일 동안 정글과 공룡발자국 배경을 사용합니다.' },
  { id:'bg-emergency', kind:'background', background:'emergency', name:'배경 · 국가비상', price:6500, hours:168, storageKey:'backgroundUntil', description:'7일 동안 경광등과 위험선 배경으로 시선 강탈합니다.' }
]);

export function buyCosmetic(pet, itemId, date = new Date()) {
  ensurePetSchema(pet, date);
  const item = COSMETIC_SHOP.find((entry) => entry.id === String(itemId));
  if (!item) return { ok:false, message:'판매하지 않는 꾸미기입니다.' };
  if (pet.stats.points < item.price) return { ok:false, message:`${item.name} 구매에는 ${item.price.toLocaleString('ko-KR')}P가 필요합니다.` };

  pet.stats.points -= item.price;
  pet.records.pointsSpent += item.price;
  if (item.background) {
    const currentExpiry = new Date(pet.cosmetics.backgroundUntil ?? '').getTime();
    const sameBackground = pet.cosmetics.backgroundId === item.background && Number.isFinite(currentExpiry) && currentExpiry > date.getTime();
    const base = sameBackground ? currentExpiry : date.getTime();
    pet.cosmetics.backgroundId = item.background;
    pet.cosmetics.backgroundUntil = new Date(base + item.hours * 3600000).toISOString();
    pet.cosmeticExpiryNotices.background = null;
  } else {
    const key = item.storageKey || `${item.id}Until`;
    // 같은 글자 부위 효과는 동시에 겹치지 않게 한다. 새 효과 구매 시 기존 효과를 즉시 교체한다.
    if (item.effectGroup === 'nameText') {
      for (const otherKey of ['nameSparkleUntil','nameRainbowUntil']) if (otherKey !== key) pet.cosmetics[otherKey] = null;
      for (const noticeKey of ['nameSparkle','nameRainbow']) pet.cosmeticExpiryNotices[noticeKey] = null;
    } else if (item.effectGroup === 'statusText') {
      for (const otherKey of ['statusGlowUntil','statusRainbowUntil']) if (otherKey !== key) pet.cosmetics[otherKey] = null;
      for (const noticeKey of ['statusGlow','statusRainbow']) pet.cosmeticExpiryNotices[noticeKey] = null;
    }
    const current = new Date(pet.cosmetics[key] ?? '').getTime();
    const base = Number.isFinite(current) && current > date.getTime() ? current : date.getTime();
    pet.cosmetics[key] = new Date(base + item.hours * 3600000).toISOString();
    pet.cosmeticExpiryNotices[item.id] = null;
  }
  return { ok:true, message:`${item.name} 구매 완료!`, itemId:item.id, cosmetic: cosmeticView(pet, date) };
}
...
export function publicProfile(state, targetPetId, viewerPetId, isOnline = false) {
  ...
  return {
    ...
    statusMessage: pet.statusMessage, cosmetics: cosmeticView(pet), seasonBadges: seasonBadgeView(pet),
