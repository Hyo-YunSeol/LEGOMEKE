...
export const BLOCK_BATTLE_STAKES = Object.freeze([100, 500, 1000]);
...
const validStake = (value) => BLOCK_BATTLE_STAKES.includes(Number(value));
...
function lockPiece(state, room, petId, date = new Date()) {
  ...
  for (const [row, col] of cellsFor(player.active)) {
    if (row < 0) {
      finishRoom(state, room, petId, `${player.displayName}의 블럭이 천장에 닿았습니다.`, date);
...
function applyAction(state, room, petId, action, date = new Date()) {
  ...
  if (action === 'tick') {
    const elapsed = date.getTime() - new Date(player.lastGravityAt).getTime();
    if (Number.isFinite(elapsed) && elapsed < blockBattleGravityMs(room, date) * 0.75) return { changed: false };
  }
  ...
  if (action === 'softDrop') {
    if (movePiece(player, 1, 0)) { player.score += 1; return { changed: true }; }
    const result = lockPiece(state, room, petId, date);
    return { changed: true, locked: true, ...result };
  }
...
function initialPlayer(pet, date = new Date()) {
  const player = {
    ...
    connected: true, reconnectDeadlineAt: null, lastGravityAt: nowIso(date)
  };
  spawnPiece(player);
  return player;
}
...
export function normalizeBlockBattle(raw, state, date = new Date()) {
  ...
  for (const [roomId, rawRoom] of Object.entries(battle.rooms)) {
    ...
    if (!host?.alive || !validStake(rawRoom?.stakePoints) || (status !== 'waiting' && !guest?.alive)) {
    ...
    room.processedBatchIds = Array.isArray(room.processedBatchIds) ? [...new Set(room.processedBatchIds.map(String))].slice(-300) : [];
...
export function createBlockBattleRoom(state, pet, stakeValue, date = new Date()) {
  ...
  if (!validStake(stakeValue)) return { ok: false, message: '판돈은 100P, 500P, 1,000P 중에서 선택해야 합니다.' };
  const existing = activePlayerRoom(battle, pet.id);
  if (existing) return { ok: true, roomId: existing.id, message: '이미 참가 중인 블럭대전 방을 열었습니다.' };
  if (Object.keys(battle.rooms).length >= BLOCK_BATTLE_MAX_ROOMS) return { ok: false, message: '동시에 운영할 수 있는 블럭대전 방 3개가 모두 사용 중입니다.' };
  ...
  const room = {
    ...
    processedBatchIds: [], rematchRequests: [], settled: false, settlementId: null,
  ...
  return { ok: true, roomId: room.id, message: `${roomNumber}번 블럭대전 방을 만들었습니다.` };
}
...
export function joinBlockBattleRoom(state, pet, roomId, date = new Date()) {
  ...
  if (!room) return { ok: false, message: '블럭대전 방을 찾을 수 없습니다.' };
  ...
  if (activePlayerRoom(battle, pet.id)) return { ok: false, message: '한 사용자는 동시에 여러 블럭대전 방의 선수가 될 수 없습니다.' };
  ...
  return { ok: true, roomId, started: true, message: '상대 참가가 확정되어 블럭대전을 시작했습니다.' };
}

export function spectateBlockBattleRoom(state, pet, roomId, date = new Date()) {
  const battle = state.blockBattle = normalizeBlockBattle(state.blockBattle, state, date);
  const room = battle.rooms[roomId];
  if (!room || room.status !== 'playing') return { ok: false, message: '진행 중인 블럭대전만 관전할 수 있습니다.' };
  ...
  return { ok: true, roomId, message: '블럭대전 관전을 시작했습니다.' };
}
...
export function leaveBlockBattleRoom(state, pet, roomId, date = new Date()) {
  ...
  return { ok: true, message: room.status === 'ended' ? '블럭대전 방에서 나갔습니다.' : '기권패 처리되었습니다.' };
}
...
export function playBlockBattleActions(state, pet, roomId, input = {}, date = new Date()) {
  ...
  if (![room.hostPetId, room.guestPetId].includes(pet.id)) return { ok: false, message: '관전자는 블럭을 조작할 수 없습니다.' };
  ...
  if (room.processedBatchIds.includes(batchId)) return { ok: true, duplicate: true, message: '이미 처리된 입력입니다.' };
  const actions = Array.isArray(input.actions) ? input.actions.slice(0, 24).map(String) : [];
  if (!actions.length || actions.some((action) => !VALID_ACTIONS.has(action))) return { ok: false, message: '블럭 입력이 올바르지 않습니다.' };
  if (!room.players[pet.id]?.connected) return { ok: false, message: '재접속 처리가 끝난 뒤 입력할 수 있습니다.' };
  if (Object.values(room.players).some((player) => !player.connected)) return { ok: false, paused: true, message: '상대 재접속을 기다리는 동안 게임이 일시정지되었습니다.' };
  room.processedBatchIds.push(batchId);
  room.processedBatchIds = room.processedBatchIds.slice(-300);
  let locked = false;
  let cleared = 0;
  let attack = 0;
  ...
  room.updatedAt = nowIso(date);
  return { ok: true, locked, cleared, attack, finished: room.status === 'ended', message: attack ? `${attack}줄 공격을 보냈습니다.` : locked ? '블럭을 고정했습니다.' : '입력을 반영했습니다.' };
}
...
export function blockBattleSetConnected(state, petId, connected, date = new Date()) {
  ...
  for (const room of Object.values(battle.rooms)) {
    ...
    if (connected) for (const member of Object.values(room.players)) member.lastGravityAt = nowIso(date);
    changed = true;
  }
  return { changed };
}

export function processBlockBattleTimers(state, date = new Date()) {
  const battle = state.blockBattle = normalizeBlockBattle(state.blockBattle, state, date);
  let changed = false;
  let settled = false;
  for (const room of Object.values(battle.rooms)) {
    ...
    for (const player of Object.values(room.players)) {
      ...
      while (steps > 0 && room.status === 'playing') {
        applyAction(state, room, player.petId, 'tick', new Date(last + gravity));
        last += gravity;
        steps -= 1;
        changed = true;
...
export function blockBattleGravityMs(room, date = new Date()) {
  const elapsed = Math.max(0, date.getTime() - new Date(room?.startedAt || date).getTime());
  return Math.max(180, 850 - Math.floor(elapsed / 30_000) * 70);
}
...
export function blockBattleView(state, viewerPetId, date = new Date()) {
  const battle = state.blockBattle = normalizeBlockBattle(state.blockBattle, state, date);
  const rooms = Object.values(battle.rooms).sort((a, b) => a.roomNumber - b.roomNumber).map((room) => {
    ...
    return {
      ...
      players: canViewBoards && room.status !== 'waiting' ? {
        [room.hostPetId]: playerPublic(room.players[room.hostPetId], { full: true }),
        [room.guestPetId]: playerPublic(room.players[room.guestPetId], { full: true })
      } : {},
