...
function randomGarbageHole() { return Math.floor(Math.random() * BLOCK_BATTLE_WIDTH); }

function legacyGarbageHole(player, offset) {
  // v6.10.5 이전 저장 데이터에는 구멍 큐가 없다. 읽을 때 Math.random()으로 채우면
  // 저장 전 재로드마다 모양이 바뀔 수 있으므로 기존 상태만 안정적인 해시로 복원한다.
  const seed = `${player.petId || ''}:${int(player.pieces)}:${int(player.receivedGarbage)}:${int(player.pendingGarbage)}:${int(offset)}`;
  let hash = 2166136261;
  for (const char of seed) { hash ^= char.codePointAt(0); hash = Math.imul(hash, 16777619); }
  return (hash >>> 0) % BLOCK_BATTLE_WIDTH;
}

function ensureGarbageHoles(player) {
  player.pendingGarbageHoles = Array.isArray(player.pendingGarbageHoles)
    ? player.pendingGarbageHoles.map((hole) => Math.max(0, Math.min(BLOCK_BATTLE_WIDTH - 1, int(hole)))).slice(0, 20)
    : [];
  while (player.pendingGarbageHoles.length < player.pendingGarbage) {
    player.pendingGarbageHoles.push(legacyGarbageHole(player, player.pendingGarbageHoles.length));
  }
  if (player.pendingGarbageHoles.length > player.pendingGarbage) player.pendingGarbageHoles.length = player.pendingGarbage;
}

function addGarbage(player, amount) {
  const lines = Math.max(0, Math.min(8, int(amount)));
  ensureGarbageHoles(player);
  for (let count = 0; count < lines; count += 1) {
    if (player.board[0].some(Boolean)) return false;
    player.board.shift();
    // 공격을 받은 시점에 이미 정해 둔 구멍을 사용한다. 클라이언트에도 같은 큐를
    // 보내므로 lock 직후 방해줄까지 로컬에서 정확히 그릴 수 있어 서버 ACK를 기다리지 않는다.
    const hole = player.pendingGarbageHoles.shift();
...
function lockPiece(state, room, petId, date = new Date()) {
  ...
  if (attack > 0) {
    ...
    ensureGarbageHoles(opponent);
    const previousPending = opponent.pendingGarbage;
    opponent.pendingGarbage = Math.min(20, opponent.pendingGarbage + attack);
    const queued = Math.max(0, opponent.pendingGarbage - previousPending);
    for (let index = 0; index < queued; index += 1) opponent.pendingGarbageHoles.push(randomGarbageHole());
