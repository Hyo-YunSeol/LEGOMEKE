...
export const BLOCK_BATTLE_CLIENT_GRAVITY_GRACE_MS = 1_400;
...
function normalizePlayer(raw, pet, date) {
  ...
  player.lastClientTickAt = Number.isFinite(new Date(player.lastClientTickAt ?? '').getTime()) ? new Date(player.lastClientTickAt).toISOString() : player.lastGravityAt;
...
function initialPlayer(pet, date = new Date()) {
  const player = {
    ...
    connected: true, reconnectDeadlineAt: null, lastGravityAt: nowIso(date), lastClientTickAt: nowIso(date)
  };
  spawnPiece(player);
  return player;
}
...
export function playBlockBattleActions(state, pet, roomId, input = {}, date = new Date()) {
  ...
  if (room.processedBatchIds.includes(batchId)) return { ok: true, duplicate: true, requestId: batchId, stateVersion: room.stateVersion, message: '이미 처리된 입력입니다.' };
  ...
  for (const action of actions) {
    ...
    // 정상 플레이 중에는 클라이언트가 700ms 주기의 자동 낙하를 전송한다. 실제로
    // 반영된 tick만 시각으로 기록해 서버 fallback 중력이 같은 블록을 다시 내리지 않게 한다.
    if (action === 'tick' && result.changed && room.players[pet.id]) room.players[pet.id].lastClientTickAt = nowIso(date);
  }
...
export function blockBattleSetConnected(state, petId, connected, date = new Date()) {
  ...
  for (const room of Object.values(battle.rooms)) {
    ...
    if (connected) for (const member of Object.values(room.players)) {
      ...
      member.lastClientTickAt = nowIso(date);
    }
    bumpRoomVersion(room, date);
    changed = true;
  }
...
export function processBlockBattleTimers(state, date = new Date(), { gravity = true } = {}) {
  const battle = state.blockBattle = normalizeBlockBattle(state.blockBattle, state, date);
  let changed = false;
  let settled = false;
  for (const room of Object.values(battle.rooms)) {
    ...
    if (Object.values(room.players).some((player) => !player.connected) || !gravity) continue;
    const gravityMs = blockBattleGravityMs(room, date);
    let roomChanged = false;
    for (const player of Object.values(room.players)) {
      // 브라우저가 정상적으로 자동 tick을 보내는 동안 서버 알람이 별도의 중력 엔진처럼
      // 경쟁하지 않는다. tick이 끊긴 경우에만 마지막 확정 중력시각 기준으로 따라잡아
      // 백그라운드/네트워크 단절로 게임이 멈추는 악용은 막는다.
      const lastClientTick = new Date(player.lastClientTickAt ?? '').getTime();
      if (Number.isFinite(lastClientTick) && date.getTime() - lastClientTick < Math.max(BLOCK_BATTLE_CLIENT_GRAVITY_GRACE_MS, gravityMs * 2)) continue;
      let last = new Date(player.lastGravityAt).getTime();
      if (!Number.isFinite(last)) last = date.getTime();
      let steps = Math.min(12, Math.floor((date.getTime() - last) / gravityMs));
      while (steps > 0 && room.status === 'playing') {
        const result = applyAction(state, room, player.petId, 'tick', new Date(last + gravityMs));
        last += gravityMs;
        steps -= 1;
