...
function fallbackGarbageHole(player, index) {
  // 구버전 저장 데이터에는 구멍 큐가 없을 수 있다. 저장 여부와 관계없이 같은 상태에서는
  // 항상 같은 보정값이 나오도록 간단한 안정 해시를 사용해 서버 snapshot마다 모양이 바뀌지 않게 한다.
  const seed = `${player.petId || ''}:${int(player.pieces)}:${int(player.attackSent)}:${int(player.receivedGarbage)}:${int(player.pendingGarbage)}:${index}`;
  let hash = 2166136261;
  for (const char of seed) hash = Math.imul(hash ^ char.codePointAt(0), 16777619) >>> 0;
  return hash % BLOCK_BATTLE_WIDTH;
}

function normalizePendingGarbageHoles(player) {
  const pending = Math.max(0, Math.min(20, int(player.pendingGarbage)));
  const holes = Array.isArray(player.pendingGarbageHoles)
    ? player.pendingGarbageHoles.map((value) => int(value, -1)).filter((value) => value >= 0 && value < BLOCK_BATTLE_WIDTH).slice(0, pending)
    : [];
  while (holes.length < pending) holes.push(fallbackGarbageHole(player, holes.length));
  player.pendingGarbageHoles = holes;
  return holes;
}

function randomGarbageHoles(amount) {
  const count = Math.max(0, int(amount));
  return Array.from({ length: count }, () => Math.floor(Math.random() * BLOCK_BATTLE_WIDTH));
}

function addGarbage(player, holesValue) {
  const holes = Array.isArray(holesValue)
    ? holesValue.map((value) => int(value, -1)).filter((value) => value >= 0 && value < BLOCK_BATTLE_WIDTH).slice(0, 8)
    : randomGarbageHoles(Math.max(0, Math.min(8, int(holesValue))));
  ...
  player.receivedGarbage += holes.length;
  return true;
}
...
function finishRoom(state, room, loserPetId, reason, date = new Date()) {
  ...
  if (winner?.alive) {
    ...
    winner.records.seasonBlockBattleWins = int(winner.records.seasonBlockBattleWins) + 1;
  }
  if (loser?.alive) {
    loser.records.blockBattleLosses = int(loser.records.blockBattleLosses) + 1;
    loser.records.seasonBlockBattleLosses = int(loser.records.seasonBlockBattleLosses) + 1;
  }
...
function lockPiece(state, room, petId, date = new Date()) {
  ...
  normalizePendingGarbageHoles(player);
  const cancelled = Math.min(player.pendingGarbage, attack);
  player.pendingGarbage -= cancelled;
  if (cancelled > 0) player.pendingGarbageHoles.splice(0, cancelled);
  attack -= cancelled;
  if (attack > 0) {
    const opponent = room.players[opponentId(room, petId)];
    normalizePendingGarbageHoles(opponent);
    const beforePending = opponent.pendingGarbage;
    opponent.pendingGarbage = Math.min(20, beforePending + attack);
    const accepted = Math.max(0, opponent.pendingGarbage - beforePending);
    if (accepted > 0) opponent.pendingGarbageHoles.push(...randomGarbageHoles(accepted));
    ...
    const incomingHoles = player.pendingGarbageHoles.slice(0, incoming);
    player.pendingGarbage -= incoming;
    player.pendingGarbageHoles.splice(0, incoming);
    if (!addGarbage(player, incomingHoles)) {
      finishRoom(state, room, petId, `${withJosa(player.displayName, '이/가')} 방해줄을 버티지 못했습니다.`, date);
...
function applyAction(state, room, petId, action, date = new Date()) {
  ...
  if (action === 'tick') {
    // 화면은 tick을 즉시 한 칸 미리 그리므로 서버도 살아 있는 클라이언트에서 온
    // 각 tick을 정확히 한 번 반영해야 한다. 벽시계 오차/네트워크 지터 때문에 서버가
    // 같은 tick을 '너무 이르다'며 무변경 ACK하면 클라이언트가 이미 그린 Y가 다음
    // snapshot에서 한 칸 위로 되감겨 통통 튄다. 정상 클라이언트는 한 번에 tick 하나만
    // 미확정으로 유지하므로 요청 수신 시각을 새 700ms 중력 기준으로 삼아 일치시킨다.
    gravityDate = date;
  }
...
function initialPlayer(pet, date = new Date()) {
  const player = {
    ...
    lines: 0, score: 0, pieces: 0, attackSent: 0, receivedGarbage: 0, pendingGarbage: 0, pendingGarbageHoles: [],
...
function occupiedNumbers(battle) { return new Set(Object.values(battle.rooms).filter((room) => room.status !== 'ended').map((room) => room.roomNumber)); }
...
export function createBlockBattleRoom(state, pet, stakeValue, date = new Date()) {
  ...
  if (Object.values(battle.rooms).filter((room) => room.status !== 'ended').length >= BLOCK_BATTLE_MAX_ROOMS) return { ok: false, message: '동시에 운영할 수 있는 테트리스대전 방 3개가 모두 사용 중입니다.' };
  ...
  const room = {
    ...
    rematchRequests: [], departedPetIds: [], settled: false, settlementId: null,
...
export function leaveBlockBattleSpectator(state, pet, roomId, date = new Date()) {
  ...
  if (!target) return { ok: true, deleted: true, message: '이미 정리된 테트리스대전 방입니다.' };
  if (!target.spectators?.[pet.id]) return { ok: true, message: '이미 관전을 종료했습니다.' };
...
export function leaveBlockBattleRoom(state, pet, roomId, date = new Date()) {
  const battle = state.blockBattle = normalizeBlockBattle(state.blockBattle, state, date);
  const room = battle.rooms[roomId];
  if (!room) return { ok: true, deleted: true, message: '이미 정리된 테트리스대전 방입니다.' };
  ...
  if (room.status === 'playing') {
    finishRoom(state, room, pet.id, `${withJosa(pet.displayName, '이/가')} 나가 기권패했습니다.`, date);
  ...
  if (players.every((petId) => room.departedPetIds.includes(petId))) {
    delete battle.rooms[roomId];
    return { ok: true, deleted: true, message: '종료된 테트리스대전 방을 정리했습니다.' };
  }
  bumpRoomVersion(room, date);
  return { ok: true, departed: true, message: '테트리스대전 방에서 나갔습니다.' };
}
...
export function playBlockBattleActions(state, pet, roomId, input = {}, date = new Date()) {
  ...
  const actions = Array.isArray(input.actions) ? input.actions.slice(0, 24).map((entry) => {
    if (typeof entry === 'string') return { action: entry, piece: null, seq: null };
    const action = String(entry?.action || '');
    const pieceValue = Number(entry?.piece);
    const seqValue = Number(entry?.seq);
    return {
      action,
      piece: Number.isInteger(pieceValue) && pieceValue >= 0 ? pieceValue : null,
      seq: Number.isInteger(seqValue) && seqValue > 0 ? seqValue : null
    };
  }) : [];
  if (!actions.length || actions.some((entry) => !VALID_ACTIONS.has(entry.action))) return { ok: false, message: '블록 입력이 올바르지 않습니다.' };
  ...
  let attack = 0;
  let changed = false;
  let discardedPieceActions = 0;
  for (const inputAction of actions) {
    if (room.status !== 'playing') break;
    const player = room.players[pet.id];
    // 입력을 누른 당시의 블록 번호가 다르면 이미 lock된 이전 블록의 늦은 입력이다.
    // 특히 늦은 Space/hardDrop이 새 블록에 적용되는 것을 서버에서 최종 차단한다.
    if (inputAction.piece != null && Number(player?.pieces || 0) !== inputAction.piece) {
      discardedPieceActions += 1;
      continue;
    }
    const result = applyAction(state, room, pet.id, inputAction.action, date);
    changed ||= Boolean(result.changed);
    locked ||= Boolean(result.locked);
    cleared += int(result.cleared);
    attack += int(result.attack);
  }
  // 벽에 붙은 채 반복된 좌우/회전처럼 실제 상태가 바뀌지 않은 입력은 확인만 하고
  // 상태 버전은 올리지 않는다. 요청 ID는 저장되므로 재전송 멱등성과 클라이언트 ACK는 유지된다.
  if (room.status === 'playing' && changed) bumpRoomVersion(room, date);
  return { ok: true, requestId: batchId, stateVersion: room.stateVersion, changed, locked, cleared, attack, discardedPieceActions, finished: room.status === 'ended', message: attack ? `${attack}줄 공격을 보냈습니다.` : locked ? '블록을 고정했습니다.' : changed ? '입력을 반영했습니다.' : '변경 없는 입력을 확인했습니다.' };
}
...
export function processBlockBattleTimers(state, date = new Date(), { roomId: targetRoomId = null } = {}) {
  ...
  for (const [roomId, room] of roomEntries) {
    ...
    if (expiredDisconnected.length) {
      ...
      else finishRoom(state, room, expiredDisconnected[0].petId, `${withJosa(expiredDisconnected[0].displayName, '이/가')} 30초 안에 재접속하지 않아 기권패했습니다.`, date);
      changed = true;
      settled = true;
      continue;
    }
...
function playerPublic(player, { full = false, gravityMs = BLOCK_BATTLE_GRAVITY_MS } = {}) {
  ...
  return {
    ...
    pendingGarbageHoles: full ? [...player.pendingGarbageHoles] : []
  };
}
...
export function blockBattleRankings(state, viewerPetId = null) {
  const entries = Object.values(state.pets ?? {}).filter((pet) => pet?.alive).map((pet) => ({
    ...
    wins: Math.max(0, int(pet.records?.seasonBlockBattleWins)), losses: Math.max(0, int(pet.records?.seasonBlockBattleLosses))
