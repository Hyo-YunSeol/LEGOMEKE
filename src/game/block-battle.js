...
export function playBlockBattleActions(state, pet, roomId, input = {}, date = new Date()) {
  ...
  if (room.status === 'playing') bumpRoomVersion(room, date);
  return { ok: true, requestId: batchId, stateVersion: room.stateVersion, locked, cleared, attack, finished: room.status === 'ended', message: attack ? `${attack}줄 공격을 보냈습니다.` : locked ? '블록을 고정했습니다.' : '입력을 반영했습니다.' };
}
