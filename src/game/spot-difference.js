...
// v6.10.17부터 신규 문제는 성인 난이도의 로컬 WebP 도감 장면을 사용한다.
// 테마 10종 × 세트 2종 × 좌우 반전으로 40가지 시각 조합을 제공한다.
// 각 세트는 실제 이미지에 맞춰 검증된 정답 7곳을 고정하며, 서버 판정 좌표와 이미지 차이를 일치시킨다.
// assetVersion이 없는 기존 진행방은 레거시 벡터 문제로 유지해 배포 중인 대전을 깨지 않는다.
...
export const SPOT_DIFFERENCE_ATLAS_VERSION = 2;
export const SPOT_DIFFERENCE_ATLAS_PUZZLES = Object.freeze([
  Object.freeze({ themeId: 'body-guide', variant: 0, differenceIds: Object.freeze(['d1', 'd3', 'd5', 'd6', 'd7', 'd8', 'd11']) }),
  Object.freeze({ themeId: 'body-guide', variant: 1, differenceIds: Object.freeze(['d0', 'd2', 'd3', 'd6', 'd7', 'd9', 'd17']) }),
  Object.freeze({ themeId: 'lego-room', variant: 0, differenceIds: Object.freeze(['d0', 'd1', 'd2', 'd3', 'd6', 'd11', 'd14']) }),
  Object.freeze({ themeId: 'lego-room', variant: 1, differenceIds: Object.freeze(['d3', 'd6', 'd7', 'd8', 'd12', 'd14', 'd17']) }),
  Object.freeze({ themeId: 'convenience', variant: 0, differenceIds: Object.freeze(['d4', 'd7', 'd8', 'd9', 'd11', 'd15', 'd16']) }),
  Object.freeze({ themeId: 'convenience', variant: 1, differenceIds: Object.freeze(['d0', 'd1', 'd4', 'd5', 'd11', 'd12', 'd15']) }),
  Object.freeze({ themeId: 'beach', variant: 0, differenceIds: Object.freeze(['d0', 'd1', 'd2', 'd3', 'd8', 'd13', 'd14']) }),
  Object.freeze({ themeId: 'beach', variant: 1, differenceIds: Object.freeze(['d1', 'd4', 'd5', 'd8', 'd10', 'd13', 'd17']) }),
  Object.freeze({ themeId: 'game-room', variant: 0, differenceIds: Object.freeze(['d1', 'd2', 'd4', 'd6', 'd8', 'd10', 'd12']) }),
  Object.freeze({ themeId: 'game-room', variant: 1, differenceIds: Object.freeze(['d1', 'd3', 'd4', 'd6', 'd11', 'd12', 'd17']) }),
  Object.freeze({ themeId: 'picnic', variant: 0, differenceIds: Object.freeze(['d1', 'd2', 'd5', 'd7', 'd10', 'd11', 'd13']) }),
  Object.freeze({ themeId: 'picnic', variant: 1, differenceIds: Object.freeze(['d0', 'd1', 'd8', 'd10', 'd11', 'd12', 'd14']) }),
  Object.freeze({ themeId: 'camping', variant: 0, differenceIds: Object.freeze(['d1', 'd5', 'd7', 'd10', 'd11', 'd12', 'd14']) }),
  Object.freeze({ themeId: 'camping', variant: 1, differenceIds: Object.freeze(['d3', 'd7', 'd11', 'd14', 'd15', 'd16', 'd17']) }),
  Object.freeze({ themeId: 'cafe', variant: 0, differenceIds: Object.freeze(['d6', 'd7', 'd9', 'd10', 'd11', 'd12', 'd15']) }),
  Object.freeze({ themeId: 'cafe', variant: 1, differenceIds: Object.freeze(['d0', 'd3', 'd4', 'd9', 'd12', 'd14', 'd16']) }),
  Object.freeze({ themeId: 'festival', variant: 0, differenceIds: Object.freeze(['d3', 'd7', 'd8', 'd11', 'd12', 'd15', 'd17']) }),
  Object.freeze({ themeId: 'festival', variant: 1, differenceIds: Object.freeze(['d2', 'd3', 'd6', 'd7', 'd9', 'd10', 'd15']) }),
  Object.freeze({ themeId: 'space-lab', variant: 0, differenceIds: Object.freeze(['d0', 'd1', 'd5', 'd8', 'd9', 'd11', 'd12']) }),
  Object.freeze({ themeId: 'space-lab', variant: 1, differenceIds: Object.freeze(['d3', 'd6', 'd7', 'd8', 'd9', 'd10', 'd12']) }),
]);
const ATLAS_PUZZLE_BY_KEY = new Map(SPOT_DIFFERENCE_ATLAS_PUZZLES.map((item) => [`${item.themeId}:${item.variant}`, item]));
...
function puzzleKey(puzzle) {
  ...
  if (assetVersion === SPOT_DIFFERENCE_ATLAS_VERSION) {
    // Atlas visuals are fixed files; seed does not change the picture.
    // Keep it out of the key so the exact same visual cannot bypass history with a new seed.
    return `v${assetVersion}:${puzzle.themeId}:${puzzle.variant}:${puzzle.mirrored ? 1 : 0}:${puzzle.differenceIds.join(',')}`;
  }
  return `${assetVersion ? `v${assetVersion}:` : ''}${puzzle.themeId}:${puzzle.variant}:${puzzle.seed}:${puzzle.mirrored ? 1 : 0}:${puzzle.differenceIds.join(',')}`;
}
...
function generatePuzzle(room = {}) {
  ...
  const eligible = SPOT_DIFFERENCE_ATLAS_PUZZLES.filter((item) => item.themeId !== previousTheme);
  ...
  for (let attempt = 0; attempt < 40; attempt += 1) {
    ...
    const candidate = {
      ...
      mirrored: random() >= 0.5,
      differenceIds: [...selected.differenceIds]
    };
...
function normalizePuzzle(raw, room) {
  ...
  if (assetVersion === SPOT_DIFFERENCE_ATLAS_VERSION) {
    ...
    const atlas = ATLAS_PUZZLE_BY_KEY.get(`${raw.themeId}:${variant}`);
    if (!atlas) return generatePuzzle(room);
    const order = (value) => int(String(value).slice(1), 0);
    const expected = [...atlas.differenceIds].sort((a, b) => order(a) - order(b));
    const actual = [...differenceIds].sort((a, b) => order(a) - order(b));
    if (expected.length !== actual.length || expected.some((value, index) => value !== actual[index])) return generatePuzzle(room);
    const puzzle = {
      assetVersion: SPOT_DIFFERENCE_ATLAS_VERSION,
      themeId: raw.themeId,
