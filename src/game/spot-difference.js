...
export const SPOT_DIFFERENCE_MATCH_SECONDS = 30;
...
// v6.10.21 신규 문제는 실제 WebP 장면을 기반으로 국소 픽셀 변형을 사용한다.
// 20개 장면 × C(18,7) = 636,480개의 정답 조합을 확보하되, 무작위 빈 좌표가 아니라
// 장면 안에 의미 있게 배치한 18개 후보 지점에서 7곳을 골라 성인 난이도를 유지한다.
// 플레이어별 최근 200판의 같은 장면+정답조합은 재출제하지 않고 직전 테마도 연속 출제하지 않는다.
// assetVersion 2~5 진행방은 배포 중 대전을 깨지 않도록 호환 유지한다.
...
export const SPOT_DIFFERENCE_ATLAS_VERSION = 6;
...
// v4 신규판은 장면별로 후보 좌표 자체가 달라진다. 같은 30개 위치를 외워 찍는 플레이를 막기 위해
// 테마+variant를 고정 시드로 사용해 각 장면의 고밀도 영역 안에서 위치를 미세 분산한다.
const SPOT_DIFFERENCE_V4_BASE_POINTS = Object.freeze([
...
function spotSceneLayoutSeed(themeId, variant) {
  const text = `${String(themeId || '')}:${Math.max(0, Math.min(1, int(variant, 0)))}`;
  let hash = 2166136261;
  for (let index = 0; index < text.length; index += 1) { hash ^= text.charCodeAt(index); hash = Math.imul(hash, 16777619); }
  return hash >>> 0;
}

export function spotDifferenceHitboxesForPuzzle(puzzle) {
  const assetVersion = Math.max(0, int(puzzle?.assetVersion));
  if (assetVersion >= 6) {
    return SPOT_DIFFERENCE_HITBOXES.map((item) => Object.freeze({ ...item, r: Math.max(0.048, item.r - 0.012) }));
  }
  if (assetVersion < 4) return SPOT_DIFFERENCE_HITBOXES;
  const seed = assetVersion >= 5
    ? (spotSceneLayoutSeed(puzzle?.themeId, puzzle?.variant) ^ (Math.max(1, int(puzzle?.seed, 1)) >>> 0)) >>> 0
    : spotSceneLayoutSeed(puzzle?.themeId, puzzle?.variant);
  return SPOT_DIFFERENCE_V4_BASE_POINTS.map(([baseX, baseY], index) => {
...
const HITBOX_IDS = new Set(Array.from({ length: 30 }, (_, index) => `d${index}`));
...
function chooseSeparatedDifferenceIds(hitboxes, random, count = SPOT_DIFFERENCE_COUNT) {
  const shuffled = shuffleWithRandom(hitboxes, random);
  const chosen = [];
  for (const candidate of shuffled) {
    if (chosen.every((other) => Math.hypot(candidate.x - other.x, candidate.y - other.y) > candidate.r + other.r + 0.016)) chosen.push(candidate);
    if (chosen.length >= count) break;
  }
  if (chosen.length < count) return shuffleWithRandom(hitboxes, random).slice(0, count).map((item) => item.id).sort((a,b) => int(a.slice(1)) - int(b.slice(1)));
  return chosen.map((item) => item.id).sort((a,b) => int(a.slice(1)) - int(b.slice(1)));
}
...
function puzzleKey(puzzle) {
  ...
  if (assetVersion >= 5) {
    return `v${assetVersion}:${sceneKey(puzzle)}:${Math.max(1, int(puzzle?.seed, 1))}:${ordered.join(',')}`;
  }
  if (assetVersion >= 4) {
...
function generatePuzzle(room = {}, game = null) {
  ...
  for (let attempt = 0; attempt < 600; attempt += 1) {
    ...
    const candidateBase = { assetVersion: SPOT_DIFFERENCE_ATLAS_VERSION, themeId: selected.themeId, variant: selected.variant, seed };
    const differenceIds = chooseSeparatedDifferenceIds(spotDifferenceHitboxesForPuzzle(candidateBase), random, SPOT_DIFFERENCE_COUNT);
    const candidate = {
      ...candidateBase,
      seed,
      mirrored: random() >= 0.5,
...
function normalizePuzzle(raw, room, game = null) {
  ...
  const allowedHitboxes = assetVersion >= 4
    ? spotDifferenceHitboxesForPuzzle({ assetVersion, themeId: raw.themeId, variant })
    : SPOT_DIFFERENCE_HITBOXES;
  const allowedIds = new Set(allowedHitboxes.map((item) => item.id));
  const differenceIds = [...new Set(Array.isArray(raw.differenceIds) ? raw.differenceIds.map(String).filter((item) => allowedIds.has(item)) : [])]
  ...
  if (assetVersion === SPOT_DIFFERENCE_ATLAS_VERSION || assetVersion === 5 || assetVersion === 4 || assetVersion === 3) {
    const puzzle = {
      assetVersion,
      themeId: raw.themeId,
...
function normalizePlayer(raw, pet, puzzle = null) {
  const puzzleIds = new Set(Array.isArray(puzzle?.differenceIds) ? puzzle.differenceIds.map(String) : []);
  const foundIds = [...new Set(Array.isArray(raw?.foundIds) ? raw.foundIds.map(String).filter((item) => HITBOX_IDS.has(item) && (!puzzleIds.size || puzzleIds.has(item))) : [])].slice(0, SPOT_DIFFERENCE_COUNT);
...
export function initialSpotDifference() {
  return { version: 4, rooms: {}, recentPuzzleKeys: [], playerPuzzleHistory: {}, playerLastSceneKey: {} };
}
...
export function normalizeSpotDifference(raw, state, date = new Date()) {
  ...
  for (const [roomId, rawRoom] of Object.entries(game.rooms)) {
    ...
    if (status !== 'waiting') {
      room.players[host.id] = normalizePlayer(room.players[host.id], host, room.puzzle);
      room.players[guest.id] = normalizePlayer(room.players[guest.id], guest, room.puzzle);
    } else room.players = {};
...
function hitTest(puzzle, xValue, yValue) {
  ...
  let best = null;
  const hitboxById = new Map(spotDifferenceHitboxesForPuzzle(puzzle).map((item) => [item.id, item]));
  for (const differenceId of puzzle.differenceIds) {
    const base = hitboxById.get(differenceId);
    if (!base) continue;
...
export function processSpotDifferenceTimers(state, date = new Date(), { roomId: targetRoomId = null } = {}) {
  ...
  for (const [roomId, room] of entries) {
    ...
    if (hostCount !== guestCount) {
      ...
      finishWin(state, room, winnerPetId, loserPetId, `${SPOT_DIFFERENCE_MATCH_SECONDS}초 종료 · ${Math.max(hostCount, guestCount)}개 대 ${Math.min(hostCount, guestCount)}개로 더 많이 찾았습니다.`, date);
    } else if (hostCount > 0) {
      ...
      if (Number.isFinite(hostAt) && Number.isFinite(guestAt) && hostAt !== guestAt) {
        ...
        const loserPetId = winnerPetId === room.hostPetId ? room.guestPetId : room.hostPetId;
        finishWin(state, room, winnerPetId, loserPetId, `${SPOT_DIFFERENCE_MATCH_SECONDS}초 종료 · 둘 다 ${hostCount}개를 찾았고 같은 개수에 먼저 도달한 플레이어가 승리했습니다.`, date);
      } else finishDraw(state, room, `${SPOT_DIFFERENCE_MATCH_SECONDS}초 종료 · 양쪽 모두 ${hostCount}개로 완전 동점입니다.`, date);
    } else finishDraw(state, room, `${SPOT_DIFFERENCE_MATCH_SECONDS}초 종료 · 양쪽 모두 정답을 찾지 못해 무승부입니다.`, date);
    changed = true;
    settled = true;
  }
