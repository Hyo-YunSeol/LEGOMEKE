...
// v6.10.20 신규 문제는 시드 기반 의미 객체 장면을 사용하고 v2~v4 진행방은 호환 유지한다.
// assetVersion 5는 장면 시드마다 30개 후보 위치와 객체 구성이 달라지고 그중 7개를 뽑는다.
// 기본 조합만 20 × C(30,7) = 40,716,000개이며 좌우 반전·미세 변형 형태는 경우의 수에 포함하지 않는다.
// 같은 장면+정답조합은 플레이어별 최근 200판 동안 재출제하지 않고, 직전 장면/테마도 연속 출제하지 않는다.
// assetVersion 2/3 진행방은 배포 중 대전을 깨지 않도록 기존 좌표 체계를 그대로 유지한다.
export const SPOT_DIFFERENCE_THEMES = Object.freeze([
  { id: 'body-guide', label: '수집가 작업실' },
  { id: 'lego-room', label: '파란 창가 방' },
  { id: 'convenience', label: '편의점 진열대' },
  { id: 'beach', label: '해변' },
  { id: 'game-room', label: '게임방' },
  { id: 'picnic', label: '공원 피크닉' },
  { id: 'camping', label: '캠핑장' },
  { id: 'cafe', label: '카페' },
  { id: 'festival', label: '축제 부스' },
  { id: 'space-lab', label: '우주 연구실' }
]);

export const SPOT_DIFFERENCE_ATLAS_VERSION = 5;
...
function normalizePuzzle(raw, room, game = null) {
  ...
  if (assetVersion === SPOT_DIFFERENCE_ATLAS_VERSION || assetVersion === 4 || assetVersion === 3) {
    const puzzle = {
      assetVersion,
      themeId: raw.themeId,
