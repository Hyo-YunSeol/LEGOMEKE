...
export const SPOT_DIFFERENCE_MATCH_SECONDS = 120;
export const SPOT_DIFFERENCE_COUNT = 7;
export const SPOT_DIFFERENCE_MAX_ROOMS = 5;
...
function generatePuzzle(room = {}) {
  ...
  for (let attempt = 0; attempt < 32; attempt += 1) {
    ...
    const ids = shuffleWithRandom(SPOT_DIFFERENCE_HITBOXES.map((item) => item.id), random).slice(0, SPOT_DIFFERENCE_COUNT).sort();
    const candidate = {
      themeId: theme.id,
      seed,
...
export function createSpotDifferenceRoom(state, pet, stakeValue, date = new Date()) {
  ...
  if (Object.values(game.rooms).filter((room) => room.status !== 'ended').length >= SPOT_DIFFERENCE_MAX_ROOMS) return { ok: false, message: '동시에 운영할 수 있는 틀린그림찾기 대전방 5개가 모두 사용 중입니다.' };
...
export function processSpotDifferenceTimers(state, date = new Date(), { roomId: targetRoomId = null } = {}) {
  ...
  for (const [roomId, room] of entries) {
    ...
    if (hostCount !== guestCount) {
      ...
      finishWin(state, room, winnerPetId, loserPetId, `2분 종료 · ${Math.max(hostCount, guestCount)}개 대 ${Math.min(hostCount, guestCount)}개로 더 많이 찾았습니다.`, date);
    } else if (hostCount > 0) {
      ...
      if (Number.isFinite(hostAt) && Number.isFinite(guestAt) && hostAt !== guestAt) {
        ...
        const loserPetId = winnerPetId === room.hostPetId ? room.guestPetId : room.hostPetId;
        finishWin(state, room, winnerPetId, loserPetId, `2분 종료 · 둘 다 ${hostCount}개를 찾았고 같은 개수에 먼저 도달한 플레이어가 승리했습니다.`, date);
      } else finishDraw(state, room, `2분 종료 · 양쪽 모두 ${hostCount}개로 완전 동점입니다.`, date);
    } else finishDraw(state, room, '2분 종료 · 양쪽 모두 정답을 찾지 못해 무승부입니다.', date);
    changed = true;
    settled = true;
  }
