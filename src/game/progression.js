...
export const TERRITORY_LIMIT_TIERS = Object.freeze([
  Object.freeze({ minLevel: 1, maxLevel: 1, limit: 1 }),
  Object.freeze({ minLevel: 2, maxLevel: 2, limit: 2 }),
  Object.freeze({ minLevel: 3, maxLevel: 3, limit: 3 }),
  Object.freeze({ minLevel: 4, maxLevel: 5, limit: 4 }),
  Object.freeze({ minLevel: 6, maxLevel: 8, limit: 5 }),
  Object.freeze({ minLevel: 9, maxLevel: 12, limit: 6 }),
  Object.freeze({ minLevel: 13, maxLevel: 17, limit: 7 }),
  Object.freeze({ minLevel: 18, maxLevel: 23, limit: 8 }),
  Object.freeze({ minLevel: 24, maxLevel: 30, limit: 9 }),
  Object.freeze({ minLevel: 31, maxLevel: null, limit: 10 })
]);

export function territoryLimitForLevel(level) {
  const safeLevel = Math.max(1, Math.floor(Number(level) || 1));
  return TERRITORY_LIMIT_TIERS.find((tier) => safeLevel >= tier.minLevel && (tier.maxLevel == null || safeLevel <= tier.maxLevel))?.limit ?? 10;
}

export function nextTerritoryUpgrade(level) {
  const safeLevel = Math.max(1, Math.floor(Number(level) || 1));
  const currentLimit = territoryLimitForLevel(safeLevel);
  const next = TERRITORY_LIMIT_TIERS.find((tier) => tier.minLevel > safeLevel && tier.limit > currentLimit);
  return next ? { level: next.minLevel, limit: next.limit } : null;
}
...
export function ensureDailyProgress(pet, date = new Date()) {
  ...
  pet.daily.legoGoals = pet.daily.legoGoals && typeof pet.daily.legoGoals === 'object' && !Array.isArray(pet.daily.legoGoals)
    ? pet.daily.legoGoals
    : {};
  // 예전 내부 키는 화면상 이미 테트리스 목표였으므로 완료 상태만 새 키로 1회 승계한다.
