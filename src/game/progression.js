...
// 5×5 영토맵의 물리적 최대치(25칸)까지만, 레벨 2단계마다 보유 한도 1칸씩 증가한다.
// Lv.1~2=1칸, Lv.3~4=2칸, ... Lv.47~48=24칸, Lv.49 이상=25칸.
export const TERRITORY_LIMIT_TIERS = Object.freeze([
  ...
  Object.freeze({ minLevel: 49, maxLevel: null, limit: 25 })
]);

export function territoryLimitForLevel(level) {
  const safeLevel = Math.max(1, Math.floor(Number(level) || 1));
  return Math.min(25, Math.ceil(safeLevel / 2));
}

export function nextTerritoryUpgrade(level) {
  const safeLevel = Math.max(1, Math.floor(Number(level) || 1));
  const currentLimit = territoryLimitForLevel(safeLevel);
  if (currentLimit >= 25) return null;
  return { level: currentLimit * 2 + 1, limit: currentLimit + 1 };
}
