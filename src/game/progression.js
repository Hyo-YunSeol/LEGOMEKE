...
// 5×5 공용맵을 고레벨 한 명이 과도하게 독점하지 않도록 보유 한도를 완만하게 늘린다.
// 3레벨마다 +1칸: Lv.1~3=1칸, Lv.4~6=2칸, ... Lv.25~27=9칸, Lv.28 이상=10칸.
export const TERRITORY_LIMIT_TIERS = Object.freeze([
  ...
  Object.freeze({ minLevel: 28, maxLevel: null, limit: 10 })
]);

export function territoryLimitForLevel(level) {
  const safeLevel = Math.max(1, Math.floor(Number(level) || 1));
  return Math.min(10, Math.ceil(safeLevel / 3));
}

export function nextTerritoryUpgrade(level) {
  const safeLevel = Math.max(1, Math.floor(Number(level) || 1));
  const currentLimit = territoryLimitForLevel(safeLevel);
  if (currentLimit >= 10) return null;
  return { level: currentLimit * 3 + 1, limit: currentLimit + 1 };
}
