...
export const DAILY_LEGO_GOALS = [
  ...
  { key: 'liarPlay', label: '라이어게임 1판 참가' },
  { key: 'liarWin', label: '라이어게임 1판 승리' },
  { key: 'bung', label: '벙 1회 정상 완료' }
];
...
export function resetDailyProgress(pet) {
  pet.daily.goalCounters = { life: 0, mini: 0, fishing: 0 };
...
export function incrementGoalCounter(pet, key, amount = 1, date = new Date()) {
  ensureDailyProgress(pet, date);
  const safeAmount = Math.max(0, Math.floor(Number(amount) || 0));
  if (!['life', 'mini', 'fishing'].includes(key) || safeAmount <= 0) return { awarded: false, amount: 0 };
...
export function dailyGoalView(pet) {
  ...
  const thresholds = { life: 3, mini: 2, fishing: 5 };
