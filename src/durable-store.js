...
function normalizeState(parsed) {
  ...
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs)
    ? state.adminAuditLogs.filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry)
      ...
      const allowedActions = ['point_adjust', 'account_delete', 'account_lock', 'pin_reset', 'bung_force_cancel', 'workout_badge', 'state_reset', 'omok_clear_ended', 'block_battle_clear_ended', 'davinci_clear_ended', 'sichuan_clear_ended', 'support_settings', 'kick'];
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    const [manifest, battleSnapshot, sichuanStateSnapshot, backupManifest] = await Promise.all([
