...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 22, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 22, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    try {
      ...
      if (storedVersion !== 22 || storedTerritoryVersion < 5 || storedTerritorySize !== 5) await this.save(this.cache);
    } catch {
