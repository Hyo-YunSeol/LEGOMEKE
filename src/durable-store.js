...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 27, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  // v27: 사천성 시즌왕 도입 시, 직전 시즌 1위로 확인된 '다매'에게 한 번만 현 시즌 만료 시각까지 소급 부여한다.
  // 저장 버전이 27로 올라간 뒤에는 다시 실행되지 않는다.
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 27, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    try {
      ...
      if (storedVersion !== 27 || storedTerritoryVersion < 6 || storedTerritorySize !== 5 || territoryRecovered || removedFeatureStatePresent) await this.save(this.cache, { forceBackup: true });
      return this.cache;
    } catch (error) {
