...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 26, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs)
    ? state.adminAuditLogs.filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry)
      ...
      const allowedActions = ['point_adjust', 'account_delete', 'account_lock', 'bung_force_cancel', 'workout_badge', 'state_reset', 'omok_clear_ended', 'block_battle_clear_ended', 'davinci_clear_ended', 'sichuan_clear_ended', 'support_settings', 'kick'];
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 26, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    try {
      ...
      if (storedVersion !== 26 || storedTerritoryVersion < 6 || storedTerritorySize !== 5 || territoryRecovered || removedFeatureStatePresent) await this.save(this.cache, { forceBackup: true });
      return this.cache;
    } catch (error) {
