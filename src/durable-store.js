...
function normalizeState(parsed) {
  ...
  for (const [userId, user] of Object.entries(state.users)) {
    ...
    user.accountLockedAt = user.accountLocked ? normalizeIsoDate(user.accountLockedAt, new Date().toISOString()) : null;
    user.sessionVersion = Math.max(1, Math.floor(Number(user.sessionVersion) || 1));
