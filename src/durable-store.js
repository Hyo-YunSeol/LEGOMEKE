...
const SOCIAL_REQUEST_TTL_MS = 24 * 60 * 60_000;
...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 25, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  for (const [requestId, request] of Object.entries(state.requests)) {
    ...
    request.expiresAt = normalizeIsoDate(request.expiresAt, new Date(createdMs + SOCIAL_REQUEST_TTL_MS).toISOString());
    request.expiredAt = normalizeIsoDate(request.expiredAt);
  }
  ...
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs)
    ? state.adminAuditLogs.filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry)
      ...
      const allowedActions = ['point_adjust', 'account_delete', 'bung_force_cancel', 'workout_badge', 'state_reset', 'omok_clear_ended', 'block_battle_clear_ended', 'kick'];
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 25, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
function pruneState(state) {
  ...
  for (const [requestId, request] of Object.entries(state.requests ?? {})) {
    ...
    if (request.status === 'pending') {
      const expires = new Date(request.expiresAt ?? 0).getTime();
      if (Number.isFinite(expires) && expires <= now) {
        request.status = 'expired';
        request.respondedAt = new Date(now).toISOString();
        request.expiredAt = request.respondedAt;
      }
      continue;
    }
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    try {
      ...
      if (storedVersion !== 25 || storedTerritoryVersion < 6 || storedTerritorySize !== 5 || territoryRecovered || removedFeatureStatePresent) await this.save(this.cache, { forceBackup: true });
      return this.cache;
    } catch (error) {
