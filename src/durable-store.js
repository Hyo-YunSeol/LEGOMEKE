...
function normalizeState(parsed) {
  ...
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs)
    ? state.adminAuditLogs.filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry)
      ...
      const allowedActions = ['point_adjust', 'account_delete', 'account_lock', 'bung_force_cancel', 'workout_badge', 'state_reset', 'omok_clear_ended', 'block_battle_clear_ended', 'kick'];
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    const [manifest, battleSnapshot, backupManifest] = await Promise.all([
    ...
    try {
      ...
      if (backupJson) {
        try {
          ...
          this.cache = applyBlockBattleSnapshot(normalizeState(recovered), battleSnapshot);
          this.manifest = manifest;
          await this.save(this.cache, { forceBackup: true });
          return this.cache;
        } catch (backupError) {
