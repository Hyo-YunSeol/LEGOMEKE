...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 14, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  for (const key of ['users', 'pets', 'relationships', 'bungs', 'requests', 'miniGameChallenges', 'pokes']) {
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 14, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    try {
      ...
      if (storedVersion !== 14 || storedTerritoryVersion < 3 || storedTerritorySize !== 4) await this.save(this.cache);
    } catch {
