...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 21, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 21, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    const [manifest, chatSnapshot] = await Promise.all([
    ...
    try {
      ...
      this.cache = applyLiarChatSnapshot(normalizeState(parsed), chatSnapshot);
      this.manifest = manifest;
      if (storedVersion !== 21 || storedTerritoryVersion < 5 || storedTerritorySize !== 5) await this.save(this.cache);
    } catch {
