...
function applyRealtimeSnapshots(state, battleSnapshot, sichuanStateSnapshot) {
  return applySichuanSnapshot(applyBlockBattleSnapshot(state, battleSnapshot), sichuanStateSnapshot);
}
...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 29, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    const [manifest, battleSnapshot, sichuanStateSnapshot, legacySpotDifferenceSnapshot, backupManifest] = await Promise.all([
    ...
    try {
      ...
      this.cache = applyRealtimeSnapshots(normalizeState(parsed), battleSnapshot, sichuanStateSnapshot);
      this.manifest = manifest;
      const territoryRecovered = storedTerritoryJson !== JSON.stringify(this.cache.territory ?? null);
      if (storedVersion !== 29 || storedTerritoryVersion < 6 || storedTerritorySize !== 5 || territoryRecovered || removedFeatureStatePresent) await this.save(this.cache, { forceBackup: true });
      return this.cache;
    } catch (error) {
      ...
      if (backupJson) {
        try {
          ...
          this.cache = applyRealtimeSnapshots(normalizeState(recovered), battleSnapshot, sichuanStateSnapshot);
          this.manifest = manifest;
          await this.save(this.cache, { forceBackup: true });
          return this.cache;
        } catch (backupError) {
