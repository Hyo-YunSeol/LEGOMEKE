...
const LIAR_CHAT_STATE_KEY = 'liar-chat-state';
const BLOCK_BATTLE_STATE_KEY = 'block-battle-state';
const CHUNK_CHARACTERS = 400_000;

function liarChatSnapshot(state) {
  const game = state?.liarGame ?? {};
  return {
    revision: Math.max(0, Math.floor(Number(state?.meta?.revision) || 0)),
    messages: Array.isArray(game.messages) ? structuredClone(game.messages) : [],
    chatOperations: game.chatOperations && typeof game.chatOperations === 'object' && !Array.isArray(game.chatOperations)
      ? structuredClone(game.chatOperations)
      : {},
    playerChatState: Object.fromEntries(Object.entries(game.players ?? {}).map(([petId, player]) => [petId, { lastChatAt: player?.lastChatAt ?? null }])),
    updatedAt: new Date().toISOString()
  };
}

function applyLiarChatSnapshot(state, snapshot) {
  if (!snapshot || typeof snapshot !== 'object' || !Array.isArray(snapshot.messages)) return state;
  state.liarGame ??= initialLiarGame();
  state.liarGame.messages = structuredClone(snapshot.messages);
  state.liarGame.chatOperations = snapshot.chatOperations && typeof snapshot.chatOperations === 'object' && !Array.isArray(snapshot.chatOperations)
    ? structuredClone(snapshot.chatOperations)
    : {};
  for (const [petId, chatState] of Object.entries(snapshot.playerChatState ?? {})) {
    if (state.liarGame.players?.[petId]) state.liarGame.players[petId].lastChatAt = chatState?.lastChatAt ?? null;
  }
...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 23, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  for (const [userId, user] of Object.entries(state.users)) {
    ...
    const temporaryNickname = user.temporaryNickname && typeof user.temporaryNickname === 'object' && !Array.isArray(user.temporaryNickname) ? user.temporaryNickname : null;
    const temporaryValue = String(temporaryNickname?.nickname ?? '').normalize('NFKC').replace(/\s+/g, ' ').trim();
    const temporaryExpiresAt = new Date(temporaryNickname?.expiresAt ?? '').getTime();
    user.temporaryNickname = [...temporaryValue].length >= 2 && [...temporaryValue].length <= 12
      && /^[가-힣a-zA-Z0-9 _-]+$/.test(temporaryValue)
      && Number.isFinite(temporaryExpiresAt) && temporaryExpiresAt > Date.now()
      ? { nickname: temporaryValue, expiresAt: new Date(temporaryExpiresAt).toISOString() }
      : null;
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 23, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    const [manifest, chatSnapshot, battleSnapshot] = await Promise.all([
      this.storage.get(STATE_MANIFEST_KEY),
      this.storage.get(LIAR_CHAT_STATE_KEY),
    ...
    try {
      ...
      this.cache = applyBlockBattleSnapshot(applyLiarChatSnapshot(normalizeState(parsed), chatSnapshot), battleSnapshot);
      this.manifest = manifest;
      const territoryRecovered = storedTerritoryJson !== JSON.stringify(this.cache.territory ?? null);
      if (storedVersion !== 23 || storedTerritoryVersion < 6 || storedTerritorySize !== 5 || territoryRecovered) await this.save(this.cache);
    } catch {
  ...
  async save(state) {
    ...
    entries[LIAR_CHAT_STATE_KEY] = liarChatSnapshot(state);
  ...
  async saveLiarChat(state) {
    state.meta ??= {};
    state.meta.revision = Math.max(0, Math.floor(Number(state.meta.revision) || 0)) + 1;
    state.meta.updatedAt = new Date().toISOString();
    state.liarGame = normalizeLiarGame(state.liarGame, state);
    await this.storage.put(LIAR_CHAT_STATE_KEY, liarChatSnapshot(state));
    this.cache = state;
  }
