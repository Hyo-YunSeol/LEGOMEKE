...
import { initialDavinci, normalizeDavinci } from './game/davinci.js';
import { initialSichuan, normalizeSichuan } from './game/sichuan.js';
import { initialSpotDifference, normalizeSpotDifference } from './game/spot-difference.js';
...
const SPOT_DIFFERENCE_STATE_KEY = 'spot-difference-state';
...
function spotDifferenceSnapshot(state) {
  return {
    revision: Math.max(0, Math.floor(Number(state?.meta?.revision) || 0)),
    spotDifference: structuredClone(state?.spotDifference ?? initialSpotDifference()),
    updatedAt: new Date().toISOString()
  };
}

function applySpotDifferenceSnapshot(state, snapshot) {
  if (!snapshot || typeof snapshot !== 'object' || !snapshot.spotDifference) return state;
  state.spotDifference = normalizeSpotDifference(structuredClone(snapshot.spotDifference), state);
  state.meta.revision = Math.max(
    Math.max(0, Math.floor(Number(state.meta?.revision) || 0)),
    Math.max(0, Math.floor(Number(snapshot.revision) || 0))
  );
...
function applyRealtimeSnapshots(state, battleSnapshot, sichuanStateSnapshot, spotDifferenceStateSnapshot) {
  return applySpotDifferenceSnapshot(
    applySichuanSnapshot(applyBlockBattleSnapshot(state, battleSnapshot), sichuanStateSnapshot),
    spotDifferenceStateSnapshot
  );
}
...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 28, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
    ...
    davinci: initialDavinci(),
    sichuan: initialSichuan(),
    spotDifference: initialSpotDifference(),
...
function normalizeState(parsed) {
  ...
  for (const [userId, user] of Object.entries(state.users)) {
    ...
    user.notifications = Array.isArray(user.notifications)
      ? user.notifications.filter((item) => {
        ...
        if (item.type === 'cosmetic-expiry' || item.type === 'warning') return false;
        if (/경고/.test(text)) return false;
  ...
  for (const [petId, pet] of Object.entries(state.pets)) {
    ...
    delete pet.warnings;
    if (pet.records && typeof pet.records === 'object') delete pet.records.warnings;
  }
  ...
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs)
    ? state.adminAuditLogs.filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry)
      ...
      const allowedActions = ['point_adjust', 'account_delete', 'account_lock', 'pin_reset', 'bung_force_cancel', 'workout_badge', 'state_reset', 'omok_clear_ended', 'block_battle_clear_ended', 'davinci_clear_ended', 'sichuan_clear_ended', 'spot_difference_clear_ended', 'support_settings', 'kick'];
  ...
  state.davinci = normalizeDavinci(state.davinci, state);
  state.sichuan = normalizeSichuan(state.sichuan, state);
  state.spotDifference = normalizeSpotDifference(state.spotDifference, state);
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 28, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    const [manifest, battleSnapshot, sichuanStateSnapshot, spotDifferenceStateSnapshot, backupManifest] = await Promise.all([
      this.storage.get(STATE_MANIFEST_KEY),
      this.storage.get(BLOCK_BATTLE_STATE_KEY),
      this.storage.get(SICHUAN_STATE_KEY),
      this.storage.get(SPOT_DIFFERENCE_STATE_KEY),
    ...
    try {
      ...
      const removedFeatureStatePresent = Boolean(parsed?.liarGame)
        ...
        || (parsed?.adminAuditLogs ?? []).some((entry) => String(entry?.action ?? '').startsWith('liar') || String(entry?.detail ?? '').includes('라이어게임') || /warning/i.test(String(entry?.action ?? '')) || /경고/.test(String(entry?.detail ?? '')));
      this.cache = applyRealtimeSnapshots(normalizeState(parsed), battleSnapshot, sichuanStateSnapshot, spotDifferenceStateSnapshot);
      this.manifest = manifest;
      const territoryRecovered = storedTerritoryJson !== JSON.stringify(this.cache.territory ?? null);
      if (storedVersion !== 28 || storedTerritoryVersion < 6 || storedTerritorySize !== 5 || territoryRecovered || removedFeatureStatePresent) await this.save(this.cache, { forceBackup: true });
      return this.cache;
    } catch (error) {
      ...
      if (backupJson) {
        try {
          const recovered = JSON.parse(backupJson);
          this.cache = applyRealtimeSnapshots(normalizeState(recovered), battleSnapshot, sichuanStateSnapshot, spotDifferenceStateSnapshot);
          this.manifest = manifest;
          await this.save(this.cache, { forceBackup: true });
          return this.cache;
        } catch (backupError) {
  ...
  async save(state, { forceBackup = false } = {}) {
    ...
    await this.storage.put({
      ...
      [SPOT_DIFFERENCE_STATE_KEY]: spotDifferenceSnapshot(state)
    });
    this.cache = state;
    this.manifest = manifest;
  ...
  async saveSpotDifference(state) {
    state.meta ??= {};
    state.meta.revision = Math.max(0, Math.floor(Number(state.meta.revision) || 0)) + 1;
    state.meta.updatedAt = new Date().toISOString();
    state.spotDifference = normalizeSpotDifference(state.spotDifference, state);
    await this.storage.put(SPOT_DIFFERENCE_STATE_KEY, spotDifferenceSnapshot(state));
    this.cache = state;
  }
