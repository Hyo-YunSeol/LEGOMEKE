...
import { normalizeMinesweeperChallenge, normalizeMinesweeperSeason } from './game/minesweeper.js';
...
import { initialSichuan, normalizeSichuan } from './game/sichuan.js';
import { initialLegodoku, normalizeLegodoku } from './game/legodoku.js';
...
const SICHUAN_STATE_KEY = 'sichuan-state';
const LEGODOKU_STATE_KEY = 'legodoku-state';
const LEGACY_SPOT_DIFFERENCE_STATE_KEY = 'spot-difference-state';
const BACKUP_MANIFEST_KEY = 'state-backup-manifest';
const BACKUP_CHUNK_PREFIX = 'state-backup-chunk-';
const BROKEN_MANIFEST_PREFIX = 'broken-state-manifest-';
const BROKEN_CHUNK_PREFIX = 'broken-state-chunk-';
const BACKUP_INTERVAL_MS = 10 * 60_000;
const SOCIAL_REQUEST_HISTORY_MS = 7 * 24 * 60 * 60_000;
const OPERATION_HISTORY_MS = 7 * 24 * 60 * 60_000;
const OPERATION_HISTORY_LIMIT = 500;
const ADMIN_POINT_HISTORY_LIMIT = 100;
const ADMIN_AUDIT_LIMIT = 100;
const NOTIFICATION_LIMIT = 50;
const COMPLETED_BUNG_LIMIT = 50;
...
function sichuanSnapshot(state) {
  return {
    revision: Math.max(0, Math.floor(Number(state?.meta?.revision) || 0)),
    sichuan: structuredClone(state?.sichuan ?? initialSichuan()),
    updatedAt: new Date().toISOString()
  };
}

function applySichuanSnapshot(state, snapshot) {
  if (!snapshot || typeof snapshot !== 'object' || !snapshot.sichuan) return state;
  state.sichuan = normalizeSichuan(structuredClone(snapshot.sichuan), state);
  state.meta.revision = Math.max(
    Math.max(0, Math.floor(Number(state.meta?.revision) || 0)),
    Math.max(0, Math.floor(Number(snapshot.revision) || 0))
  );
...
function legodokuSnapshot(state) {
  return {
    revision: Math.max(0, Math.floor(Number(state?.meta?.revision) || 0)),
    legodoku: structuredClone(state?.legodoku ?? initialLegodoku()),
    updatedAt: new Date().toISOString()
  };
}

function applyLegodokuSnapshot(state, snapshot) {
  if (!snapshot || typeof snapshot !== 'object' || !snapshot.legodoku) return state;
  state.legodoku = normalizeLegodoku(structuredClone(snapshot.legodoku), state);
  state.meta.revision = Math.max(
    Math.max(0, Math.floor(Number(state.meta?.revision) || 0)),
    Math.max(0, Math.floor(Number(snapshot.revision) || 0))
  );
...
function applyRealtimeSnapshots(state, battleSnapshot, sichuanStateSnapshot, legodokuStateSnapshot) {
  return applyLegodokuSnapshot(applySichuanSnapshot(applyBlockBattleSnapshot(state, battleSnapshot), sichuanStateSnapshot), legodokuStateSnapshot);
}
...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 30, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
    ...
    publicEvents: [],
    loudspeaker: null,
    supportSettings: { enabled: true, message: DEFAULT_SUPPORT_MESSAGE },
    omok: initialOmok(),
    blockBattle: initialBlockBattle(),
    sichuan: initialSichuan(),
    legodoku: initialLegodoku(),
    territory: initialTerritory(date),
    gameRankingSeason: normalizeGameRankingSeason(null, date),
    minesweeperSeason: normalizeMinesweeperSeason(null, date)
  };
}
...
function applyLegacyRemovedGameSnapshotForCleanup(parsed, legacySpotDifferenceSnapshot) {
  if (!parsed || typeof parsed !== 'object' || !legacySpotDifferenceSnapshot || typeof legacySpotDifferenceSnapshot !== 'object' || !legacySpotDifferenceSnapshot.spotDifference) return parsed;
  parsed.spotDifference = structuredClone(legacySpotDifferenceSnapshot.spotDifference);
  parsed.meta = parsed.meta && typeof parsed.meta === 'object' && !Array.isArray(parsed.meta) ? parsed.meta : {};
  parsed.meta.revision = Math.max(
    Math.max(0, Math.floor(Number(parsed.meta.revision) || 0)),
    Math.max(0, Math.floor(Number(legacySpotDifferenceSnapshot.revision) || 0))
  );
  return parsed;
}

function refundRemovedGameEscrow(state) {
  const refundRooms = (rooms) => {
    for (const room of Object.values(rooms ?? {})) {
      ...
      for (const [petId, rawAmount] of Object.entries(escrow)) {
        ...
        pet.stats.points = Math.max(0, Math.floor(Number(pet.stats.points) || 0)) + amount;
        pet.records.pointsSpent = Math.max(0, Math.floor(Number(pet.records.pointsSpent) || 0) - amount);
      }
    }
  };
  refundRooms(state.davinci?.rooms);
  refundRooms(state.spotDifference?.rooms);
}
...
function normalizeState(parsed) {
  ...
  for (const key of ['users', 'pets', 'relationships', 'bungs', 'requests', 'miniGameChallenges', 'shopOperations', 'foodOperations', 'bungOperations', 'territoryOperations', 'pokes', 'adminPointOperations']) {
  ...
  for (const [challengeId, challenge] of Object.entries(state.miniGameChallenges)) {
    if (!challenge || !['oddEven', 'reaction', 'number', 'apple', 'block', 'minesweeper', 'tetrisSingle'].includes(challenge.gameId)) {
      delete state.miniGameChallenges[challengeId];
      continue;
    }
    ...
    if (challenge.gameId === 'minesweeper') {
      state.miniGameChallenges[challengeId] = normalizeMinesweeperChallenge(challenge);
      continue;
    }
  ...
  const rawSupportSettings = state.supportSettings && typeof state.supportSettings === 'object' && !Array.isArray(state.supportSettings) ? state.supportSettings : {};
  const normalizedSupportMessage = [...String(rawSupportSettings.message ?? DEFAULT_SUPPORT_MESSAGE).replace(/\s+/g, ' ').trim()].slice(0, 120).join('');
  state.supportSettings = {
    enabled: rawSupportSettings.enabled !== false,
    message: normalizedSupportMessage || DEFAULT_SUPPORT_MESSAGE
  };
  ...
  state.publicEvents = Array.isArray(state.publicEvents)
    ? state.publicEvents.filter((event) => {
      ...
      if (eventType === 'liar' || /davinci|spot[-_]?difference|spotdifference/i.test(eventType)) return false;
      if (/경고|다빈치|틀린그림/.test(text)) return false;
  ...
  for (const [userId, user] of Object.entries(state.users)) {
    ...
    user.accountLocked = Boolean(user.accountLocked);
    ...
    user.notifications = Array.isArray(user.notifications)
      ? user.notifications.filter((item) => {
        ...
        if (itemType === 'liar' || /davinci|spot[-_]?difference|spotdifference/i.test(itemType)) return false;
        if (itemType === 'cosmetic-expiry' || itemType === 'warning') return false;
        if (/경고|다빈치|틀린그림/.test(text)) return false;
  ...
  for (const [petId, pet] of Object.entries(state.pets)) {
    ...
    delete pet.warnings;
    if (pet.records && typeof pet.records === 'object' && !Array.isArray(pet.records)) {
      delete pet.records.warnings;
      for (const key of ['davinciGames', 'davinciTotalWins', 'davinciTotalCorrect', 'davinciWins', 'davinciCorrect', 'spotDifferenceGames', 'spotDifferenceWins', 'spotDifferenceDraws', 'spotDifferenceLosses', 'seasonSpotDifferenceWins', 'seasonSpotDifferenceDraws', 'seasonSpotDifferenceLosses']) delete pet.records[key];
    }
    if (pet.seasonBadges && typeof pet.seasonBadges === 'object' && !Array.isArray(pet.seasonBadges)) delete pet.seasonBadges.spotDifference;
    if (pet.kingHistory && typeof pet.kingHistory === 'object' && !Array.isArray(pet.kingHistory)) delete pet.kingHistory.spotDifference;
  }
  ...
  for (const [requestId, request] of Object.entries(state.requests)) {
    ...
    request.fromPetId = String(request.fromPetId || '');
    request.toPetId = String(request.toPetId || '');
    if (!state.pets[request.fromPetId] || !state.pets[request.toPetId] || request.fromPetId === request.toPetId) {
      delete state.requests[requestId];
      continue;
    }
    request.status = ['pending', 'accepted', 'rejected', 'cancelled', 'expired'].includes(request.status) ? request.status : 'cancelled';
    request.createdAt = normalizeIsoDate(request.createdAt, new Date().toISOString());
    request.respondedAt = normalizeIsoDate(request.respondedAt);
    // 구버전의 24시간 만료 필드는 폐기하고, 미응답·처리완료 신청 모두 pruneState에서 최근 7일만 보관한다.
    delete request.expiresAt;
    delete request.expiredAt;
  }
  ...
  for (const key of ['foodOperations', 'bungOperations']) {
    for (const [operationId, operation] of Object.entries(state[key])) {
      if (!operation || typeof operation !== 'object' || Array.isArray(operation)) { delete state[key][operationId]; continue; }
      const createdAt = normalizeIsoDate(operation.createdAt);
      if (!createdAt || !operation.userId || !operation.petId || !operation.result || typeof operation.result !== 'object') { delete state[key][operationId]; continue; }
      operation.id = String(operation.id || operationId);
      operation.userId = String(operation.userId);
      operation.petId = String(operation.petId);
      operation.createdAt = createdAt;
  ...
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs)
    ? state.adminAuditLogs.filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry)
      && !String(entry.action ?? '').startsWith('liar')
      && !String(entry.detail ?? '').includes('라이어게임')
      && !/davinci|spot[-_]?difference|spotdifference/i.test(String(entry.action ?? ''))
      && !/다빈치|틀린그림/.test(String(entry.detail ?? ''))
      && !/warning/i.test(String(entry.action ?? ''))
      && !/경고/.test(String(entry.detail ?? ''))).map((entry) => {
      const allowedActions = ['point_adjust', 'account_delete', 'account_lock', 'pin_reset', 'bung_force_cancel', 'workout_badge', 'state_reset', 'omok_clear_ended', 'block_battle_clear_ended', 'sichuan_clear_ended', 'support_settings', 'kick'];
    ...
    }).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, ADMIN_AUDIT_LIMIT)
    : [];

  removeLegacyRealtimeAndNicknameState(state);
  // Removed battle modes may still hold player stakes at deploy time. Refund every
  // non-zero escrow entry before deleting those modes so the cleanup cannot burn points.
  refundRemovedGameEscrow(state);
  delete state.davinci;
  delete state.spotDifference;
  state.publicEvents = (state.publicEvents ?? []).filter((event) => !/davinci|spot[-_]?difference|spotdifference/i.test(String(event?.type ?? '')) && !/다빈치|틀린그림/.test(String(event?.text ?? '')));
  state.omok = normalizeOmok(state.omok, state);
  state.blockBattle = normalizeBlockBattle(state.blockBattle, state);
  state.sichuan = normalizeSichuan(state.sichuan, state);
  state.legodoku = normalizeLegodoku(state.legodoku, state);
  ...
  state.minesweeperSeason = normalizeMinesweeperSeason(state.minesweeperSeason, new Date());

  // v27: 사천성 시즌왕 도입 당시 확인된 직전 시즌 1위에게 한 번만 현 시즌 만료 시각까지 소급 부여한다.
  // v28에서 대상 정정이 추가되므로, 새 설치/오래된 저장본도 최종적으로 올바른 대상만 남게 된다.
  if (previousVersion < 27) {
    const damaeUser = Object.values(state.users).find((candidate) => candidate?.nickname === '다매');
    const damaePet = damaeUser ? state.pets?.[damaeUser.currentPetId] : null;
    if (damaePet?.alive && damaePet.userId === damaeUser.id) {
      damaePet.seasonBadges = damaePet.seasonBadges && typeof damaePet.seasonBadges === 'object' && !Array.isArray(damaePet.seasonBadges) ? damaePet.seasonBadges : {};
      damaePet.seasonBadges.sichuan = state.gameRankingSeason.endsAt;
    }
  }

  // v28: 이미 배포된 v27의 사천성왕 소급 대상을 '다매레고'에서 실제 직전 시즌 1위 '레알다매레고'로 정정한다.
  // v27이 한 번 실행된 운영 데이터에서도 정확히 한 번 정정되도록 저장 버전으로 가드한다.
  if (previousVersion < 28) {
    const wrongUser = Object.values(state.users).find((candidate) => candidate?.nickname === '다매');
    const wrongPet = wrongUser ? state.pets?.[wrongUser.currentPetId] : null;
    const targetPet = Object.values(state.pets ?? {}).find((candidate) => candidate?.alive && candidate?.displayName === '레알다매레고')
      ?? (() => {
        const targetUser = Object.values(state.users).find((candidate) => candidate?.nickname === '레알다매');
        return targetUser ? state.pets?.[targetUser.currentPetId] : null;
      })();
    const retroactiveExpiry = state.gameRankingSeason.endsAt;
    if (wrongPet?.alive && wrongPet.seasonBadges?.sichuan === retroactiveExpiry) {
      wrongPet.seasonBadges.sichuan = null;
      if (wrongPet.kingHistory && typeof wrongPet.kingHistory === 'object') wrongPet.kingHistory.sichuan = false;
    }
    if (targetPet?.alive) {
      targetPet.seasonBadges = targetPet.seasonBadges && typeof targetPet.seasonBadges === 'object' && !Array.isArray(targetPet.seasonBadges) ? targetPet.seasonBadges : {};
      targetPet.seasonBadges.sichuan = retroactiveExpiry;
      targetPet.kingHistory = targetPet.kingHistory && typeof targetPet.kingHistory === 'object' && !Array.isArray(targetPet.kingHistory) ? targetPet.kingHistory : {};
      targetPet.kingHistory.sichuan = true;
    }
  }

  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 30, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
function pruneState(state) {
  ...
  for (const user of Object.values(state.users ?? {})) if (user && typeof user === 'object') user.notifications = Array.isArray(user.notifications) ? user.notifications.filter(Boolean).slice(0, NOTIFICATION_LIMIT) : [];
  ...
  const keepCompleted = new Set(completedBungs.slice(0, COMPLETED_BUNG_LIMIT).map((bung) => bung.id));
  for (const [bungId, bung] of Object.entries(state.bungs ?? {})) {
  ...
  for (const [requestId, request] of Object.entries(state.requests ?? {})) {
    if (!request) { delete state.requests[requestId]; continue; }
    const reference = new Date(request.respondedAt ?? request.expiredAt ?? request.createdAt ?? 0).getTime();
    if (!Number.isFinite(reference) || now - reference > SOCIAL_REQUEST_HISTORY_MS) delete state.requests[requestId];
  }

  for (const [challengeId, challenge] of Object.entries(state.miniGameChallenges ?? {})) {
    const created = new Date(challenge?.createdAt ?? 0).getTime();
    const keepActivePersistentMini = ['block', 'minesweeper'].includes(challenge?.gameId) && challenge.completed !== true;
    if (!challenge || !Number.isFinite(created) || (!keepActivePersistentMini && now - created > 24 * 60 * 60_000)) delete state.miniGameChallenges[challengeId];
  }
  const adminPointOperations = Object.values(state.adminPointOperations ?? {})
    .filter((operation) => operation && now - new Date(operation.createdAt ?? 0).getTime() <= OPERATION_HISTORY_MS)
    .sort((a, b) => new Date(b.createdAt ?? 0) - new Date(a.createdAt ?? 0))
    .slice(0, ADMIN_POINT_HISTORY_LIMIT);
  ...
  for (const key of ['shopOperations', 'foodOperations', 'bungOperations', 'territoryOperations']) {
    const operations = Object.values(state[key] ?? {})
      .filter((operation) => operation && now - new Date(operation.createdAt ?? 0).getTime() <= OPERATION_HISTORY_MS)
      .sort((a, b) => new Date(b.createdAt ?? 0) - new Date(a.createdAt ?? 0))
      .slice(0, OPERATION_HISTORY_LIMIT);
    state[key] = Object.fromEntries(operations.map((operation) => [operation.id, operation]));
  }
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs) ? state.adminAuditLogs.filter(Boolean).sort((a, b) => new Date(b.createdAt ?? 0) - new Date(a.createdAt ?? 0)).slice(0, ADMIN_AUDIT_LIMIT) : [];
...
export class DurableJsonStore {
  ...
  async readChunked(manifest, prefix) {
    if (!manifest || !Number.isInteger(manifest.chunks) || manifest.chunks < 1) return null;
    const keys = Array.from({ length: manifest.chunks }, (_, index) => `${prefix}${index}`);
    const values = await this.storage.get(keys);
    const json = keys.map((key) => values.get(key) ?? '').join('');
    return json.length ? json : null;
  }

  async writeChunked(json, manifestKey, prefix, previousManifest = null, extraManifest = {}) {
    const chunks = [];
    for (let offset = 0; offset < json.length; offset += CHUNK_CHARACTERS) chunks.push(json.slice(offset, offset + CHUNK_CHARACTERS));
    if (!chunks.length) chunks.push('{}');
    const entries = {};
    chunks.forEach((chunk, index) => { entries[`${prefix}${index}`] = chunk; });
    const manifest = { chunks: chunks.length, characters: json.length, updatedAt: new Date().toISOString(), ...extraManifest };
    entries[manifestKey] = manifest;
    await this.storage.put(entries);
    const previousChunks = previousManifest?.chunks ?? 0;
    if (previousChunks > chunks.length) {
      const stale = Array.from({ length: previousChunks - chunks.length }, (_, index) => `${prefix}${index + chunks.length}`);
      await this.storage.delete(stale);
    }
    return manifest;
  }

  async storeBrokenSnapshot(json, manifest, error) {
    const stamp = Date.now();
    const manifestKey = `${BROKEN_MANIFEST_PREFIX}${stamp}`;
    const prefix = `${BROKEN_CHUNK_PREFIX}${stamp}-`;
    await this.writeChunked(json || '{}', manifestKey, prefix, null, {
      sourceChunks: Number(manifest?.chunks) || 0,
      error: String(error?.message || error || 'JSON parse failed').slice(0, 300)
    });
  }

  async load() {
    if (this.cache) return this.cache;
    const [manifest, battleSnapshot, sichuanStateSnapshot, legodokuStateSnapshot, legacySpotDifferenceSnapshot, backupManifest] = await Promise.all([
      this.storage.get(STATE_MANIFEST_KEY),
      this.storage.get(BLOCK_BATTLE_STATE_KEY),
      this.storage.get(SICHUAN_STATE_KEY),
      this.storage.get(LEGODOKU_STATE_KEY),
      this.storage.get(LEGACY_SPOT_DIFFERENCE_STATE_KEY),
      this.storage.get(BACKUP_MANIFEST_KEY)
    ]);
    this.backupManifest = backupManifest ?? null;
    if (!manifest || !Number.isInteger(manifest.chunks) || manifest.chunks < 1) {
      this.cache = initialState();
      await this.save(this.cache, { forceBackup: true });
      return this.cache;
    }
    const json = await this.readChunked(manifest, STATE_CHUNK_PREFIX);
    try {
      const parsed = applyLegacyRemovedGameSnapshotForCleanup(JSON.parse(json ?? ''), legacySpotDifferenceSnapshot);
      ...
      const removedFeatureStatePresent = Boolean(parsed?.liarGame)
        ...
        || (parsed?.publicEvents ?? []).some((event) => event?.type === 'liar' || /경고/.test(String(event?.text ?? '')))
        || Object.values(parsed?.users ?? {}).some((user) => (user?.notifications ?? []).some((item) => item?.type === 'liar' || item?.type === 'warning' || /경고/.test(String(item?.text ?? ''))))
        || Object.values(parsed?.pets ?? {}).some((pet) => Object.hasOwn(pet ?? {}, 'warnings') || Object.hasOwn(pet?.records ?? {}, 'warnings'))
        || Boolean(parsed?.davinci)
        || Boolean(parsed?.spotDifference)
        || Boolean(legacySpotDifferenceSnapshot)
        || Object.values(parsed?.pets ?? {}).some((pet) => Object.keys(pet?.records ?? {}).some((key) => /^(davinci|spotDifference|seasonSpotDifference)/.test(key)) || Object.hasOwn(pet?.seasonBadges ?? {}, 'spotDifference') || Object.hasOwn(pet?.kingHistory ?? {}, 'spotDifference'))
        || (parsed?.adminAuditLogs ?? []).some((entry) => String(entry?.action ?? '').startsWith('liar') || /davinci|spot[-_]?difference|spotdifference/i.test(String(entry?.action ?? '')) || /라이어게임|다빈치|틀린그림/.test(String(entry?.detail ?? '')) || /warning/i.test(String(entry?.action ?? '')) || /경고/.test(String(entry?.detail ?? '')))
        || Object.values(parsed?.users ?? {}).some((user) => (user?.notifications ?? []).some((item) => /davinci|spot[-_]?difference|spotdifference/i.test(String(item?.type ?? '')) || /다빈치|틀린그림/.test(String(item?.text ?? ''))))
        || (parsed?.publicEvents ?? []).some((event) => /davinci|spot[-_]?difference|spotdifference/i.test(String(event?.type ?? '')) || /다빈치|틀린그림/.test(String(event?.text ?? '')));
      this.cache = applyRealtimeSnapshots(normalizeState(parsed), battleSnapshot, sichuanStateSnapshot, legodokuStateSnapshot);
      this.manifest = manifest;
      const territoryRecovered = storedTerritoryJson !== JSON.stringify(this.cache.territory ?? null);
      if (storedVersion !== 30 || storedTerritoryVersion < 6 || storedTerritorySize !== 6 || territoryRecovered || removedFeatureStatePresent) await this.save(this.cache, { forceBackup: true });
      return this.cache;
    } catch (error) {
      await this.storeBrokenSnapshot(json ?? '', manifest, error);
      const backupJson = await this.readChunked(backupManifest, BACKUP_CHUNK_PREFIX);
      if (backupJson) {
        try {
          const recovered = applyLegacyRemovedGameSnapshotForCleanup(JSON.parse(backupJson), legacySpotDifferenceSnapshot);
          this.cache = applyRealtimeSnapshots(normalizeState(recovered), battleSnapshot, sichuanStateSnapshot, legodokuStateSnapshot);
          this.manifest = manifest;
          await this.save(this.cache, { forceBackup: true });
          return this.cache;
        } catch (backupError) {
          await this.storeBrokenSnapshot(backupJson, backupManifest, backupError);
        }
      }
      this.cache = null;
      throw new Error('게임 저장 데이터가 손상되어 자동 초기화를 중단했습니다. 원본 손상 데이터는 보존되어 있으며 마지막 정상 백업으로도 복구할 수 없습니다.');
    }
  }

  async save(state, { forceBackup = false } = {}) {
    state.meta ??= {};
    state.meta.revision = Math.max(0, Math.floor(Number(state.meta.revision) || 0)) + 1;
    pruneState(state);
    const json = JSON.stringify(state);
    const previousManifest = this.manifest;
    const manifest = await this.writeChunked(json, STATE_MANIFEST_KEY, STATE_CHUNK_PREFIX, previousManifest);
    await this.storage.put({
      [BLOCK_BATTLE_STATE_KEY]: blockBattleSnapshot(state),
      [SICHUAN_STATE_KEY]: sichuanSnapshot(state),
      [LEGODOKU_STATE_KEY]: legodokuSnapshot(state)
    });
    await this.storage.delete(LEGACY_SPOT_DIFFERENCE_STATE_KEY);
    this.cache = state;
    this.manifest = manifest;

    const lastBackup = new Date(this.backupManifest?.updatedAt ?? 0).getTime();
    if (forceBackup || !Number.isFinite(lastBackup) || Date.now() - lastBackup >= BACKUP_INTERVAL_MS) {
      this.backupManifest = await this.writeChunked(json, BACKUP_MANIFEST_KEY, BACKUP_CHUNK_PREFIX, this.backupManifest);
    }
  }
...
async saveSichuan(state) {
    state.meta ??= {};
    state.meta.revision = Math.max(0, Math.floor(Number(state.meta.revision) || 0)) + 1;
    state.meta.updatedAt = new Date().toISOString();
    state.sichuan = normalizeSichuan(state.sichuan, state);
    await this.storage.put(SICHUAN_STATE_KEY, sichuanSnapshot(state));
    this.cache = state;
  }
  async saveLegodoku(state) {
    state.meta ??= {};
    state.meta.revision = Math.max(0, Math.floor(Number(state.meta.revision) || 0)) + 1;
    state.meta.updatedAt = new Date().toISOString();
    state.legodoku = normalizeLegodoku(state.legodoku, state);
    await this.storage.put(LEGODOKU_STATE_KEY, legodokuSnapshot(state));
    this.cache = state;
  }
