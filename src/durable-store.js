...
export function initialState() {
  const date = new Date();
  return {
    meta: { version: 20, revision: 0, createdAt: date.toISOString(), updatedAt: date.toISOString() },
    users: {},
    pets: {},
    relationships: {},
    bungs: {},
...
function normalizeState(parsed) {
  ...
  for (const key of ['users', 'pets', 'relationships', 'bungs', 'requests', 'miniGameChallenges', 'shopOperations', 'pokes', 'adminPointOperations']) {
  ...
  state.adminAuditLogs = Array.isArray(state.adminAuditLogs)
    ? state.adminAuditLogs.filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry)).map((entry) => {
      const allowedActions = ['point_adjust', 'account_delete', 'bung_force_cancel', 'workout_badge'];
  ...
  state.meta = { ...base.meta, ...(state.meta ?? {}), version: 20, revision: Math.max(0, Math.floor(Number(state.meta?.revision) || 0)) };
  return state;
}
...
export class DurableJsonStore {
  ...
  async load() {
    ...
    try {
      ...
      if (storedVersion !== 20 || storedTerritoryVersion < 5 || storedTerritorySize !== 5) await this.save(this.cache);
    } catch {
