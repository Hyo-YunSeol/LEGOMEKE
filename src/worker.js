...
import {
  ...
  createDavinciRoom, joinDavinciRoom, setDavinciReady, kickDavinciPlayer, startDavinciRoom, placeDavinciJoker, submitDavinciRps, selectDavinciOrder,
...
const APP_VERSION = '6.9.2-davinci-ui-final';
  ...
  async handleFetch(request) {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/minigames/minesweeper/action') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => playMinesweeperGame(gameState, pet, body.challengeId, {
          action: body.action, row: body.row, col: body.col
        }, now), date);
        // 싱글게임 조작 결과와 최신 bootstrap을 HTTP 응답으로 바로 돌려주므로 자기 자신에게 별도
        // WebSocket refresh를 보내지 않는다. 매 클릭마다 이중 bootstrap/DOM 갱신이 발생하는 것을 막는다.
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      ...
      const davinciKick = matchPath(pathname, '/api/davinci/rooms/:roomId/kick');
      ...
      if (request.method === 'POST' && (davinciJoin || davinciReady || davinciKick || davinciStart || davinciJoker || davinciRps || davinciOrder || davinciGuess || davinciDecision || davinciPenalty || davinciSpectate || davinciSpectateLeave || davinciLeave || davinciRematch)) {
        const body = await readJson(request);
        const matched = davinciJoin || davinciReady || davinciKick || davinciStart || davinciJoker || davinciRps || davinciOrder || davinciGuess || davinciDecision || davinciPenalty || davinciSpectate || davinciSpectateLeave || davinciLeave || davinciRematch;
        const roomId = matched.roomId;
        const route = davinciJoin ? 'join' : davinciReady ? 'ready' : davinciKick ? 'kick' : davinciStart ? 'start' : davinciJoker ? 'joker' : davinciRps ? 'rps' : davinciOrder ? 'order' : davinciGuess ? 'guess' : davinciDecision ? 'decision' : davinciPenalty ? 'penalty' : davinciSpectate ? 'spectate' : davinciSpectateLeave ? 'spectate-leave' : davinciLeave ? 'leave' : 'rematch';
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          ...
          if (route === 'kick') return kickDavinciPlayer(gameState, pet, roomId, body.targetPetId, now);
