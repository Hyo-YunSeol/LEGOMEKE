...
import {
  LEGODOKU_MATCH_SECONDS, LEGODOKU_MAX_MISTAKES, LEGODOKU_MAX_ROOMS, LEGODOKU_SIZE, LEGODOKU_STAKES,
  clearEndedLegodokuRooms, createLegodokuRoom, joinLegodokuRoom, leaveLegodokuRoom, leaveLegodokuSpectator,
  legodokuNextAlarmAt, legodokuRanking, legodokuRoomView, legodokuView, playLegodokuCell, processLegodokuTimers,
  removePetFromLegodoku, requestLegodokuRematch, spectateLegodokuRoom
} from './game/legodoku.js';
...
const APP_VERSION = '6.10.28-full-fix-final';
...
function lockedPublicPetIds(state) {
  const lockedUsers = new Set(Object.values(state.users ?? {}).filter((user) => user?.accountLocked).map((user) => user.id));
  return new Set(Object.values(state.pets ?? {}).filter((pet) => pet && lockedUsers.has(pet.userId)).map((pet) => pet.id));
}

function publicRankingState(state, excludedPetIds = lockedPublicPetIds(state)) {
  if (!excludedPetIds.size) return state;
  const pets = Object.fromEntries(Object.entries(state.pets ?? {}).filter(([petId]) => !excludedPetIds.has(petId)));
  return { ...state, pets };
}
...
export class LegoGameRoom {
  constructor(ctx, env) {
    ...
    this.liveReactions = { omok: new Map(), blockBattle: new Map(), sichuan: new Map(), legodoku: new Map() };
  ...
  cleanLiveReactions(date = new Date()) {
    ...
    for (const scope of ['omok', 'blockBattle', 'sichuan', 'legodoku']) {
  ...
  adminView(state, user, onlineIds = this.onlineUserIds(state)) {
    ...
    return {
      ...
      endedLegodokuRooms: isAdmin ? Object.values(state.legodoku?.rooms ?? {}).filter((room) => room?.status === 'ended' && room?.settled).length : 0,
  ...
  deleteUserAccountState(state, targetUserId, date = new Date()) {
    ...
    for (const pet of targetPets) {
      ...
      removePetFromLegodoku(state, pet.id, date);
  ...
  async processTimeState(state, date = new Date()) {
    ...
    if (minesweeperSeason.changed) {
      ...
      for (const champion of minesweeperSeason.champions ?? []) {
        ...
        addPublicEvent(state, `${champion.badgeLabel} · ${withJosa(champion.displayName, '이/가')} 지뢰찾기 ${champion.label} ${seconds}초로 1위를 차지했습니다.`, 'game-season', [champion.petId], date);
    ...
    if (gameSeason.changed) {
      const labels = { apple: '🍎 사과게임', singleTetris: '🧱 싱글 테트리스', omok: '⚫ 오목', blockBattle: '🧱 테트리스대전', sichuan: '🀄 사천성대전', legodoku: '🧩 레고도쿠' };
    ...
    const legodoku = processLegodokuTimers(state, date);
    if (expiredMini.changed || bungs.changed || gameSeason.changed || territory.changed || omok.changed || sichuan.changed || legodoku.changed) changed = true;
    ...
    return { changed, nonBattleChanged, blockBattleChanged: blockBattle.changed, blockBattleSettled: blockBattle.settled, legodokuChanged: legodoku.changed };
  }
  ...
  async scheduleNextAlarm(state) {
    ...
    const legodokuAt = legodokuNextAlarmAt(state);
    ...
    if (legodokuAt) candidates.push(new Date(legodokuAt).getTime());
  ...
  onlineProfiles(state, viewerPetId, onlineIds = this.onlineUserIds(state)) {
    return [...onlineIds].map((userId) => state.users[userId]).filter((member) => member && !member.accountLocked)
  ...
  bootstrapFromState(state, userId) {
    ...
    const excludedPublicPetIds = lockedPublicPetIds(state);
    const rankingState = publicRankingState(state, excludedPublicPetIds);
    return {
      ...
      residents: Object.values(state.users ?? {}).filter((member) => !member?.accountLocked).map((member) => publicProfile(state, member.currentPetId, pet.id, onlineIds.has(member.id))).filter(Boolean).sort((a, b) => a.displayName.localeCompare(b.displayName, 'ko')),
      ...
      rankings: { ...rankingsView(rankingState, pet.id), blockBattle: blockBattleRankings(rankingState, pet.id), sichuan: sichuanRanking(rankingState, pet.id), legodoku: legodokuRanking(rankingState, pet.id) },
      ...
      legodoku: (() => { const view = legodokuView(state, pet.id); for (const room of view.rooms) room.reactions = this.liveReactionView('legodoku', room.id); return view; })(),
      territory: territoryView(state, pet.id, { excludedPetIds: excludedPublicPetIds }),
      ...
      catalog: {
        ...
        legodoku: { maxRooms: LEGODOKU_MAX_ROOMS, matchSeconds: LEGODOKU_MATCH_SECONDS, stakes: [...LEGODOKU_STAKES], size: LEGODOKU_SIZE, maxMistakes: LEGODOKU_MAX_MISTAKES },
  ...
  sendLegodokuState(state, roomId) {
    const targetRoom = state.legodoku?.rooms?.[roomId];
    if (!targetRoom) return;
    const audiencePetIds = new Set([targetRoom.hostPetId, targetRoom.guestPetId, ...Object.keys(targetRoom.spectators ?? {})].filter(Boolean));
    const date = new Date();
    const serverTime = date.getTime();
    const reactions = this.liveReactionView('legodoku', roomId, date);
    ...
    for (const socket of this.ctx.getWebSockets()) {
      try {
        ...
        if (!payload) {
          const room = legodokuRoomView(state, roomId, pet.id, date);
          if (!room) continue;
          room.reactions = reactions;
          payload = JSON.stringify({ type: 'legodoku-state', room, serverTime });
  ...
  async handleFetch(request) {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/auth/register') {
        ...
        addPublicEvent(state, `${withJosa(pet.displayName, '이/가')} 레고방에 처음 들어왔습니다.`, 'join', [pet.id], date);
        await this.store.save(state);
        await this.scheduleNextAlarm(state);
      ...
      if (request.method === 'POST' && pathname === '/api/minigames/apple/select') {
        const body = await readJson(request);
        const result = await this.mutateForUserLight(state, auth.userId, (gameState, _user, pet, now) => selectAppleGame(gameState, pet, body.challengeId, {
        ...
        // 진행 중 매 드래그마다 전체 bootstrap + 자기 자신 WebSocket refresh를 중복 수행하지 않는다.
        // 종료 순간에만 포인트/랭킹/레고 상태를 한 번에 확정해서 내려준다.
        if (result.ok && result.finished) result.bootstrap = this.bootstrapFromState(state, auth.userId);
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      ...
      if (request.method === 'POST' && pathname === '/api/minigames/block/select') {
        const body = await readJson(request);
        const result = await this.mutateForUserLight(state, auth.userId, (gameState, _user, pet, now) => selectBlockGame(gameState, pet, body.challengeId, {
        ...
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          if (route === 'join') {
            ...
            if (joined.ok && !hadGuest && hostPetId && hostPetId !== pet.id) {
              ...
              if (host?.alive) addNotification(gameState, host.userId, `⚫ ${withJosa(pet.displayName, '이/가')} 오목 대전에 들어왔습니다.`, 'omok-opponent', { roomId, petId: pet.id }, now);
            }
            return joined;
          }
      ...
      if (request.method === 'POST' && pathname === '/api/legodoku/rooms') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => createLegodokuRoom(gameState, pet, body.stakePoints, now), date);
        this.broadcastRefresh('legodoku-lobby');
        return jsonResponse(result, result.ok ? 201 : 400);
      }
      const legodokuJoin = matchPath(pathname, '/api/legodoku/rooms/:roomId/join');
      const legodokuSpectate = matchPath(pathname, '/api/legodoku/rooms/:roomId/spectate');
      const legodokuSpectateLeave = matchPath(pathname, '/api/legodoku/rooms/:roomId/spectate/leave');
      const legodokuLeave = matchPath(pathname, '/api/legodoku/rooms/:roomId/leave');
      const legodokuCell = matchPath(pathname, '/api/legodoku/rooms/:roomId/cell');
      const legodokuRematch = matchPath(pathname, '/api/legodoku/rooms/:roomId/rematch');
      if (request.method === 'POST' && legodokuCell) {
        const body = await readJson(request);
        const user = state.users[auth.userId];
        const pet = user ? currentPetForUser(state, auth.userId) : null;
        if (!user || !pet?.alive) return errorResponse('사용자를 찾을 수 없습니다.', 401);
        user.lastSeenAt = date.toISOString();
        const result = playLegodokuCell(state, pet, legodokuCell.roomId, body, date);
        const targetRoom = state.legodoku?.rooms?.[legodokuCell.roomId];
        const timedSettlement = Boolean(result.terminal && targetRoom?.status === 'ended' && targetRoom?.settled);
        const requiresFullSave = Boolean(result.finished || timedSettlement);
        const shouldPersist = Boolean((result.ok && !result.duplicate && !result.duplicateCell) || timedSettlement);
        if (shouldPersist) {
          this.sendLegodokuState(state, legodokuCell.roomId);
          if (requiresFullSave) {
            await this.store.save(state);
            await this.scheduleNextAlarm(state);
            this.broadcastRefresh('legodoku-lobby');
          } else await this.store.saveLegodoku(state);
        }
        const room = legodokuRoomView(state, legodokuCell.roomId, pet.id, date);
        if (room) room.reactions = this.liveReactionView('legodoku', room.id, date);
        return jsonResponse({ ...result, room, serverTime: date.getTime() }, result.ok || result.terminal || result.stale ? 200 : 400);
      }
      if (request.method === 'POST' && (legodokuJoin || legodokuSpectate || legodokuSpectateLeave || legodokuLeave || legodokuRematch)) {
        const matched = legodokuJoin || legodokuSpectate || legodokuSpectateLeave || legodokuLeave || legodokuRematch;
        const roomId = matched.roomId;
        const route = legodokuJoin ? 'join' : legodokuSpectate ? 'spectate' : legodokuSpectateLeave ? 'spectate-leave' : legodokuLeave ? 'leave' : 'rematch';
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          if (route === 'join') return joinLegodokuRoom(gameState, pet, roomId, now);
          if (route === 'spectate') return spectateLegodokuRoom(gameState, pet, roomId, now);
          if (route === 'spectate-leave') return leaveLegodokuSpectator(gameState, pet, roomId, now);
          if (route === 'leave') return leaveLegodokuRoom(gameState, pet, roomId, now);
          return requestLegodokuRematch(gameState, pet, roomId, now);
        }, date);
        this.sendLegodokuState(state, roomId);
        this.broadcastRefresh('legodoku-lobby');
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      ...
      const legodokuReaction = matchPath(pathname, '/api/legodoku/rooms/:roomId/reaction');
      if (request.method === 'POST' && legodokuReaction) {
        const body = await readJson(request);
        const pet = currentPetForUser(state, auth.userId);
        const room = state.legodoku?.rooms?.[legodokuReaction.roomId];
        const spectator = pet ? room?.spectators?.[pet.id] : null;
        const isPlayer = Boolean(pet && [room?.hostPetId, room?.guestPetId].includes(pet.id));
        if (!pet?.alive || room?.status !== 'playing' || (!isPlayer && !spectator)) return errorResponse('레고도쿠 참가자 또는 관전자만 공감을 보낼 수 있습니다.', 403);
        const result = this.addSpectatorReaction('legodoku', room.id, pet, body.type, date);
        if (!result.ok) return errorResponse(result.message, result.status || 400);
        this.sendLegodokuState(state, room.id);
        return jsonResponse({ ok: true, reaction: result.reaction });
      }
      ...
      if (request.method === 'POST' && pathname === '/api/territory/claim') {
        ...
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          ...
          if (claimed.ok && claimed.stolenFromPetId) {
            ...
            if (victim?.alive && victim.id !== pet.id) addNotification(gameState, victim.userId, `🗺️ ${withJosa(pet.displayName, '이/가')} 내 영토 (${Number(claimed.row) + 1}, ${Number(claimed.col) + 1})를 탈취했습니다.`, 'territory-stolen', { petId: pet.id, row: claimed.row, col: claimed.col }, now);
          }
          if (requestId && /^[A-Za-z0-9._:-]{8,120}$/.test(requestId)) {
      ...
      if (request.method === 'POST' && pathname === '/api/admin/kick') {
        ...
        removePetFromLegodoku(state, target.id, date);
        ...
        await this.store.save(state);
        this.broadcastRefresh('admin');
        return jsonResponse({ ok: true, message: `${withJosa(target.displayName, '을/를')} 강퇴 처리했습니다.`, result, bootstrap: this.bootstrapFromState(state, auth.userId) });
      }
      ...
      if (request.method === 'POST' && pathname === '/api/admin/legodoku/clear-ended') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const outcome = clearEndedLegodokuRooms(state, date);
        this.appendAdminAudit(state, user, { action: 'legodoku_clear_ended', targetDisplayName: '종료된 레고도쿠 대전방', detail: `${outcome.cleared}개 정리` }, date);
        await this.store.save(state);
        this.broadcastRefresh('legodoku-lobby');
        return jsonResponse({ ...outcome, bootstrap: this.bootstrapFromState(state, auth.userId) });
      }
  ...
  async handleAlarm() {
    ...
    if (result.legodokuChanged) {
      for (const room of Object.values(state.legodoku?.rooms ?? {})) if (room?.status === 'playing' || room?.status === 'ended') this.sendLegodokuState(state, room.id);
    }
