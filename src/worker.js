  ...
  async processTimeState(state, date = new Date()) {
    ...
    if (minesweeperSeason.changed) {
      ...
      for (const champion of minesweeperSeason.champions ?? []) {
        ...
        addNotification(state, champion.userId, `지뢰찾기 ${champion.label} 1위! 다음 게임 하루 동안 ${champion.badgeLabel} 칭호를 획득했습니다.`, 'game-season', { game: 'minesweeper', difficulty: champion.difficulty, rank: 1 }, date);
      }
    }
