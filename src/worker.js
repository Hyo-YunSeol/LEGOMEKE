...
export class LegoGameRoom {
  ...
  async processTimeState(state, date = new Date()) {
    ...
    for (const userId of Object.keys(state.users ?? {})) {
      ...
      if (beforeEffects !== JSON.stringify(pet.effects ?? null) || nickname.changed || reset.changed || hunger.changed || news) changed = true;
    }
  ...
  bootstrapFromState(state, userId) {
    ...
    return {
      ...
      catalog: {
        foods: Object.values(FOODS), shopItems: Object.values(SHOP_ITEMS), miniGames: Object.values(MINI_GAMES), fishingRewards: FISHING_REWARDS.map(({ id: rewardId, label, reward }) => ({ id: rewardId, label, reward })),
