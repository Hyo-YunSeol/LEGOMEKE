...
export class LegoGameRoom {
      ...
      members: isAdmin ? Object.values(state.users).map((member) => {
        ...
        if (!pet) {
          return {
            ...
            accountLockedAt: member.accountLockedAt ?? null,
  ...
  disconnectUserSockets(userId, code = 4001, reason = 'account session ended') {
    for (const socket of this.ctx.getWebSockets()) {
      try {
        if (socket.deserializeAttachment()?.userId !== userId) continue;
        socket.close(code, String(reason || 'account session ended').slice(0, 120));
      } catch { /* ignore */ }
    }
  }
  ...
  async processTimeState(state, date = new Date()) {
    ...
    if (expiredMini.changed || gameSeason.changed || territory.changed || omok.changed) changed = true;
  ...
  async handleFetch(request) {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/auth/register') {
        ...
        const user = { id: userId, nickname, pinSalt: pinData.salt, pinHash: pinData.hash, generation: 1, currentPetId: null, sessionVersion: 1, role: 'user', workoutBadge: false, accountLocked: false, accountLockedAt: null, notifications: [], createdAt: date.toISOString(), lastSeenAt: date.toISOString() };
      ...
      if (request.method === 'POST' && pathname === '/api/auth/login') {
        ...
        if (user.accountLocked) return errorResponse('운영자에 의해 잠긴 계정입니다. 운영자에게 잠금 해제를 요청해주세요.', 423);
        user.lastSeenAt = date.toISOString();
      ...
      if (request.method === 'POST' && blockInput) {
        ...
        const timed = processBlockBattleTimers(state, date, { gravity: false });
      ...
      const adminAccountLock = matchPath(pathname, '/api/admin/users/:userId/lock');
      if (request.method === 'POST' && adminAccountLock) {
        const adminUser = state.users[auth.userId];
        if (!this.isAdminUser(adminUser)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const targetUserId = String(adminAccountLock.userId ?? '').trim();
        if (!targetUserId) return errorResponse('잠금 상태를 변경할 회원을 지정해주세요.', 400);
        if (targetUserId === auth.userId) return errorResponse('운영자 본인 계정은 잠글 수 없습니다.', 400);
        const targetUser = state.users[targetUserId];
        if (!targetUser) return errorResponse('대상 회원 계정을 찾을 수 없습니다.', 404);
        const body = await readJson(request);
        const locked = Boolean(body.locked);
        const targetPet = currentPetForUser(state, targetUserId);
        if (Boolean(targetUser.accountLocked) === locked) {
          return jsonResponse({
            ok: true, unchanged: true,
            message: locked ? '이미 잠긴 계정입니다.' : '이미 잠금 해제된 계정입니다.',
            bootstrap: this.bootstrapFromState(state, auth.userId)
          });
        }

        targetUser.accountLocked = locked;
        targetUser.accountLockedAt = locked ? date.toISOString() : null;
        // 잠금/해제 어느 쪽이든 기존 토큰을 폐기한다. 잠금 해제 후에는 새로 로그인해야 하므로
        // 잠겨 있던 기기의 오래된 세션이 자동으로 되살아나지 않는다.
        targetUser.sessionVersion = Math.max(1, Math.floor(Number(targetUser.sessionVersion) || 1)) + 1;
        let realtimeChanged = false;
        if (locked && targetPet?.alive) {
          // 두 실시간 게임 모두 반드시 연결 해제 처리를 실행한다. `||=`로 합치면 첫 게임이
          // true일 때 두 번째 호출이 단락되어 다른 게임의 재접속 보호가 빠질 수 있다.
          const omokConnection = omokSetConnected(state, targetPet.id, false, date);
          const blockBattleConnection = blockBattleSetConnected(state, targetPet.id, false, date);
          realtimeChanged = Boolean(omokConnection.changed || blockBattleConnection.changed);
        }
        this.appendAdminAudit(state, adminUser, {
          action: locked ? 'account_lock' : 'account_unlock',
          targetUserId, targetPetId: targetPet?.id ?? null,
          targetDisplayName: targetPet?.displayName || targetUser.nickname,
          detail: locked ? '계정 로그인 잠금' : '계정 잠금 해제'
        }, date);
        await this.store.save(state);
        if (realtimeChanged) await this.scheduleNextAlarm(state);
        if (locked) this.disconnectUserSockets(targetUserId, 4003, 'account locked');
        else this.disconnectUserSockets(targetUserId, 4004, 'account unlocked; login again');
        this.broadcastRefresh(locked ? 'admin-account-lock' : 'admin-account-unlock');
        return jsonResponse({
          ok: true, locked,
          message: locked
            ? `${targetPet?.displayName || targetUser.nickname} 계정을 잠갔습니다. 기존 로그인도 종료됩니다.`
            : `${targetPet?.displayName || targetUser.nickname} 계정 잠금을 해제했습니다. 새로 로그인하면 이용할 수 있습니다.`,
          bootstrap: this.bootstrapFromState(state, auth.userId)
        });
      }
      ...
      if (request.method === 'DELETE' && adminDeleteUser) {
        ...
        await this.store.save(state);
        await this.scheduleNextAlarm(state);
        this.disconnectUserSockets(targetUserId, 4001, 'account deleted');
  ...
  async handleWebSocketMessage(socket, message) {
    try {
      ...
      const pet = socketUser && !socketUser.accountLocked ? currentPetForUser(state, userId) : null;
      if (!pet?.alive) {
        socket.send(JSON.stringify({ type: 'block-battle-error', message: socketUser?.accountLocked ? '운영자에 의해 잠긴 계정입니다.' : '사용자를 찾을 수 없습니다.' }));
        if (socketUser?.accountLocked) { try { socket.close(4003, 'account locked'); } catch { /* ignore */ } }
        return;
      }
      const date = new Date();
