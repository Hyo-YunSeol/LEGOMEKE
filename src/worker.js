...
export class LegoGameRoom {
  ...
  addSpectatorReaction(scope, roomId, pet, typeValue, date = new Date()) {
    ...
    if (date.getTime() - lastAt < 2_000) return { ok: false, status: 429, message: '리액션은 2초에 한 번 보낼 수 있습니다.' };
    this.reactionRateLimits.set(key, date.getTime());
    const item = { id: id('reaction'), petId: pet.id, displayName: pet.displayName, type, ...reaction, createdAt: date.toISOString(), expiresAt: date.getTime() + 4_000 };
  ...
  async processTimeState(state, date = new Date()) {
    ...
    for (const userId of Object.keys(state.users ?? {})) {
      ...
      if (reset.changed || hunger.changed || news) changed = true;
    }
