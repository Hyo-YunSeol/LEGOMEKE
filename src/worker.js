...
export class LegoGameRoom {
  constructor(ctx, env) {
    ...
    this.liveReactions = { omok: new Map(), blockBattle: new Map(), davinci: new Map() };
  ...
  cleanLiveReactions(date = new Date()) {
    ...
    for (const scope of ['omok', 'blockBattle', 'davinci']) {
  ...
  async processTimeState(state, date = new Date()) {
    ...
    if (gameSeason.changed) {
      const labels = { apple: '🍎 사과게임', omok: '⚫ 오목', blockBattle: '🧱 테트리스대전', davinci: '🧩 다빈치코드' };
    ...
    if (expiredMini.changed || bungs.changed || gameSeason.changed || territory.changed || omok.changed || davinci.changed) changed = true;
  ...
  bootstrapFromState(state, userId) {
    ...
    return {
      ...
      rankings: { ...rankingsView(state, pet.id), blockBattle: blockBattleRankings(state, pet.id), davinci: davinciRanking(state, pet.id) },
