...
import {
  ...
  createBung, joinBung, leaveBung, startBung, finishBung, forceCancelBung,
  ...
  purchaseShopItem, syncTemporaryNickname
} from './game/engine.js';
import {
  ...
  liarUpdateSettings, liarVote, resetLiarParticipantForAdmin,
  LIAR_BET_OPTIONS, LIAR_DISCUSSION_OPTIONS, LIAR_PLAYER_OPTIONS
...
import {
  blockBattleNextAlarmAt, blockBattleRankings, blockBattleSetConnected, blockBattleView,
...
export class LegoGameRoom {
  constructor(ctx, env) {
    ...
    this.liveReactions = { liar: [], omok: new Map(), blockBattle: new Map() };
  ...
  cleanLiveReactions(date = new Date()) {
    ...
    this.liveReactions.liar = this.liveReactions.liar.filter((item) => item.expiresAt > now);
  ...
  liveReactionView(scope, roomId = null, date = new Date()) {
    ...
    const items = scope === 'liar' ? this.liveReactions.liar : (this.liveReactions[scope]?.get(roomId) ?? []);
  ...
  addSpectatorReaction(scope, roomId, pet, typeValue, date = new Date()) {
    ...
    if (scope === 'liar') this.liveReactions.liar = [...this.liveReactions.liar, item].slice(-12);
    else this.liveReactions[scope].set(roomId, [...(this.liveReactions[scope].get(roomId) ?? []), item].slice(-12));
      ...
      members: isAdmin ? Object.values(state.users).map((member) => {
        ...
        if (!pet) {
          return {
            ...
            hasActivePet: false,
            workoutBadge: Boolean(member.workoutBadge),
  ...
  async processTimeState(state, date = new Date()) {
    ...
    for (const userId of Object.keys(state.users ?? {})) {
      ...
      const nickname = syncTemporaryNickname(state, state.users[userId], date);
      ...
      if (beforeEffects !== JSON.stringify(pet.effects ?? null) || beforeFlexItem !== JSON.stringify(pet.flexItem ?? null) || nickname.changed || reset.changed || hunger.changed || news) changed = true;
    }
    ...
    if (expiredMini.changed || gameSeason.changed || territory.changed || liar.changed || omok.changed || liarNews) changed = true;
  ...
  async scheduleNextAlarm(state) {
    ...
    for (const user of Object.values(state.users ?? {})) if (user?.temporaryNickname?.expiresAt) candidates.push(new Date(user.temporaryNickname.expiresAt).getTime());
  ...
  bootstrapFromState(state, userId) {
    ...
    return {
      ...
      residents: Object.values(state.users ?? {}).map((member) => publicProfile(state, member.currentPetId, pet.id, this.isOnline(state, member.id))).filter(Boolean).sort((a, b) => a.displayName.localeCompare(b.displayName, 'ko')),
      ...
      notifications: (user.notifications ?? []).filter((item) => !['break','break-warning'].includes(item?.type) && !/(?:파손|부서)/.test(String(item?.text ?? ''))).slice(0, 40),
      ...
      liarGame: (() => { const view = liarGameView(state, pet.id); view.reactions = this.liveReactionView('liar'); return view; })(),
      ...
      catalog: {
        ...
        blockBattle: { maxRooms: 3, stakes: [100, 500, 1000], width: 10, height: 20 },
  ...
  sendBlockBattleState(state, roomId) {
    for (const socket of this.ctx.getWebSockets()) {
      try {
        ...
        const pet = user ? state.pets?.[user.currentPetId] : null;
        if (!pet?.alive) continue;
        const view = blockBattleView(state, pet.id);
        const room = view.rooms.find((item) => item.id === roomId);
        if (!room || room.viewerRole === 'none') continue;
        room.reactions = this.liveReactionView('blockBattle', room.id);
        socket.send(JSON.stringify({ type: 'block-battle-state', room, serverTime: Date.now() }));
      } catch { /* ignore disconnected socket */ }
    }
  }

  liarAudienceUserIds(state) {
    const ids = new Set();
    for (const participant of [
      ...Object.values(state.liarGame?.players ?? {}),
      ...Object.values(state.liarGame?.spectators ?? {})
    ]) if (participant?.userId) ids.add(participant.userId);
    for (const user of Object.values(state.users ?? {})) if (this.isAdminUser(user)) ids.add(user.id);
    return ids;
  }

  broadcastLiarChatEvent(state, payload) {
    const audience = this.liarAudienceUserIds(state);
    const message = JSON.stringify({ type: 'liar-chat-event', ...payload, at: Date.now() });
    for (const socket of this.ctx.getWebSockets()) {
      try {
        if (audience.has(socket.deserializeAttachment()?.userId)) socket.send(message);
      } catch { /* ignore */ }
    }
  }
  ...
  async handleFetch(request) {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/auth/register') {
        ...
        const duplicateNickname = Object.values(state.users).some((user) => {
          if (String(user.nickname).toLocaleLowerCase('ko') === normalizedNickname) return true;
          const expiresAt = new Date(user.temporaryNickname?.expiresAt ?? '').getTime();
          return Number.isFinite(expiresAt) && expiresAt > date.getTime()
            && String(user.temporaryNickname?.nickname ?? '').toLocaleLowerCase('ko') === normalizedNickname;
        });
      ...
      if (request.method === 'POST' && pathname === '/api/liar/chat') {
        const body = await readJson(request);
        const user = state.users[auth.userId];
        const pet = currentPetForUser(state, auth.userId);
        if (!user || !pet?.alive) return errorResponse('사용자를 찾을 수 없습니다.', 401);
        const result = liarAddChat(state, pet, body.text, body.requestId, date);
        if (!result.ok) return jsonResponse(result, 400);
        if (!result.duplicate) {
          await this.store.saveLiarChat(state);
          this.broadcastLiarChatEvent(state, { action: 'append', chat: result.chat, revision: state.meta.revision });
        }
        return jsonResponse({ ...result, revision: state.meta.revision });
      }
      ...
      if (request.method === 'POST' && (blockJoin || blockSpectate || blockSpectateLeave || blockLeave || blockInput || blockRematch)) {
        const body = await readJson(request);
        const matched = blockJoin || blockSpectate || blockSpectateLeave || blockLeave || blockInput || blockRematch;
        const roomId = matched.roomId;
        const route = blockJoin ? 'join' : blockSpectate ? 'spectate' : blockSpectateLeave ? 'spectate-leave' : blockLeave ? 'leave' : blockInput ? 'input' : 'rematch';
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          ...
          if (route === 'input') {
            blockBattleSetConnected(gameState, pet.id, true, now);
            return playBlockBattleActions(gameState, pet, roomId, body, now);
          }
          return requestBlockBattleRematch(gameState, pet, roomId, now);
        }, date);
        this.sendBlockBattleState(state, roomId);
        if (route !== 'input' || result.finished) this.broadcastRefresh('block-battle-lobby');
      ...
      if (request.method === 'POST' && pathname === '/api/liar/reaction') {
        const body = await readJson(request);
        const pet = currentPetForUser(state, auth.userId);
        const spectator = pet ? state.liarGame?.spectators?.[pet.id] : null;
        if (!pet?.alive || !spectator?.connected || state.liarGame?.players?.[pet.id]) return errorResponse('라이어게임 관전자만 리액션을 보낼 수 있습니다.', 403);
        const result = this.addSpectatorReaction('liar', null, pet, body.type, date);
      ...
      if (request.method === 'POST' && blockBattleReaction) {
        ...
        const spectator = pet ? room?.spectators?.[pet.id] : null;
        if (!pet?.alive || room?.status !== 'playing' || !spectator?.connected || [room?.hostPetId, room?.guestPetId].includes(pet.id)) return errorResponse('블럭대전 관전자만 리액션을 보낼 수 있습니다.', 403);
      ...
      if (request.method === 'POST' && pathname === '/api/territory/claim') {
        ...
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          ...
          const claimed = claimTerritory(gameState, pet, body.row, body.col, now);
      ...
      if (request.method === 'DELETE' && adminLiarChat) {
        ...
        await this.store.saveLiarChat(state);
        this.broadcastLiarChatEvent(state, { action: 'delete', chatId: adminLiarChat.chatId, revision: state.meta.revision });
        return jsonResponse({ ...outcome, chatId: adminLiarChat.chatId, revision: state.meta.revision });
      }
      ...
      if (request.method === 'POST' && pathname === '/api/admin/reset-user') {
        ...
        const liarCleanup = resetLiarParticipantForAdmin(state, target.id, date);
        removePetFromOmok(state, target.id, date);
        removePetFromBlockBattle(state, target.id, date);
        this.appendAdminAudit(state, user, {
          action: 'state_reset', targetUserId: target.userId, targetPetId: target.id, targetDisplayName: target.displayName,
          detail: liarCleanup.forceEnded ? '라이어게임 안전 종료·판돈 환불 포함' : '비정상 진행 상태 초기화'
        }, date);
        addNotification(state, target.userId, '운영자가 비정상 게임 상태를 초기화했습니다.', 'info');
        await this.store.save(state);
        this.broadcastRefresh('admin');
        return jsonResponse({ ok: true, message: liarCleanup.forceEnded ? '대상의 상태를 초기화하고 라이어게임 판돈을 안전하게 환불했습니다.' : '대상의 비정상 진행 상태를 초기화했습니다.', bootstrap: this.bootstrapFromState(state, auth.userId) });
      }
      ...
      if (request.method === 'POST' && pathname === '/api/admin/liar/force-end') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const outcome = forceEndLiarGame(state, date);
        await this.store.save(state);
        await this.scheduleNextAlarm(state);
        this.broadcastRefresh('admin');
        return jsonResponse({ ...outcome, bootstrap: this.bootstrapFromState(state, auth.userId) });
  ...
  async handleWebSocketMessage(socket, message) {
    try {
      ...
      const date = new Date();
      const timed = await this.processTimeState(state, date);
      const result = playBlockBattleActions(state, pet, String(data.roomId || ''), data, date);
      if (timed.changed || (result.ok && !result.duplicate)) {
        const requiresFullSave = timed.nonBattleChanged || timed.blockBattleSettled || result.finished;
        if (requiresFullSave) await this.store.save(state);
        else await this.store.saveBlockBattle(state);
        await this.scheduleNextAlarm(state);
      }
      if (!result.ok) socket.send(JSON.stringify({ type: 'block-battle-error', roomId: data.roomId, ...result }));
  ...
  async markSocketDisconnected(socket) {
    ...
    if (liarChanged.changed || omokChanged.changed || blockBattleChanged.changed) {
