...
const APP_VERSION = '6.9.3-minigame-abandon-final';
...
export class LegoGameRoom {
  ...
  async processTimeState(state, date = new Date()) {
    ...
    if (gameSeason.changed) {
      const labels = { apple: '🍎 사과게임', omok: '⚫ 오목', blockBattle: '🧱 테트리스대전', sichuan: '🀄 사천성대전', davinci: '🧩 다빈치코드' };
  ...
  async scheduleNextAlarm(state) {
    ...
    for (const challenge of Object.values(state.miniGameChallenges ?? {})) {
      if (challenge && !['block', 'minesweeper'].includes(challenge.gameId) && !challenge.completed && challenge.expiresAt) candidates.push(new Date(challenge.expiresAt).getTime());
    }
  ...
  bootstrapFromState(state, userId) {
    ...
    return {
      ...
      activeMiniChallenge: (() => {
        const challenge = Object.values(state.miniGameChallenges ?? {}).find((item) => item.petId === pet.id && !item.completed && (['block', 'minesweeper'].includes(item.gameId) || new Date(item.expiresAt).getTime() > Date.now()));
