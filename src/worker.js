...
import {
  ...
  MINI_GAMES_PER_DAY, FISHING_PER_DAY, GAME_DAY_HOURS, GAME_DAY_RESET_HOURS_KST, BODY_STAGES, BODY_ADVANCEMENTS, BODY_ADVANCEMENT_BODY, STATUS_MESSAGE_MAX_LENGTH, STATUS_MESSAGE_ADMIN_MAX_LENGTH,
...
import {
  ...
  workAction, restAction, cookAction, eatAction, updateStatusMessage,
  ...
  playMinesweeperGame, abandonMinesweeperGame, miniChallengeView,
  startFishing, claimFishing, publishFishingNews,
  ensurePetSchema, endLifeAndRestart, nextHungerPenaltyAt,
  purchaseShopItem, loudspeakerView, selectBodyAdvancement
} from './game/engine.js';
import {
...
import {
  ...
  playOmokMove, processOmokTimers, removePetFromOmok, requestOmokRematch, spectateOmokRoom, submitOmokRps, selectOmokColor, OMOK_TURN_SECONDS
} from './game/omok.js';
import {
...
import {
  DAVINCI_MAX_ROOMS, DAVINCI_MIN_PLAYERS, DAVINCI_MAX_PLAYERS, DAVINCI_FIRST_GUESS_SECONDS, DAVINCI_CHAIN_GUESS_SECONDS,
  davinciNextAlarmAt, davinciRanking, davinciSetConnected, davinciView, clearEndedDavinciRooms,
  createDavinciRoom, joinDavinciRoom, setDavinciReady, startDavinciRoom, placeDavinciJoker, submitDavinciRps, selectDavinciOrder,
  guessDavinciTile, decideDavinciTurn, chooseDavinciPenaltyTile, spectateDavinciRoom, leaveDavinciSpectator, leaveDavinciRoom,
  requestDavinciRematch, processDavinciTimers, removePetFromDavinci
} from './game/davinci.js';
import {
  SICHUAN_MATCH_SECONDS, SICHUAN_MAX_ROOMS, SICHUAN_STAKES, clearEndedSichuanRooms, createSichuanRoom, joinSichuanRoom,
  leaveSichuanRoom, leaveSichuanSpectator, playSichuanPair, processSichuanTimers, removePetFromSichuan, requestSichuanRematch,
  sichuanNextAlarmAt, sichuanRanking, sichuanRoomView, sichuanView, spectateSichuanRoom
} from './game/sichuan.js';
import {
  SPOT_DIFFERENCE_MATCH_SECONDS, SPOT_DIFFERENCE_MAX_ROOMS, SPOT_DIFFERENCE_STAKES,
  clearEndedSpotDifferenceRooms, createSpotDifferenceRoom, joinSpotDifferenceRoom,
  leaveSpotDifferenceRoom, leaveSpotDifferenceSpectator, playSpotDifferenceClick, processSpotDifferenceTimers,
  removePetFromSpotDifference, requestSpotDifferenceRematch, spotDifferenceNextAlarmAt,
  spotDifferenceRanking, spotDifferenceRoomView, spotDifferenceView, spectateSpotDifferenceRoom
} from './game/spot-difference.js';

import { processGameRankingSeason } from './game/ranking-season.js';
import { MINESWEEPER_DIFFICULTIES, processMinesweeperSeason } from './game/minesweeper.js';
import { gameDayKey, nextGameDayAt, nextGameRankingSeasonAt } from './lib/time.js';

const APP_VERSION = '6.10.21-advancement-shop-spotdiff-semantic-final';
const BATTLE_USAGE_FORCE_RESET_ID = '2026-08-27-battle-usage-reset-v1';
...
function supportSettingsView(state) {
  const raw = state?.supportSettings && typeof state.supportSettings === 'object' ? state.supportSettings : {};
...
export class LegoGameRoom {
  constructor(ctx, env) {
    ...
    this.liveReactions = { omok: new Map(), blockBattle: new Map(), davinci: new Map(), sichuan: new Map(), spotDifference: new Map() };
    ...
    this.ctx.blockConcurrencyWhile(async () => {
      ...
      let davinciConnectionChanged = false;
      for (const room of Object.values(state.davinci?.rooms ?? {})) {
        for (const player of room?.players ?? []) {
          if (player?.connected && !socketUsers.has(player.userId)) {
            const disconnected = davinciSetConnected(state, player.petId, false);
            davinciConnectionChanged ||= disconnected.changed;
          }
        }
        for (const spectator of Object.values(room?.spectators ?? {})) {
          if (spectator?.connected && !socketUsers.has(spectator.userId)) {
            const disconnected = davinciSetConnected(state, spectator.petId, false);
            davinciConnectionChanged ||= disconnected.changed;
          }
        }
      }
      if (battleConnectionChanged || davinciConnectionChanged) {
        if (battleConnectionChanged && !davinciConnectionChanged) await this.store.saveBlockBattle(state);
        else await this.store.save(state);
  ...
  cleanLiveReactions(date = new Date()) {
    ...
    for (const scope of ['omok', 'blockBattle', 'davinci', 'sichuan', 'spotDifference']) {
  ...
  appendAdminAudit(state, user, entry, date = new Date()) {
    ...
    state.adminAuditLogs = state.adminAuditLogs.slice(0, 100);
  }
  ...
  adminView(state, user, onlineIds = this.onlineUserIds(state)) {
    ...
    return {
      ...
      members: isAdmin ? Object.values(state.users).map((member) => {
        ...
        if (!pet) {
          return {
            ...
            workoutBadge: Boolean(member.workoutBadge), accountLocked: Boolean(member.accountLocked), createdAt: member.createdAt ?? null
          };
        }
        ensurePetSchema(pet);
      ...
      endedDavinciRooms: isAdmin ? Object.values(state.davinci?.rooms ?? {}).filter((room) => room?.status === 'ended' && room?.settled).length : 0,
      endedSichuanRooms: isAdmin ? Object.values(state.sichuan?.rooms ?? {}).filter((room) => room?.status === 'ended' && room?.settled).length : 0,
      endedSpotDifferenceRooms: isAdmin ? Object.values(state.spotDifference?.rooms ?? {}).filter((room) => room?.status === 'ended' && room?.settled).length : 0,
  ...
  disconnectUserSockets(userId, reason = 'account unavailable') {
  ...
  deleteUserAccountState(state, targetUserId, date = new Date()) {
    ...
    for (const pet of targetPets) {
      ...
      removePetFromDavinci(state, pet.id, date);
      removePetFromSichuan(state, pet.id, date);
      removePetFromSpotDifference(state, pet.id, date);
  ...
  async processTimeState(state, date = new Date()) {
    ...
    // v6.10.12 일회성 복구: 이전 배포에서 6시간 경계 정규화가 메모리에서만 일어나고
    // Durable Object에 저장되지 않은 이용자가 있을 수 있어 이번 배포에서 전체 현재 레고의
    // 1:1 대전 이용량만 정확히 한 번 0으로 밀어준다. 포인트/랭킹/게임 기록은 건드리지 않는다.
    state.meta ??= {};
    if (state.meta.battleUsageForceResetId !== BATTLE_USAGE_FORCE_RESET_ID) {
      ...
      state.meta.battleUsageForceResetId = BATTLE_USAGE_FORCE_RESET_ID;
      state.meta.battleUsageForceResetAt = date.toISOString();
      changed = true;
    }

    const minesweeperSeason = processMinesweeperSeason(state, date);
    if (minesweeperSeason.changed) {
      changed = true;
      for (const champion of minesweeperSeason.champions ?? []) {
        const seconds = (Number(champion.ms || 0) / 1000).toFixed(1);
        addPublicEvent(state, `${champion.badgeLabel} · ${champion.displayName}이 지뢰찾기 ${champion.label} ${seconds}초로 1위를 차지했습니다.`, 'game-season', [champion.petId], date);
        addNotification(state, champion.userId, `지뢰찾기 ${champion.label} 1위! 다음 3일 시즌 동안 ${champion.badgeLabel} 칭호를 획득했습니다.`, 'game-season', { game: 'minesweeper', difficulty: champion.difficulty, rank: 1 }, date);
      }
    }
    for (const userId of Object.keys(state.users ?? {})) {
      ...
      // normalizeBattleDaily()는 ensurePetSchema() 안에서도 실행된다. 예전에는 여기서
      // battlePlayed/battleBonus/battleDate만 바뀌면 processTimeState의 changed가 false로
      // 남아 저장이 생략될 수 있었다. 대전 일일 필드도 전후를 비교해 반드시 영구 저장한다.
      const beforeBattleDaily = JSON.stringify([
        pet.daily?.battleDate ?? null,
        Number(pet.daily?.battlePlayed || 0),
        Number(pet.daily?.battleBonus || 0)
      ]);
      ...
      const afterBattleDaily = JSON.stringify([
        pet.daily?.battleDate ?? null,
        Number(pet.daily?.battlePlayed || 0),
        Number(pet.daily?.battleBonus || 0)
      ]);
      if (beforeEffects !== JSON.stringify(pet.effects ?? null)
        || beforeFlexItem !== JSON.stringify(pet.flexItem ?? null)
        || beforeBattleDaily !== afterBattleDaily
        || reset.changed || hunger.changed || news) changed = true;
    }
    ...
    if (gameSeason.changed) {
      const labels = { apple: '🍎 사과게임', singleTetris: '🧱 싱글 테트리스', omok: '⚫ 오목', blockBattle: '🧱 테트리스대전', sichuan: '🀄 사천성대전', spotDifference: '🔎 틀린그림찾기', davinci: '🧩 다빈치코드' };
      for (const [key, awards] of Object.entries(gameSeason.awards ?? {})) {
        for (const award of awards ?? []) {
          const winner = state.pets?.[award.petId];
          if (!winner?.alive) continue;
          const champion = award.rank === 1 && ['apple', 'omok'].includes(key) ? ` · 다음 시즌 동안 ${key === 'apple' ? '🍎왕' : '⚫오목왕'} 뱃지 획득` : '';
    ...
    const davinci = processDavinciTimers(state, date);
    const sichuan = processSichuanTimers(state, date);
    const spotDifference = processSpotDifferenceTimers(state, date);
    if (expiredMini.changed || bungs.changed || gameSeason.changed || territory.changed || omok.changed || davinci.changed || sichuan.changed || spotDifference.changed) changed = true;
  ...
  async scheduleNextAlarm(state) {
    ...
    const davinciAt = davinciNextAlarmAt(state);
    const sichuanAt = sichuanNextAlarmAt(state);
    const spotDifferenceAt = spotDifferenceNextAlarmAt(state);
    const bungAt = bungNextAlarmAt(state);
    const rankingSeasonAt = nextGameRankingSeasonAt(new Date());
    const gameDayAt = nextGameDayAt(new Date());
    ...
    if (davinciAt) candidates.push(new Date(davinciAt).getTime());
    if (sichuanAt) candidates.push(new Date(sichuanAt).getTime());
    if (spotDifferenceAt) candidates.push(new Date(spotDifferenceAt).getTime());
    if (bungAt) candidates.push(new Date(bungAt).getTime());
    if (rankingSeasonAt) candidates.push(new Date(rankingSeasonAt).getTime());
    if (gameDayAt) candidates.push(new Date(gameDayAt).getTime());
    for (const challenge of Object.values(state.miniGameChallenges ?? {})) {
      if (!challenge || ['block', 'minesweeper'].includes(challenge.gameId) || challenge.completed || !challenge.expiresAt) continue;
      const expiresAt = new Date(challenge.expiresAt).getTime();
      if (Number.isFinite(expiresAt)) candidates.push(expiresAt + (challenge.gameId === 'tetrisSingle' ? 60_000 : 0));
    }
    for (const pet of Object.values(state.pets ?? {})) {
      if (!pet?.alive) continue;
      ensurePetSchema(pet);
  ...
  async authenticate(request, url, state) {
    ...
    if (!user || user.accountLocked || (user.sessionVersion ?? 1) !== payload.sv) return null;
  ...
  onlineUserIds(state) {
    ...
    for (const user of Object.values(state.users ?? {})) {
      ...
      if (user.lastSeenAt && new Date(user.lastSeenAt).getTime() >= activeAfter) ids.add(user.id);
    }
    return ids;
  }
  ...
  bootstrapFromState(state, userId) {
    ...
    return {
      ...
      activeMiniChallenge: (() => {
        const challenge = Object.values(state.miniGameChallenges ?? {}).find((item) => item.petId === pet.id && !item.completed && (['block', 'minesweeper', 'tetrisSingle'].includes(item.gameId) || new Date(item.expiresAt).getTime() > Date.now()));
        return challenge ? miniChallengeView(challenge) : null;
      })(),
      ...
      rankings: { ...rankingsView(state, pet.id), blockBattle: blockBattleRankings(state, pet.id), sichuan: sichuanRanking(state, pet.id), spotDifference: spotDifferenceRanking(state, pet.id), davinci: davinciRanking(state, pet.id) },
      ...
      davinci: (() => { const view = davinciView(state, pet.id); for (const room of view.rooms) room.reactions = this.liveReactionView('davinci', room.id); return view; })(),
      sichuan: (() => { const view = sichuanView(state, pet.id); for (const room of view.rooms) room.reactions = this.liveReactionView('sichuan', room.id); return view; })(),
      spotDifference: (() => { const view = spotDifferenceView(state, pet.id); for (const room of view.rooms) room.reactions = this.liveReactionView('spotDifference', room.id); return view; })(),
      territory: territoryView(state, pet.id),
      loudspeaker: loudspeakerView(state),
      support: supportSettingsView(state),
      ...
      catalog: {
        ...
        davinci: { maxRooms: DAVINCI_MAX_ROOMS, minPlayers: DAVINCI_MIN_PLAYERS, maxPlayers: DAVINCI_MAX_PLAYERS, firstGuessSeconds: DAVINCI_FIRST_GUESS_SECONDS, chainGuessSeconds: DAVINCI_CHAIN_GUESS_SECONDS, stakes: [100, 500, 1000, 2000, 3000, 4000, 5000] },
        sichuan: { maxRooms: SICHUAN_MAX_ROOMS, matchSeconds: SICHUAN_MATCH_SECONDS, stakes: [...SICHUAN_STAKES], rows: 8, cols: 10 },
        spotDifference: { maxRooms: SPOT_DIFFERENCE_MAX_ROOMS, matchSeconds: SPOT_DIFFERENCE_MATCH_SECONDS, differenceCount: 7, stakes: [...SPOT_DIFFERENCE_STAKES] },
        minesweeper: Object.fromEntries(Object.entries(MINESWEEPER_DIFFICULTIES).map(([key, value]) => [key, { ...value }])),
        ...
        bodyAdvancements: Object.values(BODY_ADVANCEMENTS).map((item) => ({ ...item })),
        bodyAdvancementBody: BODY_ADVANCEMENT_BODY,
  ...
  async mutateForUserLight(state, userId, mutator, date = new Date()) {
    const user = state.users[userId];
    const pet = user ? state.pets[user.currentPetId] : null;
    if (!user || !pet) return { ok: false, status: 401, message: '사용자를 찾을 수 없습니다.' };
    const current = currentPetForUser(state, userId);
  ...
  sendSichuanState(state, roomId) {
    const targetRoom = state.sichuan?.rooms?.[roomId];
    if (!targetRoom) return;
    const audiencePetIds = new Set([targetRoom.hostPetId, targetRoom.guestPetId, ...Object.keys(targetRoom.spectators ?? {})].filter(Boolean));
    const date = new Date();
    const serverTime = date.getTime();
    for (const socket of this.ctx.getWebSockets()) {
      try {
        const attachment = socket.deserializeAttachment();
        const user = attachment?.userId ? state.users?.[attachment.userId] : null;
        const pet = user ? state.pets?.[user.currentPetId] : null;
        if (!pet?.alive || !audiencePetIds.has(pet.id)) continue;
        const room = sichuanRoomView(state, roomId, pet.id, date);
        if (!room) continue;
        room.reactions = this.liveReactionView('sichuan', roomId, date);
        socket.send(JSON.stringify({ type: 'sichuan-state', room, serverTime }));
      } catch { /* ignore disconnected socket */ }
    }
  }

  sendSpotDifferenceState(state, roomId) {
    const targetRoom = state.spotDifference?.rooms?.[roomId];
    if (!targetRoom) return;
    const audiencePetIds = new Set([targetRoom.hostPetId, targetRoom.guestPetId, ...Object.keys(targetRoom.spectators ?? {})].filter(Boolean));
    const date = new Date();
    const serverTime = date.getTime();
    const reactions = this.liveReactionView('spotDifference', roomId, date);
    ...
    for (const socket of this.ctx.getWebSockets()) {
      try {
        ...
        if (!payload) {
          const room = spotDifferenceRoomView(state, roomId, pet.id, date);
          if (!room) continue;
          room.reactions = reactions;
          payload = JSON.stringify({ type: 'spot-difference-state', room, serverTime });
  ...
  async acceptWebSocket(request, url, state) {
    ...
    if (user) {
      ...
      if (pet) {
        ...
        davinciSetConnected(state, pet.id, true);
      }
      await this.store.save(state);
      await this.scheduleNextAlarm(state);
    }
  ...
  async handleFetch(request) {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/auth/register') {
        ...
        const user = { id: userId, nickname, pinSalt: pinData.salt, pinHash: pinData.hash, generation: 1, currentPetId: null, sessionVersion: 1, role: 'user', workoutBadge: false, accountLocked: false, notifications: [], createdAt: date.toISOString(), lastSeenAt: date.toISOString() };
      ...
      if (request.method === 'POST' && pathname === '/api/auth/login') {
        ...
        if (user.accountLocked) return errorResponse('운영자에 의해 잠긴 계정입니다.', 403);
        user.lastSeenAt = date.toISOString();
      ...
      if (request.method === 'POST' && pathname === '/api/profile/body-advancement') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (_state, _user, pet, now) => selectBodyAdvancement(pet, body.key, now), date);
        this.broadcastRefresh('profile');
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      ...
      const routes = {
        ...
        '/api/actions/cook': (gameState, _user, pet, now) => cookAction(pet, now),
        // 구버전 브라우저 캐시가 남아 있어도 헬스가 실행되지 않도록 요리하기로 호환 처리한다.
        '/api/actions/exercise': (gameState, _user, pet, now) => cookAction(pet, now)
      };
      ...
      if (request.method === 'POST' && pathname === '/api/minigames/start') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => startMiniGame(gameState, pet, body.gameId, now, { stakePoints: body.stakePoints, difficulty: body.difficulty }), date);
        // 지뢰찾기는 응답 자체에 최신 bootstrap이 포함된다. 시작 직후 자기 자신에게 다시 refresh를 보내면
        // 180ms 뒤 게임판 DOM이 재생성되어 첫 모바일 pointer/click이 중간에 끊길 수 있으므로 중복 갱신하지 않는다.
        if (String(body.gameId || '') !== 'minesweeper') this.broadcastRefresh('minigame', [auth.userId]);
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      if (request.method === 'POST' && pathname === '/api/minigames/finish') {
        ...
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => finishMiniGame(gameState, pet, body.challengeId, body.value, now, {
          ...
          lineClearCounts: body.lineClearCounts,
          endReason: body.endReason
        }), date);
        this.broadcastRefresh('minigame', [auth.userId]);
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      if (request.method === 'POST' && pathname === '/api/minigames/minesweeper/action') {
        const body = await readJson(request);
        const requestedActions = Array.isArray(body.actions) && body.actions.length
          ? body.actions.slice(0, 8)
          : [{ action: body.action, row: body.row, col: body.col }];
        const result = await this.mutateForUserLight(state, auth.userId, (gameState, _user, pet, now) => {
          const results = [];
          for (const input of requestedActions) {
            const step = playMinesweeperGame(gameState, pet, body.challengeId, {
              action: input?.action, row: input?.row, col: input?.col
            }, now);
            results.push(step);
            if (!step.ok || step.finished) break;
          }
          const last = results.at(-1) ?? { ok: false, message: '처리할 지뢰찾기 입력이 없습니다.' };
          return { ...last, firstReveal: results.some((step) => step?.firstReveal === true), results, processedCount: results.length };
        }, date);
        // 진행 중에는 거대한 전체 bootstrap 대신 바뀐 지뢰판만 돌려준다.
        // 종료 순간에만 포인트·랭킹·레고 상태까지 한 번에 동기화한다.
        if (result.ok && (result.finished || result.firstReveal)) result.bootstrap = this.bootstrapFromState(state, auth.userId);
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      if (request.method === 'POST' && pathname === '/api/minigames/minesweeper/abandon') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => abandonMinesweeperGame(gameState, pet, body.challengeId, now), date);
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      ...
      const omokRps = matchPath(pathname, '/api/omok/rooms/:roomId/rps');
      const omokColor = matchPath(pathname, '/api/omok/rooms/:roomId/color');
      if (request.method === 'POST' && (omokJoin || omokSpectate || omokSpectateLeave || omokLeave || omokMove || omokRematch || omokRps || omokColor)) {
        const body = await readJson(request);
        const route = omokJoin ? 'join' : omokSpectate ? 'spectate' : omokSpectateLeave ? 'spectate-leave' : omokLeave ? 'leave' : omokMove ? 'move' : omokRps ? 'rps' : omokColor ? 'color' : 'rematch';
        const roomId = (omokJoin || omokSpectate || omokSpectateLeave || omokLeave || omokMove || omokRematch || omokRps || omokColor).roomId;
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          if (route === 'join') {
          ...
          if (route === 'rps') return submitOmokRps(gameState, pet, roomId, String(body.choice || ''), now);
          if (route === 'color') return selectOmokColor(gameState, pet, roomId, String(body.color || ''), now);
      ...
      if (request.method === 'POST' && pathname === '/api/davinci/rooms') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => createDavinciRoom(gameState, pet, body.stakePoints, now), date);
        this.broadcastRefresh('davinci');
        return jsonResponse({ ...result, bootstrap: this.bootstrapFromState(state, auth.userId) }, result.ok ? 201 : 400);
      }
      const davinciJoin = matchPath(pathname, '/api/davinci/rooms/:roomId/join');
      const davinciReady = matchPath(pathname, '/api/davinci/rooms/:roomId/ready');
      const davinciStart = matchPath(pathname, '/api/davinci/rooms/:roomId/start');
      const davinciJoker = matchPath(pathname, '/api/davinci/rooms/:roomId/joker');
      const davinciRps = matchPath(pathname, '/api/davinci/rooms/:roomId/rps');
      const davinciOrder = matchPath(pathname, '/api/davinci/rooms/:roomId/order');
      const davinciGuess = matchPath(pathname, '/api/davinci/rooms/:roomId/guess');
      const davinciDecision = matchPath(pathname, '/api/davinci/rooms/:roomId/decision');
      const davinciPenalty = matchPath(pathname, '/api/davinci/rooms/:roomId/penalty');
      const davinciSpectate = matchPath(pathname, '/api/davinci/rooms/:roomId/spectate');
      const davinciSpectateLeave = matchPath(pathname, '/api/davinci/rooms/:roomId/spectate/leave');
      const davinciLeave = matchPath(pathname, '/api/davinci/rooms/:roomId/leave');
      const davinciRematch = matchPath(pathname, '/api/davinci/rooms/:roomId/rematch');
      if (request.method === 'POST' && (davinciJoin || davinciReady || davinciStart || davinciJoker || davinciRps || davinciOrder || davinciGuess || davinciDecision || davinciPenalty || davinciSpectate || davinciSpectateLeave || davinciLeave || davinciRematch)) {
        const body = await readJson(request);
        const matched = davinciJoin || davinciReady || davinciStart || davinciJoker || davinciRps || davinciOrder || davinciGuess || davinciDecision || davinciPenalty || davinciSpectate || davinciSpectateLeave || davinciLeave || davinciRematch;
        const roomId = matched.roomId;
        const route = davinciJoin ? 'join' : davinciReady ? 'ready' : davinciStart ? 'start' : davinciJoker ? 'joker' : davinciRps ? 'rps' : davinciOrder ? 'order' : davinciGuess ? 'guess' : davinciDecision ? 'decision' : davinciPenalty ? 'penalty' : davinciSpectate ? 'spectate' : davinciSpectateLeave ? 'spectate-leave' : davinciLeave ? 'leave' : 'rematch';
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          if (route === 'join') return joinDavinciRoom(gameState, pet, roomId, now);
          if (route === 'ready') return setDavinciReady(gameState, pet, roomId, body.ready, now);
          if (route === 'start') return startDavinciRoom(gameState, pet, roomId, now);
          if (route === 'joker') return placeDavinciJoker(gameState, pet, roomId, body.position, now);
          if (route === 'rps') return submitDavinciRps(gameState, pet, roomId, String(body.choice || ''), now);
          if (route === 'order') return selectDavinciOrder(gameState, pet, roomId, String(body.choice || ''), now);
          if (route === 'guess') return guessDavinciTile(gameState, pet, roomId, body.targetPetId, body.tileId, body.guess, body.actionId, now);
          if (route === 'decision') return decideDavinciTurn(gameState, pet, roomId, String(body.decision || ''), now);
          if (route === 'penalty') return chooseDavinciPenaltyTile(gameState, pet, roomId, body.tileId, now);
          if (route === 'spectate') return spectateDavinciRoom(gameState, pet, roomId, now);
          if (route === 'spectate-leave') return leaveDavinciSpectator(gameState, pet, roomId, now);
          if (route === 'leave') return leaveDavinciRoom(gameState, pet, roomId, now);
          return requestDavinciRematch(gameState, pet, roomId, now);
        }, date);
        this.broadcastRefresh('davinci');
        return jsonResponse({ ...result, bootstrap: this.bootstrapFromState(state, auth.userId) }, result.ok ? 200 : 400);
      }

      if (request.method === 'POST' && pathname === '/api/sichuan/rooms') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => createSichuanRoom(gameState, pet, body.stakePoints, now), date);
        this.broadcastRefresh('sichuan-lobby');
        return jsonResponse(result, result.ok ? 201 : 400);
      }
      const sichuanJoin = matchPath(pathname, '/api/sichuan/rooms/:roomId/join');
      const sichuanSpectate = matchPath(pathname, '/api/sichuan/rooms/:roomId/spectate');
      const sichuanSpectateLeave = matchPath(pathname, '/api/sichuan/rooms/:roomId/spectate/leave');
      const sichuanLeave = matchPath(pathname, '/api/sichuan/rooms/:roomId/leave');
      const sichuanPair = matchPath(pathname, '/api/sichuan/rooms/:roomId/pair');
      const sichuanRematch = matchPath(pathname, '/api/sichuan/rooms/:roomId/rematch');
      if (request.method === 'POST' && sichuanPair) {
        const body = await readJson(request);
        const user = state.users[auth.userId];
        const pet = user ? currentPetForUser(state, auth.userId) : null;
        if (!user || !pet?.alive) return errorResponse('사용자를 찾을 수 없습니다.', 401);
        user.lastSeenAt = date.toISOString();
        const result = playSichuanPair(state, pet, sichuanPair.roomId, body, date);
        const targetRoom = state.sichuan?.rooms?.[sichuanPair.roomId];
        const timedSettlement = Boolean(result.terminal && targetRoom?.status === 'ended' && targetRoom?.settled);
        const requiresFullSave = Boolean(result.finished || timedSettlement);
        const shouldPersist = Boolean((result.ok && !result.duplicate) || timedSettlement);
        if (shouldPersist) {
          this.sendSichuanState(state, sichuanPair.roomId);
          if (requiresFullSave) {
            await this.store.save(state);
            await this.scheduleNextAlarm(state);
            this.broadcastRefresh('sichuan-lobby');
          } else await this.store.saveSichuan(state);
        }
        const room = sichuanRoomView(state, sichuanPair.roomId, pet.id, date);
        if (room) room.reactions = this.liveReactionView('sichuan', room.id, date);
        return jsonResponse({ ...result, room, serverTime: date.getTime() }, result.ok || result.terminal || result.stale ? 200 : 400);
      }
      if (request.method === 'POST' && (sichuanJoin || sichuanSpectate || sichuanSpectateLeave || sichuanLeave || sichuanRematch)) {
        const matched = sichuanJoin || sichuanSpectate || sichuanSpectateLeave || sichuanLeave || sichuanRematch;
        const roomId = matched.roomId;
        const route = sichuanJoin ? 'join' : sichuanSpectate ? 'spectate' : sichuanSpectateLeave ? 'spectate-leave' : sichuanLeave ? 'leave' : 'rematch';
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          if (route === 'join') return joinSichuanRoom(gameState, pet, roomId, now);
          if (route === 'spectate') return spectateSichuanRoom(gameState, pet, roomId, now);
          if (route === 'spectate-leave') return leaveSichuanSpectator(gameState, pet, roomId, now);
          if (route === 'leave') return leaveSichuanRoom(gameState, pet, roomId, now);
          return requestSichuanRematch(gameState, pet, roomId, now);
        }, date);
        this.sendSichuanState(state, roomId);
        this.broadcastRefresh('sichuan-lobby');
        return jsonResponse(result, result.ok ? 200 : 400);
      }

      if (request.method === 'POST' && pathname === '/api/spot-difference/rooms') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => createSpotDifferenceRoom(gameState, pet, body.stakePoints, now), date);
        this.broadcastRefresh('spot-difference-lobby');
        return jsonResponse(result, result.ok ? 201 : 400);
      }
      const spotJoin = matchPath(pathname, '/api/spot-difference/rooms/:roomId/join');
      const spotSpectate = matchPath(pathname, '/api/spot-difference/rooms/:roomId/spectate');
      const spotSpectateLeave = matchPath(pathname, '/api/spot-difference/rooms/:roomId/spectate/leave');
      const spotLeave = matchPath(pathname, '/api/spot-difference/rooms/:roomId/leave');
      const spotClick = matchPath(pathname, '/api/spot-difference/rooms/:roomId/click');
      const spotRematch = matchPath(pathname, '/api/spot-difference/rooms/:roomId/rematch');
      if (request.method === 'POST' && spotClick) {
        const body = await readJson(request);
        const user = state.users[auth.userId];
        const pet = user ? currentPetForUser(state, auth.userId) : null;
        if (!user || !pet?.alive) return errorResponse('사용자를 찾을 수 없습니다.', 401);
        user.lastSeenAt = date.toISOString();
        const timedBefore = processSpotDifferenceTimers(state, date, { roomId: spotClick.roomId });
        const result = playSpotDifferenceClick(state, pet, spotClick.roomId, body, date);
        const shouldPersist = Boolean(timedBefore.changed || result.correct !== undefined || result.alreadyFound);
        const requiresFullSave = Boolean(timedBefore.settled || result.finished);
        if (shouldPersist) {
          this.sendSpotDifferenceState(state, spotClick.roomId);
          if (requiresFullSave) {
            await this.store.save(state);
            await this.scheduleNextAlarm(state);
            this.broadcastRefresh('spot-difference-lobby');
          } else await this.store.saveSpotDifference(state);
        }
        const room = spotDifferenceRoomView(state, spotClick.roomId, pet.id, date);
        if (room) room.reactions = this.liveReactionView('spotDifference', room.id, date);
        return jsonResponse({ ...result, room, serverTime: date.getTime() }, result.ok || result.terminal ? 200 : 400);
      }
      if (request.method === 'POST' && (spotJoin || spotSpectate || spotSpectateLeave || spotLeave || spotRematch)) {
        const matched = spotJoin || spotSpectate || spotSpectateLeave || spotLeave || spotRematch;
        const roomId = matched.roomId;
        const route = spotJoin ? 'join' : spotSpectate ? 'spectate' : spotSpectateLeave ? 'spectate-leave' : spotLeave ? 'leave' : 'rematch';
        const result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
          if (route === 'join') return joinSpotDifferenceRoom(gameState, pet, roomId, now);
          if (route === 'spectate') return spectateSpotDifferenceRoom(gameState, pet, roomId, now);
          if (route === 'spectate-leave') return leaveSpotDifferenceSpectator(gameState, pet, roomId, now);
          if (route === 'leave') return leaveSpotDifferenceRoom(gameState, pet, roomId, now);
          return requestSpotDifferenceRematch(gameState, pet, roomId, now);
        }, date);
        this.sendSpotDifferenceState(state, roomId);
        this.broadcastRefresh('spot-difference-lobby');
        return jsonResponse(result, result.ok ? 200 : 400);
      }
      ...
      if (request.method === 'POST' && blockInput) {
        ...
        // 입력이 도착했다는 사실 자체가 클라이언트가 살아 있다는 최신 신호다.
        // 입력보다 먼저 서버 fallback 중력을 따라잡으면 네트워크가 잠깐 밀린 순간
        // 서버가 여러 칸을 먼저 내린 뒤 로컬 예측 입력까지 더해져 큰 스냅이 생길 수 있다.
        // 먼저 입력을 확정하고, 그 뒤 재접속 만료 같은 서버 타이머를 처리한다.
        const timed = processBlockBattleTimers(state, date, { roomId: blockInput.roomId });
        ...
        // lock 직후 새 active 블록은 이미 서버 메모리 상태에서 확정되어 있다.
        // Durable Storage 쓰기가 끝날 때까지 화면 전송을 늦추면 저장 지연이 그대로
        // 다음 블록 등장 지연으로 보이므로, 정확한 authoritative 상태를 먼저 전송한다.
      ...
      if (request.method === 'POST' && omokReaction) {
        ...
        const isPlayer = Boolean(pet && [room?.hostPetId, room?.guestPetId].includes(pet.id));
        if (!pet?.alive || room?.status !== 'playing' || (!isPlayer && !spectator?.connected)) return errorResponse('오목 참가자 또는 관전자만 공감을 보낼 수 있습니다.', 403);
      ...
      if (request.method === 'POST' && blockBattleReaction) {
        ...
        const spectator = pet ? room?.spectators?.[pet.id] : null;
        const isPlayer = Boolean(pet && [room?.hostPetId, room?.guestPetId].includes(pet.id));
        if (!pet?.alive || room?.status !== 'playing' || (!isPlayer && !spectator?.connected)) return errorResponse('테트리스대전 참가자 또는 관전자만 공감을 보낼 수 있습니다.', 403);
      ...
      const davinciReaction = matchPath(pathname, '/api/davinci/rooms/:roomId/reaction');
      if (request.method === 'POST' && davinciReaction) {
        const body = await readJson(request);
        const pet = currentPetForUser(state, auth.userId);
        const room = state.davinci?.rooms?.[davinciReaction.roomId];
        const spectator = pet ? room?.spectators?.[pet.id] : null;
        const isPlayer = Boolean(pet && room?.players?.some((player) => player.petId === pet.id && !player.leftRoom));
        if (!pet?.alive || room?.status !== 'playing' || (!isPlayer && !spectator?.connected)) return errorResponse('다빈치코드 참가자 또는 관전자만 공감을 보낼 수 있습니다.', 403);
        const result = this.addSpectatorReaction('davinci', room.id, pet, body.type, date);
        if (!result.ok) return errorResponse(result.message, result.status || 400);
        this.broadcastRefresh('spectator-reaction');
        return jsonResponse({ ok: true, reaction: result.reaction, bootstrap: this.bootstrapFromState(state, auth.userId) });
      }

      const sichuanReaction = matchPath(pathname, '/api/sichuan/rooms/:roomId/reaction');
      if (request.method === 'POST' && sichuanReaction) {
        const body = await readJson(request);
        const pet = currentPetForUser(state, auth.userId);
        const room = state.sichuan?.rooms?.[sichuanReaction.roomId];
        const spectator = pet ? room?.spectators?.[pet.id] : null;
        const isPlayer = Boolean(pet && [room?.hostPetId, room?.guestPetId].includes(pet.id));
        if (!pet?.alive || room?.status !== 'playing' || (!isPlayer && !spectator)) return errorResponse('사천성 대전 참가자 또는 관전자만 공감을 보낼 수 있습니다.', 403);
        const result = this.addSpectatorReaction('sichuan', room.id, pet, body.type, date);
        if (!result.ok) return errorResponse(result.message, result.status || 400);
        this.sendSichuanState(state, room.id);
        return jsonResponse({ ok: true, reaction: result.reaction });
      }

      const spotDifferenceReaction = matchPath(pathname, '/api/spot-difference/rooms/:roomId/reaction');
      if (request.method === 'POST' && spotDifferenceReaction) {
        const body = await readJson(request);
        const pet = currentPetForUser(state, auth.userId);
        const room = state.spotDifference?.rooms?.[spotDifferenceReaction.roomId];
        const spectator = pet ? room?.spectators?.[pet.id] : null;
        const isPlayer = Boolean(pet && [room?.hostPetId, room?.guestPetId].includes(pet.id));
        if (!pet?.alive || room?.status !== 'playing' || (!isPlayer && !spectator)) return errorResponse('틀린그림찾기 참가자 또는 관전자만 공감을 보낼 수 있습니다.', 403);
        const result = this.addSpectatorReaction('spotDifference', room.id, pet, body.type, date);
        if (!result.ok) return errorResponse(result.message, result.status || 400);
        this.sendSpotDifferenceState(state, room.id);
        return jsonResponse({ ok: true, reaction: result.reaction });
      }
      ...
      if (request.method === 'POST' && pathname === '/api/admin/support-settings') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const body = await readJson(request);
        const message = [...String(body.message ?? '').replace(/\s+/g, ' ').trim()].slice(0, SUPPORT_MESSAGE_MAX_LENGTH).join('');
        if (!message) return errorResponse('후원 문구를 입력해주세요.', 400);
        const enabled = body.enabled !== false;
        state.supportSettings = { enabled, message };
        this.appendAdminAudit(state, user, {
          action: 'support_settings',
          targetUserId: auth.userId,
          targetPetId: state.users[auth.userId]?.currentPetId ?? null,
          targetDisplayName: '후원 배너',
          detail: `${enabled ? '후원 배너 ON' : '후원 배너 OFF'} · 문구 변경`
        }, date);
        await this.store.save(state);
        await this.scheduleNextAlarm(state);
        this.broadcastRefresh('support-settings');
        return jsonResponse({
          ok: true,
          message: '후원 설정을 저장했습니다.',
          support: supportSettingsView(state),
          bootstrap: this.bootstrapFromState(state, auth.userId)
        });
      }

      if (request.method === 'POST' && pathname === '/api/admin/pin-reset') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const body = await readJson(request);
        const targetUserId = String(body.targetUserId ?? '').trim();
        if (!targetUserId) return errorResponse('대상 회원을 지정해주세요.', 400);
        if (targetUserId === auth.userId) return errorResponse('운영자 본인 PIN은 이 메뉴에서 변경할 수 없습니다.', 400);
        const targetUser = state.users[targetUserId];
        if (!targetUser) return errorResponse('대상 회원을 찾을 수 없습니다.', 404);
        const pinError = validatePin(body.pin);
        if (pinError) return errorResponse(pinError, 400);

        const pinData = await hashPin(body.pin, this.secret);
        targetUser.pinSalt = pinData.salt;
        targetUser.pinHash = pinData.hash;
        targetUser.sessionVersion = Math.max(1, Number(targetUser.sessionVersion) || 1) + 1;
        ...
        this.appendAdminAudit(state, user, {
          ...
          detail: 'PIN 변경 · 기존 로그인 종료'
        }, date);
        await this.store.save(state);
        await this.scheduleNextAlarm(state);
        this.disconnectUserSockets(targetUserId, 'PIN changed');
        this.broadcastRefresh('admin-pin-reset', [auth.userId]);
        return jsonResponse({
          ok: true,
          message: `${targetPet?.displayName || targetUser.nickname}의 PIN을 변경했습니다. 기존 로그인은 모두 종료됐습니다.`,
          bootstrap: this.bootstrapFromState(state, auth.userId)
        });
      }

      if (request.method === 'POST' && pathname === '/api/admin/account-lock') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const body = await readJson(request);
        const targetUserId = String(body.targetUserId ?? '').trim();
        if (!targetUserId) return errorResponse('대상 회원을 지정해주세요.', 400);
        if (targetUserId === auth.userId) return errorResponse('운영자 본인 계정은 잠글 수 없습니다.', 400);
        const targetUser = state.users[targetUserId];
        if (!targetUser) return errorResponse('대상 회원을 찾을 수 없습니다.', 404);
        const enabled = Boolean(body.enabled);
        if (Boolean(targetUser.accountLocked) !== enabled) {
          targetUser.accountLocked = enabled;
          // 잠그는 즉시 기존 로그인 토큰을 전부 무효화한다. 잠금 해제 뒤에는 새로 로그인해야 한다.
          if (enabled) targetUser.sessionVersion = Math.max(1, Number(targetUser.sessionVersion) || 1) + 1;
        }
        const targetPet = state.pets?.[targetUser.currentPetId];
        this.appendAdminAudit(state, user, {
          action: 'account_lock',
          ...
          targetPetId: targetPet?.id ?? null,
          targetDisplayName: targetPet?.displayName || targetUser.nickname,
          detail: enabled ? '계정 잠금' : '계정 잠금 해제'
        }, date);
        await this.store.save(state);
        await this.scheduleNextAlarm(state);
        if (enabled) this.disconnectUserSockets(targetUserId, 'account locked');
        this.broadcastRefresh('admin-account-lock');
        return jsonResponse({
          ok: true,
          message: enabled ? `${targetPet?.displayName || targetUser.nickname} 계정을 잠갔습니다.` : `${targetPet?.displayName || targetUser.nickname} 계정 잠금을 해제했습니다.`,
          bootstrap: this.bootstrapFromState(state, auth.userId)
        });
      }
      ...
      if (request.method === 'POST' && pathname === '/api/admin/kick') {
        ...
        removePetFromDavinci(state, target.id, date);
        removePetFromSichuan(state, target.id, date);
        removePetFromSpotDifference(state, target.id, date);
      ...
      if (request.method === 'POST' && pathname === '/api/admin/sichuan/clear-ended') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const outcome = clearEndedSichuanRooms(state, date);
        this.appendAdminAudit(state, user, { action: 'sichuan_clear_ended', targetDisplayName: '종료된 사천성 대전방', detail: `${outcome.cleared}개 정리` }, date);
        await this.store.save(state);
        this.broadcastRefresh('sichuan-lobby');
        return jsonResponse({ ...outcome, bootstrap: this.bootstrapFromState(state, auth.userId) });
      }
      if (request.method === 'POST' && pathname === '/api/admin/spot-difference/clear-ended') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const outcome = clearEndedSpotDifferenceRooms(state, date);
        this.appendAdminAudit(state, user, { action: 'spot_difference_clear_ended', targetDisplayName: '종료된 틀린그림찾기 대전방', detail: `${outcome.cleared}개 정리` }, date);
        await this.store.save(state);
        this.broadcastRefresh('spot-difference-lobby');
        return jsonResponse({ ...outcome, bootstrap: this.bootstrapFromState(state, auth.userId) });
      }
      if (request.method === 'POST' && pathname === '/api/admin/davinci/clear-ended') {
        const user = state.users[auth.userId];
        if (!this.isAdminUser(user)) return errorResponse('운영자 권한이 필요합니다.', 403);
        const outcome = clearEndedDavinciRooms(state);
        this.appendAdminAudit(state, user, { action: 'davinci_clear_ended', targetDisplayName: '종료된 다빈치코드 방', detail: `${outcome.cleared}개 정리` }, date);
        await this.store.save(state);
        this.broadcastRefresh('davinci');
        return jsonResponse({ ...outcome, bootstrap: this.bootstrapFromState(state, auth.userId) });
      }
  ...
  async handleWebSocketMessage(socket, message) {
    try {
      ...
      const userId = socket.deserializeAttachment()?.userId;
      const state = await this.store.load();
      const socketUser = state.users?.[userId];
      if (!socketUser || socketUser.accountLocked) {
        socket.send(JSON.stringify({ type: 'error', message: socketUser?.accountLocked ? '운영자에 의해 잠긴 계정입니다.' : '사용자를 찾을 수 없습니다.' }));
        try { socket.close(4001, 'account unavailable'); } catch { /* ignore */ }
        return;
      }
      const pet = currentPetForUser(state, userId);
      if (!pet?.alive) {
      ...
      // 해당 방 입력을 먼저 확정한 뒤 그 방의 재접속 만료/fallback 타이머만 처리한다.
      const result = playBlockBattleActions(state, pet, String(data.roomId || ''), data, date);
      const timed = processBlockBattleTimers(state, date, { roomId: String(data.roomId || '') });
      const shouldPersist = timed.changed || (result.ok && !result.duplicate);
      const requiresFullSave = timed.settled || result.finished;
      ...
      // WebSocket 경로도 저장 I/O보다 먼저 새 블록 상태를 내려 화면 공백을 없앤다.
  ...
  async markSocketDisconnected(socket) {
    ...
    const davinciChanged = davinciSetConnected(state, pet.id, false);
    if (omokChanged.changed || blockBattleChanged.changed || davinciChanged.changed) {
