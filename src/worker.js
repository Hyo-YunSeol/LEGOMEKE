...
export class LegoGameRoom {
  ...
  async processTimeState(state, date = new Date()) {
    ...
    if (expiredMini.changed || bungs.changed || gameSeason.changed || territory.changed || omok.changed) changed = true;
  ...
  async handleFetch(request) {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/minigames/minesweeper/action') {
        ...
        this.broadcastRefresh('minesweeper', [auth.userId]);
        return jsonResponse(result, result.ok ? 200 : 400);
      }
