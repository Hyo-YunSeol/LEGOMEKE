...
import {
  ...
  ensurePetSchema, endLifeAndRestart, nextHungerPenaltyAt, buyCosmetic, COSMETIC_SHOP
} from './game/engine.js';
import {
...
export class LegoGameRoom {
  ...
  processCosmeticExpiryNotifications(state, pet, date = new Date()) {
    if (!pet?.alive) return false;
    ensurePetSchema(pet, date);
    const now = date.getTime();
    const limit = 6 * 60 * 60 * 1000;
    let changed = false;
    for (const item of COSMETIC_SHOP.filter((entry) => entry.hours && !entry.title)) {
      if (item.background && pet.cosmetics?.backgroundId !== item.background) continue;
      const storageKey = item.storageKey || `${item.id}Until`;
      const noticeKey = item.background ? 'background' : item.id;
      const expiresAt = new Date(pet.cosmetics?.[storageKey] ?? '').getTime();
      if (!Number.isFinite(expiresAt) || expiresAt <= now || expiresAt - now > limit) continue;
      const expiresIso = new Date(expiresAt).toISOString();
      if (pet.cosmeticExpiryNotices?.[noticeKey] === expiresIso) continue;
      pet.cosmeticExpiryNotices ??= {};
      pet.cosmeticExpiryNotices[noticeKey] = expiresIso;
      addNotification(state, pet.userId, `✨ ${item.name} 효과가 6시간 안에 만료됩니다.`, 'cosmetic-expiry', { itemId: item.id, expiresAt: expiresIso }, date);
      changed = true;
    }
    return changed;
  }
  ...
  async processTimeState(state, date = new Date()) {
    ...
    for (const userId of Object.keys(state.users ?? {})) {
      ...
      const cosmeticNotice = this.processCosmeticExpiryNotifications(state, pet, date);
      if (reset.changed || hunger.changed || news || cosmeticNotice) changed = true;
    }
  ...
  bootstrapFromState(state, userId) {
    ...
    return {
      ...
      catalog: {
        ...
        statusMessageMaxLength: STATUS_MESSAGE_MAX_LENGTH, cosmetics: COSMETIC_SHOP.map((item) => ({ ...item }))
      }
    };
  }
  ...
  async handleFetch(request) {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/profile/cosmetics/buy') {
        const body = await readJson(request);
        const result = await this.mutateForUser(state, auth.userId, (_state, _user, pet, now) => buyCosmetic(pet, body.itemId, now), date);
        this.broadcastRefresh('cosmetics');
