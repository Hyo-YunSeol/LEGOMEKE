...
import {
  ...
  refreshTopPokeNews, startMiniGame, finishMiniGame, stopMiniGame, selectAppleGame, requestAppleNewBoardGame, settleExpiredMiniGames,
...
export class LegoGameRoom {
  constructor(ctx, env) {
    ...
    this.liveReactions = { liar: [], omok: new Map() };
  ...
  cleanLiveReactions(date = new Date()) {
    ...
    for (const [roomId, items] of this.liveReactions.omok.entries()) {
      const active = items.filter((item) => item.expiresAt > now);
      if (active.length) this.liveReactions.omok.set(roomId, active);
      else this.liveReactions.omok.delete(roomId);
    }
  ...
  liveReactionView(scope, roomId = null, date = new Date()) {
    ...
    const items = scope === 'liar' ? this.liveReactions.liar : (this.liveReactions.omok.get(roomId) ?? []);
  ...
  addSpectatorReaction(scope, roomId, pet, typeValue, date = new Date()) {
    ...
    else this.liveReactions.omok.set(roomId, [...(this.liveReactions.omok.get(roomId) ?? []), item].slice(-12));
