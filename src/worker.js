...
const APP_VERSION = '6.10.6-tetris-instant-spawn-final';
    ...
    try {
      ...
      if (request.method === 'POST' && (blockJoin || blockSpectate || blockSpectateLeave || blockLeave || blockRematch)) {
        ...
        if (route === 'join') {
          ...
          if (!user || !pet?.alive) return errorResponse('사용자를 찾을 수 없습니다.', 401);
          user.lastSeenAt = date.toISOString();
          result = joinBlockBattleRoom(state, pet, roomId, date);
          // 시작 상태는 Durable Storage 저장보다 먼저 선수들에게 보낸다. 시작 직후 첫 active
          // 블록이 이미 메모리에 있는데 저장 I/O 때문에 화면만 늦어지는 현상을 제거한다.
          if (result.ok) this.sendBlockBattleState(state, roomId);
          await this.store.save(state);
          await this.scheduleNextAlarm(state);
          result = { ...result, bootstrap: this.bootstrapFromState(state, auth.userId) };
        } else {
          result = await this.mutateForUser(state, auth.userId, (gameState, _user, pet, now) => {
