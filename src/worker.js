...
import {
  ...
  MINI_GAMES_PER_DAY, FISHING_PER_DAY, GAME_DAY_HOURS, BODY_STAGES, STATUS_MESSAGE_MAX_LENGTH,
  ...
  adminView(state, user) {
    ...
    return {
      ...
      members: isAdmin ? Object.values(state.users).map((member) => {
        const pet = state.pets[member.currentPetId];
        if (!pet?.alive) return null;
        ensurePetSchema(pet);
        return { userId: member.id, petId: pet.id, displayName: pet.displayName, warnings: pet.warnings, warningTotal: pet.records?.warnings ?? 0, breakWarnings: pet.integrity?.breakWarnings ?? 0, broken: Boolean(pet.integrity?.broken), points: pet.stats.points, level: privateDashboard(state, member.id).pet.stats.level, online: this.isOnline(state, member.id), isSelf: member.id === user?.id };
      }).filter(Boolean).sort((a, b) => a.displayName.localeCompare(b.displayName, 'ko')) : [],
  ...
  bootstrapFromState(state, userId) {
    ...
    return {
      ...
      catalog: {
        ...
        gameDayHours: GAME_DAY_HOURS, breakWarningMax: BREAK_WARNING_MAX, breakInactivityHours: BREAK_INACTIVITY_HOURS,
...
export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === '/healthz') return jsonResponse({ ok: true, version: APP_VERSION, platform: 'Cloudflare Workers Free' });
    if (url.pathname.startsWith('/api/')) {
      const objectId = env.GAME_ROOM.idFromName('lego-life-global-room');
      return env.GAME_ROOM.get(objectId).fetch(request);
    }
