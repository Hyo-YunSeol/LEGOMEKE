...
2. 저장소 `Hyo-YunSeol/LEGOMEKE`, 브랜치 `main` 선택
3. Worker 이름 `legomeke`
4. Build command: `npm run verify`
5. Deploy command: `npx wrangler deploy`
6. Root directory / Path: 완전히 비움
7. 저장 후 배포

`package.json`의 `deploy:cloudflare` 스크립트도 같은 배포 명령을 사용합니다.
...
운영자로 로그인하면 운영자 관리의 회원 목록에서 다른 회원의 `계정 삭제`를 사용할 수 있습니다. 운영자 본인 계정 삭제는 서버에서 차단됩니다. 삭제는 복구 기능이 없으므로 대상 회원을 다시 확인한 뒤 사용하세요.
