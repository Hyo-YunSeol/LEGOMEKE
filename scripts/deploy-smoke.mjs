...
assert.match(worker, /APP_VERSION = '6\.10\.28-full-fix-final'/);
assert.match(index, /styles\.css\?v=6101280/);
assert.match(index, /app\.js\?v=6101280/);
assert.match(app, /sw\.js\?v=6101280/);
assert.match(sw, /const CACHE = 'lego-life-v6101280-full-fix-final'/);
...
assert.match(styles, /\.sichuan-cell img\{[^}]*width:100%;height:100%;max-width:100%;max-height:100%/);
assert.match(styles, /\.ranking-section \.rank-tabs-grid\s*\{\s*grid-template-columns:repeat\(3,minmax\(0,1fr\)\)/);
assert.match(styles, /\.sichuan-board\.sichuan-concealed \.sichuan-cell:not\(\.sichuan-empty-cell\) img\{visibility:hidden\}/);
assert.doesNotMatch(styles, /\.sichuan-streak-overlay\{/);
assert.match(app, /tile\.src[^\n]*v=6101280&s=6101281/);

assert.equal(Object.keys(SICHUAN_THEMES).length, 5);
const ids = SICHUAN_THEMES.neon.tiles.map((tile) => tile.id);
...
console.log('Deploy smoke OK: v6.10.28 full fix final assets and existing game features are connected.');
