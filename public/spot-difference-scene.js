...
const SPOT_V4_BASE_POINTS = Object.freeze([
  [0.11,0.12],[0.29,0.12],[0.47,0.12],[0.65,0.12],[0.83,0.12],
  [0.17,0.26],[0.35,0.26],[0.53,0.26],[0.71,0.26],[0.89,0.26],
  [0.11,0.40],[0.29,0.40],[0.47,0.40],[0.65,0.40],[0.83,0.40],
  [0.17,0.54],[0.35,0.54],[0.53,0.54],[0.71,0.54],[0.89,0.54],
  [0.11,0.68],[0.29,0.68],[0.47,0.68],[0.65,0.68],[0.83,0.68],
  [0.17,0.82],[0.35,0.82],[0.53,0.82],[0.71,0.82],[0.89,0.82]
]);

function spotSceneLayoutSeed(themeId, variant) {
  const safeVariant = Math.max(0, Math.min(1, Number(variant || 0) | 0));
  const text = `${String(themeId || '')}:${safeVariant}`;
  let hash = 2166136261;
  for (let index = 0; index < text.length; index += 1) { hash ^= text.charCodeAt(index); hash = Math.imul(hash, 16777619); }
  return hash >>> 0;
}

export function spotClientHitboxesForPuzzle(puzzle) {
  const assetVersion = Math.max(0, Number(puzzle?.assetVersion || 0) | 0);
  if (assetVersion < 4) return SPOT_CLIENT_HITBOXES;
  const baseSeed = spotSceneLayoutSeed(puzzle?.themeId, puzzle?.variant);
  const seed = assetVersion >= 5 ? (baseSeed ^ ((Math.max(1, Number(puzzle?.seed || 1) | 0)) >>> 0)) >>> 0 : baseSeed;
  return SPOT_V4_BASE_POINTS.map(([baseX, baseY], index) => {
    const random = seededRandom(seed + Math.imul(index + 1, 104729));
    const x = Math.max(0.075, Math.min(0.925, baseX + (random() - 0.5) * (assetVersion >= 5 ? 0.048 : 0.056)));
    const y = Math.max(0.075, Math.min(0.885, baseY + (random() - 0.5) * (assetVersion >= 5 ? 0.032 : 0.040)));
    const r = assetVersion >= 5 ? (0.044 + random() * 0.004) : (0.050 + random() * 0.006);
    return Object.freeze({ id: `d${index}`, x, y, r });
  });
}

const THEMES = Object.freeze({
  'body-guide': { label: '수집가 작업실', sky: '#fff7ed', floor: '#fde68a', accent: '#ea580c', pool: ['crown','heart','star','apple','cake','cat','balloon','flower','book','moon','gift','clock'] },
  'lego-room': { label: '파란 창가 방', sky: '#eff6ff', floor: '#e7e5e4', accent: '#2563eb', pool: ['clock','plant','book','cup','cat','gamepad','camera','gift','heart','cake','star','moon'] },
  convenience: { label: '편의점 진열대', sky: '#ecfdf5', floor: '#d1fae5', accent: '#059669', pool: ['bottle','apple','cake','cup','camera','gift','star','mushroom','book','heart','clock','balloon'] },
  beach: { label: '해변', sky: '#e0f2fe', floor: '#fef3c7', accent: '#0284c7', pool: ['sun','cloud','fish','umbrella','balloon','apple','star','moon','camera','bottle','heart','flower'] },
  'game-room': { label: '게임방', sky: '#f5f3ff', floor: '#ddd6fe', accent: '#7c3aed', pool: ['gamepad','star','rocket','camera','cup','clock','moon','heart','crown','gift','book','balloon'] },
  picnic: { label: '공원 피크닉', sky: '#f0fdf4', floor: '#bbf7d0', accent: '#16a34a', pool: ['sun','cloud','flower','apple','cake','cat','balloon','umbrella','heart','book','bottle','star'] },
  camping: { label: '캠핑장', sky: '#fefce8', floor: '#d9f99d', accent: '#65a30d', pool: ['moon','star','mushroom','bottle','cup','fish','book','apple','camera','heart','cloud','rocket'] },
  cafe: { label: '카페', sky: '#fff7ed', floor: '#fed7aa', accent: '#c2410c', pool: ['cup','cake','plant','book','clock','camera','heart','flower','apple','cat','gift','star'] },
  festival: { label: '축제 부스', sky: '#fdf2f8', floor: '#fbcfe8', accent: '#db2777', pool: ['balloon','gift','star','crown','cake','heart','camera','apple','rocket','flower','moon','cup'] },
  'space-lab': { label: '우주 연구실', sky: '#eef2ff', floor: '#c7d2fe', accent: '#4338ca', pool: ['rocket','moon','star','gamepad','bottle','clock','camera','crown','heart','plant','fish','gift'] }
});


const SPOT_ATLAS_LEGACY_VERSION = 2;
const SPOT_ATLAS_ASSET_VERSION = 4;
const SPOT_ATLAS_CACHE_VERSION = '610120';
...
export function spotDifferenceAssetUrls(puzzle) {
  ...
  return {
    original,
    // v3/v4는 원본 한 장의 실제 픽셀을 국소 변형해 7개 차이를 만든다. v2 진행방만 기존 changed WebP를 사용한다.
...
function dynamicAtlasDifferenceMarkup(puzzle, href) {
  if (![3, SPOT_ATLAS_ASSET_VERSION].includes(Number(puzzle?.assetVersion || 0)) || !href) return '';
  const ids = new Set(Array.isArray(puzzle?.differenceIds) ? puzzle.differenceIds : []);
  const seedBase = Number(puzzle?.seed || 1);
  const defs = [];
  const edits = [];
  for (const base of spotClientHitboxesForPuzzle(puzzle).filter((hitbox) => ids.has(hitbox.id))) {
    const index = Number(base.id.slice(1)) || 0;
    const random = seededRandom(seedBase + (index + 1) * 104729);
    // 정답 표시용 도형을 새로 그리지 않는다. 원본의 실제 픽셀 조각만 국소적으로
    // 이동/반전/확대해 부품·배선·문양·책등·잎 같은 기존 요소 자체가 달라지게 한다.
    // 중심도 매판 약간 흔들어 같은 18개 후보 지점을 외워 찍는 플레이를 줄인다.
    const jitterX = Math.round((random() - .5) * 16);
    const jitterY = Math.round((random() - .5) * 12);
    const x = Math.max(28, Math.min(W - 28, base.x * W + jitterX));
    const y = Math.max(24, Math.min(H - 24, base.y * H + jitterY));
    const rx = 17 + Math.floor(random() * 10);
    const ry = 13 + Math.floor(random() * 8);
    const dx = (random() < .5 ? -1 : 1) * (8 + Math.floor(random() * 9));
    const dy = (random() < .5 ? -1 : 1) * (4 + Math.floor(random() * 7));
    const mode = Math.floor(random() * 6);
    const clipId = `spot-edit-${Math.abs(seedBase)}-${index}`;
    const rounded = random() > .45;
    defs.push(rounded
      ? `<clipPath id="${clipId}"><rect x="${x - rx}" y="${y - ry}" width="${rx * 2}" height="${ry * 2}" rx="${Math.max(7, Math.floor(Math.min(rx, ry) * .65))}"/></clipPath>`
      : `<clipPath id="${clipId}"><ellipse cx="${x}" cy="${y}" rx="${rx}" ry="${ry}"/></clipPath>`);

    let transform = '';
    if (mode === 0) transform = `translate(${dx} ${dy})`;
    else if (mode === 1) transform = `translate(${-dx} ${Math.round(dy * .65)})`;
    else if (mode === 2) transform = `translate(${2 * x} 0) scale(-1 1)`;
    else if (mode === 3) transform = `translate(0 ${2 * y}) scale(1 -1)`;
    else if (mode === 4) transform = `translate(${x} ${y}) scale(1.075) translate(${-x} ${-y})`;
    else transform = `translate(${x} ${y}) scale(.925) translate(${-x} ${-y})`;

    edits.push(`<g class="spot-dynamic-difference spot-natural-edit" clip-path="url(#${clipId})"><image href="${href}" x="0" y="0" width="${W}" height="${H}" preserveAspectRatio="none" transform="${transform}"/></g>`);
  }
  if (!edits.length) return '';
  const mirrorTransform = puzzle.mirrored ? ` transform="translate(${W} 0) scale(-1 1)"` : '';
  return `<defs>${defs.join('')}</defs><g class="spot-natural-edit-layer"${mirrorTransform}>${edits.join('')}</g>`;
}

function renderCandidate(type, hitbox, index, changed, accent, puzzle = null) {
  const x = hitbox.x * W;
  const y = hitbox.y * H;
  const base = symbolMarkup(type, accent);
  if (!changed) return `<g transform="translate(${x} ${y})">${base}</g>`;
  if (Number(puzzle?.assetVersion || 0) < 5) {
    const mode = index % 6;
    if (mode === 0) return '';
  ...
  const mutationSeed = (Number(puzzle?.seed || 1) + Math.imul(index + 1, 65537)) >>> 0;
  const random = seededRandom(mutationSeed);
  const mode = Math.floor(random() * 6);
  const replacements = ['cup','plant','clock','cat','fish','book','cake','gamepad','umbrella','apple','camera','flower','gift','star','heart','rocket','bottle','balloon'];
  if (mode === 0) return ''; // 물건 하나가 통째로 사라짐
  if (mode === 1) {
    let alt = replacements[Math.floor(random() * replacements.length)];
    if (alt === type) alt = replacements[(replacements.indexOf(alt) + 3) % replacements.length];
    return `<g transform="translate(${x} ${y})">${symbolMarkup(alt, accent)}</g>`; // 다른 물건으로 교체
  }
  if (mode === 2) return `<g transform="translate(${x} ${y}) rotate(90)">${base}</g>`; // 방향 변화
  if (mode === 3) return `<g transform="translate(${x - 16} ${y + 2}) scale(.82)">${base}</g><g transform="translate(${x + 24} ${y + 10}) scale(.58)">${base}</g>`; // 개수 변화
  if (mode === 4) return `<g transform="translate(${x} ${y})">${base}<path d="M-25 -25 25 25M-25 25 25-25" stroke="${accent}" stroke-width="6" stroke-linecap="round" opacity=".85"/></g>`; // 문양 변화
  return `<g transform="translate(${x} ${y}) scale(-1 1)">${base}</g>`; // 좌우 방향 변화
}
...
export function spotDifferenceHitIdAt(puzzle, xValue, yValue) {
  ...
  let best = null;
  const hitboxes = spotClientHitboxesForPuzzle(puzzle);
  for (const id of puzzle.differenceIds) {
    const base = hitboxes.find((item) => item.id === id);
    if (!base) continue;
...
export function renderSpotDifferenceAnswerMarks(puzzle, foundIds = [], revealAll = false) {
  ...
  return spotClientHitboxesForPuzzle(puzzle)
...
export function renderSpotDifferenceScene(puzzle, { changed = false, foundIds = [], revealAll = false, ariaLabel = '' } = {}) {
  ...
  const hitboxes = spotClientHitboxesForPuzzle(puzzle).map((base) => ({ ...base, x: puzzle.mirrored ? 1 - base.x : base.x }));
  const symbols = Array.from({ length: hitboxes.length }, (_, index) => symbolPool[index % symbolPool.length]);
  const differences = new Set(Array.isArray(puzzle.differenceIds) ? puzzle.differenceIds : []);
  const objects = hitboxes.map((hitbox, index) => renderCandidate(symbols[index], hitbox, index, Boolean(changed && differences.has(hitbox.id)), theme.accent, puzzle)).join('');
  const answers = renderSpotDifferenceAnswerMarks(puzzle, foundIds, revealAll);
