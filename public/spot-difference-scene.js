...
const SPOT_ATLAS_ASSET_VERSION = 3;
const SPOT_ATLAS_CACHE_VERSION = '610118';

export function spotDifferenceAssetUrls(puzzle) {
  const assetVersion = Number(puzzle?.assetVersion || 0);
  if (![SPOT_ATLAS_LEGACY_VERSION, SPOT_ATLAS_ASSET_VERSION].includes(assetVersion)) return null;
...
function dynamicAtlasDifferenceMarkup(puzzle) {
  if (Number(puzzle?.assetVersion || 0) !== SPOT_ATLAS_ASSET_VERSION) return '';
  const ids = new Set(Array.isArray(puzzle?.differenceIds) ? puzzle.differenceIds : []);
  const seedBase = Number(puzzle?.seed || 1);
  return SPOT_CLIENT_HITBOXES.filter((hitbox) => ids.has(hitbox.id)).map((base) => {
    const x = (puzzle.mirrored ? 1 - base.x : base.x) * W;
    const y = base.y * H;
    const random = seededRandom(seedBase + (Number(base.id.slice(1)) + 1) * 104729);
    const mode = Math.floor(random() * 6);
    const angle = Math.floor(random() * 70) - 35;
    const dark = random() > .5 ? '#111827' : '#334155';
    const light = random() > .5 ? '#f8fafc' : '#fef3c7';
    const accent = ['#b91c1c','#1d4ed8','#166534','#7c2d12'][Math.floor(random() * 4)];
    const tag = String((Math.floor(random() * 9) + 1));
    if (mode === 0) return `<g class="spot-dynamic-difference" transform="translate(${x} ${y}) rotate(${angle})"><rect x="-9" y="-5" width="18" height="10" rx="2" fill="${light}" stroke="${dark}" stroke-width="3"/><path d="M-5 0h10" stroke="${accent}" stroke-width="3" stroke-linecap="round"/></g>`;
    if (mode === 1) return `<g class="spot-dynamic-difference" transform="translate(${x} ${y})"><circle r="8" fill="${dark}"/><circle r="3" fill="${light}"/><circle cx="11" cy="-8" r="3.5" fill="${accent}"/></g>`;
    if (mode === 2) return `<g class="spot-dynamic-difference" transform="translate(${x} ${y}) rotate(${angle})"><path d="M0-10 10 8H-10Z" fill="${light}" stroke="${accent}" stroke-width="3"/><circle cy="2" r="2.5" fill="${dark}"/></g>`;
    if (mode === 3) return `<g class="spot-dynamic-difference" transform="translate(${x} ${y}) rotate(${angle})"><rect x="-10" y="-10" width="20" height="20" rx="5" fill="${light}" fill-opacity=".92" stroke="${dark}" stroke-width="2"/><text x="0" y="6" text-anchor="middle" font-family="system-ui,sans-serif" font-size="15" font-weight="900" fill="${accent}">${tag}</text></g>`;
    if (mode === 4) return `<g class="spot-dynamic-difference" transform="translate(${x} ${y}) rotate(${angle})"><path d="M-11-6h22M-11 0h16M-11 6h22" stroke="${light}" stroke-width="5" stroke-linecap="round"/><path d="M-11-6h22M-11 0h16M-11 6h22" stroke="${dark}" stroke-width="2" stroke-linecap="round"/></g>`;
    return `<g class="spot-dynamic-difference" transform="translate(${x} ${y}) rotate(${angle})"><circle r="10" fill="none" stroke="${light}" stroke-width="5"/><circle r="10" fill="none" stroke="${accent}" stroke-width="2.5" stroke-dasharray="7 5"/><path d="M-3 0h6" stroke="${dark}" stroke-width="3" stroke-linecap="round"/></g>`;
  }).join('');
}
...
export function renderSpotDifferenceScene(puzzle, { changed = false, foundIds = [], revealAll = false, ariaLabel = '' } = {}) {
  ...
  if (atlasUrls) {
    ...
    const dynamicDifferences = changed && assetVersion === SPOT_ATLAS_ASSET_VERSION ? dynamicAtlasDifferenceMarkup(puzzle) : '';
