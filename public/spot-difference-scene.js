...
export function spotClientHitboxesForPuzzle(puzzle) {
  ...
  if (assetVersion >= 6) {
    return SPOT_CLIENT_HITBOXES.map((item) => Object.freeze({ ...item, r: Math.max(0.048, item.r - 0.012) }));
  }
...
const SPOT_ATLAS_ASSET_VERSION = 6;
const SPOT_ATLAS_CACHE_VERSION = '610121';

export function spotDifferenceAssetUrls(puzzle) {
  const assetVersion = Number(puzzle?.assetVersion || 0);
  if (![SPOT_ATLAS_LEGACY_VERSION, 3, 4, SPOT_ATLAS_ASSET_VERSION].includes(assetVersion)) return null;
...
function dynamicAtlasDifferenceMarkup(puzzle, href) {
  if (![3, 4, SPOT_ATLAS_ASSET_VERSION].includes(Number(puzzle?.assetVersion || 0)) || !href) return '';
