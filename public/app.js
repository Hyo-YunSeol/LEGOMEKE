import { cloneBlockBattleVisualPlayer, reconcileBlockBattleVisual } from './block-battle-visual-state.js?v=6101260';
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (!appleOnlyUpdated && !blockOnlyUpdated && !blockInputProtected && !minesweeperOnlyUpdated && !blockBattleLivePatched && !sichuanLivePatched && !legodokuLivePatched) {
      render();
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=6101260" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=6101260`;
...
function patchLegodokuMarksDom(room = currentLegodokuRoom()) {
  ...
  document.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
    ...
    if (hasHead) {
      ...
      if (!cell.querySelector('.legodoku-head')) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101260" alt="" draggable="false">');
...
function patchLegodokuSpectatorBoardsDom(room = currentLegodokuRoom()) {
  ...
  root.querySelectorAll('[data-legodoku-player-board]').forEach((board) => {
    ...
    board.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
      ...
      if (hasHead && !existing) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101260" alt="" draggable="false">');
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=6101260`;
}
...
function sichuanRoomView(room) {
  ...
  if (room.status === 'waiting') {
    ...
    stage = `<div class="sichuan-match-head">${sichuanProgressHtml(self, { mine: true })}<div class="sichuan-match-center"><span>남은 시간</span>${timer}<small data-sichuan-spectator-count>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${sichuanProgressHtml(opponent)}</div><div class="sichuan-board-stage">${sichuanBoardHtml(self, { interactive })}<div class="sichuan-finish-status hidden" data-sichuan-finish-status>마지막 패 처리 중 · 승리 판정 확인 중</div>${spectatorBurstLayer(room.reactions || [], 'sichuan')}</div><p class="helper centered">같은 그림 두 개를 선택하세요. 최대 두 번 꺾어 연결할 수 있으며 판 바깥쪽 경로도 허용됩니다.</p>`;
  } else {
...
function queueBlockBattleInput(action) {
  ...
  if (unconfirmedActions >= BLOCK_BATTLE_MAX_UNCONFIRMED_ACTIONS) return false;
...
navigator.serviceWorker?.register?.('/sw.js?v=6101260')?.catch(() => {});
