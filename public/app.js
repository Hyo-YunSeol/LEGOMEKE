...
import { renderSpotDifferenceScene, spotDifferenceHitIdAt, spotDifferenceThemeLabel } from './spot-difference-scene.js?v=610114';
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610114" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610114`;
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610114`;
}
...
function spotDifferenceRoomView(room) {
  ...
  if (room.status === 'waiting') {
    ...
    const timerText = room.status === 'playing' ? '<b id="spot-difference-countdown">2:00</b>' : '<b>종료</b>';
...
navigator.serviceWorker?.register?.('/sw.js?v=610114')?.catch(() => {});
