...
import { preloadSpotDifferenceSceneAssets, renderSpotDifferenceAnswerMarks, renderSpotDifferenceScene, spotDifferenceHitIdAt, spotDifferenceThemeLabel } from './spot-difference-scene.js?v=610119';
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610119" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610119`;
...
function gamesView() {
  ...
  return `
    ...
    <section class="section personal-game-wrap">${sectionHeading('포인트 개인게임', `이번 게임 하루 ${app.data.dashboard.pet.daily.miniGamesPlayed}/${app.data.dashboard.pet.daily.miniGamesLimit || app.data.catalog.miniGamesPerDay}회`)}${resume}<div class="game-grid">${games.map((game) => `<article class="game-card"><div class="game-icon">${miniGameIcon(game.id)}</div><h3>${esc(game.name)}</h3><button class="primary wide" data-action="${active?.gameId === game.id ? 'resume-mini' : 'start-mini'}" data-id="${game.id}" type="button" ${active && active.gameId !== game.id ? 'disabled' : ''}>${active?.gameId === game.id ? (game.id === 'minesweeper' ? '게임으로' : '이어하기') : '시작'}</button></article>`).join('')}</div></section>
...
function omokLobby() {
  ...
  return `${sectionHeading('오목게임', `15×15 · 30초/수 · ${battleUsageText()}`, `<button class="primary" data-action="omok-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="omok-lobby-list">${cards}</div>`;
}
...
function blockBattleLobby() {
  ...
  return `${sectionHeading('테트리스대전', `10×20 · 1:1 · ${battleUsageText()}`, `<button class="primary" data-action="block-battle-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="block-battle-lobby-list">${cards}</div>`;
}
...
function sichuanLobby() {
  ...
  return `${sectionHeading('사천성대전', `2:30 · 막히면 패배 · ${battleUsageText()}`, `<button class="primary" data-action="sichuan-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="sichuan-lobby-list">${cards}</div>`;
}
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610119`;
}
...
function spotDifferenceLobby() {
  ...
  const canCreate = rooms.length < (game?.maxRooms || 3);
  return `${sectionHeading('틀린그림찾기', `7개 · 45초 · 오답 5회 · ${battleUsageText()}`, `<button class="primary" data-action="spot-difference-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="spot-lobby-list">${cards}</div>`;
}
...
function spotDifferenceRoomView(room) {
  ...
  const body = `${sectionHeading('틀린그림찾기', `${room.roomNumber}번방 · 판돈 ${points(room.stakePoints)} · ${spotDifferenceStatusLabel(room.status)}`, '<button class="text-button" data-action="spot-difference-back" type="button">로비 보기</button>')}${stage}<div class="button-row">${rematch}${leave}</div>${reaction}${resultOverlay}`;
...
function spotDifferenceLiveDomMatches(room, scope = document) {
  if (!room || room.status !== 'playing' || !room.puzzle) return false;
  const root = scope?.querySelector?.('.spot-room-view') || $('.spot-room-view');
  if (!root) return false;
  return String(root.dataset.spotRoomId || '') === String(room.id || '')
    && String(root.dataset.spotStatus || '') === 'playing'
    && String(root.dataset.spotPuzzleKey || '') === String(room.puzzle.key || '');
}

function patchSpotDifferenceLiveRoom(room) {
  if (!room || room.status !== 'playing' || !room.puzzle) return false;
  const root = $('.spot-room-view');
  if (!root || !spotDifferenceLiveDomMatches(room)) return false;
...
function davinciLobby() {
  ...
  return `${sectionHeading('다빈치코드', `2~4인 · 조커 포함 · ${battleUsageText()}`, `<button class="primary" data-action="davinci-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="davinci-lobby-list">${cards}</div>`;
}
...
function renderTab(tab, { force = false } = {}) {
  ...
  if (tab === 'games' && alreadyRendered) {
    ...
    const liveSpotDifferenceRoom = currentSpotDifferenceRoom();
    if (liveSpotDifferenceRoom?.status === 'playing'
      && spotDifferenceLiveDomMatches(liveSpotDifferenceRoom, pane)
      && patchSpotDifferenceLiveRoom(liveSpotDifferenceRoom)) {
      pane.dataset.revision = String(app.revision);
      updateAppChrome();
      return;
    }
...
function render() {
  ...
  const liveSpotDifferenceRoom = app.tab === 'games' ? currentSpotDifferenceRoom() : null;
  if (pane.dataset.rendered === 'true' && app.dirtyTabs.has('games')
    && liveSpotDifferenceRoom?.status === 'playing'
    && spotDifferenceLiveDomMatches(liveSpotDifferenceRoom, pane)
    && patchSpotDifferenceLiveRoom(liveSpotDifferenceRoom)) {
    requestAnimationFrame(syncGameResultScrollLock);
    return;
...
navigator.serviceWorker?.register?.('/sw.js?v=610119')?.catch(() => {});
