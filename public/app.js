...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610103" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610103`;
...
function restoreSingleTetrisState(challenge) {
  try {
    ...
    // v6.10.3: 싱글도 대전과 동일하게 새 블록은 0행에서 즉시 보인다.
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610103`;
}
...
navigator.serviceWorker?.register?.('/sw.js?v=610103')?.catch(() => {});
