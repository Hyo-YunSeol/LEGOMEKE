...
function applyBootstrap(next) {
  ...
  for (let index = 0; index < (app.data?.blockBattle?.rooms || []).length; index += 1) {
    ...
    blockBattleVisualSelf(room, { force: safeVisualReconcile });
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    const blockBattleLivePatched = Boolean(liveBlockBattleRoom?.status === 'playing' && patchBlockBattleDynamic(liveBlockBattleRoom));
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610111" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610111`;
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610111`;
}
...
function queueBlockBattleInput(action) {
  ...
  // lock/줄삭제/다음 블록 생성은 서버 ACK에서만 확정한다. 로컬에서 board까지 미리 바꾸면
  // 서버의 실제 garbage/line-clear 결과와 교차할 때 이전 블록이 잔상처럼 남을 수 있다.
  // hardDrop은 현재 active를 바닥 위치까지만 즉시 보여주고, 새 블록은 서버 확정 뒤 표시한다.
...
function renderTab(tab, { force = false } = {}) {
  ...
  if (tab === 'games' && alreadyRendered) {
    ...
    if (liveBlockBattleRoom?.status === 'playing'
      ...
      && patchBlockBattleDynamic(liveBlockBattleRoom)) {
...
function scheduleTabRender(tab, { afterPaint = false } = {}) {
  ...
  const run = () => {
    ...
    if (tab === 'games' && pane.dataset.rendered === 'true') {
      const liveBlockBattleRoom = currentBlockBattleRoom();
      if (liveBlockBattleRoom?.status === 'playing' && patchBlockBattleDynamic(liveBlockBattleRoom)) {
...
function render() {
  ...
  const liveBlockBattleRoom = app.tab === 'games' ? currentBlockBattleRoom() : null;
  if (pane.dataset.rendered === 'true' && app.dirtyTabs.has('games') && liveBlockBattleRoom?.status === 'playing' && patchBlockBattleDynamic(liveBlockBattleRoom)) {
    syncBlockBattleGravity();
    requestAnimationFrame(syncGameResultScrollLock);
    return;
  }
...
navigator.serviceWorker?.register?.('/sw.js?v=610111')?.catch(() => {});
