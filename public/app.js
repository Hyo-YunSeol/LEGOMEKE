...
async function api(path, options = {}) {
  ...
  const data = await response.json().catch(() => ({ ok: false, message: '서버 응답을 읽을 수 없습니다.' }));
...
function showBrokenPromptIfNeeded() {
  ...
  if (!pet?.integrity?.broken || app.brokenPromptPetId === pet.id || app.modal) return;
  app.brokenPromptPetId = pet.id;
  const nextGeneration = Math.max(1, Number(pet.generation) || 1) + 1;
  openModal(`${modalHeader('💥 레고가 부숴졌습니다.', `${pet.integrity.causeLabel || '반복된 무리한 생활'}로 현재 세대가 끝났습니다.`)}<div class="broken-modal"><p>${esc(pet.displayName)}의 결합부가 완전히 파손되었습니다.</p><strong>${nextGeneration}세대 레고로 다시 시작하시겠습니까?</strong><p class="warning-box">다음 세대를 시작하면 현재 포인트·레고력·레벨·몸집·관계·영토·상태메시지 등은 모두 초기화되고 과거 레고 기록만 남습니다.</p><button class="primary wide" data-action="restart-broken" type="button">🧱 ${nextGeneration}세대 레고 시작하기</button></div>`, { type: 'broken' });
}
...
function updateVisualViewportVars() {
  ...
  document.documentElement.style.setProperty('--visual-viewport-height', `${Math.max(1, height)}px`);
  document.documentElement.style.setProperty('--visual-viewport-top', `${Math.max(0, top)}px`);
}

function setLiarKeyboardMode(active) {
  const scrollSnapshot = captureLiarChatScrollState();
  const mobile = window.matchMedia?.('(max-width: 720px)')?.matches ?? window.innerWidth <= 720;
  app.liarKeyboardActive = Boolean(active && mobile && app.tab === 'games');
  document.body.classList.toggle('liar-keyboard-open', app.liarKeyboardActive);
  updateVisualViewportVars();
  requestAnimationFrame(() => restoreLiarChatScrollState(scrollSnapshot));
}
...
const BODY_STAGE_FALLBACK = [
  ...
  { min: 200, max: null, key: 'daeruk', label: '돼룩돼룩레고', activityHungerCost: 4 }
];
...
function avatar(stage, { mini = false } = {}) {
  ...
  return `<div class="pet-visual ${mini ? 'mini' : ''} ${stageClass}"><span class="pet-shadow"></span><img src="/pets/${esc(stage.key)}.svg" alt="${esc(stage.label)}" draggable="false"></div>`;
}
...
function lifeActionsLocked() {
  const daily = app.data?.dashboard?.pet?.daily;
  if (app.data?.dashboard?.pet?.integrity?.broken || !daily || daily.actionsLeft <= 0) return true;
...
function homeView() {
  ...
  const integrity = pet.integrity ?? { warnings: 0, maxWarnings: 3, broken: false };
  const broken = Boolean(integrity.broken);
  const breakPanel = broken
    ? `<div class="break-panel broken"><strong>💥 레고가 부숴졌습니다.</strong><p>${esc(integrity.causeLabel || '반복된 무리한 생활')}로 현재 세대가 종료되었습니다.</p><button class="primary wide" data-action="restart-broken" type="button">다음 세대 레고 시작하기</button></div>`
    : integrity.warnings > 0
      ? `<div class="break-panel warning"><strong>파손 경고 ${integrity.warnings}/${integrity.maxWarnings}</strong><p>${esc(integrity.stageMessage || '레고의 생활 균형을 확인해주세요.')}</p><small>한쪽 행동만 계속 반복하지 않고 균형 있게 지내면 경고를 줄일 수 있습니다.</small></div>`
      : '';
  return `
    <section class="hero-card">
      <div class="hero-main">${avatar(stage)}<div class="hero-copy"><span class="eyebrow"><button class="body-stage-link" data-action="body-guide" data-body="${pet.stats.body}" type="button">${esc(stage.label)}</button> · ${pet.generation}세대</span><h1>${esc(pet.displayName)}</h1><div class="status-message-row"><span class="status-message-text ${pet.statusMessage ? '' : 'muted'}">${esc(pet.statusMessage || '상태메시지 없음')}</span><button class="status-edit-button" data-action="edit-status-message" type="button" ${broken ? 'disabled' : ''}>수정</button></div><p class="system-status">${esc(pet.status)}</p><div class="hero-tags"><span>Lv.${pet.stats.level}</span><span>레고력 ${pet.stats.legoPower}</span><span>${pet.partnerPetId ? `커플 D+${pet.coupleDay}` : '솔로'}</span></div></div></div>
    ...
    ${breakPanel}
    <section class="section stat-panel">${bar('체력', pet.stats.stamina, { badLow: true })}${bar('배고픔', pet.stats.hunger, { badLow: true })}<div class="break-summary"><span>파손 경고 <b>${integrity.warnings}/${integrity.maxWarnings}</b></span><small>특정 행동을 여러 게임날 연속 과하게 반복하면 경고가 쌓입니다.</small></div></section>

    <section class="section">
      ${sectionHeading('생활 행동', '게임 하루 5회 · 30분 쿨타임', `<button class="soft-button" data-action="open-food" type="button" ${broken ? 'disabled' : ''}>🍚 밥 먹기</button>`)}
    ...
    <section class="section daily-goals-compact">
      ${sectionHeading('오늘의 레고력', '6시간마다 목표가 초기화됩니다.', `<span class="tag">${goals.completed}/${goals.total}</span>`)}
      ${goalList(goals)}
      <p class="helper daily-goals-note">벙 정상 종료마다 레고력 +1 추가</p>
    </section>

    <section class="section fishing-card">
      ${sectionHeading('30초 낚시', '미니게임 횟수와 별도입니다.', `<button class="text-button" data-action="show-fishing-rewards" type="button">낚이는 것 보기</button>`)}
      <div class="fishing-status"><div><strong id="fishing-state">${fishing ? '낚시 중' : '낚시 가능'}</strong><small id="fishing-countdown">${fishing ? durationText(new Date(fishing.readyAt).getTime() - Date.now()) : `남은 횟수 ${Math.max(0, fishingLeft)}/${app.data.catalog.fishingPerDay}`}</small></div><button class="primary" data-action="start-fishing" type="button" ${broken || fishing || fishingLeft <= 0 ? 'disabled' : ''}>낚시 시작</button></div>
...
function rankingsSection() {
  ...
  return `<section class="section ranking-section">${sectionHeading('레고방 순위', '포인트·레벨·커플·찌르기 현황')}<div class="rank-tabs-grid"><article class="rank-card"><h3>포인트 TOP 5</h3>${rankRows(rankings.points, (item) => points(item.points))}</article><article class="rank-card"><h3>레벨 TOP 5</h3>${rankRows(rankings.levels, (item) => `Lv.${item.level} · ${item.legoPower}`)}</article><article class="rank-card"><h3>커플</h3>${couples}</article><article class="rank-card"><h3>찌르기 TOP 5</h3>${pokes}</article></div></section>`;
}
...
function appleRankingCard() {
  const ranking = app.data.rankings?.apple ?? [];
  const rows = ranking.length
    ? ranking.map((item) => `<div class="rank-row"><b>${item.rank}</b><button data-action="profile" data-id="${item.petId}" type="button">${esc(item.displayName)}</button><strong>${Number(item.score || 0).toLocaleString('ko-KR')}점</strong></div>`).join('')
    : '<div class="empty">아직 사과게임 기록이 없습니다.</div>';
  return `<article class="rank-card apple-rank-card"><h3>🍎 사과게임 TOP 5</h3>${rows}</article>`;
}
...
function gamesView() {
  ...
  const resume = active
    ...
    : '';
  return `
    <section class="page-title"><span class="eyebrow">포인트 게임</span><h1>게임</h1><p>라이어게임·오목게임·개인게임을 즐길 수 있습니다.</p></section>
    <section class="section liar-wrap">${liarSection()}</section>
    <section class="section omok-wrap">${omokSection()}</section>
    <section class="section">${sectionHeading('개인게임', `이번 게임 하루 ${app.data.dashboard.pet.daily.miniGamesPlayed}/${app.data.catalog.miniGamesPerDay}회`)}${resume}<div class="game-grid">${games.map((game) => `<article class="game-card"><div class="game-icon">${miniGameIcon(game.id)}</div><h3>${esc(game.name)}</h3><p>${esc(game.description)}</p><button class="primary wide" data-action="${active?.gameId === game.id ? 'resume-mini' : 'start-mini'}" data-id="${game.id}" type="button" ${active && active.gameId !== game.id ? 'disabled' : ''}>${active?.gameId === game.id ? '이어하기' : '시작'}</button></article>`).join('')}</div><div class="single-rank-grid">${appleRankingCard()}</div></section>
  `;
}
...
function omokLobbyRanking() {
  const rows = app.data.omok?.ranking ?? [];
  return rows.length
    ? rows.map((item) => `<div class="rank-row"><b>${item.rank}</b><button data-action="profile" data-id="${item.petId}" type="button">${esc(item.displayName)}</button><strong>${item.wins}승 ${item.draws}무 ${item.losses}패</strong></div>`).join('')
    : '<div class="empty">아직 오목 전적이 없습니다.</div>';
}
...
function omokLobby() {
  ...
  return `${sectionHeading('오목게임', '15×15 · 1:1 실시간 대전 · 한 수 30초', `<button class="primary" data-action="omok-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="omok-lobby-list">${cards}</div><div class="single-rank-grid"><article class="rank-card"><h3>오목 TOP 5</h3>${omokLobbyRanking()}</article></div>`;
}
...
function omokRoomView(room) {
  ...
  const board = room.board.map((line, row) => line.map((cell, col) => `<button class="omok-cell ${cell || ''}" data-action="omok-move" data-row="${row}" data-col="${col}" type="button" ${!isPlayer || room.status !== 'playing' || !room.isMyTurn || cell ? 'disabled' : ''} aria-label="${row + 1}행 ${col + 1}열">${cell === 'black' ? '<span class="omok-stone black"></span>' : cell === 'white' ? '<span class="omok-stone white"></span>' : ''}</button>`).join('')).join('');
...
function liarSection() {
  ...
  if (spectator) {
    ...
    return `${sectionHeading('라이어게임 관전', `${liarPhaseLabel(game.phase)} · 관전자 ${game.spectatorCount || 0}명`, game.phaseEndsAt ? '<span id="liar-countdown" class="tag"></span>' : '')}<div class="liar-layout"><div><div class="liar-player-list">${players}</div><div class="spectator-card"><strong>👀 관전 중</strong><p>투표·정답·게임 조작·채팅은 사용할 수 없습니다. 제시어와 라이어 정보는 게임 종료 전 서버에서 전송되지 않습니다.</p></div>${spectatorResult}<button class="ghost" data-action="liar-spectate-leave" type="button">관전 나가기</button></div>${liarChat(game, me.id, true)}</div>`;
  }
  ...
  return `${sectionHeading('라이어게임', `${liarPhaseLabel(game.phase)} · 1라운드`, game.phaseEndsAt ? '<span id="liar-countdown" class="tag"></span>' : '')}<div class="liar-layout"><div><div class="liar-player-list">${players}</div>${waitingControls}${role}${vote}${guess}${result}${gameOver}</div>${liarChat(game, me.id, false)}</div>`;
}
...
function territoryViewPage() {
  ...
  const rows = Array.from({ length: mapSize }, (_, row) => {
    const rowCells = Array.from({ length: mapSize }, (_, col) => {
      ...
      const title = cell ? `${cell.ownerDisplayName}${home ? ' · 본진' : ''}${golden ? ' · 황금 영토' : ''}` : (golden ? '황금 영토 · 빈 땅' : '빈 땅');
  ...
  const goldenStatus = territory.goldenRevealed
    ? '<b>👑 황금 영토 공개됨</b>'
    : `👑 황금 영토 공개까지 <b id="territory-golden-countdown">${durationText(new Date(territory.goldenRevealAt).getTime() - Date.now())}</b>`;
  return `<section class="page-title"><span class="eyebrow">6시간 시즌</span><h1>제${territory.seasonNumber}회 레고 영토전</h1><p>본진을 지키면서 주변 8칸으로 땅을 넓히고 상대 영토를 빼앗는 게임입니다.</p></section><section class="section territory-summary"><div class="metric-grid">${compactMetric('남은 시간', `<span id="territory-countdown">${durationText(new Date(territory.endsAt).getTime() - Date.now())}</span>`, '', { raw: true })}${compactMetric('내 레벨', `Lv.${territory.my.level}`)}${compactMetric('내 영토', `${territory.my.owned}/${territory.my.limit}`)}${compactMetric('현재 순위', territory.my.rank ? `${territory.my.rank}위` : '-')}</div><div class="territory-golden-status">${goldenStatus}</div><div class="territory-rule"><b>룰:</b> 첫 회색칸은 🏠 본진이 되어 빼앗기지 않습니다. 이후 내 영토의 앞·뒤·옆·대각선 주변 8칸만 점령/탈취할 수 있습니다. 한도가 꽉 차면 가장 오래된 일반 영토가 자동으로 비워지며 본진은 유지됩니다.</div></section><section class="section territory-section">${sectionHeading('공용 맵', `${mapSize} × ${mapSize}`)}<div class="territory-scroll"><div class="territory-map" role="grid" aria-label="${mapSize}행 ${mapSize}열 레고 영토전 맵">${rows}</div></div><p class="helper centered">회색칸은 빈 땅, 색깔 닉네임은 소유자, 🏠 표시는 빼앗을 수 없는 본진입니다.</p></section><section class="section">${sectionHeading('현재 순위', '단독 1위만 300P를 받습니다.')}<div class="rank-list">${ranking}</div></section>`;
}
...
function profileMiniCard(profile) {
  ...
  return `<button class="profile-mini" data-action="profile" data-id="${profile.id}" type="button">${avatar(stage, { mini: true })}<strong>${esc(profile.displayName)}</strong>${profile.statusMessage ? `<span class="profile-status-message">${esc(profile.statusMessage)}</span>` : ''}<small>Lv.${profile.stats.level} · ${points(profile.stats.points)}</small></button>`;
}
...
function recordsView() {
  ...
  const integrity = pet.integrity ?? { warnings: 0, maxWarnings: 3 };
  return `<section class="page-title"><span class="eyebrow">기록</span><h1>${esc(pet.displayName)}의 기록</h1><p>현재 레고의 핵심 기록만 표시합니다.</p></section><section class="section"><div class="record-grid">${compactMetric('세대', `${pet.generation}세대`)}${compactMetric('생존', `${records.days}게임일`)}${compactMetric('레벨', `Lv.${pet.stats.level}`)}${compactMetric('레고력', `${pet.stats.legoPower}`)}${compactMetric('포인트', points(pet.stats.points))}${compactMetric('최고 포인트', points(records.maxPoints))}${compactMetric('현재 경고', `${pet.warnings}회`)}${compactMetric('누적 경고', `${pet.records.warnings}회`)}${compactMetric('파손 경고', `${integrity.warnings}/${integrity.maxWarnings}`)}${compactMetric('영토전 우승', `${records.territoryWins}회`)}${compactMetric('라이어 승리', `${records.liarWins}회`)}${compactMetric('홀짝 최고', `${records.oddEvenBest}연승`)}${compactMetric('낚시', `${records.fishing}회`)}</div></section><section class="section">${sectionHeading('과거 레고', '파손·강퇴 등으로 끝난 이전 세대')}<div class="history-list">${history.length ? history.map((item) => `<article><div><strong>${esc(item.displayName)}</strong><small>${esc(item.endReason || '종료')} · ${dateText(item.endedAt)}${item.endDetail ? ` · ${esc(item.endDetail)}` : ''}</small></div><span>최고 Lv.${item.records?.maxLevel || 1} · ${points(item.records?.maxPoints || 0)}</span></article>`).join('') : '<div class="empty">아직 과거 레고가 없습니다.</div>'}</div></section>${app.data.admin.isAdmin ? `<section class="section admin-callout"><div><h2>운영자 관리</h2><p>일반 경고, 강퇴, 상태 초기화와 라이어게임 관리를 할 수 있습니다.</p></div><button class="primary" data-action="open-admin" type="button">관리 열기</button></section>` : ''}<section class="section"><button class="danger-button wide" data-action="logout" type="button">모든 기기에서 로그아웃</button></section>`;
}
...
function openBodyGuide(bodyValue = app.data?.dashboard?.pet?.stats?.body) {
  ...
  const cards = bodyStages().map((stage, index) => `<article class="body-guide-card ${stage.key === current.key ? 'current' : ''}"><span class="body-guide-step">${index + 1}단계</span>${avatar(stage, { mini: true })}<div><strong>${esc(stage.label)}</strong><small>몸집 ${esc(bodyRangeLabel(stage))}</small><small>게임·교미·영토 등 활동 시 배고픔 -${Math.max(1, Number(stage.activityHungerCost) || 1)}</small>${stage.key === current.key ? '<b>현재 단계</b>' : ''}</div></article>`).join('');
...
function openFoodShop() {
  ...
  openModal(`${modalHeader('음식 상점', `보유 포인트 ${points(app.data.dashboard.pet.stats.points)}`)}<div class="food-grid">${foods.map((food) => `<article class="food-card"><div><strong>${esc(food.name)}</strong><p>${esc(food.description)}</p><small>배고픔 +${food.hunger} · 몸집 +${food.body}</small></div><button class="primary" data-action="eat" data-id="${food.id}" type="button">${points(food.price)}</button></article>`).join('')}</div>`, { type: 'food' });
}

function openFishingRewards() {
  openModal(`${modalHeader('낚이는 것', '30초 후 아래 결과 중 하나가 나옵니다.')}<div class="reward-grid">${app.data.catalog.fishingRewards.map((item) => `<article><strong>${esc(item.label)}</strong><span>${item.reward ? points(item.reward) : '꽝'}</span></article>`).join('')}</div>`, { type: 'fishingRewards' });
}
...
async function openProfile(petId) {
  try {
    ...
    openModal(`${modalHeader(profile.displayName, profile.relationshipLabel || '')}<div class="profile-detail">${avatar(stage)}<button class="body-stage-profile-link" data-action="body-guide" data-body="${profile.stats.body}" type="button">${esc(stage.label)} · 몸집 ${profile.stats.body} · 단계 보기</button>${profile.statusMessage ? `<p class="profile-status-detail">${esc(profile.statusMessage)}</p>` : ''}<div class="metric-grid">${compactMetric('포인트', points(profile.stats.points))}${compactMetric('레벨', `Lv.${profile.stats.level}`)}${compactMetric('레고력', `${profile.stats.legoPower}`)}${compactMetric('몸집', `${profile.stats.body}`)}${compactMetric('경고', `${profile.warnings}회`)}${compactMetric('파손 경고', `${profile.integrity?.warnings ?? 0}/${profile.integrity?.maxWarnings ?? 3}`)}${compactMetric('상태', profile.partnerPetId ? `커플 D+${profile.coupleDay}` : '솔로')}</div>${relationshipActions}</div>`, { type: 'profile', petId });
...
function openMiniGame(challenge) {
  ...
  if (challenge.gameId === 'reaction') content = `<div class="mini-center"><div class="game-icon big">⚡</div><h3 id="reaction-title">신호를 기다리세요</h3><p>너무 일찍 누르면 0P입니다.</p><button id="reaction-button" class="reaction-button" data-action="finish-mini" data-value="1" type="button">기다리세요</button></div>`;
...
function openAdmin() {
  ...
  const members = admin.members.map((member) => `<article class="admin-member"><div><strong>${esc(member.displayName)}</strong><small>Lv.${member.level} · ${points(member.points)} · 일반 경고 ${member.warnings} · 누적 ${member.warningTotal} · 파손 경고 ${member.breakWarnings ?? 0}/${app.data.catalog.breakWarningMax ?? 3}${member.broken ? ' · 💥 파손' : ''}</small></div><div class="admin-buttons"><button data-action="admin-warning" data-id="${member.petId}" data-value="1" type="button">경고+</button><button data-action="admin-warning" data-id="${member.petId}" data-value="-1" type="button">경고-</button><button data-action="admin-reset-user" data-id="${member.petId}" type="button">상태 초기화</button><button class="danger-button" data-action="admin-kick" data-id="${member.petId}" type="button">강퇴</button></div></article>`).join('');
  ...
  openModal(`${modalHeader('운영자 관리', `내 User ID: ${admin.userId}`)}<div class="admin-top"><button class="warning-button wide" data-action="admin-force-liar" type="button">진행 중 라이어게임 강제 종료</button></div><h3>회원 관리</h3><div class="admin-list">${members || '<div class="empty">회원이 없습니다.</div>'}</div><h3>라이어 채팅 관리</h3><div class="admin-list">${chats || '<div class="empty">채팅이 없습니다.</div>'}</div>`, { type: 'admin' });
}
...
function updateReactionButton() {
  ...
  const ready = Date.now() >= challenge.readyAt;
  button.textContent = ready ? '지금!' : '기다리세요';
  button.classList.toggle('ready', ready);
  title.textContent = ready ? '지금 누르세요!' : '신호를 기다리세요';
}
...
async function tick() {
  ...
  if (appleCountdown && appleChallenge?.gameId === 'apple') {
    ...
    if (remaining <= 0 && !app.appleFinishInFlight) {
      ...
      try { await loadBootstrap({ silent: true }); } finally { app.appleFinishInFlight = false; }
    }
  }
  ...
  const territoryGoldenCountdown = $('#territory-golden-countdown');
  if (territoryGoldenCountdown) territoryGoldenCountdown.textContent = durationText(new Date(app.data.territory.goldenRevealAt).getTime() - Date.now());
  updateReactionButton();
}
...
async function handleAction(button) {
  ...
  if (action === 'finish-mini') return perform('/api/minigames/finish', { challengeId: app.data.activeMiniChallenge?.id, value: button.dataset.value });
  if (action === 'stop-mini') return perform('/api/minigames/stop', { challengeId: app.data.activeMiniChallenge?.id });
  ...
  if (action === 'restart-broken') {
    if (!confirm('현재 레고의 모든 진행 상태를 초기화하고 다음 세대로 다시 시작할까요? 과거 레고 기록은 남습니다.')) return;
    const result = await perform('/api/life/restart', {});
    if (result?.ok) {
      app.brokenPromptPetId = null;
      closeModal();
      app.tab = 'home';
...
document.addEventListener('submit', async (event) => {
  ...
  if (form.id === 'number-game-form') await perform('/api/minigames/finish', { challengeId: app.data.activeMiniChallenge?.id, value: data.guess });
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=640').catch(() => {});
