...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69501" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69501`;
...
const SINGLE_TETRIS_STORAGE_KEY = 'lego_single_tetris_state_v1';
...
function singleTetrisSpawn(state) {
  ...
  state.active = { type, rotation: 0, row: -1, col: 3 };
  singleTetrisFillQueue(state);
...
function sichuanBoardHtml(player, { interactive = false, compact = false } = {}) {
  ...
  return `<div class="sichuan-board ${compact ? 'compact' : ''}" role="grid" aria-label="${esc(player?.displayName || '플레이어')} 사천성 판">${board.map((tileId, index) => {
    if (!tileId) return `<span class="sichuan-cell empty" role="gridcell" aria-hidden="true"></span>`;
    const tile = tiles.get(tileId);
    const selected = !compact && Number(app.sichuanSelectedIndex) === index;
    const tileSrc = tile?.src ? `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=69501` : '';
    return `<button class="sichuan-cell ${selected ? 'selected' : ''}" data-action="sichuan-tile" data-index="${index}" data-tile="${esc(tileId)}" type="button" ${interactive ? '' : 'disabled'} aria-label="${esc(tile?.label || tileId)}${selected ? ' 선택됨' : ''}"><img src="${esc(tileSrc)}" alt="" draggable="false" loading="eager" decoding="sync"><span>${esc(tile?.label || '')}</span></button>`;
  }).join('')}</div>`;
}
...
navigator.serviceWorker?.register?.('/sw.js?v=69501')?.catch(() => {});
