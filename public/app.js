...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69103" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69103`;
...
async function drainMinesweeperActionQueue() {
  ...
  const cell = challenge.board?.[item.row]?.[item.col];
  if (!cell || (item.action === 'reveal' && (cell.revealed || cell.flagged)) || (item.action === 'flag' && cell.revealed)) {
    item.resolve?.(null);
    return drainMinesweeperActionQueue();
  }
...
const MINESWEEPER_MOVE_CANCEL_PX = 12;
...
navigator.serviceWorker?.register?.('/sw.js?v=69103')?.catch(() => {});
