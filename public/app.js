...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610102" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610102`;
...
function restoreSingleTetrisState(challenge) {
  try {
    ...
    // v6.10.2: 싱글도 대전과 동일하게 새 블록은 0행에서 즉시 보인다.
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610102`;
}
...
function syncAppleGameLayout() {
  ...
  if (!modal || !game || !board) return;
  const fixedNodes = [
    $('.apple-abandon-controls', game), $('.apple-hud', game), $('.apple-rules', game),
    $('#apple-selection-info', game), $('#apple-refresh-region', game)
  ];
  const fixedHeight = fixedNodes.reduce((sum, node) => sum + miniGameOuterHeight(node), 0);
  const boardStyle = getComputedStyle(board);
  const boardMargins = (parseFloat(boardStyle.marginTop) || 0) + (parseFloat(boardStyle.marginBottom) || 0);
  // game.clientHeight is the real flex-space left after the modal header. Using it avoids
  // double-counting the header and keeps the bottom refresh buttons inside the visual viewport.
  const availableHeight = Math.max(1, game.clientHeight - fixedHeight - boardMargins - 4);
  const availableWidth = Math.max(1, game.clientWidth);
...
navigator.serviceWorker?.register?.('/sw.js?v=610102')?.catch(() => {});
