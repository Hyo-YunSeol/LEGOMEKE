...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (!chatOnlyUpdated && !composerProtected && !appleOnlyUpdated) {
      render();
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=672" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=672`;
...
function shopActionFor(item, shop) {
  if (item.id === 'miniGame10' && shop.miniGameTicketBought) return '<button class="primary" type="button" disabled>오늘 구매 완료</button>';
  if (item.id === 'fishing5' && shop.fishingTicketBought) return '<button class="primary" type="button" disabled>오늘 구매 완료</button>';
...
function shopStatusFor(item, shop) {
  if (item.id === 'miniGame10' && shop.miniGameTicketBought) return `오늘 구매 완료 · +${shop.miniGameBonus}회 · 총 ${shop.miniGamesLimit}회`;
  if (item.id === 'fishing5' && shop.fishingTicketBought) return `오늘 구매 완료 · +${shop.fishingBonus}회 · 총 ${shop.fishingLimit}회`;
...
function blockBoardHtml(challenge) {
  ...
  return (challenge.blockBoard ?? []).map((row, rowIndex) => row.map((value, colIndex) => {
    if (value == null) return '<span class="block-cell empty" aria-hidden="true"></span>';
    const color = Math.max(0, Math.min(4, Number(value) || 0));
    return `<button class="block-cell color-${color}" data-action="block-select" data-row="${rowIndex}" data-col="${colIndex}" data-version="${version}" type="button" aria-label="${BLOCK_COLOR_NAMES[color]} 블록"><span>${BLOCK_SYMBOLS[color]}</span></button>`;
  }).join('')).join('');
}
...
function syncBlockGameLayout() {
  ...
  const fixed = ['.modal-head', '.block-hud', '.block-rules', '.block-reward-guide', '.block-helper']
...
function openMiniResult(result, challenge, { previousBest = 0 } = {}) {
  ...
  if (gameId === 'apple') {
    ...
    headline = allClear ? 'ALL CLEAR!' : '게임 종료';
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=672').catch(() => {});
