...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610110" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610110`;
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610110`;
}
...
function replayBlockBattlePendingInputs(room, { paint = true } = {}) {
  ...
  // 자동 tick은 로컬에서 선행 표시하지 않으므로 상대 push/bootstrap 수신 때도 재생하지 않는다.
  // ACK 전 tick을 replay하면 서버의 아직 이전 행인 snapshot 위에 다시 한 칸을 더 그리게 되어
  // push 순서에 따라 아래/위가 교차하는 시각적 반짝임이 생길 수 있다.
  const replayablePendingActions = pendingActions.filter((action) => action !== 'tick');
  ...
  const actions = awaitingLock ? replayablePendingActions : [...replayablePendingActions, ...app.blockBattleInputBuffer.filter((action) => action !== 'tick')];
  ...
  if (predictedLock) {
    ...
    for (const action of replayablePendingActions) previewBlockBattleInput(action, { room, paint: false });
    const reconstructed = previewBlockBattleLock(room, { paint: false, mark: false });
    if (reconstructed) {
      const bufferedReplay = app.blockBattleInputBuffer.filter((action) => action !== 'tick');
      for (const action of bufferedReplay) previewBlockBattleInput(action, { room, paint: false });
      replayedActions = [...replayablePendingActions, ...bufferedReplay];
    } else {
      app.blockBattlePredictedLock = null;
      replayedActions = replayablePendingActions;
    }
  } else {
  ...
  const hasPredictedGravityAction = replayedActions.some((action) => ['softDrop', 'hardDrop'].includes(action))
    || pendingActions.includes('tick') || app.blockBattleInputBuffer.includes('tick');
...
function queueBlockBattleInput(action) {
  ...
  // 자동 중력 tick은 서버 확정 전에는 화면을 미리 움직이지 않는다.
  // 1:1에서는 상대 상태 push와 내 tick ACK가 교차할 수 있어 로컬 선행 낙하를 재생하면
  // 같은 블록이 한 칸 되감겼다가 다시 내려오는 것처럼 보여 PC에서 반짝임이 발생할 수 있다.
  // 수동 soft/hard drop만 기존의 즉시 반응/lock 예측을 유지한다.
  const willLock = action === 'hardDrop'
    || (action === 'softDrop' && blockBattleVerticalWouldLock(room));
  ...
  // tick은 여기서 paint/preview하지 않는다. 서버가 정확히 한 번 적용한 authoritative
  // snapshot이 도착했을 때만 한 칸 내려 보이게 해서 자동 낙하의 이중 렌더를 제거한다.
...
navigator.serviceWorker?.register?.('/sw.js?v=610110')?.catch(() => {});
