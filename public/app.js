...
function avatar(stage, { mini = false } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=653`;
...
function warmInactiveTabs() {
  clearTimeout(app.tabWarmTimer);
  // 최초 1회만 빈 탭을 미리 만들어 둔다. 이미 렌더된 dirty 탭은 눌렀을 때만 갱신해 실시간 업데이트가 전체 화면을 계속 재생성하지 않게 한다.
  const queue = MAIN_TABS.filter((tab) => tab !== app.tab && !app.renderedTabs.has(tab));
  const next = () => {
    const tab = queue.shift();
    if (!tab || !app.data) return;
    renderTab(tab);
    if (queue.length) app.tabWarmTimer = setTimeout(next, 45);
  };
  if (queue.length) app.tabWarmTimer = setTimeout(next, 60);
}
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=653').catch(() => {});
