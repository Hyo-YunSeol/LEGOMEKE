...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=682" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=682`;
...
function renderBlockBattleRegion() {
  ...
  app.blockBattleRenderFrame = 0;
  app.blockBattleNeedsLayout = false;
  // 누르고 있던 조작 버튼이 DOM 교체로 사라지면 pointerup을 놓칠 수 있으므로
  // 반복 입력 타이머를 먼저 정리한다.
  stopBlockBattleHold();
...
function scheduleBlockBattleDomUpdate(needsLayout = false) {
  ...
  app.blockBattleRenderFrame = requestAnimationFrame(() => {
    ...
    else if (room) for (const player of Object.values(room.players || {})) paintBlockBattleBoard(player);
  });
}

function blockBattleLayoutSignature(room) {
  const players = Object.values(room?.players || {}).map((player) => [
    player.petId, player.connected, player.lines, player.attackSent, player.pendingGarbage
  ].join(':')).sort().join('|');
  const reactions = (room?.reactions || []).map((item) => item.reactionId).join(':');
  return [room?.status, room?.viewerRole, room?.spectatorCount, room?.winnerPetId, room?.resultReason,
    room?.lastAttack?.at, players, reactions].join('~');
}
...
function queueBlockBattleInput(action) {
  ...
  const room = currentBlockBattleRoom();
  if (app.tab !== 'games' || !navigator.onLine || blockBattleInputStopped(room)) return;
  ...
  if (action === 'tick') {
    ...
    if (tickPending || awaitingHardDrop) return;
  } else {
  ...
  if (app.blockBattleInputBuffer.length >= 48) return;
  if (action !== 'tick' && !awaitingHardDrop) {
    const changed = previewBlockBattleInput(action);
    // 벽에 닿은 좌우/회전 입력은 서버에서도 결과가 같으므로 75ms 반복 전송하지 않는다.
    if (!changed && ['left', 'right', 'rotate'].includes(action)) return;
...
function territoryViewPage() {
  ...
  const reentryNotice = battleUnlocked && Number(territory.my?.owned || 0) === 0
    ? `<div class="warning-box">⚔️ 현재 내 영토가 없습니다. 빈칸이 있으면 무료로 새 본진을 세울 수 있고, 맵이 꽉 찼다면 상대 건물을 눌러 ${Number(app.data?.catalog?.territoryStealCost || 50)}P로 빼앗아 🏠 새 본진을 세울 수 있습니다.</div>`
    : '';
  ...
  return `<section class="page-title"><span class="eyebrow">6시간 시즌</span><h1>제${territory.seasonNumber}회 레고 영토전</h1><p>본진을 지키면서 주변 8칸으로 땅을 넓히고 상대 영토를 빼앗는 게임입니다.</p></section>${previousResult}<section class="section territory-summary"><div class="metric-grid">${compactMetric('남은 시간', `<span id="territory-countdown">${durationText(new Date(territory.endsAt).getTime() - serverNow)}</span>`, '', { raw: true })}${compactMetric('내 레벨', `Lv.${territory.my.level}`)}${compactMetric('내 영토', `${territory.my.owned}/${territory.my.limit}`)}${compactMetric('현재 순위', territory.my.rank ? `${territory.my.rank}위` : '-')}</div><div class="territory-next-reset"><strong>다음 초기화: ${territoryResetText(territory.endsAt)}</strong></div><div class="territory-limit-strip"><span>${esc(territoryLimitHint)}</span><button class="soft-button" data-action="territory-limits" type="button">ⓘ 레벨별 한도</button></div><div class="territory-golden-status">${goldenStatus}</div><div class="territory-golden-status"><b>${battleUnlocked ? '⚔️ 전면전 진행 중 · 본진 탈취 가능' : `🛡️ 본진 보호 중 · ${territory.cells.length}/${mapSize * mapSize}칸 점유`}</b></div>${reentryNotice}<div class="territory-rule"><b>안내:</b> 영토전은 한국시간 00시·06시·12시·18시에 새 회차가 시작됩니다. 점령 시각부터 6시간을 계산하는 방식이 아니며, 현재 영토는 표시된 다음 초기화 시각까지 유지됩니다.</div><div class="territory-rule"><b>룰:</b> 빈 땅 점령은 무료, 상대 땅 탈취는 50P입니다. 처음에는 🏠 본진이 보호됩니다. 맵 25칸이 한 번이라도 모두 차면 ⚔️ 전면전이 열려 본진도 탈취할 수 있습니다. 25칸이 모두 각자의 본진이어도 예외 없이 서로 탈취할 수 있으며, 신규 참가자나 영토가 0칸이 된 사람은 상대 건물을 50P로 빼앗아 새 본진으로 다시 참가할 수 있습니다. 보유 한도가 꽉 차면 빈 땅 추가 점령은 막히지만, 상대 땅은 50P를 내고 가장 오래된 일반 영토 한 칸을 옮기는 방식으로 계속 탈취할 수 있습니다.</div></section><section class="section territory-section">${sectionHeading('공용 맵', `${mapSize} × ${mapSize}`)}<div class="territory-scroll"><div class="territory-map" role="grid" aria-label="${mapSize}행 ${mapSize}열 레고 영토전 맵">${rows}</div></div><p class="helper centered">회색칸은 빈 땅, 색깔 닉네임은 소유자입니다. 🏠 본진은 전면전 전까지만 보호되며, 25/25칸이 모두 차면 이후 탈취 가능합니다. 황금 영토 위치는 종료 전에는 공개되지 않습니다.</p></section><section class="section">${sectionHeading('현재 순위', '단독 1위만 500P를 받습니다. 동률이면 종료 후 공개되는 황금 영토가 최종 승자를 가릅니다.')}<div class="rank-list">${ranking}</div></section>`;
}
...
document.addEventListener('pointerdown', (event) => {
  ...
  if (blockControl && event.isPrimary !== false && (event.pointerType !== 'mouse' || event.button === 0)) {
    ...
    const room = currentBlockBattleRoom();
    const holdKey = room ? `${room.id}:${room.matchId}` : '';
    ...
    if (['left', 'right', 'softDrop'].includes(action)) {
      app.blockBattleHoldDelay = setTimeout(() => {
        const repeat = () => {
          ...
          if (!blockControl.isConnected || document.hidden || app.tab !== 'games'
            || !current || `${current.id}:${current.matchId}` !== holdKey || blockBattleInputStopped(current)) {
            stopBlockBattleHold();
            return;
          }
          queueBlockBattleInput(action);
          app.blockBattleHoldTimer = setTimeout(repeat, 75);
        };
        repeat();
      }, 180);
    }
...
document.addEventListener('lostpointercapture', stopBlockBattleHold, { passive: true });
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=682').catch(() => {});
