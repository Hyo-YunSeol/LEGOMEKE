...
function previewBlockBattleLock(room = currentBlockBattleRoom(), { paint = true, mark = true } = {}) {
  ...
  attack -= cancelled;
  // 방해줄의 구멍 위치는 서버에서 확정되지만, 그 ACK를 기다리느라 다음 블록 표시까지
  // 멈추면 네트워크 지연이 그대로 체감된다. 화면용 visualSelf에서는 lock/줄삭제/다음 블록
  // 생성까지만 즉시 예측하고, 실제 방해줄 모양과 최종 board는 ACK 수신 시 서버 상태로 교정한다.
  // 게임 규칙/서버 판정은 전혀 바꾸지 않는다.
...
function queueBlockBattleInput(action) {
  ...
  // ACK가 오면 기존 force reconcile 경로가 실제 방해줄/공격/board 결과로 정확히 교정한다.
