...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69301" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69301`;
...
function miniGameIcon(gameId) {
  return ({ oddEven: '🌓', apple: '🍎', minesweeper: '💣', block: '🧱' })[gameId] || '🎮';
}
...
function gamesView() {
  ...
  const resume = active
    ...
    : '';
  return `
    <section class="page-title"><span class="eyebrow">포인트 게임</span><h1>게임</h1><p>개인게임으로 포인트를 벌고, 단체게임에서 오목·테트리스대전·사천성·다빈치코드를 즐길 수 있습니다.</p></section>
...
function omokRoomView(room) {
  ...
  const resultText = room.status === 'ended' ? room.result === 'draw' ? '무승부 · 판돈 반환' : `${esc(room.winnerPetId === room.black?.petId ? room.black?.displayName : room.white?.displayName || '승자')} 승리` : null;
...
function sichuanBoardHtml(player, { interactive = false, compact = false } = {}) {
  ...
  return `<div class="sichuan-board ${compact ? 'compact' : ''}" role="grid" aria-label="${esc(player?.displayName || '플레이어')} 사천성 판">${board.map((tileId, index) => {
    ...
    return `<button class="sichuan-cell ${selected ? 'selected' : ''}" data-action="sichuan-tile" data-index="${index}" data-tile="${esc(tileId)}" type="button" ${interactive ? '' : 'disabled'} aria-label="${esc(tile?.label || tileId)}${selected ? ' 선택됨' : ''}"><img src="${esc(tile?.src || '')}" alt="" draggable="false"><span>${esc(tile?.label || '')}</span></button>`;
  }).join('')}</div>`;
}
...
function sichuanRoomView(room) {
  ...
  const result = room.status === 'ended'
    ? room.result === 'draw'
      ? `<div class="result-card"><strong>무승부 · 판돈 반환</strong><p>${esc(room.resultReason || '')}</p></div>`
      : `<div class="result-card"><strong>${room.winnerPetId === app.data?.dashboard?.pet?.id ? '승리했습니다!' : `${esc(room.players?.[room.winnerPetId]?.displayName || '상대')} 승리`}</strong><p>${esc(room.resultReason || '')}</p><small>승자가 판돈 ${points(Number(room.stakePoints || 0) * 2)} 획득</small></div>`
    : '';
  ...
  return `${sectionHeading('사천성대전', `${room.roomNumber}번방 · 판돈 ${points(room.stakePoints)} · ${sichuanStatusLabel(room.status)}`, '<button class="text-button" data-action="sichuan-back" type="button">로비 보기</button>')}<div class="sichuan-rule-strip compact"><span>동일 시작판</span><span>2:30</span><span>막히면 패배</span></div>${stage}${result}<div class="button-row">${rematch}${leave}</div>${reaction}`;
}
...
async function handleAction(button, event = null) {
  ...
  if (action === 'omok-rps') { const room = currentOmokRoom(); if (!room) return; return perform(`/api/omok/rooms/${encodeURIComponent(room.id)}/rps`, { choice: button.dataset.value }, null, 'POST', { toastDuration: 700, toastType: 'game-start' }); }
  if (action === 'omok-color') { const room = currentOmokRoom(); if (!room) return; return perform(`/api/omok/rooms/${encodeURIComponent(room.id)}/color`, { color: button.dataset.value }); }
...
navigator.serviceWorker?.register?.('/sw.js?v=69301')?.catch(() => {});
