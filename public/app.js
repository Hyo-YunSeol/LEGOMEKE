import { cloneBlockBattleVisualPlayer, reconcileBlockBattleVisual } from './block-battle-visual-state.js?v=6101240';
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=6101240" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=6101240`;
...
function patchLegodokuMarksDom(room = currentLegodokuRoom()) {
  ...
  document.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
    ...
    if (hasHead) {
      ...
      if (!cell.querySelector('.legodoku-head')) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101240" alt="" draggable="false">');
...
function patchLegodokuSpectatorBoardsDom(room = currentLegodokuRoom()) {
  ...
  root.querySelectorAll('[data-legodoku-player-board]').forEach((board) => {
    ...
    board.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
      ...
      if (hasHead && !existing) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101240" alt="" draggable="false">');
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=6101240`;
}
...
function resetBlockGameInputQueue({ keepLocal = false } = {}) {
...
async function correctBlockGameFromServer(message = '') {
  resetBlockGameInputQueue();
...
async function drainBlockGameInputQueue() {
  ...
  try {
    const result = await api('/api/minigames/block/select', {
      method: 'POST',
    ...
    if (result?.stale || !result?.ok) {
      if (result?.stale && applyBlockGameServerResult(result)) {
        ...
        await correctBlockGameFromServer(result?.message || '블록게임 상태를 다시 불러왔습니다.');
      }
      return;
    }
    ...
    if (generation === app.blockGameQueueGeneration) await correctBlockGameFromServer(error.message || '블록게임 연결을 다시 맞췄습니다.');
    return;
  } finally {
    if (generation === app.blockGameQueueGeneration) {
      app.blockGameInputInFlight = false;
      if (!app.blockGameInputQueue.length) syncBlockGameLocalChallenge(app.data?.activeMiniChallenge, { force: true });
...
function refreshBlockMiniOnly(result = null) {
  ...
  if (remaining) remaining.textContent = `${Math.max(0, Number(optimistic ? challenge.blockRemainingCount : (result?.remainingCount ?? challenge.blockRemainingCount)) || 0)}개`;
  if (groups) groups.textContent = `${Math.max(0, Number(optimistic ? challenge.blockAvailableGroups : (result?.availableGroups ?? challenge.blockAvailableGroups)) || 0)}개`;
  if (pending) pending.textContent = points(pendingPoints);
  if (stop) stop.textContent = '포기하기';
  if (helper) helper.textContent = optimistic ? '입력을 순서대로 처리 중입니다. 계속 클릭해도 됩니다.' : (result?.message || '제거한 자리 위의 블록만 아래로 바로 내려옵니다. 좌우로는 움직이지 않습니다.');
...
navigator.serviceWorker?.register?.('/sw.js?v=6101240')?.catch(() => {});
