import { cloneBlockBattleVisualPlayer, reconcileBlockBattleVisual } from './block-battle-visual-state.js?v=6101280';
...
const CLIENT_VERSION = '6.10.28';
const DEFAULT_API_TIMEOUT_MS = 12_000;
const BLOCK_GAME_API_TIMEOUT_MS = 6_500;
const BLOCK_GAME_SETTLEMENT_WATCH_MS = 15_000;
const MINESWEEPER_ACTION_QUEUE_LIMIT = 64;
const APPLE_INPUT_QUEUE_LIMIT = 48;
...
const app = {
  ...
  // Space는 물리 keydown 1회당 hardDrop 1회만 허용한다. ACK 전 다음 블록에 누른 별도 keydown은 순서대로 보존한다.
  ...
  appleInputQueue: [],
  appleInputInFlight: false,
  appleInputGeneration: 0,
  appleCurrentInput: null,
  appleLastErrorKey: '',
  appleLastErrorAt: 0,
  blockGameInputQueue: [],
  blockGameInputInFlight: false,
  blockGameLocalChallenge: null,
  blockGameAcceptedVersions: new Map(),
  blockGameQueueGeneration: 0,
  blockGameLastErrorKey: '',
  blockGameLastErrorAt: 0,
  blockGameRecoveryItem: null,
  blockGameRecoveryChallenge: null,
  blockGameAutoSettleChallengeId: null,
  blockGameSettlementWatchTimer: null,
  blockGameRecoveryInProgress: false,
...
const CLIENT_PARTICLES = Object.freeze({
  '이/가': ['이', '가'], '을/를': ['을', '를'], '은/는': ['은', '는'], '과/와': ['과', '와'], '으로/로': ['으로', '로']
});
function clientHasFinalConsonant(value) {
...
function clientHasRieulFinal(value) {
  const text = String(value ?? '').trim();
  if (!text) return false;
  const code = text.charCodeAt(text.length - 1);
...
function josa(value, pair) {
  ...
  const particles = CLIENT_PARTICLES[pair];
  if (!particles) return text;
  if (pair === '으로/로') return `${text}${!clientHasFinalConsonant(text) || clientHasRieulFinal(text) ? particles[1] : particles[0]}`;
  return `${text}${clientHasFinalConsonant(text) ? particles[0] : particles[1]}`;
}
...
async function api(path, options = {}) {
  const { timeoutMs: timeoutValue, signal: externalSignal, ...fetchOptions } = options;
  const timeoutMs = Math.max(1_000, Number(timeoutValue) || DEFAULT_API_TIMEOUT_MS);
  ...
  try {
    ...
    if (timedOut) throw new Error('서버 응답이 지연되어 요청을 중단했습니다. 현재 상태를 다시 확인해주세요.');
...
function applyBootstrap(next) {
  ...
  const previousBlockChallenge = app.data?.activeMiniChallenge?.gameId === 'block' ? structuredClone(app.data.activeMiniChallenge) : null;
  ...
  app.data = next;
  // 블록게임도 같은 challenge에서는 boardVersion이 절대 뒤로 가지 않는다.
  // 늦은 bootstrap/복구 응답이 이미 제거한 블록을 되살리는 현상을 여기서 한 번 더 차단한다.
  const incomingBlockChallenge = app.data?.activeMiniChallenge;
  if (previousBlockChallenge && incomingBlockChallenge?.gameId === 'block' && incomingBlockChallenge.id === previousBlockChallenge.id) {
    const previousVersion = Math.max(1, Number(previousBlockChallenge.blockBoardVersion) || 1);
    const incomingVersion = Math.max(1, Number(incomingBlockChallenge.blockBoardVersion) || 1);
    const acceptedVersion = Math.max(previousVersion, Number(app.blockGameAcceptedVersions.get(incomingBlockChallenge.id)) || 1);
    if (incomingVersion < acceptedVersion) app.data.activeMiniChallenge = previousBlockChallenge;
    else app.blockGameAcceptedVersions.set(incomingBlockChallenge.id, incomingVersion);
  } else if (incomingBlockChallenge?.gameId === 'block') {
    app.blockGameAcceptedVersions.set(incomingBlockChallenge.id, Math.max(1, Number(incomingBlockChallenge.blockBoardVersion) || 1));
  }
  ...
  for (let index = 0; index < (app.data?.blockBattle?.rooms || []).length; index += 1) {
    ...
    const beforeActiveKey = JSON.stringify(visualBefore?.active ?? null);
...
function logout(callServer = true) {
  ...
  clearTimeout(app.sichuanFinishWatchTimer);
  app.sichuanFinishWatchTimer = null;
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  const modalEpochAtRequest = app.modalEpoch;
  ...
  try {
    ...
    if (!app.data) return;
    // 모바일 브라우저/PWA가 메모리 회수 뒤 페이지를 다시 만들 수 있으므로 bootstrap만으로
    // 지뢰찾기를 포기 처리하지 않는다. 사용자가 명시적으로 닫기/포기했을 때만 abandon API를 호출한다.
    ...
    const appleInputProtected = renderMode === 'full'
      && app.modal?.type === 'mini'
      && app.modal?.gameId === 'apple'
      && appleGameHasPendingInputs();
    if (appleInputProtected) refreshAppleMiniOnly();
    const blockOnlyUpdated = renderMode === 'block' && refreshBlockMiniOnly();
    // 블록게임은 서버 ACK를 기다리는 동안에도 다음 클릭을 로컬 판에 낙관적으로 반영한다.
    // 이때 5초 bootstrap이 서버의 한 단계 이전 판으로 모달을 재생성하면 클릭이 사라져 보이므로,
    // 일반 동기화는 데이터만 저장하고 블록 전용 응답 경로가 판 DOM을 교정하도록 둔다.
    const blockInputProtected = renderMode === 'full'
      && app.modal?.type === 'mini'
      && app.modal?.gameId === 'block'
      && blockGameHasPendingInputs();
    ...
    if (blockInputProtected || blockBattleLivePatched || sichuanLivePatched || legodokuLivePatched) markTabDirty('games');
    if (!appleOnlyUpdated && !appleInputProtected && !blockOnlyUpdated && !blockInputProtected && !minesweeperOnlyUpdated && !blockBattleLivePatched && !sichuanLivePatched && !legodokuLivePatched) {
      render();
      const keepSingleTetrisResult = app.modal?.type === 'mini' && app.modal?.gameId === 'tetrisSingle' && app.singleTetrisState?.ended;
      if (app.modal && app.modalEpoch === modalEpochAtRequest && dataChanged && !keepSingleTetrisResult) refreshModal();
...
function closeModal() {
  app.modalEpoch += 1;
  app.profileClickSuppressUntil = Date.now() + 750;
  if (app.modal?.type === 'mini' && app.modal?.gameId === 'apple') resetAppleInputQueue();
  if (app.modal?.type === 'mini' && app.modal?.gameId === 'block') resetBlockGameInputQueue();
  stopSingleTetrisTimers();
  cancelMinesweeperPointerGesture();
...
async function refreshModal() {
  ...
  if (descriptor.type === 'admin') return refreshAdminModalPartial();
}
...
async function perform(path, body = {}, successMessage = null, method = 'POST', {
  ...
  try {
    ...
    try {
      ...
      if (!appleOnlyUpdated && !blockOnlyUpdated && !minesweeperOnlyUpdated && !shopOnlyUpdated) {
        render();
        if (app.modal && app.modalEpoch === modalEpochAtRequest) {
          await refreshModal();
...
const BODY_STAGE_FALLBACK = [
  { min: 0, max: null, key: 'normal', label: '보통레고', activityHungerCost: 1 }
];
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=6101280" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function lifeHungerCostsForStage(stage) {
  ...
  const table = {
    1:[4,2,6], 2:[4,2,6], 3:[5,2,7], 4:[5,3,7], 5:[6,3,8],
    6:[6,3,8], 7:[7,4,9], 8:[7,4,9], 9:[8,4,10], 10:[8,5,10]
  };
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=6101280`;
  ...
  return `
    ...
    <section class="section">
      ...
      <div class="action-grid">
        <button class="action-card life-action" data-action="work" type="button" ${lifeActionsLocked() ? 'disabled' : ''}><span>💼</span><strong>일하기</strong><small>+500P · 체력 -8 · 포만감 -${lifeHunger.work}</small></button>
        <button class="action-card life-action" data-action="cook" type="button" ${lifeActionsLocked() || pet.stats.hunger >= 100 ? 'disabled' : ''}><span>🍳</span><strong>요리하기</strong><small>포만감 +50 · 체력 -5 · 최대 100</small></button>
...
function commonBattleResultOverlay(game, room, { totalPot = 0 } = {}) {
  ...
  const main = isDraw ? '게임이 무승부로 종료되었습니다.' : `${esc(josa(winnerName, '이/가'))} 이겼습니다.`;
...
const BLOCK_BATTLE_MAX_IMPORTANT_UNCONFIRMED_ACTIONS = 24;
...
function legodokuBoardHtml(room, { interactive = false, revealSolution = false, playerPetId = null } = {}) {
  ...
  const boardPlayer = playerPetId ? room.players?.[playerPetId] : (room.viewerRole === 'player' ? room.players?.[room.selfPetId] : null);
  const confirmed = new Set((boardPlayer?.confirmed ?? []).map(Number));
  ...
  const playerAttr = playerPetId ? ` data-legodoku-player-board="${esc(playerPetId)}"` : '';
  return `<div class="legodoku-board${interactive ? ' interactive' : ''}" role="grid" aria-rowcount="8" aria-colcount="8" data-legodoku-board${playerAttr}>${regions.map((region, index) => {
    ...
    const left = col * 12.5, top = row * 12.5;
    return `<button class="${classes}" style="--legodoku-left:${left}%;--legodoku-top:${top}%" data-action="legodoku-cell" data-index="${index}" data-row="${row}" data-col="${col}" type="button" ${interactive && !hasHead ? '' : 'disabled'} role="gridcell" aria-label="${label}">${hasHead ? '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101280" alt="" draggable="false">' : ''}<span class="legodoku-x" data-legodoku-mark>${marked ? '×' : ''}</span></button>`;
  }).join('')}</div>`;
}
...
function legodokuRoomView(room) {
  ...
  if (room.status === 'waiting') {
    ...
    stage = `<div class="legodoku-match-head">${legodokuProgressHtml(self, { mine: true })}<div class="legodoku-match-center"><span>남은 시간</span>${timer}<small>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${legodokuProgressHtml(opponent)}</div><div class="legodoku-board-stage">${legodokuBoardHtml(room, { interactive: true })}<div class="legodoku-feedback-layer" aria-live="polite"></div>${spectatorBurstLayer(room.reactions || [], 'legodoku')}</div>`;
  } else if (isSpectator) {
    stage = `<div class="legodoku-match-head">${legodokuProgressHtml(hostPlayer)}<div class="legodoku-match-center"><span>남은 시간</span>${timer}<small>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${legodokuProgressHtml(guestPlayer)}</div><div class="legodoku-spectator-boards"><article class="legodoku-spectator-board"><div class="legodoku-spectator-board-title"><strong>${esc(hostPlayer?.displayName || room.host?.displayName || '-')}</strong><span>레고 ${Number(hostPlayer?.foundCount || 0)}/8</span></div><div class="legodoku-board-stage">${legodokuBoardHtml(room, { playerPetId: room.host?.petId })}</div></article><article class="legodoku-spectator-board"><div class="legodoku-spectator-board-title"><strong>${esc(guestPlayer?.displayName || room.guest?.displayName || '-')}</strong><span>레고 ${Number(guestPlayer?.foundCount || 0)}/8</span></div><div class="legodoku-board-stage">${legodokuBoardHtml(room, { playerPetId: room.guest?.petId })}</div></article></div>${spectatorBurstLayer(room.reactions || [], 'legodoku')}`;
  } else {
  ...
  const reaction = isSpectator ? `${spectatorReactionBar('legodoku', room.id, room.reactions || [], true)}<p class="helper">관전자는 두 플레이어의 현재 레고 위치를 모두 볼 수 있지만 게임 입력에는 참여할 수 없습니다.</p>` : isPlayer ? spectatorReactionBar('legodoku', room.id, room.reactions || [], true, true) : '';
  const info = room.status === 'waiting' ? '문제는 대전 시작 시 생성' : `난이도 ${esc(room.puzzle?.difficulty || '중')}`;
  const inputGuide = isPlayer && room.status === 'playing' ? '<div class="legodoku-input-guide" aria-label="레고도쿠 조작 방법"><span><b>누르면 레고</b>빈칸을 한 번 누르면 레고를 놓음</span><span><b>드래그하면 X</b>빈칸에서 그대로 끌면 X 표시</span><span><b>X 지우기</b>X에서 시작해 드래그하면 지움</span></div>' : '';
  const body = `${sectionHeading('레고도쿠', `${room.roomNumber}번방 · ${info} · 판돈 ${points(room.stakePoints)} · ${legodokuStatusLabel(room.status)}`, '<button class="text-button" data-action="legodoku-back" type="button">로비 보기</button>')}${inputGuide}${stage}<div class="button-row">${rematch}${leave}</div>${reaction}${commonBattleResultOverlay('legodoku', room, { totalPot: Number(room.stakePoints || 0) * 2 })}`;
...
function patchLegodokuMarksDom(room = currentLegodokuRoom()) {
  ...
  document.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
    ...
    if (hasHead) {
      ...
      if (!cell.querySelector('.legodoku-head')) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101280" alt="" draggable="false">');
...
function patchLegodokuSpectatorBoardsDom(room = currentLegodokuRoom()) {
  if (!room || room.viewerRole !== 'spectator' || room.status !== 'playing') return false;
  const root = document.querySelector('.legodoku-room-view');
  if (!root) return false;
  root.querySelectorAll('[data-legodoku-player-board]').forEach((board) => {
    const petId = String(board.dataset.legodokuPlayerBoard || '');
    const player = room.players?.[petId];
    const confirmed = new Set((player?.confirmed ?? []).map(Number));
    board.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
      const index = Number(cell.dataset.index);
      const hasHead = confirmed.has(index);
      cell.classList.toggle('confirmed', hasHead);
      const existing = cell.querySelector('.legodoku-head');
      if (hasHead && !existing) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101280" alt="" draggable="false">');
      if (!hasHead && existing) existing.remove();
      const row = Number(cell.dataset.row) + 1, col = Number(cell.dataset.col) + 1;
      cell.setAttribute('aria-label', `${row}행 ${col}열${hasHead ? ' · 레고' : ''}`);
    });
    const titleCount = board.closest('.legodoku-spectator-board')?.querySelector('.legodoku-spectator-board-title span');
    if (titleCount) titleCount.textContent = `레고 ${Math.max(0, Math.min(8, Number(player?.foundCount || 0)))}/8`;
  });
  return true;
}
...
function patchLegodokuLiveRoom(room) {
  ...
  for (const player of Object.values(room.players || {})) {
    ...
    if (!panel) continue;
    const found = Math.max(0, Math.min(8, Number(player?.foundCount || 0)));
    const maxMistakes = legodokuMaxMistakes(room);
    const mistakes = Math.max(0, Math.min(maxMistakes, Number(player?.mistakes || 0)));
    ...
    if (mistakeNode) mistakeNode.textContent = `실수 ${mistakes}/${maxMistakes}`;
  }
  if (room.viewerRole === 'player') patchLegodokuMarksDom(room);
  else if (room.viewerRole === 'spectator') patchLegodokuSpectatorBoardsDom(room);
...
function sichuanPenaltyRemainingMs(room, petId) {
  if (!room || room.status !== 'playing' || !petId) return 0;
  const until = new Date(room.streakPenaltyUntil?.[petId] ?? '').getTime();
  if (!Number.isFinite(until)) return 0;
  return Math.max(0, until - sichuanServerNow());
}

function sichuanPenaltyActive(room, petId) {
  return sichuanPenaltyRemainingMs(room, petId) > 0;
}

function sichuanPenaltyNoticeHtml(room) {
  if (room?.viewerRole !== 'player' || room.status !== 'playing') return '';
  const mine = sichuanPenaltyRemainingMs(room, room.selfPetId);
  const opponent = sichuanPenaltyRemainingMs(room, room.opponentPetId);
  if (mine <= 0 && opponent <= 0) return '<div class="sichuan-penalty-notices hidden" data-sichuan-penalty-notices></div>';
  const parts = [];
  if (mine > 0) parts.push(`<span class="mine" data-sichuan-penalty-self>연승 패널티 · 패 공개까지 ${Math.ceil(mine / 1000)}초</span>`);
  if (opponent > 0) parts.push(`<span data-sichuan-penalty-opponent>상대 연승 패널티 · ${Math.ceil(opponent / 1000)}초</span>`);
  return `<div class="sichuan-penalty-notices" data-sichuan-penalty-notices>${parts.join('')}</div>`;
}

function updateSichuanPenaltyUi(room = currentSichuanRoom()) {
  if (!room || room.viewerRole !== 'player' || room.status !== 'playing') return false;
  const mineMs = sichuanPenaltyRemainingMs(room, room.selfPetId);
  const opponentMs = sichuanPenaltyRemainingMs(room, room.opponentPetId);
  const mineActive = mineMs > 0;
  const notices = document.querySelector('[data-sichuan-penalty-notices]');
  if (notices) {
    notices.classList.toggle('hidden', !mineActive && opponentMs <= 0);
    notices.innerHTML = `${mineActive ? `<span class="mine" data-sichuan-penalty-self>연승 패널티 · 패 공개까지 ${Math.ceil(mineMs / 1000)}초</span>` : ''}${opponentMs > 0 ? `<span data-sichuan-penalty-opponent>상대 연승 패널티 · ${Math.ceil(opponentMs / 1000)}초</span>` : ''}`;
  }
  const self = room.players?.[room.selfPetId];
  if (self) patchSichuanBoard(self, { interactive: true, conceal: mineActive });
  return mineActive || opponentMs > 0;
}
...
function sichuanTheme(room = currentSichuanRoom()) {
  ...
  return themes.find((theme) => theme.key === wanted)
    ?? themes[0]
    ?? { key: 'neon', label: '네온시티', tiles: game.tiles ?? [] };
}
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=6101280&s=6101281`;
}
...
function updateSichuanFinishState(room = currentSichuanRoom()) {
  const status = document.querySelector('[data-sichuan-finish-status]');
  const self = room?.viewerRole === 'player' ? room.players?.[room.selfPetId] : null;
  const awaitingFinish = Boolean(room?.status === 'playing' && self && sichuanEffectiveRemovedCount(self, room) >= 80);
  if (status) status.classList.toggle('hidden', !awaitingFinish);
  if (!awaitingFinish) {
    clearTimeout(app.sichuanFinishWatchTimer);
    app.sichuanFinishWatchTimer = null;
    return false;
  }
  if (!app.sichuanFinishWatchTimer) {
    app.sichuanFinishWatchTimer = setTimeout(() => {
      app.sichuanFinishWatchTimer = null;
      const latest = currentSichuanRoom();
      if (latest?.status === 'playing' && latest.viewerRole === 'player') loadBootstrap({ silent: true, renderMode: 'games-live' });
    }, 1200);
  }
  return true;
}
...
function sichuanBoardHtml(player, { interactive = false, compact = false, conceal = false } = {}) {
  const tiles = sichuanTileMap();
  const board = sichuanEffectiveBoard(player);
  const petId = String(player?.petId || '');
  return `<div class="sichuan-board ${compact ? 'compact' : ''}${conceal ? ' sichuan-concealed' : ''}" data-sichuan-pet-id="${esc(petId)}" role="grid" aria-rowcount="8" aria-colcount="10" aria-label="${esc(player?.displayName || '플레이어')} 사천성 판">${board.map((tileId, index) => {
...
function sichuanRoomView(room) {
  ...
  if (room.status === 'waiting') {
    ...
    stage = `<div class="sichuan-match-head">${sichuanProgressHtml(self, { mine: true })}<div class="sichuan-match-center"><span>남은 시간</span>${timer}<small data-sichuan-spectator-count>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${sichuanProgressHtml(opponent)}</div>${sichuanPenaltyNoticeHtml(room)}<div class="sichuan-board-stage">${sichuanBoardHtml(self, { interactive, conceal: sichuanPenaltyActive(room, room.selfPetId) })}<div class="sichuan-finish-status hidden" data-sichuan-finish-status>마지막 패 처리 중 · 승리 판정 확인 중</div>${spectatorBurstLayer(room.reactions || [], 'sichuan')}</div><p class="helper centered">같은 그림 두 개를 선택하세요. 최대 두 번 꺾어 연결할 수 있으며 판 바깥쪽 경로도 허용됩니다.</p>`;
  } else {
...
function renderSichuanRegion() {
  ...
  const room = currentSichuanRoom();
  if (room?.status === 'playing') { updateSichuanCountdown(room); updateSichuanPenaltyUi(room); }
  syncSupportBanner();
  requestAnimationFrame(syncGameResultScrollLock);
  return true;
}
...
function patchSichuanBoard(player, { interactive = false, compact = false, conceal = false } = {}) {
  if (!player) return false;
  const boardNode = findSichuanNodeByPet('.sichuan-board', player.petId);
  if (!boardNode) return false;
  const tiles = sichuanTileMap();
  boardNode.classList.toggle('sichuan-concealed', Boolean(conceal));
  const board = sichuanEffectiveBoard(player);
...
function patchSichuanLiveRoom(room) {
  ...
  if (isPlayer) {
    ...
    if (!patchSichuanBoard(self, { interactive: true, conceal: sichuanPenaltyActive(room, room.selfPetId) })) return false;
  } else if (isSpectator) {
  ...
  updateSichuanPenaltyUi(room);
  updateSichuanFinishState(room);
  requestAnimationFrame(syncGameResultScrollLock);
  return true;
}
...
async function drainSichuanPairQueue() {
  ...
  const requestTimeout = setTimeout(() => requestController.abort(), 8000);
  try {
    result = await api(`/api/sichuan/rooms/${encodeURIComponent(item.roomId)}/pair`, {
      method: 'POST', signal: requestController.signal, body: JSON.stringify({ matchId: item.matchId, first: item.first, second: item.second, actionId: item.actionId })
    });
    ...
    toast(requestController.signal.aborted ? '사천성 처리 응답이 늦어 상태를 다시 확인합니다.' : error.message, requestController.signal.aborted ? '' : 'error');
    // Reconcile from the server after a transport error/timeout; idempotent actionId prevents a retry/double-removal race.
    ...
    app.sichuanActionInFlight = false;
    const latest = currentSichuanRoom();
    if (latest?.id === item.roomId && latest?.matchId === item.matchId && latest.status === 'playing') {
      patchSichuanLiveRoom(latest);
      updateSichuanFinishState(latest);
    }
    queueMicrotask(drainSichuanPairQueue);
  }
  return result;
...
function blockBattleAttackHtml(room) {
  ...
  const attackMessage = isPlayer
    ...
    : `🔥 ${esc(josa(room.players?.[room.lastAttack?.fromPetId]?.displayName || '플레이어', '이/가'))} ${Number(room.lastAttack.lines)}줄 공격`;
...
function blockBattleHasUnconfirmedVerticalInput() {
  const tickPending = app.blockBattleInputBuffer.some((entry) => BLOCK_BATTLE_VERTICAL_ACTIONS.has(blockBattleActionName(entry)))
    || app.blockBattlePendingBatches.some((batch) =>
      batch.message.actions.some((entry) => BLOCK_BATTLE_VERTICAL_ACTIONS.has(blockBattleActionName(entry))));
  return tickPending;
}
...
function blockBattleActionName(entry) {
  return typeof entry === 'string' ? entry : String(entry?.action || '');
}

function blockBattleActionPiece(entry) {
  const value = Number(entry?.piece);
...
function acknowledgeBlockBattleBatch(room) {
  ...
  if (acknowledged.some((batch) => batch.expectsLock)) {
    const anotherPredictedLock = app.blockBattleLockQueued
      || app.blockBattlePendingBatches.some((batch) => batch.expectsLock)
      || app.blockBattleInputBuffer.some((entry) => blockBattleActionName(entry) === 'hardDrop');
    if (!anotherPredictedLock) app.blockBattlePredictedLock = null;
  }
...
function applyBlockBattleRoomState(payload) {
  ...
  const beforePieceCount = Number(visualBefore?.pieces || 0);
  const beforeActiveType = String(visualBefore?.active?.type || '');
  const beforeActiveKey = safeVisualReconcile ? JSON.stringify(visualBefore?.active ?? null) : '';
  const beforeBoardKey = safeVisualReconcile ? JSON.stringify(visualBefore?.board ?? null) : '';
  const visualAfter = blockBattleVisualSelf(room, { force: safeVisualReconcile });
...
function queueBlockBattleInput(action) {
  ...
  const visualAtInput = blockBattleVisualSelf(room) || room?.players?.[room.selfPetId];
  const inputPiece = Math.max(0, Number(visualAtInput?.pieces) || 0);
  ...
  const awaitingHardDrop = app.blockBattlePendingBatches.some((batch) => batch.message.actions.some((entry) => blockBattleActionName(entry) === 'hardDrop'))
    || app.blockBattleInputBuffer.some((entry) => blockBattleActionName(entry) === 'hardDrop');
  if (action === 'tick') {
  ...
  const importantAction = action === 'hardDrop' || action === 'rotate';
  const actionLimit = importantAction ? BLOCK_BATTLE_MAX_IMPORTANT_UNCONFIRMED_ACTIONS : BLOCK_BATTLE_MAX_UNCONFIRMED_ACTIONS;
  if (unconfirmedActions >= actionLimit) return false;
  ...
  // 이미 로컬 lock을 예측해 다음 블록이 보이는 상태라면 새 물리 입력도 그 다음 블록에 즉시 적용한다.
  // ACK 대기 여부는 독립적인 Space keydown을 폐기하는 근거가 아니며, 큐 상한으로만 폭주를 막는다.
  ...
  app.blockBattleInputBuffer.push({ action, piece: inputPiece, seq: ++app.blockBattleInputSeq });
...
function territoryViewPage() {
  ...
  const mapSize = Math.max(1, Math.floor(Number(territory.size) || 6));
...
function openBodyGuide(bodyValue = app.data?.dashboard?.pet?.stats?.body) {
  ...
  const cards = stages.map((stage, index) => {
    ...
    return `<article class="body-guide-card ${stage.key === current.key ? 'current' : ''}"><span class="body-guide-step">${index + 1}단계</span>${avatar(stage, { mini: true })}<div><strong>${esc(stage.label)}</strong><small>몸집 ${esc(bodyRangeLabel(stage))}</small>${evolution}<small>게임·교미·영토·낚시 포만감 -1</small><small>생활: 일 포만감 -${life.work}/체력 -8 · 쉬기 포만감 -${life.rest}/체력 +40 · 헬스 포만감 -${life.exercise}/체력 -12 · 요리 체력 -5</small>${currentHint}</div></article>`;
  }).join('');
...
async function buyFlexItem(itemId) {
  ...
  if (active) {
    ...
    const question = same
      ? `${josa(item.name, '을/를')} 다시 구매하면 남은 시간이 사라지고 지금부터 24시간으로 갱신됩니다. ${points(item.price)}로 구매할까요?`
      : `${active.name}의 남은 시간이 사라지고 ${item.name}(으)로 즉시 교체됩니다. ${points(item.price)}로 구매할까요?`;
    if (!confirm(question)) return null;
  } else if (!confirm(`${josa(item.name, '을/를')} ${points(item.price)}로 구매해 24시간 장착할까요?`)) return null;
...
function blockCellHtml(value, rowIndex, colIndex, version) {
  ...
  const className = empty ? 'block-cell block-empty-cell' : `block-cell color-${color}`;
  const label = empty ? '빈 칸' : `${BLOCK_COLOR_NAMES[color]} 블록`;
  // 120개 셀 DOM을 판이 끝날 때까지 고정해 pointerdown→pointerup 사이 노드 교체로 인한 모바일 클릭 유실을 막는다.
  return `<button class="${className}" data-action="block-select" data-row="${rowIndex}" data-col="${colIndex}" data-version="${version}" type="button" ${empty ? 'disabled' : ''} aria-label="${label}"></button>`;
}
...
function blockGameCloneChallenge(challenge) {
  if (!challenge || challenge.gameId !== 'block') return null;
  return { ...challenge, blockBoard: (challenge.blockBoard ?? []).map((row) => [...row]) };
}

function blockGameGroupAt(board, startRow, startCol) {
  const row = Number(startRow);
  const col = Number(startCol);
  if (!Number.isInteger(row) || !Number.isInteger(col) || row < 0 || row >= 12 || col < 0 || col >= 10) return [];
  ...
  while (stack.length) {
    ...
    for (const [nextRow, nextCol] of [[currentRow - 1, currentCol], [currentRow + 1, currentCol], [currentRow, currentCol - 1], [currentRow, currentCol + 1]]) {
      if (nextRow < 0 || nextRow >= 12 || nextCol < 0 || nextCol >= 10 || board?.[nextRow]?.[nextCol] !== color) continue;
...
function collapseBlockGameBoard(board) {
  const next = board.map((row) => [...row]);
  for (let col = 0; col < 10; col += 1) {
    const values = [];
    for (let row = 0; row < 12; row += 1) if (next[row][col] != null) values.push(next[row][col]);
    const empty = 12 - values.length;
    for (let row = 0; row < 12; row += 1) next[row][col] = row < empty ? null : values[row - empty];
  }
  return next;
}

function blockGameStats(board) {
  const seen = new Set();
  ...
  for (let row = 0; row < 12; row += 1) {
    for (let col = 0; col < 10; col += 1) {
      ...
      const group = blockGameGroupAt(board, row, col);
      for (const [groupRow, groupCol] of group) seen.add(groupRow * 10 + groupCol);
      if (group.length >= 2) removableGroups += 1;
    }
  }
  return { remainingBlocks, removableGroups };
}

function blockGameRewardForSize(sizeValue) {
  const size = Math.max(0, Math.floor(Number(sizeValue) || 0));
  if (size < 2) return 0;
  if (size === 2) return 5;
  if (size === 3) return 9;
  if (size === 4) return 13;
...
function blockGameHasPendingInputs() {
  return app.blockGameInputInFlight || app.blockGameInputQueue.length > 0;
}

function syncBlockGameLocalChallenge(challenge = app.data?.activeMiniChallenge, { force = false } = {}) {
  if (!challenge || challenge.gameId !== 'block') {
    if (force || !blockGameHasPendingInputs()) app.blockGameLocalChallenge = null;
    return null;
  }
  if (!force && blockGameHasPendingInputs() && app.blockGameLocalChallenge?.id === challenge.id) return app.blockGameLocalChallenge;
  app.blockGameLocalChallenge = blockGameCloneChallenge(challenge);
  app.blockGameAcceptedVersions.set(challenge.id, Math.max(1, Number(challenge.blockBoardVersion) || 1));
  return app.blockGameLocalChallenge;
}

function clearBlockGameSettlementWatch() {
  clearTimeout(app.blockGameSettlementWatchTimer);
  app.blockGameSettlementWatchTimer = null;
}

function resetBlockGameInputQueue({ keepLocal = false, keepRecovery = false } = {}) {
  app.blockGameQueueGeneration += 1;
  app.blockGameInputQueue.length = 0;
  app.blockGameInputInFlight = false;
  app.blockGameAutoSettleChallengeId = null;
  clearBlockGameSettlementWatch();
  if (!keepLocal) app.blockGameLocalChallenge = null;
  if (!keepRecovery) {
    app.blockGameRecoveryItem = null;
    app.blockGameRecoveryChallenge = null;
  }
}

function armBlockGameSettlementWatch(challengeId) {
  clearBlockGameSettlementWatch();
  const expectedId = String(challengeId || '');
  if (!expectedId) return;
  app.blockGameSettlementWatchTimer = setTimeout(() => {
    app.blockGameSettlementWatchTimer = null;
    const challenge = app.data?.activeMiniChallenge;
    if (!challenge || challenge.gameId !== 'block' || challenge.id !== expectedId || challenge.completed) return;
    if (Math.max(0, Number(challenge.blockAvailableGroups) || 0) > 0) return;
    // 정산 요청/응답이 비정상적으로 오래 걸리면 서버 상태를 다시 읽고 같은 requestId로 안전 복구한다.
    recoverBlockGameAfterAmbiguousFailure().catch(() => {});
  }, BLOCK_GAME_SETTLEMENT_WATCH_MS);
}

function applyBlockGameServerResult(result) {
  const challenge = app.data?.activeMiniChallenge;
  if (!challenge || challenge.gameId !== 'block' || !Array.isArray(result?.board)) return false;
  const incomingVersion = Math.max(1, Number(result.boardVersion) || 1);
  const acceptedVersion = Math.max(1, Number(app.blockGameAcceptedVersions.get(challenge.id)) || Number(challenge.blockBoardVersion) || 1);
  // 늦게 도착한 이전 선택/복구 응답이 더 최신 판을 덮어써 이미 지운 블럭을 되살리지 못하게 한다.
  if (incomingVersion < acceptedVersion) return false;
  app.blockGameAcceptedVersions.set(challenge.id, incomingVersion);
  challenge.blockBoard = result.board.map((row) => [...row]);
  challenge.blockBoardVersion = incomingVersion;
  challenge.blockPendingPoints = Math.max(0, Number(result.pendingPoints) || 0);
  challenge.blockRemovedCount = Math.max(0, Number(result.removedCount) || 0);
  challenge.blockMoveCount = Math.max(0, Number(result.moveCount) || 0);
  challenge.blockAvailableGroups = Math.max(0, Number(result.availableGroups) || 0);
  challenge.blockRemainingCount = Math.max(0, Number(result.remainingCount) || 0);
  challenge.blockAllClear = Boolean(result.allClear);
  return true;
}

function showBlockGameErrorOnce(message) {
  const key = String(message || '블록게임 상태를 다시 맞췄습니다.');
  const now = monotonicNow();
  if (app.blockGameLastErrorKey === key && now - app.blockGameLastErrorAt < 2_500) return;
  app.blockGameLastErrorKey = key;
  app.blockGameLastErrorAt = now;
  toast(key, 'error', 2200);
}

function previewBlockGameSelection(rowValue, colValue) {
  const challenge = syncBlockGameLocalChallenge();
  if (!challenge) return null;
  const row = Number(rowValue);
  const col = Number(colValue);
  const group = blockGameGroupAt(challenge.blockBoard, row, col);
  if (group.length < 2) return null;
  const boardVersion = Math.max(1, Number(challenge.blockBoardVersion) || 1);
  for (const [groupRow, groupCol] of group) challenge.blockBoard[groupRow][groupCol] = null;
  challenge.blockBoard = collapseBlockGameBoard(challenge.blockBoard);
  challenge.blockPendingPoints = Math.max(0, Number(challenge.blockPendingPoints) || 0) + blockGameRewardForSize(group.length);
  challenge.blockRemovedCount = Math.max(0, Number(challenge.blockRemovedCount) || 0) + group.length;
  challenge.blockMoveCount = Math.max(0, Number(challenge.blockMoveCount) || 0) + 1;
  challenge.blockBoardVersion = boardVersion + 1;
  const stats = blockGameStats(challenge.blockBoard);
  ...
  return { row, col, boardVersion, selectedCount: group.length };
}

async function requestBlockGameInput(item) {
  let lastError = null;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      return await api('/api/minigames/block/select', {
        method: 'POST',
        body: JSON.stringify(item),
        timeoutMs: BLOCK_GAME_API_TIMEOUT_MS
      });
    } catch (error) {
      lastError = error;
  ...
  throw lastError || new Error('블록게임 서버 응답을 확인할 수 없습니다.');
}

async function recoverBlockGameAfterAmbiguousFailure(message = '') {
  app.blockGameRecoveryInProgress = true;
  const recoveryItem = app.blockGameRecoveryItem ? { ...app.blockGameRecoveryItem } : null;
  const recoveryChallenge = app.blockGameRecoveryChallenge ? blockGameCloneChallenge(app.blockGameRecoveryChallenge) : null;
  try {
    resetBlockGameInputQueue({ keepRecovery: true });
    if (message) showBlockGameErrorOnce(message);
    await loadBootstrap({ silent: true, renderMode: 'block' });

    // 서버가 마지막 선택을 이미 정산했는데 응답만 끊겼다면 같은 requestId를 재전송한다.
    // 서버는 완료된 challenge의 저장된 reward만 돌려주므로 중복 지급 없이 결과창을 복구할 수 있다.
    if (recoveryItem && recoveryChallenge) {
      try {
        const replay = await api('/api/minigames/block/select', {
          method: 'POST',
          body: JSON.stringify(recoveryItem),
          timeoutMs: BLOCK_GAME_API_TIMEOUT_MS
        });
        if (replay?.ok && replay?.finished) {
          applyBlockGameServerResult(replay);
          if (replay.bootstrap) applyBootstrap(replay.bootstrap);
          app.blockGameRecoveryItem = null;
          app.blockGameRecoveryChallenge = null;
          openMiniResult(replay, recoveryChallenge);
          return;
        }
        if (replay?.ok && Array.isArray(replay.board)) applyBlockGameServerResult(replay);
      } catch { /* 연결이 계속 끊긴 경우 서버 상태를 유지하고 다음 동기화에서 다시 불러온다. */ }
    }

    app.blockGameRecoveryItem = null;
    app.blockGameRecoveryChallenge = null;
    syncBlockGameLocalChallenge(app.data?.activeMiniChallenge, { force: true });
    refreshBlockMiniOnly();
  } finally {
    app.blockGameRecoveryInProgress = false;
    queueBlockGameSettlementIfNeeded(app.data?.activeMiniChallenge);
  }
}

function queueBlockGameSettlementIfNeeded(challenge = app.data?.activeMiniChallenge) {
  if (!challenge || challenge.gameId !== 'block' || challenge.completed || app.blockGameRecoveryInProgress) return false;
  if (Math.max(0, Number(challenge.blockAvailableGroups) || 0) > 0) return false;
  if (blockGameHasPendingInputs() || app.blockGameAutoSettleChallengeId === challenge.id) return false;
  app.blockGameAutoSettleChallengeId = challenge.id;
  const requestId = crypto.randomUUID?.() || `block-finish-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  app.blockGameInputQueue.push({
    challengeId: challenge.id,
    row: -1,
    col: -1,
    boardVersion: Math.max(1, Number(challenge.blockBoardVersion) || 1),
    requestId,
    autoSettle: true
  });
  const helper = $('#block-helper');
  if (helper) helper.textContent = '더 이상 제거할 그룹이 없어 포인트를 확정하는 중입니다.';
  armBlockGameSettlementWatch(challenge.id);
  drainBlockGameInputQueue();
  return true;
}

async function drainBlockGameInputQueue() {
  if (app.blockGameInputInFlight || !app.blockGameInputQueue.length) return;
  const generation = app.blockGameQueueGeneration;
  const item = app.blockGameInputQueue.shift();
  app.blockGameInputInFlight = true;
  app.blockGameRecoveryItem = { ...item };
  app.blockGameRecoveryChallenge = blockGameCloneChallenge(app.blockGameLocalChallenge || app.data?.activeMiniChallenge);
  try {
    const result = await requestBlockGameInput(item);
    if (generation !== app.blockGameQueueGeneration) return;
    if (result?.stale || !result?.ok) {
      app.blockGameRecoveryItem = null;
      app.blockGameRecoveryChallenge = null;
      if (result?.stale && applyBlockGameServerResult(result)) {
        resetBlockGameInputQueue();
        syncBlockGameLocalChallenge(app.data?.activeMiniChallenge, { force: true });
        refreshBlockMiniOnly(result);
        showBlockGameErrorOnce(result.message || '블록판이 변경되어 최신 상태로 맞췄습니다.');
        queueBlockGameSettlementIfNeeded(app.data?.activeMiniChallenge);
      } else {
        await recoverBlockGameAfterAmbiguousFailure(result?.message || '블록게임 상태를 다시 불러왔습니다.');
      }
      return;
    }
    applyBlockGameServerResult(result);
    app.blockGameRecoveryItem = null;
    app.blockGameRecoveryChallenge = null;
    if (result.finished) {
      const finishedChallenge = blockGameCloneChallenge(app.blockGameLocalChallenge || app.data?.activeMiniChallenge);
      resetBlockGameInputQueue();
      if (result.bootstrap) applyBootstrap(result.bootstrap);
      openMiniResult(result, finishedChallenge);
      return;
    }
    refreshBlockMiniOnly(result);
  } catch (error) {
    if (generation === app.blockGameQueueGeneration) await recoverBlockGameAfterAmbiguousFailure(error.message || '블록게임 연결을 다시 맞췄습니다.');
    return;
  } finally {
    if (generation === app.blockGameQueueGeneration) {
      app.blockGameInputInFlight = false;
      if (!app.blockGameInputQueue.length) {
        syncBlockGameLocalChallenge(app.data?.activeMiniChallenge, { force: true });
        queueBlockGameSettlementIfNeeded(app.data?.activeMiniChallenge);
      } else queueMicrotask(drainBlockGameInputQueue);
    }
  }
}

function queueBlockGameSelection(button) {
  const challenge = app.data?.activeMiniChallenge;
  if (!challenge || challenge.gameId !== 'block' || app.blockGameInputQueue.length >= 64) return false;
  const preview = previewBlockGameSelection(button.dataset.row, button.dataset.col);
  ...
  app.blockGameInputQueue.push({
    ...
    row: preview.row,
    col: preview.col,
    boardVersion: preview.boardVersion,
    requestId
  });
  refreshBlockMiniOnly();
  drainBlockGameInputQueue();
...
function submitMinesweeperAction(action, rowValue, colValue) {
  ...
  // 네트워크가 잠깐 멎은 동안 무제한 입력이 쌓여 모바일 메모리를 압박하지 않게 상한을 둔다.
  if (app.minesweeperActionQueue.length >= MINESWEEPER_ACTION_QUEUE_LIMIT) {
    toast('입력이 많이 밀렸습니다. 잠시 후 다시 눌러주세요.', 'error', 1400);
    return Promise.resolve(null);
  }
  // 같은 칸 공개는 결과가 같으므로 ACK 전 중복 터치만 조용히 합친다. 깃발은 토글 동작이라 합치지 않는다.
  if (action === 'reveal' && app.minesweeperActionQueue.some((item) => item.challengeId === challenge.id && item.action === 'reveal' && item.row === row && item.col === col)) {
    return Promise.resolve(null);
  }
...
function openMiniGame(challenge) {
  ...
  cleanupAppleBoardUi();
  if (challenge.gameId === 'block') challenge = syncBlockGameLocalChallenge(challenge) || challenge;
  let content = '';
  ...
  requestAnimationFrame(() => {
    ...
    if (challenge.gameId === 'block') { syncBlockGameLayout(); queueBlockGameSettlementIfNeeded(app.data?.activeMiniChallenge || challenge); }
...
function appleGameHasPendingInputs() {
  return Boolean(app.appleInputInFlight || app.appleCurrentInput || app.appleInputQueue.length);
}

function resetAppleInputQueue() {
  app.appleInputGeneration += 1;
  app.appleInputQueue = [];
  app.appleCurrentInput = null;
  app.appleInputInFlight = false;
}

function showAppleErrorOnce(message) {
  const key = String(message || '사과게임 입력을 처리하지 못했습니다.');
  const now = monotonicNow();
  if (app.appleLastErrorKey === key && now - app.appleLastErrorAt < 2_000) return;
  app.appleLastErrorKey = key;
  app.appleLastErrorAt = now;
  toast(key, 'error', 1200);
}

function appleSelectionBounds(start, end) {
  return {
...
function applyAppleOptimisticSelection(item, ui = app.appleBoardUi) {
  if (!ui || ui.challengeId !== item.challengeId || ui.boardGeneration !== item.boardGeneration) return false;
  const bounds = appleSelectionBounds(item.start, item.end);
  let changed = false;
  for (let row = bounds.minRow; row <= bounds.maxRow; row += 1) {
    for (let col = bounds.minCol; col <= bounds.maxCol; col += 1) {
      if (ui.values[row]?.[col] == null) continue;
      ui.values[row][col] = null;
      const cell = ui.cells[row * 10 + col];
      if (cell) { cell.textContent = ''; cell.classList.add('removed'); cell.classList.remove('selected'); }
      changed = true;
    }
  }
  if (changed) {
    ui.sumPrefix = buildApplePrefix(ui.values);
    ui.countPrefix = buildApplePrefix(ui.values, true);
    clearAppleSelection(ui);
  }
  return changed;
}

function overlayApplePendingSelections(values, challengeId, boardGeneration) {
  const next = values.map((row) => [...row]);
  const pending = [app.appleCurrentInput, ...app.appleInputQueue].filter(Boolean);
  for (const item of pending) {
    if (item.challengeId !== challengeId || item.boardGeneration !== boardGeneration) continue;
    const bounds = appleSelectionBounds(item.start, item.end);
    for (let row = bounds.minRow; row <= bounds.maxRow; row += 1) {
      for (let col = bounds.minCol; col <= bounds.maxCol; col += 1) next[row][col] = null;
    }
  }
  return next;
}

function applyAppleServerResult(result) {
  const challenge = app.data?.activeMiniChallenge;
  if (!challenge || challenge.gameId !== 'apple' || !result) return false;
  if (Array.isArray(result.board)) challenge.appleBoard = result.board.map((row) => [...row]);
  if (Number.isFinite(Number(result.pendingPoints))) challenge.applePendingPoints = Math.max(0, Number(result.pendingPoints));
  if (Number.isFinite(Number(result.score))) challenge.appleScore = Math.max(0, Number(result.score));
  if (Number.isFinite(Number(result.removedCount))) challenge.appleRemovedCount = Math.max(0, Number(result.removedCount));
  if (Number.isFinite(Number(result.availableMoves))) challenge.appleAvailableMoves = Math.max(0, Number(result.availableMoves));
  if (typeof result.newBoardAvailable === 'boolean') challenge.appleNewBoardAvailable = result.newBoardAvailable;
  if (Number.isFinite(Number(result.boardsGenerated))) challenge.appleBoardsGenerated = Math.max(1, Number(result.boardsGenerated));
  if (Number.isFinite(Number(result.serverTime))) app.data.serverTime = Number(result.serverTime);
  if (Number.isFinite(Number(result.revision))) app.revision = Math.max(app.revision, Number(result.revision));
  return true;
}

async function drainAppleInputQueue() {
  if (app.appleInputInFlight || !app.appleInputQueue.length) return;
  const generation = app.appleInputGeneration;
  const item = app.appleInputQueue.shift();
  const finishedChallenge = structuredClone(app.data?.activeMiniChallenge);
  app.appleCurrentInput = item;
  app.appleInputInFlight = true;
  try {
    const result = await api('/api/minigames/apple/select', {
      method: 'POST',
      body: JSON.stringify({
        challengeId: item.challengeId,
        startRow: item.start.row, startCol: item.start.col,
        endRow: item.end.row, endCol: item.end.col,
        boardGeneration: item.boardGeneration,
        requestId: item.requestId
      }),
      timeoutMs: BLOCK_GAME_API_TIMEOUT_MS
    });
    if (generation !== app.appleInputGeneration) return;
    if (result?.bootstrap) applyBootstrap(result.bootstrap);
    else applyAppleServerResult(result);
    app.appleCurrentInput = null;
    if (result?.boardRefreshed) {
      // 가능한 합10 영역이 사라져 서버가 새 판을 만든 순간, 이전 판에서 미리 받은
      // 좌표 입력은 새 판에 절대 재사용하지 않는다.
      resetAppleInputQueue();
      refreshAppleMiniOnly(result);
      return;
    }
    if (!result?.ok || result?.stale) {
      resetAppleInputQueue();
      showAppleErrorOnce(result?.message || '사과판이 변경되어 최신 상태로 맞췄습니다.');
      await loadBootstrap({ silent: true, renderMode: 'apple' });
      return;
    }
    if (result.finished) {
      resetAppleInputQueue();
      openMiniResult(result, finishedChallenge, { previousBest: Number(app.data?.dashboard?.pet?.records?.appleBestScore || 0) });
      return;
    }
    refreshAppleMiniOnly(result);
  } catch (error) {
    if (generation !== app.appleInputGeneration) return;
    resetAppleInputQueue();
    showAppleErrorOnce(error.message || '사과게임 연결을 다시 맞췄습니다.');
    await loadBootstrap({ silent: true, renderMode: 'apple' });
    return;
  } finally {
    if (generation === app.appleInputGeneration) {
      app.appleInputInFlight = false;
      app.appleCurrentInput = null;
      if (app.appleInputQueue.length) queueMicrotask(drainAppleInputQueue);
    }
  }
}

function submitAppleSelection(start, end, boardGeneration) {
  const challenge = app.data?.activeMiniChallenge;
  const ui = app.appleBoardUi;
  if (!challenge || challenge.gameId !== 'apple' || !ui || app.appleInputQueue.length >= APPLE_INPUT_QUEUE_LIMIT) return false;
  if (ui.challengeId !== challenge.id || Number(boardGeneration) !== Number(ui.boardGeneration)) return false;
  const requestId = crypto.randomUUID?.() || `apple-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const item = { challengeId: challenge.id, start: { ...start }, end: { ...end }, boardGeneration: Number(boardGeneration), requestId };
  if (!applyAppleOptimisticSelection(item, ui)) return false;
  app.appleInputQueue.push(item);
  drainAppleInputQueue();
  return true;
}
...
function refreshAppleMiniOnly(result = null) {
  ...
  const nextGeneration = Number(result?.boardsGenerated ?? challenge.appleBoardsGenerated ?? 1);
  const serverValues = (result?.board ?? challenge.appleBoard ?? []).map((row) => [...row]);
  const nextValues = overlayApplePendingSelections(serverValues, challenge.id, nextGeneration);
  for (let row = 0; row < 10; row += 1) {
    for (let col = 0; col < 10; col += 1) {
  ...
  if (score) score.textContent = `${Number(result?.score ?? challenge.appleScore ?? 0).toLocaleString('ko-KR')}점`;
  if (pending) pending.textContent = points(result?.pendingPoints ?? challenge.applePendingPoints ?? 0);
...
function refreshBlockMiniOnly(result = null) {
  ...
  const serverChallenge = app.data?.activeMiniChallenge;
  const optimistic = blockGameHasPendingInputs() && app.blockGameLocalChallenge?.gameId === 'block'
    && (!serverChallenge || app.blockGameLocalChallenge.id === serverChallenge.id);
  const challenge = optimistic ? app.blockGameLocalChallenge : serverChallenge;
  const board = $('#block-board');
  if (!challenge || challenge.gameId !== 'block' || !board || board.children.length !== 120) return false;
  const nextBoard = optimistic ? challenge.blockBoard : (result?.board ?? challenge.blockBoard);
  if (!Array.isArray(nextBoard) || nextBoard.length !== 12 || nextBoard.some((row) => !Array.isArray(row) || row.length !== 10)) return false;
  const version = Math.max(1, Number(optimistic ? challenge.blockBoardVersion : (result?.boardVersion ?? challenge.blockBoardVersion)) || 1);
  for (let row = 0; row < 12; row += 1) {
    for (let col = 0; col < 10; col += 1) {
      const index = row * 10 + col;
      ...
      const empty = value == null;
      const color = Math.max(0, Math.min(4, Number(value) || 0));
      current.className = empty ? 'block-cell block-empty-cell' : `block-cell color-${color}`;
      current.dataset.version = String(version);
      current.disabled = empty;
      current.setAttribute('aria-label', empty ? '빈 칸' : `${BLOCK_COLOR_NAMES[color]} 블록`);
    }
  }
  ...
  const helper = $('#block-helper');
  const pendingPoints = Math.max(0, Number(optimistic ? challenge.blockPendingPoints : (result?.pendingPoints ?? challenge.blockPendingPoints)) || 0);
  const remainingCount = Math.max(0, Number(optimistic ? challenge.blockRemainingCount : (result?.remainingCount ?? challenge.blockRemainingCount)) || 0);
  const availableGroups = Math.max(0, Number(optimistic ? challenge.blockAvailableGroups : (result?.availableGroups ?? challenge.blockAvailableGroups)) || 0);
  if (remaining) remaining.textContent = `${remainingCount}개`;
  if (groups) groups.textContent = `${availableGroups}개`;
  if (pending) pending.textContent = points(pendingPoints);
  if (stop) stop.textContent = '포기하기';
  if (helper) helper.textContent = availableGroups === 0
    ? '더 이상 제거할 그룹이 없어 포인트를 확정하는 중입니다.'
    : optimistic ? '입력을 순서대로 처리 중입니다. 계속 클릭해도 됩니다.' : (result?.message || '제거한 자리 위의 블록만 아래로 바로 내려옵니다. 좌우로는 움직이지 않습니다.');
  requestAnimationFrame(syncBlockGameLayout);
  if (!optimistic && availableGroups === 0) queueMicrotask(() => queueBlockGameSettlementIfNeeded(app.data?.activeMiniChallenge));
  return true;
}
...
function adminAuditLogsHtml(admin = app.data?.admin) {
  return (admin?.auditLogs ?? []).map((entry) => {
    ...
    else if (entry.action === 'legodoku_clear_ended') detail = `${esc(entry.adminDisplayName)} → 종료된 레고도쿠 방 비우기`;
  ...
  }).join('') || '<div class="empty">아직 운영 기록이 없습니다.</div>';
}

function refreshAdminModalPartial() {
  if (app.modal?.type !== 'admin' || !app.data?.admin?.isAdmin) return false;
  const modal = $('#modal-content');
  if (!modal) return false;
  const scrollTop = modal.scrollTop;
  const textarea = $('#admin-support-message');
  const toggle = $('#admin-support-form input[name="enabled"]');
  const draft = textarea?.value;
  const checked = toggle?.checked;
  const hadFocus = document.activeElement === textarea;
  const selectionStart = hadFocus ? textarea.selectionStart : null;
  const selectionEnd = hadFocus ? textarea.selectionEnd : null;
  const admin = app.data.admin;
  const memberList = $('#admin-member-list');
  const activeBungs = $('#admin-active-bungs');
  const auditLogs = $('#admin-audit-logs');
  if (memberList) memberList.innerHTML = adminMembersHtml(admin);
  if (activeBungs) activeBungs.innerHTML = adminActiveBungsHtml(admin);
  if (auditLogs) auditLogs.innerHTML = adminAuditLogsHtml(admin);
  const counts = {
    'admin-ended-omok-count': admin.endedOmokRooms,
    'admin-ended-block-count': admin.endedBlockBattleRooms,
    'admin-ended-sichuan-count': admin.endedSichuanRooms,
    'admin-ended-legodoku-count': admin.endedLegodokuRooms
  };
  for (const [id, value] of Object.entries(counts)) {
    const node = $(`#${id}`);
    if (node) node.textContent = String(Number(value || 0));
  }
  // 자동 bootstrap은 사용자가 편집 중인 후원 폼을 서버 값으로 덮지 않는다.
  if (textarea && draft != null) textarea.value = draft;
  if (toggle && checked != null) toggle.checked = checked;
  modal.scrollTop = scrollTop;
  if (hadFocus && textarea) {
    try { textarea.focus({ preventScroll: true }); } catch { textarea.focus(); }
    try { textarea.setSelectionRange(selectionStart, selectionEnd); } catch {}
    modal.scrollTop = scrollTop;
  }
  return true;
}

function diagnosticWebSocketState() {
  const state = Number(app.ws?.readyState);
  return ({ 0: 'connecting', 1: 'open', 2: 'closing', 3: 'closed' })[state] || 'none';
}

function buildAdminDiagnosticText(serverVersion = 'unknown') {
  const active = app.data?.activeMiniChallenge;
  const blockRoom = currentBlockBattleRoom();
  const sichuanRoom = currentSichuanRoom();
  const legodokuRoom = currentLegodokuRoom();
  const omokRoom = currentOmokRoom();
  const syncedAgoMs = Math.max(0, Math.round(monotonicNow() - Number(app.bootstrapSyncedAt || monotonicNow())));
  return [
    `LEGO LIFE GAME ${CLIENT_VERSION}`,
    `server=${serverVersion}`,
    `online=${navigator.onLine ? 'yes' : 'no'} visibility=${document.visibilityState || 'unknown'} ws=${diagnosticWebSocketState()}`,
    `revision=${Math.max(0, Number(app.revision) || 0)} bootstrapAgoMs=${syncedAgoMs}`,
    `tab=${app.tab || '-'} modal=${app.modal?.type || '-'}:${app.modal?.gameId || '-'}`,
    `mini=${active?.gameId || '-'}:${active?.id || '-'} completed=${Boolean(active?.completed)}`,
    `minesweeper=session:${app.minesweeperSessionActive ? 1 : 0} inFlight:${app.minesweeperActionInFlight ? 1 : 0} queued:${app.minesweeperActionQueue.length}`,
    `block=inFlight:${app.blockGameInputInFlight ? 1 : 0} queued:${app.blockGameInputQueue.length} recovery:${app.blockGameRecoveryInProgress ? 1 : 0} settle:${app.blockGameAutoSettleChallengeId || '-'}`,
    `rooms=omok:${omokRoom?.status || '-'} tetris:${blockRoom?.status || '-'} sichuan:${sichuanRoom?.status || '-'} legodoku:${legodokuRoom?.status || '-'}`
  ].join('\n');
}
...
async function copyAdminDiagnostics(button) {
  if (!app.data?.admin?.isAdmin) return;
  const original = button?.textContent || '진단 정보 복사';
  if (button) { button.disabled = true; button.textContent = '진단 확인 중…'; }
  let serverVersion = 'unavailable';
  try {
    const health = await api('/api/health', { timeoutMs: 5_000 });
    serverVersion = String(health?.version || 'unknown');
  } catch { /* 진단 복사는 서버가 불안정할 때도 클라이언트 상태를 남겨야 한다. */ }
  const text = buildAdminDiagnosticText(serverVersion);
  const copied = await copyPlainText(text);
  if (copied) toast('진단 정보를 복사했습니다.', 'success', 1800);
  else {
    console.info(text);
    toast('자동 복사에 실패했습니다. 개발자 콘솔에 진단 정보를 남겼습니다.', 'error', 2600);
  }
...
async function tick() {
  ...
  if (sichuanCountdown && sichuanRoom?.status === 'playing' && sichuanRoom.deadlineAt) {
    ...
    updateSichuanPenaltyUi(sichuanRoom);
...
async function handleAction(button, event = null) {
  ...
  if (action === 'profile') { if (Date.now() < app.profileClickSuppressUntil) return; return openProfile(idValue); }
  ...
  if (action === 'block-select') return queueBlockGameSelection(button);
  ...
  if (action === 'abandon-mini') {
    ...
    if (!confirm(`${josa(gameName, '을/를')} 포기할까요? 현재까지 획득한 포인트는 정산되며 사용한 개인게임 횟수는 돌아오지 않습니다.`)) return null;
    if (challenge.gameId === 'block') resetBlockGameInputQueue();
    const previousBest = Number(app.data?.dashboard?.pet?.records?.appleBestScore || 0);
  ...
  if (action === 'apple-new-board') {
    ...
    if (!challenge || challenge.gameId !== 'apple') return;
    const finishedChallenge = structuredClone(challenge);
    if (appleGameHasPendingInputs()) { toast('현재 선택을 처리 중입니다. 잠시 후 새 판을 눌러주세요.', 'error', 1100); return; }
    ...
    if (result?.ok) resetAppleInputQueue();
  ...
  if (action === 'admin-copy-diagnostics') return copyAdminDiagnostics(button);
...
function beginMinesweeperPointerGesture(event) {
  ...
  if (event.pointerType === 'mouse') {
    ...
    submitMinesweeperAction('reveal', cell.dataset.row, cell.dataset.col);
    return;
  }
...
navigator.serviceWorker?.register?.('/sw.js?v=6101280')?.catch(() => {});
