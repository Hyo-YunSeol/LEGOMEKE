...
function legodokuRoomView(room) {
  ...
  if (room.status === 'waiting') {
    ...
    stage = `<div class="legodoku-match-head">${legodokuProgressHtml(self, { mine: true })}<div class="legodoku-match-center"><span>남은 시간</span>${timer}<small>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${legodokuProgressHtml(opponent)}</div><div class="legodoku-board-stage">${legodokuBoardHtml(room, { interactive: true })}<div class="legodoku-feedback-layer" aria-live="polite"></div>${spectatorBurstLayer(room.reactions || [], 'legodoku')}</div><p class="helper centered">탭은 레고 놓기 · 빈칸을 드래그하면 X 표시 · X에서 드래그하면 지우기</p>`;
  } else if (isSpectator) {
