  ...
  socket.addEventListener('message', (event) => {
    try {
      ...
      if (payload.type === 'refresh') {
        ...
        if (app.modal?.type === 'shop') openShop();
...
function recordsView() {
  ...
  return `<section class="page-title"><span class="eyebrow">기록</span><h1>${esc(pet.displayName)}의 기록</h1><p>현재 레고의 핵심 기록만 표시합니다.</p></section><section class="section"><div class="record-grid">${compactMetric('세대', `${pet.generation}세대`)}${compactMetric('생존', `${records.days}게임일`)}${compactMetric('레벨', `Lv.${pet.stats.level}`)}${compactMetric('레고력', `${pet.stats.legoPower}`)}${compactMetric('포인트', points(pet.stats.points))}${compactMetric('최고 포인트', points(records.maxPoints))}${compactMetric('현재 경고', `${pet.warnings}회`)}${compactMetric('누적 경고', `${pet.records.warnings}회`)}${compactMetric('영토전 우승', `${records.territoryWins}회`)}${compactMetric('테트리스대전', `${records.blockBattleWins || 0}승 ${records.blockBattleLosses || 0}패`)}${compactMetric('번개 최고', records.bestReactionMs ? `${(records.bestReactionMs/1000).toFixed(3)}초` : '-')}${compactMetric('낚시', `${records.fishing}회`)}</div></section>${app.data.admin.isAdmin ? `<section class="section admin-callout"><div><h2>운영자 관리</h2><p>포인트 지급·회수, 일반 경고, 강퇴, 계정 삭제, 상태 초기화와 진행 중 벙·오목방 관리를 할 수 있습니다.</p></div><button class="primary" data-action="open-admin" type="button">관리 열기</button></section>` : ''}<section class="section"><button class="danger-button wide" data-action="logout" type="button">모든 기기에서 로그아웃</button></section>`;
}
...
async function buyLoudspeaker() {
  ...
  const raw = prompt(`확성기 메시지를 입력하세요. 최대 ${maxLength}자 · ${points(item.price)} · ${durationSeconds}초 표시`);
  if (raw == null) return null;
  const message = String(raw).replace(/\s+/g, ' ').trim();
  if (!message) {
    toast('확성기 메시지를 입력해주세요.', 'error');
    return null;
  }
  if ([...message].length > maxLength) {
    toast(`확성기 메시지는 ${maxLength}자 이내로 입력해주세요.`, 'error');
    return null;
  }
