...
function applyBootstrap(next) {
  ...
  const changed = !app.data || revision !== app.revision;
  app.data = next;
...
function logout(callServer = true) {
  ...
  clearInterval(app.blockBattleGravityTimer);
  clearTimeout(app.blockBattleFlushTimer);
  clearTimeout(app.blockBattleHoldDelay);
  clearInterval(app.blockBattleHoldTimer);
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (!chatOnlyUpdated && !composerProtected && !appleOnlyUpdated && !numberInputProtected) {
      render();
  ...
  socket.addEventListener('message', (event) => {
    try {
      ...
      if (payload.type === 'refresh') {
        clearTimeout(app.refreshTimer);
        const renderMode = payload.reason === 'liar-chat' ? 'liar-chat' : payload.reason === 'apple' ? 'apple' : payload.reason === 'spectator-reaction' ? 'games-live' : 'full';
        ...
        toast(payload.message || '블럭대전 입력을 처리하지 못했습니다.', 'error');
        if (payload.stale) loadBootstrap({ silent: true });
      }
    } catch { /* ignore */ }
  });
...
async function perform(path, body = {}, successMessage = null, method = 'POST', { renderMode = 'full', toastDuration = 3400, toastType = null } = {}) {
  ...
  const lockedControls = $$('#view .tab-pane:not(.hidden) button, #view .tab-pane:not(.hidden) input[type="submit"], #modal-root.open button, #modal-root.open input[type="submit"], .topbar button');
  ...
  try {
    ...
    if (!chatOnlyUpdated && !appleOnlyUpdated && !numberOnlyUpdated && !shopOnlyUpdated) {
      render();
      if (app.modal) {
        await refreshModal();
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=680" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=680`;
...
function rankingsSection() {
  ...
  return `<section class="section ranking-section">${sectionHeading('레고방 순위', '포인트·레벨과 게임 TOP 5')}<div class="rank-tabs-grid"><article class="rank-card"><h3>포인트 TOP 5</h3>${rankRows(rankings.points, (item) => points(item.points))}</article><article class="rank-card"><h3>레벨 TOP 5</h3>${rankRows(rankings.levels, (item) => `Lv.${item.level} · ${item.legoPower}`)}</article></div><div class="game-ranking-heading"><strong>게임 순위</strong><small>번개반응 · 사과게임 · 오목은 3일 시즌제 · 블럭대전은 누적 승패</small></div><div class="game-rank-grid"><article class="rank-card"><h3>⚡ 번개반응 TOP 5</h3>${reaction}${myRank(myGameRanks.reaction, (item) => `${(Number(item.ms || 0) / 1000).toFixed(3)}초`)}</article><article class="rank-card"><h3>🍎 사과게임 TOP 5</h3>${apple}${myRank(myGameRanks.apple, (item) => `${Number(item.score || 0).toLocaleString('ko-KR')}점`)}</article><article class="rank-card"><h3>⚫ 오목 TOP 5</h3>${omok}${myRank(myGameRanks.omok, (item) => `${item.wins}승 ${item.draws}무 ${item.losses}패`)}</article><article class="rank-card"><h3>🧱 블럭대전 TOP 5</h3>${blockBattle}${myRank(rankings.blockBattle?.mine, (item) => `${item.wins}승 ${item.losses}패`)}</article></div><div class="game-ranking-heading relation-ranking-heading"><strong>관계 현황</strong><small>기존 커플·찌르기 기록</small></div><div class="rank-tabs-grid"><article class="rank-card"><h3>커플</h3>${couples}</article><article class="rank-card"><h3>찌르기 TOP 5</h3>${pokes}</article></div></section>`;
}
...
function gamesView() {
  ...
  const resume = active
    ...
    : '';
  return `
    <section class="page-title"><span class="eyebrow">포인트 게임</span><h1>게임</h1><p>개인게임으로 포인트를 벌고, 단체게임에서 오목·블럭대전을 즐길 수 있습니다.</p></section>
...
function blockBattleLobby() {
  ...
  return `${sectionHeading('블럭대전', '10×20 · 7종 블럭 · 1:1 실시간 대전', `<button class="primary" data-action="block-battle-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<p class="helper block-battle-intro">여러 줄을 한 번에 지우면 상대에게 방해줄을 보냅니다. 시간이 지날수록 낙하 속도가 빨라지며 먼저 천장에 닿으면 패배합니다.</p><div class="block-battle-lobby-list">${cards}</div>`;
}
...
function blockBattleBoardHtml(player, { compact = false } = {}) {
  ...
  return `<div class="block-battle-board ${compact ? 'compact' : ''}" data-block-player="${esc(player.petId)}" role="grid" aria-label="${esc(player.displayName)}의 10열 20행 블럭판">${board.flatMap((row, rowIndex) => row.map((cell, colIndex) => `<span class="block-battle-cell ${cell ? `piece-${esc(cell)}` : ''}" data-row="${rowIndex}" data-col="${colIndex}"></span>`)).join('')}</div>`;
}
...
function blockBattlePlayerPanel(player, { mine = false, compact = false } = {}) {
  ...
  return `<article class="block-battle-player ${mine ? 'mine' : ''} ${compact ? 'compact' : ''}"><header><div><strong>${esc(player.displayName)}</strong>${mine ? '<small>내 게임판</small>' : ''}</div>${connection}</header>${blockBattleBoardHtml(player, { compact })}<div class="block-battle-stats"><span>제거 <b>${Number(player.lines || 0)}줄</b></span><span>공격 <b>${Number(player.attackSent || 0)}줄</b></span><span>대기 방해 <b>${Number(player.pendingGarbage || 0)}줄</b></span></div></article>`;
}
...
function blockBattleRoomView(room) {
  ...
  const attackNotice = room.status === 'playing' && Number(room.lastAttack?.lines) > 0
    ? `<div class="block-battle-attack ${attackFromMe || isSpectator ? 'sent' : 'incoming'}">${attackMessage}</div>` : '';
  const controls = isPlayer && room.status === 'playing'
    ? `<div class="block-battle-controls" aria-label="블럭 조작"><button data-action="block-battle-control" data-value="left" type="button">←<small>왼쪽</small></button><button data-action="block-battle-control" data-value="rotate" type="button">↻<small>회전</small></button><button data-action="block-battle-control" data-value="right" type="button">→<small>오른쪽</small></button><button data-action="block-battle-control" data-value="softDrop" type="button">↓<small>내리기</small></button><button class="hard-drop" data-action="block-battle-control" data-value="hardDrop" type="button">⇩<small>바로 내리기</small></button></div><p class="helper centered">키보드: ← → 이동 · ↑ 회전 · ↓ 내리기 · Space 바로 내리기</p>` : '';
  ...
  const boards = room.status === 'waiting'
    ...
    : `<div class="block-battle-stage">${spectatorBurstLayer(room.reactions || [], 'blockBattle')}${attackNotice}<div class="block-battle-versus">${blockBattlePlayerPanel(self, { mine: isPlayer })}<div class="block-battle-vs"><b>VS</b><span>속도 ${Math.max(1, Math.round(850 / Math.max(180, Number(room.gravityMs || 850))))}단계</span><span>관전자 ${Number(room.spectatorCount || 0)}명</span></div>${blockBattlePlayerPanel(opponent, { compact: true })}</div></div>`;
  return `${sectionHeading('블럭대전', `${room.roomNumber}번방 · 판돈 ${points(room.stakePoints)} · ${blockBattleStatusLabel(room.status)}`, '<button class="text-button" data-action="block-battle-back" type="button">로비 보기</button>')}${reconnecting}${boards}${result}${controls}<div class="button-row">${rematch}${leave}</div>${isSpectator ? `${spectatorReactionBar('block-battle', room.id, room.reactions || [], true)}<p class="helper">관전자는 블럭 조작·판돈·재대결에 참여할 수 없습니다.</p>` : ''}`;
}
...
function blockBattleMatrix(player) {
  const board = player?.board?.map((row) => [...row]) ?? Array.from({ length: 20 }, () => Array(10).fill(null));
  if (player?.active && LOCAL_TETROMINO_SHAPES[player.active.type]) {
    for (const [x, y] of LOCAL_TETROMINO_SHAPES[player.active.type][Number(player.active.rotation) || 0]) {
      const row = Number(player.active.row) + y;
      const col = Number(player.active.col) + x;
...
function blockBattleLocalCollision(player, piece) {
  return LOCAL_TETROMINO_SHAPES[piece.type][piece.rotation].some(([x, y]) => {
    const row = piece.row + y;
    const col = piece.col + x;
    return col < 0 || col >= 10 || row >= 20 || (row >= 0 && player.board[row]?.[col]);
  });
}

function paintBlockBattleBoard(player) {
  const board = document.querySelector(`.block-battle-board[data-block-player="${CSS.escape(String(player?.petId || ''))}"]`);
  if (!board) return false;
  const cells = board.children;
  const matrix = blockBattleMatrix(player);
  let index = 0;
  for (const row of matrix) for (const cell of row) {
    const className = `block-battle-cell${cell ? ` piece-${cell}` : ''}`;
...
function previewBlockBattleInput(action) {
  const room = currentBlockBattleRoom();
  ...
  if (action === 'rotate') {
    ...
    for (const offset of [0, -1, 1, -2, 2]) {
      ...
      if (!blockBattleLocalCollision(player, candidate)) { player.active = candidate; break; }
    }
  } else {
    ...
    } else if (!blockBattleLocalCollision(player, candidate)) player.active = candidate;
  }
  paintBlockBattleBoard(player);
  return true;
}

function syncBlockBattleGravity() {
  const room = currentBlockBattleRoom();
  const active = room?.viewerRole === 'player' && room.status === 'playing' && Object.values(room.players || {}).every((player) => player.connected);
  ...
  if (!active) return;
  app.blockBattleGravityTimer = setInterval(() => {
...
function applyBlockBattleRoomState(payload) {
  ...
  const previousSelf = previous?.players?.[previous.selfPetId];
  const nextSelf = room.players?.[room.selfPetId];
  const inputPaused = room.status !== 'playing' || Object.values(room.players || {}).some((player) => !player.connected);
  if (inputPaused || (previous?.matchId && previous.matchId !== room.matchId)) {
    clearTimeout(app.blockBattleFlushTimer);
    app.blockBattleFlushTimer = null;
    app.blockBattleInputBuffer.length = 0;
  }
  const needsLayout = !previous || previous.status !== room.status || previous.viewerRole !== room.viewerRole
    || Number(previousSelf?.lines || 0) !== Number(nextSelf?.lines || 0)
    || Number(previousSelf?.pendingGarbage || 0) !== Number(nextSelf?.pendingGarbage || 0)
    || previous.lastAttack?.at !== room.lastAttack?.at;
  if (app.blockBattleLobbyForced) {
    syncBlockBattleGravity();
    return true;
  }
  if (app.tab === 'games' && !needsLayout) {
    for (const player of Object.values(room.players || {})) paintBlockBattleBoard(player);
  } else if (app.tab === 'games') renderBlockBattleRegion();
  else markTabDirty('games');
  syncBlockBattleGravity();
  return true;
}

async function flushBlockBattleInputs() {
  clearTimeout(app.blockBattleFlushTimer);
  app.blockBattleFlushTimer = null;
  const room = currentBlockBattleRoom();
  const actions = app.blockBattleInputBuffer.splice(0, 24);
  ...
  if (app.ws?.readyState === WebSocket.OPEN) app.ws.send(JSON.stringify(message));
  else {
    try {
      const result = await api(`/api/block-battle/rooms/${encodeURIComponent(room.id)}/input`, { method: 'POST', body: JSON.stringify(message) });
      const next = result.blockBattle?.rooms?.find((item) => item.id === room.id);
      if (next) applyBlockBattleRoomState({ room: next, serverTime: result.blockBattle.serverTime });
    } catch (error) { toast(error.message, 'error'); await loadBootstrap({ silent: true }); }
  }
  if (app.blockBattleInputBuffer.length) app.blockBattleFlushTimer = setTimeout(flushBlockBattleInputs, 35);
}

function queueBlockBattleInput(action) {
  if (!['left', 'right', 'rotate', 'softDrop', 'hardDrop', 'tick'].includes(action)) return;
  const room = currentBlockBattleRoom();
  if (!room || room.viewerRole !== 'player' || room.status !== 'playing' || Object.values(room.players || {}).some((player) => !player.connected)) return;
  previewBlockBattleInput(action);
  app.blockBattleInputBuffer.push(action);
  if (app.blockBattleInputBuffer.length >= 12) flushBlockBattleInputs();
  else if (!app.blockBattleFlushTimer) app.blockBattleFlushTimer = setTimeout(flushBlockBattleInputs, 35);
}
...
function territoryViewPage() {
  ...
  return `<section class="page-title"><span class="eyebrow">6시간 시즌</span><h1>제${territory.seasonNumber}회 레고 영토전</h1><p>본진을 지키면서 주변 8칸으로 땅을 넓히고 상대 영토를 빼앗는 게임입니다.</p></section>${previousResult}<section class="section territory-summary"><div class="metric-grid">${compactMetric('남은 시간', `<span id="territory-countdown">${durationText(new Date(territory.endsAt).getTime() - serverNow)}</span>`, '', { raw: true })}${compactMetric('내 레벨', `Lv.${territory.my.level}`)}${compactMetric('내 영토', `${territory.my.owned}/${territory.my.limit}`)}${compactMetric('현재 순위', territory.my.rank ? `${territory.my.rank}위` : '-')}</div><div class="territory-next-reset"><strong>다음 초기화: ${territoryResetText(territory.endsAt)}</strong></div><div class="territory-limit-strip"><span>${esc(territoryLimitHint)}</span><button class="soft-button" data-action="territory-limits" type="button">ⓘ 레벨별 한도</button></div><div class="territory-golden-status">${goldenStatus}</div><div class="territory-golden-status"><b>${battleUnlocked ? '⚔️ 전면전 진행 중 · 본진 탈취 가능' : `🛡️ 본진 보호 중 · ${territory.cells.length}/${mapSize * mapSize}칸 점유`}</b></div><div class="territory-rule"><b>안내:</b> 영토전은 한국시간 00시·06시·12시·18시에 새 회차가 시작됩니다. 점령 시각부터 6시간을 계산하는 방식이 아니며, 현재 영토는 표시된 다음 초기화 시각까지 유지됩니다.</div><div class="territory-rule"><b>룰:</b> 빈 땅 점령은 무료, 상대 땅 탈취는 50P입니다. 처음에는 🏠 본진이 보호됩니다. 맵 25칸이 한 번이라도 모두 차면 ⚔️ 전면전이 열려 본진도 탈취할 수 있습니다. 전면전 이후 영토가 0칸이 되어도 다른 칸을 차지해 다시 참가할 수 있습니다. 보유 한도가 꽉 차면 빈 땅 추가 점령은 막히지만, 상대 땅은 50P를 내고 가장 오래된 일반 영토 한 칸을 옮기는 방식으로 계속 탈취할 수 있습니다.</div></section><section class="section territory-section">${sectionHeading('공용 맵', `${mapSize} × ${mapSize}`)}<div class="territory-scroll"><div class="territory-map" role="grid" aria-label="${mapSize}행 ${mapSize}열 레고 영토전 맵">${rows}</div></div><p class="helper centered">회색칸은 빈 땅, 색깔 닉네임은 소유자입니다. 🏠 본진은 전면전 전까지만 보호되며, 25/25칸이 모두 차면 이후 탈취 가능합니다. 황금 영토 위치는 종료 전에는 공개되지 않습니다.</p></section><section class="section">${sectionHeading('현재 순위', '단독 1위만 500P를 받습니다. 동률이면 종료 후 공개되는 황금 영토가 최종 승자를 가릅니다.')}<div class="rank-list">${ranking}</div></section>`;
}
...
function openTerritoryLimits() {
  ...
  openModal(`${modalHeader('영토 레벨별 보유 한도', note)}<div class="territory-limit-table">${rows}</div><p class="warning-box">한도가 꽉 차면 빈 땅 추가 점령은 제한됩니다. 상대 영토 탈취는 50P이며, 한도에 꽉 찬 상태에서는 가장 오래된 일반 영토 한 칸을 비우고 탈취한 칸으로 이동해 총 보유 수를 유지합니다. 전면전 전에는 본진은 자동으로 비우지 않습니다.</p>`, { type:'territoryLimits' });
}
...
function recordsView() {
  ...
  return `<section class="page-title"><span class="eyebrow">기록</span><h1>${esc(pet.displayName)}의 기록</h1><p>현재 레고의 핵심 기록만 표시합니다.</p></section><section class="section"><div class="record-grid">${compactMetric('세대', `${pet.generation}세대`)}${compactMetric('생존', `${records.days}게임일`)}${compactMetric('레벨', `Lv.${pet.stats.level}`)}${compactMetric('레고력', `${pet.stats.legoPower}`)}${compactMetric('포인트', points(pet.stats.points))}${compactMetric('최고 포인트', points(records.maxPoints))}${compactMetric('현재 경고', `${pet.warnings}회`)}${compactMetric('누적 경고', `${pet.records.warnings}회`)}${compactMetric('영토전 우승', `${records.territoryWins}회`)}${compactMetric('블럭대전', `${records.blockBattleWins || 0}승 ${records.blockBattleLosses || 0}패`)}${compactMetric('번개 최고', records.bestReactionMs ? `${(records.bestReactionMs/1000).toFixed(3)}초` : '-')}${compactMetric('낚시', `${records.fishing}회`)}</div></section>${app.data.admin.isAdmin ? `<section class="section admin-callout"><div><h2>운영자 관리</h2><p>포인트 지급·회수, 일반 경고, 강퇴, 계정 삭제, 상태 초기화와 기존 라이어게임 관리를 할 수 있습니다.</p></div><button class="primary" data-action="open-admin" type="button">관리 열기</button></section>` : ''}<section class="section"><button class="danger-button wide" data-action="logout" type="button">모든 기기에서 로그아웃</button></section>`;
}
...
function openMiniGame(challenge) {
  ...
  if (challenge.gameId === 'block') content = `<div class="block-game"><div class="block-hud"><div><small>남은 블록</small><strong>${Number(challenge.blockRemainingCount || 0)}개</strong></div><div><small>제거 가능 그룹</small><strong>${Number(challenge.blockAvailableGroups || 0)}개</strong></div><div><small>획득 예정</small><strong>${points(challenge.blockPendingPoints || 0)}</strong></div></div><p class="helper block-rules">같은 색 2개 이상 클릭 · 상하좌우만 연결 · 시간제한 없음</p><div class="block-controls"><button class="soft-button wide" data-action="stop-mini" type="button">그만하고 ${points(challenge.blockPendingPoints || 0)} 받기</button></div><div id="block-board" class="block-board" role="grid" aria-label="블록게임 12행 10열 색상판">${blockBoardHtml(challenge)}</div><p class="block-helper">제거한 자리 위의 블록만 아래로 바로 내려옵니다. 좌우로는 움직이지 않습니다.</p><p class="block-reward-guide"><b>포인트</b> 2개 5P · 3개 9P · 4개 13P · 5개 18P · 6개 23P · 7개 29P · 8개 35P · 9개 42P · 10~12개 52P · 13~15개 65P · 16개+ 80P · ALL CLEAR +100P</p></div>`;
...
function openCreateBlockBattle() {
  ...
  openModal(`${modalHeader('블럭대전 방 만들기', `보유 포인트 ${points(balance)}`)}<form id="block-battle-create-form" class="stack-form"><label>판돈<select name="stakePoints">${options}</select></label><p class="warning-box">상대가 참가해 대전이 확정될 때 양쪽에서 같은 판돈을 한 번만 보관합니다. 승자는 두 사람의 판돈 전액을 받습니다.</p><button class="primary wide" type="submit">방 만들기</button></form>`, { type: 'blockBattleCreate' });
}
...
async function handleAction(button) {
  ...
  if (action === 'block-select') {
    ...
    const result = await perform('/api/minigames/block/select', {
      ...
      requestId
    }, null, 'POST', { toastDuration: 800, toastType: 'game-start' });
  ...
  if (action === 'block-battle-control') {
    if (Date.now() - Number(button.dataset.pointerHandledAt || 0) < 500) return;
    return queueBlockBattleInput(button.dataset.value);
  }
  if (action === 'block-battle-reaction') return api(`/api/block-battle/rooms/${encodeURIComponent(idValue)}/reaction`, { method: 'POST', body: JSON.stringify({ type: button.dataset.reaction }) }).catch((error) => toast(error.message, 'error'));
  ...
  if (action === 'claim-territory-direct') {
    ...
    try {
      return await perform('/api/territory/claim', { row, col, requestId });
    } finally {
...
function switchMainTab(tabName, { smooth = false } = {}) {
  ...
  if (!changed) {
    updateAppChrome();
    if (app.dirtyTabs.has(tabName)) scheduleTabRender(tabName);
    window.scrollTo({ top: 0, behavior: smooth ? 'smooth' : 'auto' });
    return;
  }

  if (app.tab === 'social') renderResidentRegion(false);
  if (app.tab === 'games') disconnectOmokBoardObserver();
  app.tab = tabName;
...
document.addEventListener('pointerdown', (event) => {
  ...
  if (blockControl && event.isPrimary !== false && (event.pointerType !== 'mouse' || event.button === 0)) {
    ...
    blockControl.dataset.pointerHandledAt = String(Date.now());
    queueBlockBattleInput(action);
    clearTimeout(app.blockBattleHoldDelay);
    clearInterval(app.blockBattleHoldTimer);
    if (['left', 'right', 'softDrop'].includes(action)) {
      app.blockBattleHoldDelay = setTimeout(() => {
        app.blockBattleHoldTimer = setInterval(() => queueBlockBattleInput(action), 75);
      }, 180);
    }
    return;
  }
...
const stopBlockBattleHold = () => {
  clearTimeout(app.blockBattleHoldDelay);
  clearInterval(app.blockBattleHoldTimer);
  app.blockBattleHoldDelay = null;
  app.blockBattleHoldTimer = null;
};
document.addEventListener('pointerup', stopBlockBattleHold, { passive: true });
document.addEventListener('pointercancel', stopBlockBattleHold, { passive: true });
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=680').catch(() => {});
