...
function openModal(html, descriptor = {}) {
  ...
  setAppleModalMode(descriptor.type === 'mini' && descriptor.gameId === 'apple');
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=671" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=671`;
...
function preserveLiarScrollDuringViewportChange() {
  if (app.appleModalActive) requestAnimationFrame(syncAppleGameLayout);
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=671').catch(() => {});
