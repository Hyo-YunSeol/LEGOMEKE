...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (!chatOnlyUpdated && !appleOnlyUpdated) {
      render();
...
async function perform(path, body = {}, successMessage = null, method = 'POST', { renderMode = 'full', toastDuration = 3400, toastType = null } = {}) {
  ...
  try {
    ...
    if (!chatOnlyUpdated && !appleOnlyUpdated && !shopOnlyUpdated) {
      render();
...
function avatar(stage, { mini = false } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=661`;
...
function recordsView() {
  ...
  return `<section class="page-title"><span class="eyebrow">기록</span><h1>${esc(pet.displayName)}의 기록</h1><p>현재 레고의 핵심 기록만 표시합니다.</p></section><section class="section"><div class="record-grid">${compactMetric('세대', `${pet.generation}세대`)}${compactMetric('생존', `${records.days}게임일`)}${compactMetric('레벨', `Lv.${pet.stats.level}`)}${compactMetric('레고력', `${pet.stats.legoPower}`)}${compactMetric('포인트', points(pet.stats.points))}${compactMetric('최고 포인트', points(records.maxPoints))}${compactMetric('현재 경고', `${pet.warnings}회`)}${compactMetric('누적 경고', `${pet.records.warnings}회`)}${compactMetric('영토전 우승', `${records.territoryWins}회`)}${compactMetric('라이어 승리', `${records.liarWins}회`)}${compactMetric('번개 최고', records.bestReactionMs ? `${(records.bestReactionMs/1000).toFixed(3)}초` : '-')}${compactMetric('낚시', `${records.fishing}회`)}</div></section>${app.data.admin.isAdmin ? `<section class="section admin-callout"><div><h2>운영자 관리</h2><p>포인트 지급·회수, 일반 경고, 강퇴, 계정 삭제, 상태 초기화와 라이어게임 관리를 할 수 있습니다.</p></div><button class="primary" data-action="open-admin" type="button">관리 열기</button></section>` : ''}<section class="section"><button class="danger-button wide" data-action="logout" type="button">모든 기기에서 로그아웃</button></section>`;
}
...
function updateVisualViewportVars() {
  ...
  const rawHeight = Number(viewport?.height ?? window.innerHeight ?? root.clientHeight ?? 0);
  const rawTop = Number(viewport?.offsetTop ?? 0);
  const height = Number.isFinite(rawHeight) ? Math.max(0, Math.round(rawHeight)) : 0;
  const top = Number.isFinite(rawTop) ? Math.max(0, Math.round(rawTop)) : 0;
...
function render() {
  ...
  ensureTabPanes();
  showTabPane(app.tab);
  // 서버 데이터가 실제로 바뀐 경우에만 현재 탭을 갱신한다. 보이지 않는 탭은 눌렀을 때까지 렌더하지 않는다.
  renderTab(app.tab);
  updateAppChrome();
}
...
function shopActionFor(item, shop) {
  ...
  if (item.id === 'lottery') {
    ...
    if (plays >= 2) return '<button class="primary" type="button" disabled>오늘 완료</button>';
...
function shopStatusFor(item, shop) {
  if (item.id === 'miniGame10' && shop.miniGameBonus) return `오늘 +${shop.miniGameBonus}회 · 총 ${shop.miniGamesLimit}회`;
  if (item.id === 'fishing5' && shop.fishingBonus) return `오늘 +${shop.fishingBonus}회 · 총 ${shop.fishingLimit}회`;
  ...
  if (item.id === 'lottery' && shop.lastLotteryResult) return `오늘 ${shop.lotteryPlays}/2회 · 최근 ${shop.lastLotteryResult.prize ? points(shop.lastLotteryResult.prize) : '꽝'}`;
...
function openShop() {
  ...
  openModal(`${modalHeader('상점', `보유 포인트 ${points(app.data.dashboard.pet.stats.points)}`)}<div class="shop-grid">${cards}</div><p class="helper centered">구매와 지급은 서버에서 한 번만 처리됩니다. 횟수권과 복권은 한국시간 00·06·12·18시 초기화 기준입니다.</p>`, { type: 'shop' });
}
...
function setupAppleBoardInteractions() {
  ...
  board.addEventListener('pointerdown', (event) => {
    ...
    const cell = appleCellFromPoint(event.clientX, event.clientY, ui.layout);
    if (!cell) return;
    event.preventDefault();
    ui.drag = { pointerId: event.pointerId, start: cell, end: cell };
  ...
  board.addEventListener('pointermove', (event) => {
    ...
    event.preventDefault();
    ui.pendingPoint = { clientX: event.clientX, clientY: event.clientY };
...
function switchMainTab(tabName, { smooth = false } = {}) {
  ...
  if (!app.renderedTabs.has(tabName)) {
    renderTab(tabName, { force: true });
  } else if (app.dirtyTabs.has(tabName)) {
    // 캐시된 화면을 먼저 한 프레임 실제로 그린 뒤 최신 데이터로 교체한다.
    // 빠르게 여러 탭을 눌렀을 때 예전 탭의 무거운 렌더가 뒤늦게 실행되지 않게 토큰으로 취소한다.
    const token = ++app.tabSwitchToken;
    requestAnimationFrame(() => {
      if (!app.data || app.tab !== tabName || app.tabSwitchToken !== token) return;
      requestAnimationFrame(() => {
        if (!app.data || app.tab !== tabName || app.tabSwitchToken !== token || !app.dirtyTabs.has(tabName)) return;
        renderTab(tabName, { force: true });
        updateAppChrome();
      });
    });
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=661').catch(() => {});
