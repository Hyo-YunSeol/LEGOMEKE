import { cloneBlockBattleVisualPlayer, reconcileBlockBattleVisual } from './block-battle-visual-state.js?v=6101250';
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (numberInputProtected || blockInputProtected || blockBattleLivePatched || sichuanLivePatched || legodokuLivePatched) markTabDirty('games');
    if (!appleOnlyUpdated && !blockOnlyUpdated && !blockInputProtected && !minesweeperOnlyUpdated && !numberInputProtected && !blockBattleLivePatched && !sichuanLivePatched && !legodokuLivePatched) {
      render();
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=6101250" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=6101250`;
...
function patchLegodokuMarksDom(room = currentLegodokuRoom()) {
  ...
  document.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
    ...
    if (hasHead) {
      ...
      if (!cell.querySelector('.legodoku-head')) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101250" alt="" draggable="false">');
...
function patchLegodokuSpectatorBoardsDom(room = currentLegodokuRoom()) {
  ...
  root.querySelectorAll('[data-legodoku-player-board]').forEach((board) => {
    ...
    board.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
      ...
      if (hasHead && !existing) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101250" alt="" draggable="false">');
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=6101250`;
}
...
navigator.serviceWorker?.register?.('/sw.js?v=6101250')?.catch(() => {});
