...
import { renderSpotDifferenceAnswerMarks, renderSpotDifferenceScene, spotDifferenceHitIdAt, spotDifferenceThemeLabel } from './spot-difference-scene.js?v=610115';
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610115" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610115`;
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610115`;
}
...
function spotDifferenceLobby() {
  ...
  const rooms = (game?.rooms ?? []).filter((room) => room.status !== 'ended');
  const byNumber = new Map(rooms.map((room) => [room.roomNumber, room]));
  const cards = Array.from({ length: game?.maxRooms || 5 }, (_, index) => {
    const room = byNumber.get(index + 1);
...
navigator.serviceWorker?.register?.('/sw.js?v=610115')?.catch(() => {});
