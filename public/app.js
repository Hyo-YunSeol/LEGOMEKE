...
import { renderSpotDifferenceScene, spotDifferenceHitIdAt, spotDifferenceThemeLabel } from './spot-difference-scene.js?v=610116';
...
const app = {
  ...
  tickTimer: null,
  countdownFloors: new Map(),
  elapsedCeilings: new Map(),
  spotDifferenceClockTimer: null,
  ...
  spotDifferenceRoomId: null,
  spotDifferenceLobbyForced: false,
  spotDifferenceServerSyncedAt: monotonicNow(),
  spotDifferenceLastRefreshAt: 0,
  spotDifferencePointerSuppressUntil: 0,
  spotDifferenceRevealUntil: new Map(),
  spotDifferencePendingClicks: new Set(),
  spotDifferenceWrongFlashAt: 0,
  davinciRoomId: null,
  ...
  // 첫 lock ACK 전에 사용자가 Space를 다시 누르면 입력을 버리지 않고 다음 블록용으로 예약한다.
  blockBattleDeferredHardDrops: 0,
  ...
  blockBattleViewportLayoutKey: '',
  blockBattleViewportLockedHeight: 0,
  blockBattleViewportLockedWidth: 0,
...
function stableCountdownSeconds(key, deadlineMs, nowMs) {
  const deadline = Number(deadlineMs);
  const now = Number(nowMs);
  if (!Number.isFinite(deadline) || !Number.isFinite(now)) return 0;
  const raw = Math.max(0, Math.ceil((deadline - now) / 1000));
  const previous = app.countdownFloors.get(key);
  if (!previous || previous.deadline !== deadline) {
    app.countdownFloors.set(key, { deadline, seconds: raw });
    return raw;
  }
  // 같은 phase/deadline에서는 서버 재동기화나 네트워크 지터가 있어도 숫자가 역행하지 않는다.
  const seconds = Math.min(previous.seconds, raw);
  previous.seconds = seconds;
  return seconds;
}

function clearCountdownKeys(prefix) {
  for (const key of app.countdownFloors.keys()) if (String(key).startsWith(prefix)) app.countdownFloors.delete(key);
}

function stableElapsedMilliseconds(key, startedAtMs, nowMs) {
  const startedAt = Number(startedAtMs);
  const now = Number(nowMs);
  if (!Number.isFinite(startedAt) || !Number.isFinite(now)) return 0;
  const raw = Math.max(0, now - startedAt);
  const previous = app.elapsedCeilings.get(key);
  if (!previous || previous.startedAt !== startedAt) {
    app.elapsedCeilings.set(key, { startedAt, milliseconds: raw });
    return raw;
  }
  // 경과시간은 서버 시각이 재동기화되어도 뒤로 가지 않는다.
  const milliseconds = Math.max(previous.milliseconds, raw);
...
function supportBannerShouldHide() {
  ...
  return Boolean(currentOmokRoom() || currentBlockBattleRoom() || currentSichuanRoom() || currentSpotDifferenceRoom() || currentDavinciRoom());
}
...
function applyBootstrap(next) {
  ...
  app.spotDifferenceServerSyncedAt = app.bootstrapSyncedAt;
  preloadSichuanTiles();
...
function logout(callServer = true) {
  ...
  clearInterval(app.spotDifferenceClockTimer);
  app.spotDifferenceClockTimer = null;
  app.countdownFloors.clear();
  app.elapsedCeilings.clear();
  app.pollTimer = null;
  ...
  app.spotDifferenceRoomId = null;
  app.spotDifferenceLobbyForced = false;
  app.spotDifferenceServerSyncedAt = monotonicNow();
  app.spotDifferenceLastRefreshAt = 0;
  app.spotDifferencePointerSuppressUntil = 0;
  app.spotDifferenceRevealUntil = new Map();
  app.spotDifferencePendingClicks = new Set();
  ...
  app.blockBattleDeferredHardDrops = 0;
  ...
  app.blockBattleViewportLayoutKey = '';
  app.blockBattleViewportLockedHeight = 0;
  app.blockBattleViewportLockedWidth = 0;
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (!hadData && (currentOmokRoom() || currentBlockBattleRoom() || currentSichuanRoom() || currentSpotDifferenceRoom() || currentDavinciRoom())) {
      app.tab = 'games';
      app.omokLobbyForced = false;
      app.blockBattleLobbyForced = false;
      app.sichuanLobbyForced = false;
    ...
    if (numberInputProtected || blockBattleLivePatched || sichuanLivePatched) markTabDirty('games');
    if (!appleOnlyUpdated && !blockOnlyUpdated && !minesweeperOnlyUpdated && !numberInputProtected && !blockBattleLivePatched && !sichuanLivePatched) {
      render();
    ...
    } else if (blockBattleLivePatched || sichuanLivePatched) {
      updateAppChrome();
...
function connectRealtime() {
  ...
  app.elapsedCeilings.clear();
      ...
      } else if (payload.type === 'spot-difference-state') {
        applySpotDifferenceRoomState(payload);
...
async function refreshModal() {
  ...
  if (descriptor.type === 'omokCreate' || descriptor.type === 'blockBattleCreate' || descriptor.type === 'sichuanCreate' || descriptor.type === 'spotDifferenceCreate' || descriptor.type === 'davinciCreate') return;
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610116" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610116`;
...
function rankingsSection() {
  ...
  const spotDifference = rankRows(rankings.spotDifference?.top ?? [], (item) => `${item.wins}승 ${item.draws}무 ${item.losses}패`);
...
function gamesView() {
  ...
  const resume = active
    ...
    : '';
  return `
    <section class="page-title"><span class="eyebrow">포인트 게임</span><h1>게임</h1><p>개인게임으로 포인트를 벌고, 싱글 테트리스 기록에 도전하거나 단체게임에서 오목·테트리스대전·사천성·틀린그림찾기·다빈치코드를 즐길 수 있습니다.</p></section>
    ...
    <section class="section spot-difference-wrap">${spotDifferenceSection()}</section>
...
function battleUsageText() {
  ...
  return `대전 ${used}/${limit}회 · 오목·테트리스·사천성·틀린그림·다빈치 합산`;
}
...
function omokLobby() {
  ...
  return `${sectionHeading('오목게임', `15×15 · 1:1 실시간 대전 · 한 수 30초 · ${battleUsageText()}`, `<button class="primary" data-action="omok-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="omok-lobby-list">${cards}</div>`;
}
...
function commonBattleResultOverlay(game, room, { totalPot = 0 } = {}) {
  ...
  let winnerName = '';
  if (game === 'davinci') winnerName = davinciPlayerName(room, room.winnerPetId);
  else winnerName = room.players?.[room.winnerPetId]?.displayName
...
async function dismissBattleResult(game, roomId, key, viewerRole = 'none') {
  ...
  const basePath = ({
    ...
    davinci: '/api/davinci/rooms',
    spotDifference: '/api/spot-difference/rooms'
  })[game];
  ...
  if (game === 'spotDifference') { app.spotDifferenceRoomId = null; app.spotDifferenceLobbyForced = true; app.spotDifferencePendingClicks.clear(); }
  if (game === 'davinci') { app.davinciRoomId = null; app.davinciLobbyForced = true; app.davinciGuessTarget = null; }
  markTabDirty('games');
  render();
...
function omokRoomView(room) {
  ...
  if (room.status === 'playing' && room.phase === 'rps') {
    ...
    const controls = isPlayer ? (mine ? `<div class="davinci-selected">${emoji[mine]} ${label[mine]} 선택 완료 · 상대를 기다리는 중</div>` : `<div class="davinci-rps-buttons"><button data-action="omok-rps" data-value="scissors" type="button">✌️<small>가위</small></button><button data-action="omok-rps" data-value="rock" type="button">✊<small>바위</small></button><button data-action="omok-rps" data-value="paper" type="button">✋<small>보</small></button></div>`) : '<p class="helper centered">가위바위보 결과를 기다리는 중입니다.</p>';
    pregame = `<div class="davinci-action-card"><strong>가위바위보 · ${Number(room.rpsRound || 1)}라운드</strong><p>승자가 흑돌 또는 백돌을 선택합니다.</p>${controls}</div>`;
...
const BLOCK_BATTLE_MAX_DEFERRED_HARD_DROPS = 8;
...
function singleTetrisTimeText(ms = singleTetrisTimeLeftMs()) {
  const state = app.singleTetrisState;
  const end = new Date(state?.expiresAt || '').getTime();
  let total = Math.max(0, Math.ceil(Math.max(0, Number(ms) || 0) / 1000));
  if (state?.challengeId && Number.isFinite(end)) {
    const now = end - Math.max(0, Number(ms) || 0);
    total = stableCountdownSeconds(`single-tetris:${state.challengeId}:${state.expiresAt}`, end, now);
  }
...
function blockBattleLobby() {
  ...
  return `${sectionHeading('테트리스대전', `10×20 · 7종 블록 · 1:1 실시간 대전 · ${battleUsageText()}`, `<button class="primary" data-action="block-battle-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<p class="helper block-battle-intro">여러 줄을 한 번에 지우면 상대에게 방해줄을 보냅니다. 낙하 속도는 일정하며 블록이 천장까지 쌓이면 패배합니다.</p><div class="block-battle-lobby-list">${cards}</div>`;
}
...
function sichuanLobby() {
  ...
  return `${sectionHeading('사천성대전', `8×10 · 동일한 시작판 · 1:1 속도전 · 2분 30초 · ${battleUsageText()}`, `<button class="primary" data-action="sichuan-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="sichuan-rule-strip"><span>🀄 같은 그림 2개</span><span>↪ 최대 2번 꺾기</span><span>🚫 막히면 즉시 패배</span></div><p class="helper sichuan-intro">두 플레이어가 완전히 같은 배치로 동시에 시작합니다. 각자 자기 판을 풀며 먼저 전부 지우면 승리합니다.</p><div class="sichuan-lobby-list">${cards}</div>`;
}

function sichuanTileMap() {
  return new Map((app.data?.sichuan?.tiles ?? []).map((tile) => [tile.id, tile]));
}
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610116`;
}

function preloadSichuanTiles() {
  if (typeof Image !== 'function') return;
  for (const tile of app.data?.sichuan?.tiles ?? []) {
...
function sichuanRoomView(room) {
  ...
  const body = `${sectionHeading('사천성대전', `${room.roomNumber}번방 · 판돈 ${points(room.stakePoints)} · ${sichuanStatusLabel(room.status)}`, '<button class="text-button" data-action="sichuan-back" type="button">로비 보기</button>')}<div class="sichuan-rule-strip compact"><span>동일 시작판</span><span>2:30</span><span>막히면 패배</span></div>${stage}<div class="button-row">${rematch}${leave}</div>${reaction}${commonBattleResultOverlay('sichuan', room, { totalPot: Number(room.stakePoints || 0) * 2 })}`;
...
function spotDifferenceServerNow() {
  const base = Number(app.data?.spotDifference?.serverTime || app.data?.serverTime || Date.now());
  return base + (monotonicNow() - app.spotDifferenceServerSyncedAt);
}

function currentSpotDifferenceRoom() {
  if (app.spotDifferenceLobbyForced) return null;
  const rooms = app.data?.spotDifference?.rooms ?? [];
  let room = rooms.find((item) => item.id === app.spotDifferenceRoomId);
  if (!room) room = rooms.find((item) => item.viewerRole === 'player' || item.viewerRole === 'spectator');
  if (room) app.spotDifferenceRoomId = room.id;
  return room ?? null;
}

function spotDifferenceStatusLabel(status) {
  return ({ waiting: '대기중', playing: '게임중', ended: '종료' })[status] || status;
}

function spotDifferenceLobby() {
  const game = app.data?.spotDifference;
  const rooms = (game?.rooms ?? []).filter((room) => room.status !== 'ended');
  const byNumber = new Map(rooms.map((room) => [room.roomNumber, room]));
  const cards = Array.from({ length: game?.maxRooms || 3 }, (_, index) => {
    const room = byNumber.get(index + 1);
    if (!room) return `<article class="spot-lobby-card empty-room"><div><strong>${index + 1}번방</strong><small>비어있음</small></div></article>`;
    const versus = room.guest ? `${esc(room.host?.displayName || '-')} VS ${esc(room.guest?.displayName || '-')}` : esc(room.host?.displayName || '-');
    let action = '';
    if (room.viewerRole !== 'none') action = `<button class="soft-button" data-action="spot-difference-open" data-id="${esc(room.id)}" type="button">열기</button>`;
    else if (room.status === 'waiting') action = `<button class="primary" data-action="spot-difference-join" data-id="${esc(room.id)}" type="button">참가</button>`;
    else if (room.status === 'playing') action = `<button class="soft-button" data-action="spot-difference-spectate" data-id="${esc(room.id)}" type="button">관전</button>`;
    else action = `<button class="ghost" data-action="spot-difference-open" data-id="${esc(room.id)}" type="button">결과</button>`;
    return `<article class="spot-lobby-card"><div><strong>${room.roomNumber}번방 · ${versus}</strong><small>판돈 ${points(room.stakePoints)} · ${spotDifferenceStatusLabel(room.status)}</small></div>${action}</article>`;
  }).join('');
  const canCreate = rooms.length < (game?.maxRooms || 3);
  return `${sectionHeading('틀린그림찾기', `7개 · 45초 · 힌트 없음 · ${battleUsageText()}`, `<button class="primary" data-action="spot-difference-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="spot-rule-strip"><span>🔎 틀린 곳 7개</span><span>⏱ 45초</span><span>❌ 오답 1초 잠금</span><span>👁 종료 후 정답 공개</span></div><p class="helper spot-intro">테마·장면·틀린 위치가 매판 자동으로 바뀝니다. 원본과 변경본 어느 쪽을 눌러도 정답으로 인정됩니다.</p><div class="spot-lobby-list">${cards}</div>`;
}

function spotDifferenceResultKey(room) {
  return `${room?.id || 'room'}:${room?.matchId || room?.endedAt || 'match'}`;
}

function spotDifferenceRevealUntilFor(room) {
  if (!room || room.status !== 'ended') return 0;
  const key = spotDifferenceResultKey(room);
  if (!app.spotDifferenceRevealUntil.has(key)) app.spotDifferenceRevealUntil.set(key, Date.now() + 5_000);
  return Number(app.spotDifferenceRevealUntil.get(key) || 0);
}

function spotDifferenceProgress(player, { mine = false } = {}) {
  const found = Math.max(0, Math.min(7, Number(player?.foundCount || player?.foundIds?.length || 0)));
  return `<div class="spot-progress ${mine ? 'mine' : ''}" data-spot-pet-id="${esc(player?.petId || '')}"><div><strong>${esc(player?.displayName || '-')}</strong>${mine ? '<small>내 진행</small>' : ''}</div><div class="spot-progress-track"><i style="width:${(found / 7) * 100}%"></i></div><b>${found}/7</b></div>`;
}

function spotDifferencePicture(room, { changed = false, foundIds = [], revealAll = false, interactive = false } = {}) {
  const label = changed ? '변경된 그림' : '원본 그림';
  const scene = renderSpotDifferenceScene(room.puzzle, { changed, foundIds, revealAll, ariaLabel: `${spotDifferenceThemeLabel(room.puzzle?.themeId)} ${label}` });
  return `<article class="spot-picture-panel"><header><strong>${label}</strong><small>${changed ? '다른 부분을 찾아보세요' : '기준 그림'}</small></header><button class="spot-picture-button" data-action="spot-difference-image" data-side="${changed ? 'changed' : 'original'}" data-id="${esc(room.id)}" type="button" aria-label="${label}에서 틀린 곳 선택" ${interactive ? '' : 'disabled'}>${scene}<span class="spot-picture-blind" aria-hidden="true"><b data-spot-start-countdown>3</b><small>잠시 후 시작</small></span></button></article>`;
}

function spotDifferenceRoomView(room) {
  const isPlayer = room.viewerRole === 'player';
  const isSpectator = room.viewerRole === 'spectator';
  const self = isPlayer ? room.players?.[room.selfPetId] : null;
  const opponent = isPlayer ? room.players?.[room.opponentPetId] : null;
  const hostPlayer = room.players?.[room.host?.petId];
  const guestPlayer = room.players?.[room.guest?.petId];
  const now = spotDifferenceServerNow();
  const revealAt = new Date(room.revealAt || '').getTime();
  const ready = room.status === 'playing' && (!Number.isFinite(revealAt) || now >= revealAt);
  const interactive = Boolean(isPlayer && ready && self);
  const revealAll = room.status === 'ended';
  const foundIds = isPlayer ? (self?.foundIds ?? []) : [];
  const themeLabel = room.puzzle ? spotDifferenceThemeLabel(room.puzzle.themeId) : '랜덤 테마';
  let stage = '';
  if (room.status === 'waiting') {
    stage = `<div class="omok-wait"><strong>${esc(room.host?.displayName || '')}</strong><p>랜덤 문제로 대결할 상대를 기다리는 중입니다.</p></div>`;
  } else {
    const leftProgress = isPlayer ? self : hostPlayer;
    const rightProgress = isPlayer ? opponent : guestPlayer;
    const timerText = room.status === 'playing' ? '<b id="spot-difference-countdown">0:45</b>' : '<b>종료</b>';
    stage = `<div class="spot-match-head">${spotDifferenceProgress(leftProgress, { mine: isPlayer })}<div class="spot-match-center"><span>${esc(themeLabel)}</span>${timerText}<small>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${spotDifferenceProgress(rightProgress)}</div><div class="spot-picture-grid ${ready || revealAll ? 'ready' : 'counting'}">${spotDifferencePicture(room, { changed: false, foundIds, revealAll, interactive })}${spotDifferencePicture(room, { changed: true, foundIds, revealAll, interactive })}${spectatorBurstLayer(room.reactions || [], 'spotDifference')}</div>`;
    if (room.status === 'playing') stage += `<p class="helper centered">두 그림 어디를 눌러도 됩니다. 틀리면 1초 동안 입력이 잠기며 상대에게는 찾은 개수만 보입니다.</p>`;
    if (revealAll) {
      const revealUntil = spotDifferenceRevealUntilFor(room);
      const remain = Math.max(0, Math.ceil((revealUntil - Date.now()) / 1000));
      if (remain > 0) stage += `<div class="spot-answer-reveal"><strong>정답 7곳 공개</strong><span>결과까지 <b data-spot-answer-countdown>${remain}</b>초</span></div>`;
    }
  }
  const rematch = isPlayer && room.status === 'ended' ? `<button class="primary" data-action="spot-difference-rematch" data-id="${esc(room.id)}" type="button">${room.rematchRequestedByMe ? '재대결 수락 대기 중' : '재대결'}</button>` : '';
  const leave = isSpectator
    ? `<button class="ghost" data-action="spot-difference-spectate-leave" data-id="${esc(room.id)}" type="button">관전 나가기</button>`
    : isPlayer ? `<button class="ghost" data-action="spot-difference-leave" data-id="${esc(room.id)}" type="button">${room.status === 'playing' ? '기권하고 나가기' : '나가기'}</button>` : '<button class="ghost" data-action="spot-difference-back" type="button">로비로</button>';
  const reaction = (isPlayer || isSpectator) ? `${spectatorReactionBar('spot-difference', room.id, room.reactions || [], true, isPlayer)}${isSpectator ? '<p class="helper">관전자는 그림을 눌러 정답을 제출할 수 없습니다.</p>' : ''}` : '';
  const revealUntil = room.status === 'ended' ? spotDifferenceRevealUntilFor(room) : 0;
  const resultOverlay = room.status === 'ended' && Date.now() >= revealUntil ? commonBattleResultOverlay('spotDifference', room, { totalPot: Number(room.stakePoints || 0) * 2 }) : '';
  const body = `${sectionHeading('틀린그림찾기', `${room.roomNumber}번방 · 판돈 ${points(room.stakePoints)} · ${spotDifferenceStatusLabel(room.status)}`, '<button class="text-button" data-action="spot-difference-back" type="button">로비 보기</button>')}<div class="spot-rule-strip compact"><span>랜덤 테마</span><span>7개</span><span>0:45</span><span>힌트 없음</span></div>${stage}<div class="button-row">${rematch}${leave}</div>${reaction}${resultOverlay}`;
  return `<div class="spot-room-view" data-spot-room-id="${esc(room.id || '')}" data-spot-match-id="${esc(room.matchId || '')}" data-spot-status="${esc(room.status || '')}" data-spot-role="${esc(room.viewerRole || 'none')}">${body}</div>`;
}

function spotDifferenceSection() {
  const room = currentSpotDifferenceRoom();
  return room ? spotDifferenceRoomView(room) : spotDifferenceLobby();
}

function renderSpotDifferenceRegion() {
  const region = $('.spot-difference-wrap');
  if (!region) { markTabDirty('games'); return false; }
  region.innerHTML = spotDifferenceSection();
  const room = currentSpotDifferenceRoom();
  if (room?.status === 'playing') updateSpotDifferenceClock(room);
  syncSpotDifferenceClockLoop();
  syncSupportBanner();
  requestAnimationFrame(syncGameResultScrollLock);
  return true;
}

function applySpotDifferenceRoomState(payload) {
  const room = payload?.room;
  if (!room || !app.data?.spotDifference) return false;
  const rooms = app.data.spotDifference.rooms ??= [];
  const index = rooms.findIndex((item) => item.id === room.id);
  const previous = index >= 0 ? rooms[index] : null;
  if (previous?.matchId && room.matchId && previous.matchId !== room.matchId) {
    app.spotDifferencePendingClicks.clear();
    clearCountdownKeys(`spot-start:${room.id}:`);
    clearCountdownKeys(`spot-match:${room.id}:`);
    app.spotDifferenceRevealUntil.delete(spotDifferenceResultKey(previous));
  }
  if (index >= 0) rooms[index] = room;
  else rooms.push(room);
  app.data.spotDifference.serverTime = Number(payload.serverTime || room.serverTime || Date.now());
  app.spotDifferenceServerSyncedAt = monotonicNow();
  if (!app.spotDifferenceLobbyForced) app.spotDifferenceRoomId = room.id;
  if (app.tab === 'games') renderSpotDifferenceRegion();
  else markTabDirty('games');
  updateAppChrome();
  return true;
}

function updateSpotDifferenceClock(room = currentSpotDifferenceRoom()) {
  if (!room) return false;
  const now = spotDifferenceServerNow();
  const revealAt = new Date(room.revealAt || '').getTime();
  const deadline = new Date(room.deadlineAt || '').getTime();
  const grid = $('.spot-picture-grid');
  const ready = room.status === 'playing' && (!Number.isFinite(revealAt) || now >= revealAt);
  if (grid) {
    grid.classList.toggle('ready', ready || room.status === 'ended');
    grid.classList.toggle('counting', !ready && room.status === 'playing');
  }

  if (!ready && room.status === 'playing' && Number.isFinite(revealAt)) {
    const startKey = `spot-start:${room.id}:${room.matchId || room.revealAt}`;
    const startSeconds = Math.min(3, Math.max(1, stableCountdownSeconds(startKey, revealAt, now)));
    $$('[data-spot-start-countdown]').forEach((node) => { if (node.textContent !== String(startSeconds)) node.textContent = String(startSeconds); });
  }
  $$('.spot-picture-button').forEach((button) => { if (room.viewerRole === 'player' && room.status === 'playing') button.disabled = !ready; });
  const countdown = $('#spot-difference-countdown');
  if (countdown && Number.isFinite(deadline)) {
    // 3·2·1 동안은 경기시간을 미리 48/47/46초처럼 보여주지 않고 0:45로 고정한다.
    const seconds = ready
      ? stableCountdownSeconds(`spot-match:${room.id}:${room.matchId || room.deadlineAt}`, deadline, now)
      : 45;
...
function syncSpotDifferenceClockLoop() {
  clearInterval(app.spotDifferenceClockTimer);
  app.spotDifferenceClockTimer = setInterval(() => {
    if (!app.data || app.tab !== 'games' || document.hidden) return;
    const room = currentSpotDifferenceRoom();
    if (room?.status === 'playing') updateSpotDifferenceClock(room);
  }, 100);
}

function flashSpotDifferenceWrong(button, x, y) {
  if (!button) return;
  const mark = document.createElement('span');
  mark.className = 'spot-wrong-mark';
  mark.textContent = '×';
  mark.style.left = `${Math.max(0, Math.min(100, x * 100))}%`;
  mark.style.top = `${Math.max(0, Math.min(100, y * 100))}%`;
  button.append(mark);
  setTimeout(() => mark.remove(), 650);
}

async function submitSpotDifferenceClick(button, pointerEvent) {
  const room = currentSpotDifferenceRoom();
  if (!room || room.id !== button?.dataset.id || room.viewerRole !== 'player' || room.status !== 'playing' || !room.puzzle) return null;
  const now = spotDifferenceServerNow();
  const revealAt = new Date(room.revealAt || '').getTime();
  if (Number.isFinite(revealAt) && now < revealAt) return null;
  const self = room.players?.[room.selfPetId];
  const lockedUntil = new Date(self?.lockedUntil || '').getTime();
  if (Number.isFinite(lockedUntil) && lockedUntil > now) return null;
  const rect = button.getBoundingClientRect();
  if (!(rect.width > 0 && rect.height > 0)) return null;
  const x = Math.max(0, Math.min(1, (Number(pointerEvent.clientX) - rect.left) / rect.width));
  const y = Math.max(0, Math.min(1, (Number(pointerEvent.clientY) - rect.top) / rect.height));
  const actionId = `spot:${room.matchId}:${Date.now().toString(36)}:${Math.random().toString(36).slice(2, 10)}`;
  if (app.spotDifferencePendingClicks.has(actionId)) return null;
  app.spotDifferencePendingClicks.add(actionId);
  const localHit = spotDifferenceHitIdAt(room.puzzle, x, y);
  if (localHit && self && !self.foundIds?.includes(localHit)) {
    self.foundIds = [...(self.foundIds ?? []), localHit];
    self.foundCount = self.foundIds.length;
    renderSpotDifferenceRegion();
  } else if (!localHit) flashSpotDifferenceWrong(button, x, y);
  try {
    const result = await api(`/api/spot-difference/rooms/${encodeURIComponent(room.id)}/click`, {
      method: 'POST', body: JSON.stringify({ matchId: room.matchId, actionId, x, y, side: button.dataset.side || 'original' })
    });
    if (result?.room) applySpotDifferenceRoomState({ room: result.room, serverTime: result.serverTime });
    if (result?.correct === false && localHit) {
      const liveButton = $(`.spot-picture-button[data-side="${button.dataset.side || 'original'}"]`);
      flashSpotDifferenceWrong(liveButton, x, y);
    }
    return result;
  } catch (error) {
    toast(error.message, 'error');
    await loadBootstrap({ silent: true, renderMode: 'games-live' });
    return null;
  } finally {
    app.spotDifferencePendingClicks.delete(actionId);
  }
}
...
function updateSichuanCountdown(room = currentSichuanRoom()) {
  ...
  const deadline = new Date(room.deadlineAt).getTime();
  const seconds = stableCountdownSeconds(`sichuan:${room.id}:${room.matchId || room.deadlineAt}`, deadline, sichuanServerNow());
  const text = `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, '0')}`;
  if (countdown.textContent !== text) countdown.textContent = text;
  return true;
}
...
function davinciLobby() {
  ...
  return `${sectionHeading('다빈치코드', `2~4인 · 숫자 추리 · 조커 포함 · 승자 판돈 독식 · ${battleUsageText()}`, `<button class="primary" data-action="davinci-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<p class="helper davinci-intro">검정·흰색 0~11과 조커 2개를 사용합니다. 숨겨진 상대 코드의 숫자를 맞히고 마지막까지 내 코드를 지켜내세요.</p><div class="davinci-lobby-list">${cards}</div>`;
}
...
function syncBlockBattleViewport({ force = false } = {}) {
  if (app.blockBattleViewportFrame) return;
  app.blockBattleViewportFrame = requestAnimationFrame(() => {
    app.blockBattleViewportFrame = 0;
    const region = $('.block-battle-wrap');
    const room = currentBlockBattleRoom();
    if (!region || room?.viewerRole !== 'player' || room.status !== 'playing') {
      app.blockBattleViewportLayoutKey = '';
      return;
    }
    const viewport = window.visualViewport;
    const height = Math.max(320, Math.round(Number(viewport?.height) || window.innerHeight || document.documentElement.clientHeight || 320));
    const width = Math.max(280, Math.round(region.getBoundingClientRect().width || Number(viewport?.width) || window.innerWidth || 320));
    const orientation = window.matchMedia?.('(orientation: landscape)')?.matches
      ? 'landscape'
      : (window.matchMedia?.('(orientation: portrait)')?.matches ? 'portrait' : (width > height ? 'landscape' : 'portrait'));
    // 주소창 show/hide로 visualViewport.height만 출렁이는 것은 레이아웃 변경으로 보지 않는다.
    // 같은 match + 같은 방향 + 비슷한 폭에서는 보드 크기를 고정해 모바일에서 판이 커졌다 작아졌다 하지 않게 한다.
    const widthBucket = Math.round(width / 24);
    const layoutKey = `${room.id}:${room.matchId || 'match'}:${orientation}:${widthBucket}`;
    if (!force && app.blockBattleViewportLayoutKey === layoutKey) return;

    app.blockBattleViewportLayoutKey = layoutKey;
    app.blockBattleViewportLockedHeight = height;
    app.blockBattleViewportLockedWidth = width;
    const reservedHeight = orientation === 'landscape' ? 155 : 220;
    const byHeight = Math.max(112, Math.floor((height - reservedHeight) / 2));
    const opponent = Math.max(56, Math.min(82, Math.floor(width * (orientation === 'landscape' ? .15 : .19))));
    const center = orientation === 'landscape' ? 24 : 22;
    const stagePadding = 18;
    const byWidth = Math.max(112, width - opponent - center - stagePadding - 12);
    const cap = orientation === 'landscape' ? 185 : 260;
    const main = Math.max(112, Math.min(cap, byHeight, byWidth));
    const opponentWidth = Math.max(54, Math.min(opponent, Math.floor(main * .38)));

    region.dataset.viewportHeight = String(height);
    region.style.setProperty('--block-battle-viewport-height', `${height}px`);
    region.style.setProperty('--block-battle-main-width', `${main}px`);
    region.style.setProperty('--block-battle-opponent-width', `${opponentWidth}px`);
  });
}
...
function applyBlockBattleRoomState(payload) {
  ...
  updateAppChrome();
  syncBlockBattleGravity();
  // 이전 lock ACK가 확정된 바로 그 snapshot에서 예약된 Space를 다음 블록에 적용한다.
  // 먼저 로컬 hardDrop/lock 예측을 만들고 그 뒤 서버 전송하므로 Space가 씹히거나 화면이 되감기지 않는다.
  drainBlockBattleDeferredHardDrop(room);
  if (!app.blockBattleSending && app.blockBattleInputBuffer.length && !app.blockBattleFlushTimer) {
    app.blockBattleFlushTimer = setTimeout(flushBlockBattleInputs, 0);
  }
  return true;
...
function queueBlockBattleInput(action) {
  ...
  // 한 번에 lock 하나만 서버보다 앞서 예측한다. 다만 Space hardDrop 입력 자체는 버리지 않는다.
  // 첫 lock ACK가 오면 다음 블록에 같은 hardDrop을 즉시 실행해 빠르게 연속 Space를 눌러도 입력이 유실되지 않는다.
  if (awaitingLock && predictedLock && willLock) {
    if (action === 'hardDrop') {
      app.blockBattleDeferredHardDrops = Math.min(
        BLOCK_BATTLE_MAX_DEFERRED_HARD_DROPS,
        app.blockBattleDeferredHardDrops + 1
      );
      return true;
    }
    return false;
...
function drainBlockBattleDeferredHardDrop(room = currentBlockBattleRoom()) {
  if (!room || room.viewerRole !== 'player' || room.status !== 'playing') {
    app.blockBattleDeferredHardDrops = 0;
    return false;
  }
  if (app.blockBattleDeferredHardDrops <= 0 || blockBattleAwaitingLock()) return false;
  app.blockBattleDeferredHardDrops -= 1;
  // ACK로 이전 lock이 확정된 뒤에만 다음 hardDrop을 일반 입력 경로로 다시 넣는다.
  // 이때 visualSelf는 다음 블록 상태이므로 서버 순서와 로컬 표시가 동시에 맞는다.
  const queued = queueBlockBattleInput('hardDrop');
  if (!queued) app.blockBattleDeferredHardDrops += 1;
  return queued;
}
...
function kingHistoryText(history = {}) {
  const defs = [
    ...
    ['blockBattle','🧱 테트리스왕'], ['sichuan','🀄 사천성왕'], ['spotDifference','🔎 틀린그림왕'],
...
function appleTimeText(challenge) {
  ...
  const remaining = stableCountdownSeconds(`apple:${challenge.id}:${challenge.expiresAt}`, deadline, serverAlignedNow(app.data?.serverTime));
...
function syncMinesweeperClock() {
  ...
  const elapsed = stableElapsedMilliseconds(`minesweeper:${challenge.id}:${challenge.startedAt}`, startedAt, serverAlignedNow(app.data?.serverTime));
...
function cleanupAppleBoardUi() {
  ...
  app.appleLayoutObserver?.disconnect?.();
  app.appleLayoutObserver = null;
...
function observeAppleGameLayout(stage) {
  if (!stage || !('ResizeObserver' in window)) return;
  app.appleLayoutObserver?.disconnect?.();
  app.appleLayoutObserver = new ResizeObserver(() => syncAppleGameLayout());
  app.appleLayoutObserver.observe(stage);
}
...
function syncAppleGameLayout() {
  ...
  app.appleLayoutFrame = requestAnimationFrame(() => {
    ...
    const size = Math.max(1, Math.floor(Math.min(560, availableWidth, availableHeight)));
    board.style.width = `${size}px`;
    board.style.height = `${size}px`;
    if (!app.appleLayoutObserver) observeAppleGameLayout(stage);
  });
}
...
function openCreateSpotDifference() {
  const balance = Math.max(0, Number(app.data?.dashboard?.pet?.stats?.points) || 0);
  const presets = app.data?.spotDifference?.stakes?.length ? app.data.spotDifference.stakes : [100, 500, 1000, 2000, 3000];
  const options = presets.map((stake) => `<option value="${stake}">${Number(stake).toLocaleString('ko-KR')}P</option>`).join('');
  openModal(`${modalHeader('틀린그림찾기 방 만들기', `보유 포인트 ${points(balance)}`)}<form id="spot-difference-create-form" class="stack-form"><label>판돈<select name="preset" id="spot-difference-stake-preset">${options}<option value="custom">직접 입력</option></select></label><label id="spot-difference-custom-stake-wrap" class="hidden">직접 입력<input name="customStake" type="number" inputmode="numeric" min="1000" step="1000" placeholder="4,000 / 5,000 / ..."></label><p class="warning-box">3초 카운트다운 뒤 같은 랜덤 문제로 시작합니다. 45초 안에 틀린 곳 7개를 먼저 찾으면 승리하며, 오답은 1초 입력 잠금입니다. 힌트는 없고 종료 뒤 정답 7곳을 공개합니다.</p><button class="primary wide" type="submit">방 만들기</button></form>`, { type: 'spotDifferenceCreate' });
  requestAnimationFrame(() => {
    const preset = $('#spot-difference-stake-preset');
    preset?.addEventListener('change', () => $('#spot-difference-custom-stake-wrap')?.classList.toggle('hidden', preset.value !== 'custom'));
  });
}
...
function openAdmin() {
  ...
  const auditLogs = (admin.auditLogs ?? []).map((entry) => {
    ...
    else if (entry.action === 'spot_difference_clear_ended') detail = `${esc(entry.adminDisplayName)} → 종료된 틀린그림찾기 방 비우기`;
    else if (entry.action === 'davinci_clear_ended') detail = `${esc(entry.adminDisplayName)} → 종료된 다빈치코드 방 비우기`;
...
async function tick() {
  ...
  if (omokCountdown && omokRoom?.status === 'playing') {
    ...
    const now = serverAlignedNow(app.data.omok.serverTime);
    const remaining = deadline - now;
    const secondsLeft = stableCountdownSeconds(`omok:${omokRoom.id}:${omokRoom.phase}:${startedAt}`, deadline, now);
    omokCountdown.textContent = `${secondsLeft}초`;
  ...
  if (davinciCountdown && davinciRoom?.status === 'playing' && davinciRoom.deadlineAt) {
    const deadline = new Date(davinciRoom.deadlineAt).getTime();
    const now = serverAlignedNow(app.data?.davinci?.serverTime || app.data?.serverTime);
    const remaining = deadline - now;
    const secondsLeft = stableCountdownSeconds(`davinci:${davinciRoom.id}:${davinciRoom.deadlineAt}`, deadline, now);
    davinciCountdown.textContent = `${secondsLeft}초`;
  ...
  const spotRoom = currentSpotDifferenceRoom();
  if (spotRoom) {
    if (spotRoom.status === 'playing') {
      updateSpotDifferenceClock(spotRoom);
      const remaining = new Date(spotRoom.deadlineAt || '').getTime() - spotDifferenceServerNow();
      if (Number.isFinite(remaining) && remaining <= 0 && Date.now() - (app.spotDifferenceLastRefreshAt || 0) > 1200) {
        app.spotDifferenceLastRefreshAt = Date.now();
        loadBootstrap({ silent: true, renderMode: 'games-live' });
      }
    } else if (spotRoom.status === 'ended') {
      const revealUntil = spotDifferenceRevealUntilFor(spotRoom);
      const remaining = Math.max(0, Math.ceil((revealUntil - Date.now()) / 1000));
      const revealNode = $('[data-spot-answer-countdown]');
      if (revealNode) revealNode.textContent = String(remaining);
      if (remaining <= 0 && !$('.common-game-result-overlay') && app.tab === 'games') renderSpotDifferenceRegion();
    }
  }
  ...
  if (appleCountdown && appleChallenge?.gameId === 'apple') {
    const deadline = new Date(appleChallenge.expiresAt).getTime();
    const now = serverAlignedNow(app.data?.serverTime);
    const remaining = deadline - now;
    const secondsLeft = stableCountdownSeconds(`apple:${appleChallenge.id}:${appleChallenge.expiresAt}`, deadline, now);
    appleCountdown.textContent = `${Math.floor(secondsLeft / 60)}:${String(secondsLeft % 60).padStart(2, '0')}`;
...
async function handleAction(button, event = null) {
  ...
  if (action === 'spot-difference-create') return openCreateSpotDifference();
  if (action === 'spot-difference-open') { app.spotDifferenceLobbyForced = false; app.spotDifferenceRoomId = idValue; renderSpotDifferenceRegion(); return; }
  if (action === 'spot-difference-back') { app.spotDifferenceRoomId = null; app.spotDifferenceLobbyForced = true; renderSpotDifferenceRegion(); return; }
  if (action === 'spot-difference-join') {
    const result = await perform(`/api/spot-difference/rooms/${encodeURIComponent(idValue)}/join`, {}, null, 'POST', { toastDuration: 900, toastType: 'game-start' });
    if (result?.ok) { app.spotDifferenceLobbyForced = false; app.spotDifferenceRoomId = result.roomId || idValue; app.spotDifferencePendingClicks.clear(); renderSpotDifferenceRegion(); }
    return result;
  }
  if (action === 'spot-difference-spectate') {
    const result = await perform(`/api/spot-difference/rooms/${encodeURIComponent(idValue)}/spectate`, {});
    if (result?.ok) { app.spotDifferenceLobbyForced = false; app.spotDifferenceRoomId = result.roomId || idValue; renderSpotDifferenceRegion(); }
    return result;
  }
  if (action === 'spot-difference-spectate-leave') {
    const result = await perform(`/api/spot-difference/rooms/${encodeURIComponent(idValue)}/spectate/leave`, {});
    if (result?.ok) { app.spotDifferenceRoomId = null; app.spotDifferenceLobbyForced = true; renderSpotDifferenceRegion(); }
    return result;
  }
  if (action === 'spot-difference-leave') {
    const room = currentSpotDifferenceRoom();
    if (room?.status === 'playing' && !confirm('게임 중 나가면 즉시 기권패 처리되고 판돈은 상대에게 지급됩니다. 나갈까요?')) return;
    const result = await perform(`/api/spot-difference/rooms/${encodeURIComponent(idValue)}/leave`, {});
    if (result?.ok) { app.spotDifferenceRoomId = null; app.spotDifferenceLobbyForced = true; app.spotDifferencePendingClicks.clear(); renderSpotDifferenceRegion(); }
    return result;
  }
  if (action === 'spot-difference-rematch') {
    const result = await perform(`/api/spot-difference/rooms/${encodeURIComponent(idValue)}/rematch`, {}, null, 'POST', { toastDuration: 1000, toastType: 'game-start' });
    if (result?.ok) { app.spotDifferenceLobbyForced = false; app.spotDifferenceRoomId = idValue; app.spotDifferencePendingClicks.clear(); renderSpotDifferenceRegion(); }
    return result;
  }
  if (action === 'spot-difference-reaction') {
    try {
      return await api(`/api/spot-difference/rooms/${encodeURIComponent(idValue)}/reaction`, { method: 'POST', body: JSON.stringify({ type: button.dataset.reaction }) });
    } catch (error) { toast(error.message, 'error'); return null; }
  }
  if (action === 'spot-difference-image') return;
  ...
  if (action === 'admin-clear-ended-spot-difference') return perform('/api/admin/spot-difference/clear-ended', {});
...
function switchMainTab(tabName, { smooth = false } = {}) {
  ...
  const realtimeGameOpen = tabName === 'games' && Boolean(app.data && (currentOmokRoom() || currentBlockBattleRoom() || currentSichuanRoom() || currentSpotDifferenceRoom() || currentDavinciRoom()));
...
document.addEventListener('click', (event) => {
  ...
  if (button) {
    ...
    if (action === 'spot-difference-image' && event.detail !== 0 && Date.now() <= app.spotDifferencePointerSuppressUntil) {
      event.preventDefault();
      return;
    }
...
document.addEventListener('pointerdown', (event) => {
  if (event.target.closest?.('#loudspeaker-message-input')) app.loudspeakerEditing = true;
  const spotPictureButton = event.target.closest?.('[data-action="spot-difference-image"]');
  if (spotPictureButton && event.isPrimary !== false && (event.pointerType !== 'mouse' || event.button === 0)) {
    // 좌표가 필요한 틀린그림 입력은 focus/click이 아니라 실제 포인터가 눌린 순간 처리한다.
    // 뒤따르는 합성 click은 억제해 한 번의 터치가 두 번 전송되지 않게 한다.
    event.preventDefault();
    app.spotDifferencePointerSuppressUntil = Date.now() + 900;
    submitSpotDifferenceClick(spotPictureButton, event);
    return;
  }
...
document.addEventListener('pointerup', (event) => { stopSingleTetrisHold('pointer', event.pointerId); stopBlockBattleHold('pointer', event.pointerId); }, { passive: true });
document.addEventListener('pointercancel', (event) => { stopSingleTetrisHold('pointer', event.pointerId); stopBlockBattleHold('pointer', event.pointerId); }, { passive: true });
...
window.addEventListener('blur', () => { stopSingleTetrisHold(); stopBlockBattleHold(); });
...
document.addEventListener('submit', async (event) => {
  ...
  if (form.id === 'spot-difference-create-form') {
    const preset = String(data.preset ?? '100');
    const stakePoints = preset === 'custom' ? Number(data.customStake) : Number(preset);
    const result = await perform('/api/spot-difference/rooms', { stakePoints });
    if (result?.ok) {
      app.spotDifferenceLobbyForced = false;
      app.spotDifferenceRoomId = result.roomId;
      app.spotDifferencePendingClicks.clear();
      closeModal();
      markTabDirty('games');
      render();
...
window.addEventListener('orientationchange', () => { app.blockBattleViewportLayoutKey = ''; syncBlockBattleViewport({ force: true }); });
...
navigator.serviceWorker?.register?.('/sw.js?v=610116')?.catch(() => {});
