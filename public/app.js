...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610108" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610108`;
...
function battleUsageText() {
  ...
  return `대전 ${used}/${limit}회 · 오목·테트리스·사천성·다빈치 합산`;
}
...
function blockBattleUsesDesktopPaint() {
  return Boolean(window.matchMedia?.('(hover: hover) and (pointer: fine)').matches);
}

function blockBattleActiveCellStyle(cell, desktop = blockBattleUsesDesktopPaint()) {
  if (!cell) return '';
  return desktop
    ? `left:${cell.col * 10}%;top:${cell.row * 5}%;transform:none`
...
function blockBattleActiveLayerHtml(player) {
  ...
  const desktop = blockBattleUsesDesktopPaint();
  return `<div class="block-battle-active-layer${desktop ? ' desktop-paint' : ''}" aria-hidden="true" data-active-ready="${cells ? '1' : '0'}" data-paint-mode="${desktop ? 'desktop' : 'touch'}">${Array.from({ length: 4 }, (_, index) => {
    const cell = cells?.[index] || null;
    return `<i class="block-battle-active-cell${cell ? ` piece-${esc(cell.type)}` : ''}" data-block-active-cell="${index}" ${cell ? `style="${blockBattleActiveCellStyle(cell, desktop)}"` : 'hidden'}></i>`;
  }).join('')}</div>`;
}
...
function blockBattleBoardHtml(player, { compact = false } = {}) {
  ...
  // 모바일은 GPU 합성 이동을 유지하고, PC(hover+fine pointer)는 left/top 직접 배치로 분리해
  // Windows Chromium의 합성 레이어 재래스터화와 모바일 Safari의 셀 class flash를 각각 피한다.
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610108`;
}
...
function paintBlockBattleActiveLayer(player, board, { preserveOnMissing = false } = {}) {
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    const desktop = layer.dataset.paintMode === 'desktop';
    if (desktop) {
      const left = `${cell.col * 10}%`;
      ...
      if (node.style.left !== '0px') node.style.left = '0px';
      if (node.style.top !== '0px') node.style.top = '0px';
    }
    node.hidden = false;
  }
...
function paintBlockBattleBoard(player, root = document, { activeOnly = false, preserveActiveOnMissing = false } = {}) {
  ...
  // 이동/회전/자동 낙하는 고정 200칸을 전혀 다시 칠하지 않고 별도 active 레이어 4칸만 이동한다.
  // 모바일은 translate3d, PC는 left/top을 사용하고 lock/방해줄/서버 확정 때만 고정 셀을 갱신한다.
  if (!activeOnly) {
...
function previewBlockBattleLock(room = currentBlockBattleRoom(), { paint = true, mark = true } = {}) {
  ...
  attack -= cancelled;
  // 서버가 이 순간 임의의 구멍 위치로 방해줄을 넣어야 한다면 클라이언트가 결과를 정확히 예측할 수 없다.
  // 그 경우에만 기존의 짧은 ACK 장벽을 유지하고, 일반적인 lock은 즉시 다음 블록을 보여준다.
  if (attack <= 0 && pendingGarbage > 0) return false;
...
navigator.serviceWorker?.register?.('/sw.js?v=610108')?.catch(() => {});
