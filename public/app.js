...
function territoryViewPage() {
  ...
  const previousResult = last?.goldenCell
    ...
    : '';
  return `<section class="page-title"><span class="eyebrow">6시간 시즌</span><h1>제${territory.seasonNumber}회 레고 영토전</h1><p>본진을 지키면서 주변 8칸으로 땅을 넓히고 상대 영토를 빼앗는 게임입니다.</p></section>${previousResult}<section class="section territory-summary"><div class="metric-grid">${compactMetric('남은 시간', `<span id="territory-countdown">${durationText(new Date(territory.endsAt).getTime() - Date.now())}</span>`, '', { raw: true })}${compactMetric('내 레벨', `Lv.${territory.my.level}`)}${compactMetric('내 영토', `${territory.my.owned}/${territory.my.limit}`)}${compactMetric('현재 순위', territory.my.rank ? `${territory.my.rank}위` : '-')}</div><div class="territory-golden-status">${goldenStatus}</div><div class="territory-golden-status"><b>${battleUnlocked ? '⚔️ 전면전 진행 중 · 본진 탈취 가능' : `🛡️ 본진 보호 중 · ${territory.cells.length}/${mapSize * mapSize}칸 점유`}</b></div><div class="territory-rule"><b>룰:</b> 처음에는 🏠 본진이 보호됩니다. 맵 16칸이 한 번이라도 모두 차면 ⚔️ 전면전이 열려 본진도 탈취할 수 있습니다. 전면전 이후 영토가 0칸이 되어도 다른 칸을 차지해 다시 참가할 수 있습니다. 보유 한도가 꽉 차면 가장 오래된 일반 영토가 이동하며, 일반 영토가 없는 1칸 보유자는 전면전에서 본진을 옮겨 공격할 수 있습니다.</div></section><section class="section territory-section">${sectionHeading('공용 맵', `${mapSize} × ${mapSize}`)}<div class="territory-scroll"><div class="territory-map" role="grid" aria-label="${mapSize}행 ${mapSize}열 레고 영토전 맵">${rows}</div></div><p class="helper centered">회색칸은 빈 땅, 색깔 닉네임은 소유자입니다. 🏠 본진은 전면전 전까지만 보호되며, 16/16칸이 모두 차면 이후 탈취 가능합니다. 황금 영토 위치는 종료 전에는 공개되지 않습니다.</p></section><section class="section">${sectionHeading('현재 순위', '단독 1위만 300P를 받습니다. 동률이면 종료 후 공개되는 황금 영토가 최종 승자를 가릅니다.')}<div class="rank-list">${ranking}</div></section>`;
}
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=644').catch(() => {});
