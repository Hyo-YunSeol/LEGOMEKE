...
function supportBannerShouldHide() {
  ...
  return Boolean(currentOmokRoom() || currentBlockBattleRoom() || currentSichuanRoom() || currentDavinciRoom());
}
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (!hadData && (currentOmokRoom() || currentBlockBattleRoom() || currentSichuanRoom() || currentDavinciRoom())) {
      app.tab = 'games';
      app.omokLobbyForced = false;
      app.blockBattleLobbyForced = false;
      app.sichuanLobbyForced = false;
    ...
    if (numberInputProtected || sichuanLivePatched) markTabDirty('games');
    if (!appleOnlyUpdated && !blockOnlyUpdated && !minesweeperOnlyUpdated && !numberInputProtected && !sichuanLivePatched) {
      render();
...
async function refreshModal() {
  ...
  if (descriptor.type === 'omokCreate' || descriptor.type === 'blockBattleCreate' || descriptor.type === 'sichuanCreate' || descriptor.type === 'davinciCreate') return;
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610106" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610106`;
...
function gamesView() {
  ...
  const resume = active
    ...
    : '';
  return `
    <section class="page-title"><span class="eyebrow">포인트 게임</span><h1>게임</h1><p>개인게임으로 포인트를 벌고, 싱글 테트리스 기록에 도전하거나 단체게임에서 오목·테트리스대전·사천성·다빈치코드를 즐길 수 있습니다.</p></section>
...
function paintSingleTetris() {
  ...
  for (let index=0; index<200; index+=1) {
    ...
    board.children[index].className=`block-battle-cell${type ? ` piece-${type}` : ''}`;
  }
  const score=$('#single-tetris-score'); if(score) score.textContent=`${state.score}점`;
  const lines=$('#single-tetris-lines'); if(lines) lines.textContent=`${state.lines}줄`;
  const time=$('#single-tetris-time'); if(time) time.textContent=singleTetrisTimeText();
  const next=$('#single-tetris-next'); if(next) next.innerHTML=singleTetrisNextHtml(state.queue[0]);
  saveSingleTetrisState();
}
...
function blockBattleActiveRenderCells(player) {
  ...
  if (!shape || !Number.isFinite(baseRow) || !Number.isFinite(baseCol)) return null;
  return shape.map(([x, y]) => {
    const row = baseRow + y;
    const col = baseCol + x;
    return row >= 0 && row < 20 && col >= 0 && col < 10
      ? { row, col, type: active.type }
      : null;
  });
}

function blockBattleActiveLayerHtml(player) {
  const cells = blockBattleActiveRenderCells(player);
  return `<div class="block-battle-active-layer" aria-hidden="true" data-active-ready="${cells ? '1' : '0'}">${Array.from({ length: 4 }, (_, index) => {
    const cell = cells?.[index] || null;
    return `<i class="block-battle-active-cell${cell ? ` piece-${esc(cell.type)}` : ''}" data-block-active-cell="${index}" ${cell ? `style="transform:translate3d(${cell.col * 100}%,${cell.row * 100}%,0)"` : 'hidden'}></i>`;
  }).join('')}</div>`;
}

function blockBattleBoardHtml(player, { compact = false } = {}) {
  if (!player) return '';
  // 고정된 board와 낙하 중 active 블록을 DOM 레이어부터 분리한다.
  // iOS Safari에서 셀 class를 빈칸→블록으로 반복 교체할 때 발생할 수 있는 한 프레임 flash를 막고,
  // 서버 snapshot 교체 중에도 낙하 블록 레이어 자체는 계속 살아 있게 한다.
  const board = Array.from({ length: 20 }, (_, rowIndex) =>
    Array.from({ length: 10 }, (_, colIndex) => player.board?.[rowIndex]?.[colIndex] ?? null));
  return `<div class="block-battle-board ${compact ? 'compact' : ''}" data-block-player="${esc(player.petId)}" role="grid" aria-label="${esc(player.displayName)}의 10열 20행 테트리스 판">${board.flatMap((row, rowIndex) => row.map((cell, colIndex) => `<span class="block-battle-cell ${cell ? `piece-${esc(cell)}` : ''}" data-row="${rowIndex}" data-col="${colIndex}"></span>`)).join('')}${blockBattleActiveLayerHtml(player)}</div>`;
}
...
function sichuanLobby() {
  ...
  return `${sectionHeading('사천성대전', '8×10 · 동일한 시작판 · 1:1 속도전 · 2분 30초', `<button class="primary" data-action="sichuan-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="sichuan-rule-strip"><span>🀄 같은 그림 2개</span><span>↪ 최대 2번 꺾기</span><span>🚫 막히면 즉시 패배</span></div><p class="helper sichuan-intro">두 플레이어가 완전히 같은 배치로 동시에 시작합니다. 각자 자기 판을 풀며 먼저 전부 지우면 승리합니다.</p><div class="sichuan-lobby-list">${cards}</div>`;
}
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610106`;
}
...
function paintBlockBattleActiveLayer(player, board, { preserveOnMissing = false } = {}) {
  const layer = board?.querySelector('.block-battle-active-layer');
  if (!layer) return false;
  const nodes = [...layer.querySelectorAll('[data-block-active-cell]')];
  if (nodes.length !== 4) return false;
  const active = player?.active;
  const cells = blockBattleActiveRenderCells(player);
  if (!cells) {
    // 게임 진행 중 내 active가 네트워크 snapshot에서 일시적으로 비어도 이미 보이는 블록을
    // 지우지 않는다. 정상 서버 상태에서는 playing 중 active=null이 될 이유가 없다.
    if (preserveOnMissing && layer.dataset.activeReady === '1') return true;
    for (const node of nodes) node.hidden = true;
    layer.dataset.activeReady = '0';
    layer.dataset.activeSignature = '';
    return true;
  }

  const signature = `${active.type}:${Number(active.rotation) || 0}:${Number(active.row)}:${Number(active.col)}`;
  if (layer.dataset.activeSignature === signature && layer.dataset.activeReady === '1') return true;
  for (let index = 0; index < 4; index += 1) {
    ...
    const className = `block-battle-active-cell piece-${cell.type}`;
    if (node.className !== className) node.className = className;
    const transform = `translate3d(${cell.col * 100}%,${cell.row * 100}%,0)`;
    if (node.style.transform !== transform) node.style.transform = transform;
    node.hidden = false;
  }
  layer.dataset.activeReady = '1';
  layer.dataset.activeSignature = signature;
  return true;
}
...
function paintBlockBattleBoard(player, root = document, { activeOnly = false, preserveActiveOnMissing = false } = {}) {
  ...
  // 이동/회전/자동 낙하는 고정 200칸을 전혀 다시 칠하지 않고 GPU 합성 active 레이어만 이동한다.
  // lock/방해줄/서버 확정처럼 board 자체가 바뀔 때에만 고정 셀을 갱신한다.
  if (!activeOnly) {
    const cache = board.__blockBattlePaintCache || (board.__blockBattlePaintCache = Array.from(cells, (cell) => cell.className));
    for (let index = 0; index < 200; index += 1) {
      const row = Math.floor(index / 10);
      const col = index % 10;
      const type = player?.board?.[row]?.[col] ?? null;
  ...
  return paintBlockBattleActiveLayer(player, board, { preserveOnMissing: preserveActiveOnMissing });
}
...
function patchBlockBattleDynamic(room) {
  ...
  for (const player of Object.values(room.players || {})) {
    const preserveActiveOnMissing = room.status === 'playing' && room.viewerRole === 'player'
      && String(player.petId) === String(room.selfPetId);
    paintBlockBattleBoard(player, region, { preserveActiveOnMissing });
...
function previewBlockBattleLock(room = currentBlockBattleRoom(), { paint = true, mark = true } = {}) {
  ...
  let pendingGarbageHoles = Array.isArray(player.pendingGarbageHoles)
    ? player.pendingGarbageHoles.map((hole) => Math.max(0, Math.min(9, Math.floor(Number(hole) || 0)))).slice(0, pendingGarbage)
    : [];
  // 서버는 공격을 받은 순간 방해줄 구멍 큐를 확정해서 내려준다. 따라서 lock 직후에도
  // ACK를 기다리지 않고 서버와 똑같은 방해줄을 로컬에서 적용할 수 있다.
  if (pendingGarbageHoles.length < pendingGarbage) return false;
  const cancelled = Math.min(pendingGarbage, attack);
  pendingGarbage -= cancelled;
  if (cancelled > 0) pendingGarbageHoles.splice(0, cancelled);
  attack -= cancelled;
  if (attack <= 0 && pendingGarbage > 0) {
    const incoming = Math.min(8, pendingGarbage);
    for (let index = 0; index < incoming; index += 1) {
      if (kept[0]?.some(Boolean)) return false;
      kept.shift();
      const hole = pendingGarbageHoles.shift();
      kept.push(Array.from({ length: 10 }, (_, col) => col === hole ? null : 'G'));
    }
    pendingGarbage -= incoming;
  }

  const nextActive = { type: nextType, rotation: 0, row: 0, col: 3 };
  const testPlayer = { ...player, board: kept, active: nextActive };
  ...
  if (paint) patchBlockBattleDynamic(room);
  return true;
}
...
function replayBlockBattlePendingInputs(room, { paint = true } = {}) {
  ...
  const predictedLock = awaitingLock && blockBattlePredictedLockActive(room);
  const actions = awaitingLock ? pendingActions : [...pendingActions, ...app.blockBattleInputBuffer];
  let replayedActions = actions;
  if (predictedLock) {
    // 아직 ACK 전인 lock 입력까지 먼저 재적용한 뒤, 예측 가능한 경우에만 새 블록을 로컬로 생성한다.
    // 그 다음 입력 버퍼는 새 블록에 적용되어 싱글처럼 lock 직후 바로 조작할 수 있다.
    for (const action of pendingActions) previewBlockBattleInput(action, { room, paint: false });
    const reconstructed = previewBlockBattleLock(room, { paint: false, mark: false });
    if (reconstructed) {
      for (const action of app.blockBattleInputBuffer) previewBlockBattleInput(action, { room, paint: false });
      replayedActions = [...pendingActions, ...app.blockBattleInputBuffer];
    } else {
      app.blockBattlePredictedLock = null;
      replayedActions = pendingActions;
    }
  } else {
  ...
  const hasPredictedGravityAction = replayedActions.some((action) => ['softDrop', 'hardDrop', 'tick'].includes(action));
  ...
  if (paint) paintBlockBattleBoard(room.players?.[room.selfPetId], document, { preserveActiveOnMissing: true });
}
...
function applyBlockBattleRoomState(payload) {
  ...
  const playerMatchStarted = Boolean(previous?.status === 'waiting' && room.status === 'playing' && room.viewerRole === 'player');
  const sameMatch = Boolean(previous?.matchId && previous.matchId === room.matchId);
  const incomingVersion = Math.max(0, Number(room.stateVersion) || 0);
  const acceptedVersion = sameMatch ? Math.max(0, Number(app.blockBattleServerVersions.get(room.id)) || 0) : 0;
  ...
  // 대기 중 상대가 참가해 실제 경기가 시작되면 선수는 로비 상태에 남지 않는다.
  // 시작 snapshot 자체가 화면 전환 신호가 되어 첫 active 블록을 즉시 렌더한다.
  if (playerMatchStarted) {
    app.blockBattleLobbyForced = false;
    app.blockBattleRoomId = room.id;
  } else if (!app.blockBattleLobbyForced) app.blockBattleRoomId = room.id;
...
function queueBlockBattleInput(action) {
  ...
  // lock 결과를 안전하게 예측한 일반 상황에서는 다음 블록의 좌우/회전을 즉시 허용한다.
  // 방해줄의 임의 구멍 등 예측 불가능한 경우에만 기존 ACK 장벽을 유지한다.
  ...
  // 한 번에 lock 하나만 서버보다 앞서 예측한다. 두 번째 lock은 첫 ACK 뒤에 처리해 난수/공격 상태와 어긋나지 않게 한다.
  if (awaitingLock && predictedLock && willLock) return false;

  if (action !== 'tick' && (!awaitingHardDrop || predictedLock)) {
  ...
  if (willLock && !predictedLock) previewBlockBattleLock(room, { paint: true, mark: true });
...
function render() {
  ...
  updateAppChrome();
  if (!pane) return;
  // While a Sichuan match is actively visible, a generic bootstrap/render must not
  // replace the 80 image nodes. Patch the live room and defer the rest of Games until
  // the user leaves the match or switches tabs. This removes device-speed-dependent flicker.
  const liveSichuanRoom = app.tab === 'games' ? currentSichuanRoom() : null;
...
async function handleAction(button, event = null) {
  ...
  if (action === 'block-battle-join') {
    // 참가 요청을 보내기 전에 이 방을 실시간 렌더 대상으로 지정한다. 서버가 저장보다 먼저
    // 보내는 시작 snapshot을 즉시 받을 수 있어 API 응답을 기다리며 첫 블록이 늦게 뜨지 않는다.
    const previousRoomId = app.blockBattleRoomId;
    const previousLobbyForced = app.blockBattleLobbyForced;
    app.blockBattleLobbyForced = false;
    app.blockBattleRoomId = idValue;
    const result = await perform(`/api/block-battle/rooms/${encodeURIComponent(idValue)}/join`, {});
    if (result?.ok) {
      app.blockBattleLobbyForced = false;
      app.blockBattleRoomId = result.roomId || idValue;
      renderBlockBattleRegion();
    } else {
      app.blockBattleRoomId = previousRoomId;
      app.blockBattleLobbyForced = previousLobbyForced;
      renderBlockBattleRegion();
    }
    return result;
  }
...
function switchMainTab(tabName, { smooth = false } = {}) {
  ...
  const realtimeGameOpen = tabName === 'games' && Boolean(app.data && (currentOmokRoom() || currentBlockBattleRoom() || currentSichuanRoom() || currentDavinciRoom()));
...
navigator.serviceWorker?.register?.('/sw.js?v=610106')?.catch(() => {});
