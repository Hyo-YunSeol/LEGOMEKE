...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=687" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=687`;
...
function switchMainTab(tabName, { smooth = false } = {}) {
  ...
  const realtimeGameOpen = tabName === 'games' && Boolean(currentOmokRoom() || currentBlockBattleRoom());
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=687').catch(() => {});
