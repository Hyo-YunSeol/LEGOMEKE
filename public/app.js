import { cloneBlockBattleVisualPlayer, reconcileBlockBattleVisual } from './block-battle-visual-state.js?v=6101270';
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=6101270" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=6101270`;
...
function patchLegodokuMarksDom(room = currentLegodokuRoom()) {
  ...
  document.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
    ...
    if (hasHead) {
      ...
      if (!cell.querySelector('.legodoku-head')) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101270" alt="" draggable="false">');
...
function patchLegodokuSpectatorBoardsDom(room = currentLegodokuRoom()) {
  ...
  root.querySelectorAll('[data-legodoku-player-board]').forEach((board) => {
    ...
    board.querySelectorAll('.legodoku-cell[data-index]').forEach((cell) => {
      ...
      if (hasHead && !existing) cell.insertAdjacentHTML('afterbegin', '<img class="legodoku-head" src="/legodoku/bi-tteop-head.svg?v=6101270" alt="" draggable="false">');
...
function sichuanPenaltyNoticeHtml(room) {
  ...
  const parts = [];
  if (mine > 0) parts.push(`<span class="mine" data-sichuan-penalty-self>연승 패널티 · ${Math.ceil(mine / 1000)}초</span>`);
...
function updateSichuanPenaltyUi(room = currentSichuanRoom()) {
  ...
  if (notices) {
    ...
    notices.innerHTML = `${mineActive ? `<span class="mine" data-sichuan-penalty-self>연승 패널티 · ${Math.ceil(mineMs / 1000)}초</span>` : ''}${opponentMs > 0 ? `<span data-sichuan-penalty-opponent>상대 연승 패널티 · ${Math.ceil(opponentMs / 1000)}초</span>` : ''}`;
  }
  const overlay = document.querySelector('[data-sichuan-streak-overlay]');
  if (overlay) {
    overlay.classList.toggle('hidden', !mineActive);
    const seconds = overlay.querySelector('[data-sichuan-penalty-seconds]');
    if (seconds) seconds.textContent = String(Math.max(0, Math.ceil(mineMs / 1000)));
  }
  const self = room.players?.[room.selfPetId];
  if (self) patchSichuanBoard(self, { interactive: !mineActive });
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=6101270`;
}
...
function sichuanRoomView(room) {
  ...
  const hostPlayer = room.players?.[room.host?.petId];
  const guestPlayer = room.players?.[room.guest?.petId];
  const interactive = Boolean(isPlayer && room.status === 'playing' && self && !sichuanPenaltyActive(room, room.selfPetId));
  ...
  if (room.status === 'waiting') {
    ...
    stage = `<div class="sichuan-match-head">${sichuanProgressHtml(self, { mine: true })}<div class="sichuan-match-center"><span>남은 시간</span>${timer}<small data-sichuan-spectator-count>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${sichuanProgressHtml(opponent)}</div>${sichuanPenaltyNoticeHtml(room)}<div class="sichuan-board-stage">${sichuanBoardHtml(self, { interactive })}<div class="sichuan-streak-overlay ${sichuanPenaltyActive(room, room.selfPetId) ? '' : 'hidden'}" data-sichuan-streak-overlay><strong>연승 패널티</strong><span>3연승 이상 · 패 <b data-sichuan-penalty-seconds>${Math.max(0, Math.ceil(sichuanPenaltyRemainingMs(room, room.selfPetId) / 1000))}</b>초 후 공개</span></div><div class="sichuan-finish-status hidden" data-sichuan-finish-status>마지막 패 처리 중 · 승리 판정 확인 중</div>${spectatorBurstLayer(room.reactions || [], 'sichuan')}</div><p class="helper centered">같은 그림 두 개를 선택하세요. 최대 두 번 꺾어 연결할 수 있으며 판 바깥쪽 경로도 허용됩니다.</p>`;
  } else {
...
function patchSichuanLiveRoom(room) {
  ...
  if (isPlayer) {
    ...
    if (!patchSichuanBoard(self, { interactive: !sichuanPenaltyActive(room, room.selfPetId) })) return false;
  } else if (isSpectator) {
...
function applyBlockGameServerResult(result) {
  ...
  challenge.blockBoardVersion = Math.max(1, Number(result.boardVersion) || 1);
...
async function handleAction(button, event = null) {
  ...
  if (action === 'sichuan-tile') {
    const room = currentSichuanRoom();
    if (!room || room.viewerRole !== 'player' || room.status !== 'playing') return;
    if (sichuanPenaltyActive(room, room.selfPetId)) { toast('연승 패널티 · 시작 후 10초 동안 패가 가려집니다.', 'game-start', 900); return; }
...
navigator.serviceWorker?.register?.('/sw.js?v=6101270')?.catch(() => {});
