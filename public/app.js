...
function minesweeperCountdownText(endsAt, now = serverAlignedNow(app.data?.serverTime)) {
  const text = gameSeasonCountdownText(endsAt, now);
  return text.replace('시즌 종료 시간 확인 중', '초기화 시간 확인 중').replace('시즌 정산 중', '초기화 중').replace('시즌 종료까지', '초기화까지');
}
...
async function refreshModal() {
  ...
  if (descriptor.type === 'omokCreate' || descriptor.type === 'blockBattleCreate') return;
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69102" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69102`;
...
function rankingsSection() {
  ...
  const mineCard = `<article class="rank-card mine-rank-card"><h3>💣 지뢰찾기 TOP 5</h3>${mineTabs}<small id="minesweeper-season-countdown" class="mine-rank-reset">${minesweeperCountdownText(mineRankings.season?.endsAt)}</small>${mineRows}${mineMyRank}</article>`;
...
function recordsView() {
  ...
  return `<section class="page-title"><span class="eyebrow">기록</span><h1>${esc(pet.displayName)}의 기록</h1><p>현재 레고의 핵심 기록만 표시합니다.</p></section><section class="section"><div class="record-grid">${compactMetric('세대', `${pet.generation}세대`)}${compactMetric('생존', `${records.days}게임일`)}${compactMetric('레벨', `Lv.${pet.stats.level}`)}${compactMetric('레고력', `${pet.stats.legoPower}`)}${compactMetric('포인트', points(pet.stats.points))}${compactMetric('최고 포인트', points(records.maxPoints))}${compactMetric('개인게임', `${records.miniGames || 0}회`)}${compactMetric('벙 완료', `${records.bungs || 0}회`)}${compactMetric('영토전 우승', `${records.territoryWins}회`)}${compactMetric('테트리스대전', `${records.blockBattleWins || 0}승 ${records.blockBattleLosses || 0}패`)}${compactMetric('번개 최고', records.bestReactionMs ? `${(records.bestReactionMs/1000).toFixed(3)}초` : '-')}${compactMetric('낚시', `${records.fishing}회`)}</div></section>${app.data.admin.isAdmin ? `<section class="section admin-callout"><div><h2>운영자 관리</h2><p>포인트 지급·회수, 강퇴, 계정 삭제, 상태 초기화와 진행 중 벙·오목·테트리스대전 방 관리를 할 수 있습니다.</p></div><button class="primary" data-action="open-admin" type="button">관리 열기</button></section>` : ''}<section class="section"><button class="danger-button wide" data-action="logout" type="button">모든 기기에서 로그아웃</button></section>`;
}
...
function minesweeperCellHtml(value, row, col, finished = false) {
  ...
  return `<button class="${className}" data-action="minesweeper-cell" data-row="${row}" data-col="${col}" type="button" ${finished ? 'disabled' : ''} aria-label="${label}">${content}</button>`;
}
...
function refreshMinesweeperMiniOnly(result = null) {
  ...
  $$('.minesweeper-mode-button').forEach((button) => button.classList.toggle('active', button.dataset.value === app.minesweeperMode));
  const helper = $('#minesweeper-helper');
  if (helper) helper.textContent = challenge.startedAt ? 'PC: 좌클릭 열기 · 우클릭 깃발 / 모바일: 열기·깃발 모드 선택' : '첫 칸을 열면 시간이 시작되고 개인게임 1회가 차감됩니다.';
  requestAnimationFrame(syncMinesweeperGameLayout);
  startMinesweeperClock();
  return true;
}

async function submitMinesweeperAction(action, rowValue, colValue) {
  const challenge = app.data?.activeMiniChallenge;
  if (!challenge || challenge.gameId !== 'minesweeper' || app.busy || app.minesweeperActionInFlight) return null;
  if (action === 'flag' && !challenge.startedAt) {
  ...
  try {
    const result = await perform('/api/minigames/minesweeper/action', { challengeId: challenge.id, action, row, col }, null, 'POST', {
...
function openMiniGame(challenge) {
  ...
  if (challenge.gameId === 'minesweeper') {
    ...
    content = `<div class="minesweeper-game ${challenge.difficulty === 'hard' ? 'hard' : 'normal'}"><div class="minesweeper-hud"><div><small>시간</small><strong id="minesweeper-timer">${minesweeperTimeText(elapsed)}</strong></div><div><small>깃발</small><strong id="minesweeper-flags">${Number(challenge.flagCount || 0)}/${Number(challenge.mines || 0)}</strong></div><div><small>안전칸</small><strong id="minesweeper-opened">${Number(challenge.revealedSafeCount || 0)}/${Number(challenge.safeCellCount || 0)}</strong></div></div><p class="helper minesweeper-rules">안전칸을 모두 열면 클리어 · 첫 클릭과 주변 8칸은 지뢰 없음</p><div class="minesweeper-controls"><button class="soft-button minesweeper-mode-button ${app.minesweeperMode === 'reveal' ? 'active' : ''}" data-action="minesweeper-mode" data-value="reveal" type="button">⛏️ 열기</button><button class="soft-button minesweeper-mode-button ${app.minesweeperMode === 'flag' ? 'active' : ''}" data-action="minesweeper-mode" data-value="flag" type="button">🚩 깃발</button><button class="ghost minesweeper-abandon-button" data-action="minesweeper-abandon" type="button">포기하기</button></div><div id="minesweeper-board" class="minesweeper-board" style="--mine-cols:${Number(challenge.cols || 10)}" role="grid" aria-label="지뢰찾기 ${Number(challenge.rows || 10)}행 ${Number(challenge.cols || 10)}열">${minesweeperBoardHtml(challenge)}</div><p id="minesweeper-helper" class="minesweeper-helper">${challenge.startedAt ? 'PC: 좌클릭 열기 · 우클릭 깃발 / 모바일: 열기·깃발 모드 선택' : '첫 칸을 열면 시간이 시작되고 개인게임 1회가 차감됩니다.'}</p></div>`;
  }
...
async function tick() {
  ...
  const mineSeasonCountdown = $('#minesweeper-season-countdown');
  if (mineSeasonCountdown) {
    const nextText = minesweeperCountdownText(app.data.rankings?.minesweeper?.season?.endsAt);
    if (mineSeasonCountdown.textContent !== nextText) mineSeasonCountdown.textContent = nextText;
  }
...
async function handleAction(button, event = null) {
  ...
  if (action === 'minesweeper-mode') {
    app.minesweeperMode = button.dataset.value === 'flag' ? 'flag' : 'reveal';
    $$('.minesweeper-mode-button').forEach((item) => item.classList.toggle('active', item.dataset.value === app.minesweeperMode));
    return;
  }
  if (action === 'minesweeper-cell') {
    const desktopPointer = Boolean(window.matchMedia?.('(pointer: fine)').matches) && Number(navigator.maxTouchPoints || 0) === 0;
    return submitMinesweeperAction(desktopPointer ? 'reveal' : app.minesweeperMode, button.dataset.row, button.dataset.col);
  }
...
document.addEventListener('contextmenu', (event) => {
  ...
  if (!cell || app.data?.activeMiniChallenge?.gameId !== 'minesweeper' || app.modal?.gameId !== 'minesweeper') return;
  event.preventDefault();
...
navigator.serviceWorker?.register?.('/sw.js?v=69102')?.catch(() => {});
