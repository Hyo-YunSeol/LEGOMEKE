...
import { preloadSpotDifferenceSceneAssets, renderSpotDifferenceAnswerMarks, renderSpotDifferenceScene, spotDifferenceHitIdAt, spotDifferenceThemeLabel } from './spot-difference-scene.js?v=610117';
...
function applyBootstrap(next) {
  ...
  const previousSpotDifferenceRooms = new Map((app.data?.spotDifference?.rooms || []).map((room) => [room.id, room]));
  ...
  // 틀린그림찾기도 늦은 bootstrap이 최신 정답/실수 횟수를 되돌리지 못하게 stateVersion을 보호한다.
  for (let index = 0; index < (app.data?.spotDifference?.rooms || []).length; index += 1) {
    const room = app.data.spotDifference.rooms[index];
    const previous = previousSpotDifferenceRooms.get(room.id);
    const samePuzzle = Boolean(previous?.puzzle?.key && previous.puzzle.key === room.puzzle?.key);
    if (samePuzzle && Number(room.stateVersion || 0) < Number(previous.stateVersion || 0)) app.data.spotDifference.rooms[index] = previous;
  }
...
function logout(callServer = true) {
  ...
  app.spotDifferenceLastServerNow = 0;
...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    const liveSpotDifferenceRoom = app.tab === 'games' ? currentSpotDifferenceRoom() : null;
    const spotDifferenceLivePatched = Boolean(liveSpotDifferenceRoom?.status === 'playing' && patchSpotDifferenceLiveRoom(liveSpotDifferenceRoom));
    ...
    if (numberInputProtected || blockBattleLivePatched || sichuanLivePatched || spotDifferenceLivePatched) markTabDirty('games');
    if (!appleOnlyUpdated && !blockOnlyUpdated && !minesweeperOnlyUpdated && !numberInputProtected && !blockBattleLivePatched && !sichuanLivePatched && !spotDifferenceLivePatched) {
      render();
    ...
    } else if (blockBattleLivePatched || sichuanLivePatched || spotDifferenceLivePatched) {
      updateAppChrome();
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610117" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610117`;
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610117`;
}
...
function spotDifferenceServerNow() {
  ...
  const computed = base + (monotonicNow() - app.spotDifferenceServerSyncedAt);
  // 새 bootstrap/WebSocket 응답이 네트워크 지연 때문에 과거 시각으로 보이더라도
  // 틀린그림찾기 카운트다운/45초 타이머가 절대 뒤로 올라가지 않게 단조 증가시킨다.
  app.spotDifferenceLastServerNow = Math.max(Number(app.spotDifferenceLastServerNow || 0), computed);
  return app.spotDifferenceLastServerNow;
}
...
function spotDifferenceLobby() {
  ...
  const canCreate = rooms.length < (game?.maxRooms || 3);
  return `${sectionHeading('틀린그림찾기', `7개 · 45초 · 오답 5회 패배 · ${battleUsageText()}`, `<button class="primary" data-action="spot-difference-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="spot-rule-strip"><span>🔎 틀린 곳 7개</span><span>⏱ 45초</span><span>🧠 성인 난이도</span><span>❌ 오답 5회 패배</span><span>👁 종료 후 정답 공개</span></div><p class="helper spot-intro">성인 난이도 고밀도 도감 20세트가 랜덤으로 출제됩니다. 원본과 변경본 어느 쪽을 눌러도 정답으로 인정됩니다.</p><div class="spot-lobby-list">${cards}</div>`;
}
...
function spotDifferenceMaxWrongClicks(room = null) {
  return Math.max(1, Number(room?.maxWrongClicks || app.data?.spotDifference?.maxWrongClicks || 5));
}

function spotDifferenceProgress(player, { mine = false, room = null } = {}) {
  const found = Math.max(0, Math.min(7, Number(player?.foundCount || player?.foundIds?.length || 0)));
  const maxWrong = spotDifferenceMaxWrongClicks(room);
  const wrong = Math.max(0, Math.min(maxWrong, Number(player?.wrongClicks || 0)));
  return `<div class="spot-progress ${mine ? 'mine' : ''}" data-spot-pet-id="${esc(player?.petId || '')}"><div><strong>${esc(player?.displayName || '-')}</strong>${mine ? '<small>내 진행</small>' : ''}</div><div class="spot-progress-track"><i style="width:${(found / 7) * 100}%"></i></div><b><span data-spot-found-count>${found}/7</span><em data-spot-wrong-count>❌${wrong}/${maxWrong}</em></b></div>`;
}

function spotDifferencePicture(room, { changed = false, foundIds = [], revealAll = false, interactive = false, startSeconds = 3 } = {}) {
  preloadSpotDifferenceSceneAssets(room?.puzzle).catch(() => false);
...
function spotDifferenceRoomView(room) {
  ...
  const deadlineAt = new Date(room.deadlineAt || '').getTime();
  const startSeconds = Number.isFinite(revealAt) ? Math.max(0, Math.ceil((revealAt - now) / 1000)) : 0;
  ...
  if (room.status === 'waiting') {
    ...
    const configuredSeconds = Math.max(1, Number(app.data?.spotDifference?.matchSeconds || 45));
    const initialSeconds = ready && Number.isFinite(deadlineAt)
      ? Math.max(0, Math.min(configuredSeconds, Math.ceil((deadlineAt - now) / 1000)))
      : configuredSeconds;
    const timerText = room.status === 'playing' ? `<b id="spot-difference-countdown">${Math.floor(initialSeconds / 60)}:${String(initialSeconds % 60).padStart(2, '0')}</b>` : '<b>종료</b>';
    stage = `<div class="spot-match-head">${spotDifferenceProgress(leftProgress, { mine: isPlayer, room })}<div class="spot-match-center"><span>${esc(themeLabel)}</span>${timerText}<small data-spot-spectator-count>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${spotDifferenceProgress(rightProgress, { room })}</div><div class="spot-picture-grid ${ready || revealAll ? 'ready' : 'counting'}">${spotDifferencePicture(room, { changed: false, foundIds, revealAll, interactive, startSeconds })}${spotDifferencePicture(room, { changed: true, foundIds, revealAll, interactive, startSeconds })}${spectatorBurstLayer(room.reactions || [], 'spotDifference')}</div>`;
    if (room.status === 'playing') stage += `<p class="helper centered">두 그림 어디를 눌러도 됩니다. 오답 ${spotDifferenceMaxWrongClicks(room)}회째는 즉시 패배하며, 1~4회 오답은 1초 동안 입력이 잠깁니다.</p>`;
    if (revealAll) {
...
function findSpotDifferenceProgressNode(petId) {
  const wanted = String(petId || '');
  return $$('.spot-progress[data-spot-pet-id]').find((node) => String(node.dataset.spotPetId || '') === wanted) || null;
}

function patchSpotDifferenceProgress(player, room) {
  if (!player) return false;
  const node = findSpotDifferenceProgressNode(player.petId);
  if (!node) return false;
  const found = Math.max(0, Math.min(7, Number(player.foundCount || player.foundIds?.length || 0)));
  const maxWrong = spotDifferenceMaxWrongClicks(room);
  const wrong = Math.max(0, Math.min(maxWrong, Number(player.wrongClicks || 0)));
  const track = node.querySelector('.spot-progress-track i');
  if (track) track.style.width = `${(found / 7) * 100}%`;
  const foundNode = node.querySelector('[data-spot-found-count]');
  if (foundNode) foundNode.textContent = `${found}/7`;
  const wrongNode = node.querySelector('[data-spot-wrong-count]');
  if (wrongNode) wrongNode.textContent = `❌${wrong}/${maxWrong}`;
...
function patchSpotDifferenceAnswers(room) {
  if (!room?.puzzle) return false;
  const self = room.viewerRole === 'player' ? room.players?.[room.selfPetId] : null;
  const foundIds = self?.foundIds ?? [];
  const answerMarkup = renderSpotDifferenceAnswerMarks(room.puzzle, foundIds, false);
  const layers = $$('[data-spot-answer-layer]');
  if (layers.length !== 2) return false;
  for (const layer of layers) {
    if (layer.innerHTML !== answerMarkup) layer.innerHTML = answerMarkup;
  }
  return true;
}

function patchSpotDifferenceLiveRoom(room) {
  if (!room || room.status !== 'playing' || !room.puzzle) return false;
  const root = $('.spot-room-view');
  if (!root) return false;
  if (String(root.dataset.spotRoomId || '') !== String(room.id || '')) return false;
  if (String(root.dataset.spotStatus || '') !== 'playing') return false;
  if (String(root.dataset.spotPuzzleKey || '') !== String(room.puzzle.key || '')) return false;

  const isPlayer = room.viewerRole === 'player';
  const left = isPlayer ? room.players?.[room.selfPetId] : room.players?.[room.host?.petId];
  const right = isPlayer ? room.players?.[room.opponentPetId] : room.players?.[room.guest?.petId];
  if (!patchSpotDifferenceProgress(left, room) || !patchSpotDifferenceProgress(right, room)) return false;
  if (!patchSpotDifferenceAnswers(room)) return false;
  const spectator = $('[data-spot-spectator-count]');
  if (spectator) spectator.textContent = `관전자 ${Number(room.spectatorCount || 0)}명`;
  updateSpotDifferenceClock(room);
  return true;
}

function applySpotDifferenceRoomState(payload) {
  const incoming = payload?.room;
  if (!incoming || !app.data?.spotDifference) return false;
  const rooms = app.data.spotDifference.rooms ??= [];
  const index = rooms.findIndex((item) => item.id === incoming.id);
  const previous = index >= 0 ? rooms[index] : null;
  const samePuzzle = Boolean(previous?.puzzle?.key && previous.puzzle.key === incoming.puzzle?.key);
  const stale = samePuzzle && Number(incoming.stateVersion || 0) < Number(previous.stateVersion || 0);
  const room = stale ? previous : incoming;
  if (previous?.puzzle?.key && incoming.puzzle?.key && previous.puzzle.key !== incoming.puzzle.key) {
    app.spotDifferencePendingClicks.clear();
    app.spotDifferenceRevealUntil.delete(spotDifferenceResultKey(previous));
  }
  if (!stale) {
    if (index >= 0) rooms[index] = incoming;
    else rooms.push(incoming);
  }
  app.data.spotDifference.serverTime = Number(payload.serverTime || incoming.serverTime || Date.now());
  app.spotDifferenceServerSyncedAt = monotonicNow();
  preloadSpotDifferenceSceneAssets(room?.puzzle).catch(() => false);
  if (!app.spotDifferenceLobbyForced) app.spotDifferenceRoomId = room.id;
  if (app.tab === 'games') {
    if (room.status === 'playing' && patchSpotDifferenceLiveRoom(room)) markTabDirty('games');
    else renderSpotDifferenceRegion();
...
function updateSpotDifferenceClock(room = currentSpotDifferenceRoom()) {
  ...
  const startSeconds = Number.isFinite(revealAt) ? Math.max(0, Math.ceil((revealAt - now) / 1000)) : 0;
  $$('[data-spot-start-countdown]').forEach((node) => {
    node.textContent = startSeconds > 0 ? String(Math.max(1, Math.min(3, startSeconds))) : '';
  });
  $$('.spot-picture-button').forEach((button) => {
    if (room.viewerRole === 'player' && room.status === 'playing') button.disabled = !ready;
  });
  const countdown = $('#spot-difference-countdown');
  if (countdown && Number.isFinite(deadline)) {
    const configured = Math.max(1, Number(app.data?.spotDifference?.matchSeconds || 45));
    const rawSeconds = ready ? Math.ceil((deadline - now) / 1000) : configured;
    const seconds = Math.max(0, Math.min(configured, rawSeconds));
...
async function submitSpotDifferenceClick(button, pointerEvent) {
  ...
  if (localHit && self && !self.foundIds?.includes(localHit)) {
    ...
    patchSpotDifferenceLiveRoom(room);
...
document.addEventListener('pointerdown', (event) => {
  ...
  if (spotPictureButton && event.isPrimary !== false && (event.pointerType !== 'mouse' || event.button === 0)) {
    // 틀린그림찾기는 한 번의 물리 입력을 pointerdown 한 경로에서만 처리한다.
    // focus/click 합성 이벤트와 실시간 DOM 갱신이 중복 판정을 만들지 못하게 한다.
    event.preventDefault();
...
navigator.serviceWorker?.register?.('/sw.js?v=610117')?.catch(() => {});
