...
function applyBootstrap(next) {
  ...
  for (let index = 0; index < (app.data?.blockBattle?.rooms || []).length; index += 1) {
    ...
    replayBlockBattlePendingInputs(room);
  }
  ...
  socket.addEventListener('message', (event) => {
    try {
      ...
      if (payload.type === 'refresh') {
        ...
        const pendingIndex = app.blockBattlePendingBatches.findIndex((batch) => batch.message.requestId === payload.requestId);
        if (pendingIndex >= 0) {
          clearTimeout(app.blockBattlePendingBatches[pendingIndex].retryTimer);
          app.blockBattlePendingBatches.splice(pendingIndex, 1);
          app.blockBattleSending = app.blockBattlePendingBatches.length > 0;
        }
        toast(payload.message || '테트리스대전 입력을 처리하지 못했습니다.', 'error');
        app.blockBattleInputBuffer.length = 0;
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=681" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=681`;
...
function paintBlockBattleBoard(player) {
  const playerId = String(player?.petId || '');
  const board = [...document.querySelectorAll('.block-battle-board')].find((item) => item.dataset.blockPlayer === playerId);
...
function previewBlockBattleInput(action, { room = currentBlockBattleRoom(), paint = true } = {}) {
  ...
  if (paint) paintBlockBattleBoard(player);
  return true;
}
...
function replayBlockBattlePendingInputs(room) {
  ...
  paintBlockBattleBoard(room.players?.[room.selfPetId]);
}
...
function applyBlockBattleRoomState(payload) {
  ...
  const previousReactionKey = (previous?.reactions || []).map((item) => item.reactionId).join(':');
  const nextReactionKey = (room.reactions || []).map((item) => item.reactionId).join(':');
  const needsLayout = !previous || previous.status !== room.status || previous.viewerRole !== room.viewerRole
    ...
    || Number(previous?.spectatorCount || 0) !== Number(room.spectatorCount || 0)
    || previousReactionKey !== nextReactionKey
...
function queueBlockBattleInput(action) {
  ...
  if (app.blockBattleInputBuffer.length >= 96) return;
  ...
  if (action !== 'tick' && !awaitingHardDrop) previewBlockBattleInput(action);
  app.blockBattleInputBuffer.push(action);
  if (action === 'hardDrop' || app.blockBattleInputBuffer.length >= 12) flushBlockBattleInputs();
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=681').catch(() => {});
