...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69901" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69901`;
...
function restoreSingleTetrisState(challenge) {
  try {
    ...
    // v6.9.9: 싱글도 대전과 동일하게 새 블록은 0행에서 즉시 보인다.
...
function sichuanBoardHtml(player, { interactive = false, compact = false } = {}) {
  ...
  return `<div class="sichuan-board ${compact ? 'compact' : ''}" role="grid" aria-rowcount="8" aria-colcount="10" aria-label="${esc(player?.displayName || '플레이어')} 사천성 판">${board.map((tileId, index) => {
    ...
    const tileSrc = tile?.src ? `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=69901` : '';
...
function singleTetrisOuterHeight(node) {
  if (!node) return 0;
...
function syncSingleTetrisLayout() {
  ...
  const fixedHeight = fixedNodes.reduce((sum, node) => sum + singleTetrisOuterHeight(node), 0);
...
navigator.serviceWorker?.register?.('/sw.js?v=69901')?.catch(() => {});
