...
function avatar(stage, { mini = false } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=651`;
...
function openTerritoryLimits() {
  ...
  openModal(`${modalHeader('영토 레벨별 보유 한도', note)}<div class="territory-limit-table">${rows}</div><p class="warning-box">한도가 꽉 차면 새 땅 점령과 상대 땅 탈취가 제한됩니다. 내 영토를 빼앗겨 빈자리가 생기거나 다음 한도 레벨에 도달하면 다시 차지할 수 있습니다. 상대 영토 탈취는 20P입니다.</p>`, { type:'territoryLimits' });
}
...
bottomNav?.addEventListener('click', (event) => {
  ...
  // 하단 5개 메뉴는 PC/모바일 모두 가장 단순한 click 한 번으로 즉시 이동한다.
  // 진행 중인 API 요청과 무관하게 화면 이동은 막지 않고, 열려 있던 일반 모달만 정리한다.
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=651').catch(() => {});
