...
function rankingsSection() {
  ...
  const oddEven = rankRows(rankings.oddEven ?? [], (item) => `${item.streak}연승`);
  ...
  return `<section class="section ranking-section">${sectionHeading('레고방 순위', '포인트·레벨과 게임 TOP 5')}<div class="rank-tabs-grid"><article class="rank-card"><h3>포인트 TOP 5</h3>${rankRows(rankings.points, (item) => points(item.points))}</article><article class="rank-card"><h3>레벨 TOP 5</h3>${rankRows(rankings.levels, (item) => `Lv.${item.level} · ${item.legoPower}`)}</article></div><div class="game-ranking-heading"><strong>게임 순위</strong><small>홀짝 · 사과게임 · 오목 TOP 5</small></div><div class="game-rank-grid"><article class="rank-card"><h3>🌓 홀짝 TOP 5</h3>${oddEven}</article><article class="rank-card"><h3>🍎 사과게임 TOP 5</h3>${apple}</article><article class="rank-card"><h3>⚫ 오목 TOP 5</h3>${omok}</article></div><div class="game-ranking-heading relation-ranking-heading"><strong>관계 현황</strong><small>기존 커플·찌르기 기록</small></div><div class="rank-tabs-grid"><article class="rank-card"><h3>커플</h3>${couples}</article><article class="rank-card"><h3>찌르기 TOP 5</h3>${pokes}</article></div></section>`;
}
...
function openAdmin() {
  ...
  const members = admin.members.map((member) => `<article class="admin-member"><div><strong>${esc(member.displayName)}${member.isSelf ? ' <span class="admin-self-badge">내 계정</span>' : ''}</strong><small>Lv.${member.level} · ${points(member.points)} · 일반 경고 ${member.warnings} · 누적 ${member.warningTotal} · 파손 경고 ${member.breakWarnings ?? 0}/${app.data.catalog.breakWarningMax ?? 3}${member.broken ? ' · 💥 파손' : ''}</small></div><div class="admin-buttons"><button class="admin-point-give" data-action="admin-points" data-id="${member.petId}" data-value="1" type="button">포인트+</button><button class="admin-point-take" data-action="admin-points" data-id="${member.petId}" data-value="-1" type="button">포인트-</button><button data-action="admin-warning" data-id="${member.petId}" data-value="1" type="button">경고+</button><button data-action="admin-warning" data-id="${member.petId}" data-value="-1" type="button">경고-</button><button data-action="admin-reset-user" data-id="${member.petId}" type="button">상태 초기화</button><button class="danger-button" data-action="admin-kick" data-id="${member.petId}" type="button">강퇴</button>${member.isSelf ? '' : `<button class="danger-button admin-delete-account" data-action="admin-delete-account" data-user-id="${member.userId}" type="button">계정 삭제</button>`}</div></article>`).join('');
  ...
  openModal(`${modalHeader('운영자 관리', `내 User ID: ${admin.userId}`)}<div class="admin-top"><button class="warning-button wide" data-action="admin-force-liar" type="button">진행 중 라이어게임 강제 종료</button></div><h3>회원 관리</h3><p class="helper">포인트+ / 포인트-로 회원 포인트를 직접 지급하거나 회수할 수 있습니다. 포인트는 0P 아래로 내려가지 않습니다. 계정 삭제는 회원가입 정보와 모든 세대 레고 데이터를 제거하며 복구할 수 없습니다.</p><div class="admin-list">${members || '<div class="empty">회원이 없습니다.</div>'}</div><h3>라이어 채팅 관리</h3><div class="admin-list">${chats || '<div class="empty">채팅이 없습니다.</div>'}</div>`, { type: 'admin' });
}
...
async function handleAction(button) {
  ...
  if (action === 'admin-points') {
    ...
    if (direction < 0 && !confirm(`${amount.toLocaleString('ko-KR')}P를 이 회원에게서 회수할까요?`)) return;
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=641').catch(() => {});
