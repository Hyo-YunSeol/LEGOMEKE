...
import { preloadSpotDifferenceSceneAssets, renderSpotDifferenceAnswerMarks, renderSpotDifferenceScene, spotDifferenceHitIdAt, spotDifferenceThemeLabel } from './spot-difference-scene.js?v=610120';
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610120" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610120`;
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610120`;
}
...
function spotDifferenceLobby() {
  ...
  const canCreate = rooms.length < (game?.maxRooms || 3);
  return `${sectionHeading('틀린그림찾기', battleUsageText(), `<button class="primary" data-action="spot-difference-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="spot-lobby-list">${cards}</div>`;
}
...
function spotDifferenceRoomView(room) {
  ...
  if (room.status === 'waiting') {
    ...
    if (room.status === 'playing') stage += `<p class="helper centered">7개 먼저 찾기 · 실수 ${spotDifferenceMaxWrongClicks(room)}회 패배</p>`;
    if (revealAll) {
...
function davinciLobby() {
  ...
  return `${sectionHeading('다빈치코드', battleUsageText(), `<button class="primary" data-action="davinci-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<div class="davinci-lobby-list">${cards}</div>`;
}
...
function renderTab(tab, { force = false } = {}) {
  ...
  if (tab === 'games' && alreadyRendered) {
    ...
    if (liveSpotDifferenceRoom?.status === 'playing' && patchSpotDifferenceLiveRoom(liveSpotDifferenceRoom)) {
      // 틀린그림찾기는 두 WebP/SVG 판을 최초 1회만 만들고 경기 중에는 절대 교체하지 않는다.
      pane.dataset.revision = String(app.revision);
      updateAppChrome();
      return;
    }
...
function scheduleTabRender(tab, { afterPaint = false } = {}) {
  ...
  const run = () => {
    ...
    if (tab === 'games' && pane.dataset.rendered === 'true') {
      ...
      const liveSpotDifferenceRoom = currentSpotDifferenceRoom();
      if (liveSpotDifferenceRoom?.status === 'playing' && patchSpotDifferenceLiveRoom(liveSpotDifferenceRoom)) {
        // 예약된 generic render도 진행 중인 틀린그림 SVG를 제거하지 못하게 한다.
        updateAppChrome();
        return;
      }
    }
...
function render() {
  ...
  const liveSpotDifferenceRoom = app.tab === 'games' ? currentSpotDifferenceRoom() : null;
  if (pane.dataset.rendered === 'true' && app.dirtyTabs.has('games') && liveSpotDifferenceRoom?.status === 'playing' && patchSpotDifferenceLiveRoom(liveSpotDifferenceRoom)) {
    // presence/알림/다른 게임 갱신이 와도 틀린그림 두 판은 그대로 유지한다.
    requestAnimationFrame(syncGameResultScrollLock);
    return;
  }
...
navigator.serviceWorker?.register?.('/sw.js?v=610120')?.catch(() => {});
