...
function cosmeticClasses(c = {}) {
  const background = c.background ? `cos-bg-${String(c.background).replace(/[^a-z0-9-]/gi, '')}` : '';
  return [c.goldBorder ? 'cos-gold' : '', c.royalCard ? 'cos-royal' : '', c.aurora ? 'cos-aura' : '', background].filter(Boolean).join(' ');
}

function cosmeticNameClass(c = {}) {
  if (c.nameRainbow) return 'cos-name-rainbow';
  if (c.nameSparkle) return 'cos-name-sparkle';
  return '';
}

function cosmeticStatusClass(c = {}) {
  if (c.statusRainbow) return 'cos-status-rainbow';
  if (c.statusGlow) return 'cos-status-glow';
  return '';
}

function cosmeticRemaining(until) {
...
function avatar(stage, { mini = false } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=652`;
...
function homeView() {
  ...
  return `
    <section class="hero-card ${cosmeticClasses(pet.cosmetics)}">
      <div class="hero-main">${avatar(stage)}<div class="hero-copy"><span class="eyebrow"><button class="body-stage-link" data-action="body-guide" data-body="${pet.stats.body}" type="button">${esc(stage.label)}</button> · ${pet.generation}세대</span><h1 class="${cosmeticNameClass(pet.cosmetics)}">${esc(pet.displayName)}${seasonBadgesHtml(pet)}</h1><div class="status-message-row"><span class="status-message-text ${pet.statusMessage ? '' : 'muted'} ${cosmeticStatusClass(pet.cosmetics)}">${esc(pet.statusMessage || '상태메시지 없음')}</span><button class="status-edit-button" data-action="edit-status-message" type="button" >수정</button></div><p class="system-status">${esc(pet.status)}</p><div class="hero-tags"><span>Lv.${pet.stats.level}</span><span>레고력 ${pet.stats.legoPower}</span><span>${pet.partnerPetId ? `커플 D+${pet.coupleDay}` : '솔로'}</span></div></div></div>
...
function profileMiniCard(profile) {
  ...
  return `<button class="${classes}" data-action="profile" data-id="${profile.id}" type="button">${seasonBadgesHtml(profile)}${avatar(stage, { mini: true })}<strong class="${cosmeticNameClass(c)}">${esc(profile.displayName)}</strong>${profile.statusMessage ? `<span class="profile-status-message ${cosmeticStatusClass(c)}">${esc(profile.statusMessage)}</span>` : ''}<small>${profile.online ? '🟢 ' : ''}Lv.${profile.stats.level} · ${points(profile.stats.points)}</small></button>`;
}

function openStyleShop() {
  const me = app.data.dashboard.pet;
  const items = app.data.catalog.cosmetics ?? [];
  const cosmetics = me.cosmetics ?? {};
  const groups = [
    ['effect', '효과', '닉네임·상태메시지·레고 본체를 기간제로 꾸밉니다.'],
    ['background', '배경', '도감 그림 자체는 투명하게 유지하고 내 카드 뒤에만 적용됩니다.']
  ];
  const statusFor = (item) => {
    if (item.kind === 'background') return cosmetics.background === item.background ? cosmeticRemaining(cosmetics.expiresAt?.background) : '';
    return cosmetics[item.id] ? cosmeticRemaining(cosmetics.expiresAt?.[item.id]) : '';
  };
  const sections = groups.map(([kind, title, description]) => {
    const groupItems = items.filter((item) => (item.kind || 'effect') === kind);
    if (!groupItems.length) return '';
    const cards = groupItems.map((item) => {
      const activeText = statusFor(item);
      const previewClass = item.background ? ` style-bg-preview cos-bg-${String(item.background).replace(/[^a-z0-9-]/gi, '')}` : '';
      return `<article class="style-shop-item${previewClass}"><div><strong>${esc(item.name)}${activeText ? ` <span class="shop-active">${esc(activeText)}</span>` : ''}</strong><p>${esc(item.description)}</p></div><button class="primary" data-action="buy-cosmetic" data-id="${esc(item.id)}" type="button" ${me.stats.points < item.price ? 'disabled' : ''}>${activeText ? `${points(item.price)} · 연장` : points(item.price)}</button></article>`;
    }).join('');
    return `<section class="style-shop-group"><header><h3>${esc(title)}</h3><p>${esc(description)}</p></header>${cards}</section>`;
  }).join('');
  openModal(`${modalHeader('꾸미기 상점', `보유 ${points(me.stats.points)} · 내 레고/주민목록/프로필에 공통 표시`)}<div class="style-shop">${sections}</div>`, { type:'styleShop' });
}
...
function switchMainTab(tabName, { smooth = false } = {}) {
  ...
  // 같은 탭을 다시 눌렀을 때 무거운 화면 전체를 재렌더링하지 않는다.
  if (!changed) {
    $$('.nav-item').forEach((button) => button.classList.toggle('active', button.dataset.tab === app.tab));
  ...
  // 눌린 탭의 시각 피드백을 DOM 재생성보다 먼저 줘서 PC/모바일 모두 즉시 반응하게 한다.
  $$('.nav-item').forEach((button) => button.classList.toggle('active', button.dataset.tab === app.tab));
  render();
  window.scrollTo({ top: 0, behavior: smooth ? 'smooth' : 'auto' });
}
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=652').catch(() => {});
