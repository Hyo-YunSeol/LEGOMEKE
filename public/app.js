...
const BODY_STAGE_FALLBACK = [
  ...
  { min: 7200, max: null, key: 'argentinosaurus', assetKey: 'dino-titan', label: '아르헨티노사우루스레고', activityHungerCost: 4 }
];
...
function avatar(stage, { mini = false } = {}) {
  ...
  return `<div class="pet-visual ${mini ? 'mini' : ''} ${stageClass}"><span class="pet-shadow"></span><img src="/pets/${esc(stage.assetKey || stage.key)}.svg" alt="${esc(stage.label)}" draggable="false"></div>`;
}
...
function openAdmin() {
  ...
  const members = admin.members.map((member) => {
    ...
    const summary = member.hasActivePet === false
      ...
      : `Lv.${member.level} · ${points(member.points)} · 일반 경고 ${member.warnings} · 누적 ${member.warningTotal} · 파손 경고 ${member.breakWarnings ?? 0}/${app.data.catalog.breakWarningMax ?? 3}${member.broken ? ' · 💥 파손' : ''}`;
...
async function handleAction(button) {
  ...
  if (action === 'admin-refresh') return loadBootstrap({ silent: false });
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=645').catch(() => {});
