...
import { preloadSpotDifferenceSceneAssets, renderSpotDifferenceAnswerMarks, renderSpotDifferenceScene, spotDifferenceHitIdAt, spotDifferenceThemeLabel } from './spot-difference-scene.js?v=610121';
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610121" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610121`;
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610121`;
}
...
function spotDifferenceServerNow() {
  ...
  // 틀린그림찾기 카운트다운/30초 타이머가 절대 뒤로 올라가지 않게 단조 증가시킨다.
...
navigator.serviceWorker?.register?.('/sw.js?v=610121')?.catch(() => {});
