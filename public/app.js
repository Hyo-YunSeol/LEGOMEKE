...
function applyBootstrap(next) {
  ...
  for (let index = 0; index < (app.data?.sichuan?.rooms || []).length; index += 1) {
    ...
    if (sameMatch && Number(room.stateVersion || 0) < Number(previous.stateVersion || 0)) app.data.sichuan.rooms[index] = previous;
  }
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69801" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69801`;
...
function dismissBattleResult(game, roomId, key) {
  ...
  if (game === 'sichuan') { app.sichuanRoomId = null; app.sichuanLobbyForced = true; app.sichuanSelectedIndex = null; }
...
function restoreSingleTetrisState(challenge) {
  try {
    ...
    // v6.9.8: 싱글도 대전과 동일하게 새 블록은 0행에서 즉시 보인다.
...
async function finishSingleTetris(reason) {
  ...
  try {
    ...
    // 정산 재시도 중에도 종료된 판은 계속 얼려둔다. 입력이 다시 살아나 점수가 바뀌는 경계조건을 막는다.
    state.ending=true; toast(error.message,'error');
    clearTimeout(app.singleTetrisFinishRetryTimer);
    app.singleTetrisFinishRetryTimer=setTimeout(()=>{ if(app.singleTetrisState===state && !state.ended){ state.ending=false; finishSingleTetris(reason); } },1200);
  }
}
...
function sichuanBoardHtml(player, { interactive = false, compact = false } = {}) {
  ...
  return `<div class="sichuan-board ${compact ? 'compact' : ''}" role="grid" aria-rowcount="8" aria-colcount="10" aria-label="${esc(player?.displayName || '플레이어')} 사천성 판">${board.map((tileId, index) => {
    // Every slot owns a fixed 8x10 grid coordinate. Removed pairs stay as invisible
    // slots instead of reflowing the remaining tiles. Do not use the global .empty
    // helper here: it intentionally spans grid-column:1/-1 for empty-state messages.
    const row = Math.floor(index / 10) + 1;
    const column = (index % 10) + 1;
    const position = `grid-row:${row};grid-column:${column}`;
    if (!tileId) return `<span class="sichuan-cell sichuan-empty-cell" data-index="${index}" role="gridcell" aria-rowindex="${row}" aria-colindex="${column}" aria-hidden="true" style="${position}"></span>`;
    const tile = tiles.get(tileId);
    const selected = !compact && Number(app.sichuanSelectedIndex) === index;
    const tileSrc = tile?.src ? `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=69801` : '';
    return `<button class="sichuan-cell ${selected ? 'selected' : ''}" data-action="sichuan-tile" data-index="${index}" data-tile="${esc(tileId)}" type="button" ${interactive ? '' : 'disabled'} role="gridcell" aria-rowindex="${row}" aria-colindex="${column}" aria-label="${esc(tile?.label || tileId)}${selected ? ' 선택됨' : ''}" style="${position}"><img src="${esc(tileSrc)}" alt="" draggable="false" loading="eager" decoding="sync"><span>${esc(tile?.label || '')}</span></button>`;
  }).join('')}</div>`;
}

function sichuanProgressHtml(player, { mine = false } = {}) {
  const removed = Math.max(0, Math.min(80, Number(player?.removedCount || 0)));
  const remaining = Math.max(0, 80 - removed);
  return `<div class="sichuan-progress ${mine ? 'mine' : ''}"><div><strong>${esc(player?.displayName || '-')}</strong>${mine ? '<small>내 진행</small>' : ''}</div><div class="sichuan-progress-track"><i style="width:${(removed / 80) * 100}%"></i></div><b>${remaining}개 남음</b></div>`;
}
...
function sichuanRoomView(room) {
  ...
  const interactive = Boolean(isPlayer && room.status === 'playing' && self && !app.sichuanActionInFlight);
  const timer = room.status === 'playing' ? '<b id="sichuan-countdown">2:30</b>' : '<b>--:--</b>';
  let stage = '';
  if (room.status === 'waiting') {
    ...
    stage = `<div class="sichuan-match-head">${sichuanProgressHtml(self, { mine: true })}<div class="sichuan-match-center"><span>남은 시간</span>${timer}<small>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${sichuanProgressHtml(opponent)}</div><div class="sichuan-board-stage">${sichuanBoardHtml(self, { interactive })}${spectatorBurstLayer(room.reactions || [], 'sichuan')}</div><p class="helper centered">같은 그림 두 개를 선택하세요. 최대 두 번 꺾어 연결할 수 있으며 판 바깥쪽 경로도 허용됩니다.</p>`;
  } else {
    stage = `<div class="sichuan-match-head">${sichuanProgressHtml(hostPlayer)}<div class="sichuan-match-center"><span>남은 시간</span>${timer}<small>관전자 ${Number(room.spectatorCount || 0)}명</small></div>${sichuanProgressHtml(guestPlayer)}</div>`;
  }
...
function applySichuanRoomState(payload) {
  ...
  const sameMatch = Boolean(previous?.matchId && previous.matchId === room.matchId);
  if (sameMatch && Number(room.stateVersion || 0) < Number(previous?.stateVersion || 0)) return false;
  ...
  if (app.tab === 'games') renderSichuanRegion();
...
async function submitSichuanPair(room, first, second) {
  if (!room || app.sichuanActionInFlight) return null;
  app.sichuanActionInFlight = true;
  try {
    const actionId = crypto.randomUUID?.() || `sichuan-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const result = await api(`/api/sichuan/rooms/${encodeURIComponent(room.id)}/pair`, {
      method: 'POST', body: JSON.stringify({ matchId: room.matchId, first, second, actionId })
    });
    app.sichuanActionInFlight = false;
    app.sichuanSelectedIndex = null;
    if (result.room) applySichuanRoomState({ room: result.room, serverTime: result.serverTime });
    if (result.ok && result.removed === false) flashSichuanPair([first, second]);
    return result;
  } catch (error) {
    app.sichuanActionInFlight = false;
    app.sichuanSelectedIndex = null;
...
function openMiniGame(challenge) {
  ...
  if (challenge.gameId === 'apple') content = `<div class="apple-game"><div class="mini-abandon-controls apple-abandon-controls"><button class="ghost mini-abandon-button" data-action="abandon-mini" type="button">포기하기</button></div><div class="apple-hud"><div><small>남은 시간</small><strong id="apple-countdown">${appleTimeText(challenge)}</strong></div><div><small>게임 점수</small><strong id="apple-score">${Number(challenge.appleScore || 0).toLocaleString('ko-KR')}점</strong></div><div><small>획득 예정</small><strong id="apple-pending">${points(challenge.applePendingPoints || 0)}</strong></div></div><p class="helper apple-rules">사각형 합 10이면 제거 · 2개 +5P · 3개 이상 +6P</p><div id="apple-board" class="apple-board" aria-label="사과게임 10 곱하기 10 숫자판">${appleBoardHtml(challenge)}</div><div id="apple-selection-info" class="apple-selection-info">드래그해서 숫자를 선택하세요.</div><div id="apple-refresh-region">${appleRefreshOfferHtml(challenge)}</div></div>`;
  ...
  requestAnimationFrame(() => {
    ...
    if (challenge.gameId === 'tetrisSingle') { paintSingleTetris(); startSingleTetrisTimers(); }
...
function syncAppleGameLayout() {
  ...
  const modalStyle = getComputedStyle(modal);
  const verticalPadding = (parseFloat(modalStyle.paddingTop) || 0) + (parseFloat(modalStyle.paddingBottom) || 0);
  const abandonControls = $('.apple-abandon-controls', game);
  const fixedHeight = [header, hud, rules, abandonControls, selection, refresh].reduce((sum, node) => sum + (node?.offsetHeight || 0), 0);
...
async function tick() {
  ...
  if (sichuanCountdown && sichuanRoom?.status === 'playing' && sichuanRoom.deadlineAt) {
    ...
    sichuanCountdown.textContent = `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, '0')}`;
...
async function handleAction(button, event = null) {
  ...
  if (action === 'battle-result-confirm') { dismissBattleResult(button.dataset.game, idValue, button.dataset.key); return; }
  ...
  if (action === 'single-tetris-restart') { stopSingleTetrisTimers(); app.singleTetrisState=null; closeModal(); requestAnimationFrame(syncGameResultScrollLock); await loadBootstrap({silent:true}); const result=await perform('/api/minigames/start',{gameId:'tetrisSingle'},null,'POST',{toastDuration:900,toastType:'game-start'}); if(result?.bootstrap?.activeMiniChallenge) openMiniGame(result.bootstrap.activeMiniChallenge); return; }
  ...
  if (action === 'sichuan-open') { app.sichuanLobbyForced = false; app.sichuanRoomId = idValue; app.sichuanSelectedIndex = null; renderSichuanRegion(); return; }
  if (action === 'sichuan-back') { app.sichuanRoomId = null; app.sichuanLobbyForced = true; app.sichuanSelectedIndex = null; renderSichuanRegion(); return; }
  ...
  if (action === 'sichuan-tile') {
    const room = currentSichuanRoom();
    if (!room || room.viewerRole !== 'player' || room.status !== 'playing' || app.sichuanActionInFlight) return;
    const index = Number(button.dataset.index);
    const self = room.players?.[room.selfPetId];
    const tileId = self?.board?.[index];
    ...
    const firstTileId = self?.board?.[first];
...
navigator.serviceWorker?.register?.('/sw.js?v=69801')?.catch(() => {});
