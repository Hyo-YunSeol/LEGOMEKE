...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=684" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=684`;
...
function blockBattleAttackHtml(room) {
  if (room?.status !== 'playing' || Number(room.lastAttack?.lines) <= 0) return '';
  const isPlayer = room.viewerRole === 'player';
  const isSpectator = room.viewerRole === 'spectator';
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=684').catch(() => {});
