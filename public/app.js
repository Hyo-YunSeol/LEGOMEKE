...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69000" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69000`;
...
function socialView() {
  ...
  const sentRequests = app.data.sentRequests ?? [];
  ...
  const requestCards = requests.map((request) => request.type === 'mating'
    ? `<article class="request-card mating-request"><div><strong>💕 ${esc(request.fromDisplayName)}</strong><small>교미 신청 · 24시간 내 응답</small></div><div class="button-row"><button class="primary" data-action="accept-mating" data-id="${request.fromPetId}" data-request-id="${request.id}" type="button">수락</button><button class="ghost" data-action="reject-mating" data-id="${request.fromPetId}" data-request-id="${request.id}" type="button">거절</button></div></article>`
    : `<article class="request-card"><div><strong>${esc(request.fromDisplayName)}</strong><small>매칭 신청 · 24시간 내 응답</small></div><div class="button-row"><button class="primary" data-action="accept-match" data-id="${request.fromPetId}" data-request-id="${request.id}" type="button">수락</button><button class="ghost" data-action="reject-match" data-id="${request.fromPetId}" data-request-id="${request.id}" type="button">거절</button></div></article>`).join('');
  const sentRequestCards = sentRequests.map((request) => `<article class="request-card sent-request"><div><strong>${request.type === 'mating' ? '💕 ' : ''}${esc(request.toDisplayName)}</strong><small>${request.type === 'mating' ? '교미' : '매칭'} 신청 보냄 · ${dateText(request.expiresAt)} 만료</small></div><button class="ghost" data-action="cancel-social-request" data-id="${request.toPetId}" data-request-id="${request.id}" type="button">신청 취소</button></article>`).join('');
  ...
  return `
    ...
    ${sentRequests.length ? `<section class="section">${sectionHeading('보낸 신청', '24시간이 지나면 자동 만료됩니다.')}<div class="request-list">${sentRequestCards}</div></section>` : ''}
...
async function buyLoudspeaker() {
  ...
  if (result?.loudspeaker) showLoudspeaker(result.loudspeaker);
  return result;
}
...
async function handleAction(button, event = null) {
  ...
  if (action === 'cancel-social-request') return perform('/api/social/action', { targetPetId: idValue, requestId: button.dataset.requestId, action: 'cancelRequest' });
...
document.addEventListener('input', (event) => {
  ...
  if (loudspeakerInput) {
    ...
    if (chars.length > maxLength) loudspeakerInput.value = chars.slice(0, maxLength).join('');
    const count = $('#loudspeaker-message-count');
    if (count) count.textContent = `${[...loudspeakerInput.value].length}/${maxLength}`;
  }
});
...
navigator.serviceWorker?.register?.('/sw.js?v=69000')?.catch(() => {});
