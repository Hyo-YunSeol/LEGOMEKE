...
const BODY_STAGE_FALLBACK = [
  ...
  { min: 4520, max: null, key: 'argentinosaurus', assetKey: 'lego-argentinosaurus', label: '아르헨티노사우루스레고', activityHungerCost: 10 }
];
...
function avatar(stage, { mini = false } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=647`;
...
function liarSection() {
  ...
  return `${sectionHeading('라이어게임', `${liarPhaseLabel(game.phase)} · 1라운드`, game.phaseEndsAt ? '<span id="liar-countdown" class="tag"></span>' : '')}<div class="liar-layout"><div><div class="liar-player-list">${players}</div>${waitingControls}${role}${vote}${guess}${result}${spectatorReactionBar('liar', null, game.reactions || [], false)}${gameOver}</div>${liarChat(game, me.id, false)}</div>`;
}
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=647').catch(() => {});
