...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    if (!hadData && (currentOmokRoom() || currentBlockBattleRoom())) {
      app.tab = 'games';
      app.omokLobbyForced = false;
      app.blockBattleLobbyForced = false;
      markTabDirty('games');
    ...
    if (!appleOnlyUpdated && !blockOnlyUpdated && !numberInputProtected) {
      render();
...
async function perform(path, body = {}, successMessage = null, method = 'POST', {
  ...
  try {
    ...
    if (!appleOnlyUpdated && !blockOnlyUpdated && !numberOnlyUpdated && !shopOnlyUpdated) {
      render();
      if (app.modal) {
        await refreshModal();
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=68946" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=68946`;
...
function blockBattleLobby() {
  ...
  return `${sectionHeading('테트리스대전', '10×20 · 7종 블록 · 1:1 실시간 대전', `<button class="primary" data-action="block-battle-create" type="button" ${canCreate ? '' : 'disabled'}>방 만들기</button>`)}<p class="helper block-battle-intro">여러 줄을 한 번에 지우면 상대에게 방해줄을 보냅니다. 자동 낙하는 한 판 내내 같은 속도로 유지되며 먼저 천장에 닿으면 패배합니다.</p><div class="block-battle-lobby-list">${cards}</div>`;
}
...
function syncBlockBattleViewport() {
  ...
  app.blockBattleViewportFrame = requestAnimationFrame(() => {
    ...
    const mobilePlaying = window.matchMedia?.('(max-width: 520px)')?.matches
      && document.body.classList.contains('block-battle-playing');
    if (!mobilePlaying) {
      region.style.removeProperty('--block-battle-board-size');
      delete region.dataset.boardSize;
      return;
    }
    const stage = $('.block-battle-stage', region);
    const minePanel = $('.block-battle-player.mine', region);
    const mineBoard = minePanel ? $('.block-battle-board', minePanel) : null;
    const controls = $('.block-battle-controls', region);
    if (!stage || !minePanel || !mineBoard || !controls) return;

    // 고정된 `화면높이 - npx` 추정값 대신 실제 현재 UI 높이를 잰다. 상단바/닉네임/
    // 통계/조작부가 기기마다 달라도 내 20번째 줄이 조작부 아래로 숨지 않게 한다.
    const stageRect = stage.getBoundingClientRect();
    const panelRect = minePanel.getBoundingClientRect();
    const boardRect = mineBoard.getBoundingClientRect();
    const controlsRect = controls.getBoundingClientRect();
    const stageStyle = getComputedStyle(stage);
    const stagePadding = (parseFloat(stageStyle.paddingTop) || 0) + (parseFloat(stageStyle.paddingBottom) || 0);
    const panelChrome = Math.max(0, panelRect.height - boardRect.height);
    const viewportBottom = viewportTop + height;
    const usableBoardHeight = viewportBottom - stageRect.top - controlsRect.height - panelChrome - stagePadding - 14;
    const byHeight = Math.floor(usableBoardHeight / 2);
    const byWidth = Math.floor(panelRect.width);
    const size = Math.max(72, Math.min(220, byHeight, byWidth));
    if (Number.isFinite(size) && size > 0 && region.dataset.boardSize !== String(size)) {
      region.dataset.boardSize = String(size);
      region.style.setProperty('--block-battle-board-size', `${size}px`);
    }
  });
}
...
const BLOCK_BATTLE_LOCAL_ATTACK_LINES = Object.freeze({ 1: 0, 2: 1, 3: 2, 4: 4 });

function previewBlockBattleLockAndSpawn(player) {
  const active = player?.active;
  const shape = active && LOCAL_TETROMINO_SHAPES[active.type]?.[Number(active.rotation) || 0];
  const nextType = Array.isArray(player?.next) ? player.next[0] : null;
  if (!shape || !LOCAL_TETROMINO_SHAPES[nextType] || !Array.isArray(player?.board)) return { changed: false, boardChanged: false };

  const board = Array.from({ length: 20 }, (_, row) => Array.from({ length: 10 }, (_, col) => player.board?.[row]?.[col] ?? null));
  for (const [x, y] of shape) {
    const row = Number(active.row) + y;
    const col = Number(active.col) + x;
    if (row < 0 || row >= 20 || col < 0 || col >= 10) return { changed: false, boardChanged: false };
  ...
  let attack = BLOCK_BATTLE_LOCAL_ATTACK_LINES[cleared] || 0;
  const cancelled = Math.min(pendingGarbage, attack);
  pendingGarbage -= cancelled;
  attack -= cancelled;
  // 방해줄 구멍은 서버 난수이므로 이 경우에만 낙관적 고정을 하지 않고 ACK를 기다린다.
  if (attack <= 0 && pendingGarbage > 0) return { changed: false, boardChanged: false };

  const nextActive = { type: nextType, rotation: 0, row: 0, col: 3 };
  if (blockBattleLocalCollision({ board: kept }, nextActive)) return { changed: false, boardChanged: false };
  player.board = kept;
  player.active = nextActive;
  player.next = player.next.slice(1);
  player.lines = Math.max(0, Number(player.lines) || 0) + cleared;
  player.pieces = Math.max(0, Number(player.pieces) || 0) + 1;
  player.pendingGarbage = pendingGarbage;
  if (attack > 0) player.attackSent = Math.max(0, Number(player.attackSent) || 0) + attack;
  return { changed: true, boardChanged: true, locked: true };
}
...
function previewBlockBattleInput(action, { room = currentBlockBattleRoom(), paint = true } = {}) {
  ...
  if (action === 'rotate') {
    ...
    if (action === 'hardDrop') {
      ...
      const locked = previewBlockBattleLockAndSpawn(player);
      changed ||= locked.changed;
      boardChanged ||= locked.boardChanged;
    } else if (!blockBattleLocalCollision(player, candidate)) {
      player.active = candidate;
      changed = true;
    } else if (action === 'softDrop' || action === 'tick') {
      // 바닥 충돌 순간에도 서버 왕복을 기다리며 멈춰 있지 않도록, 서버와 결과가
      // 결정적으로 같은 경우에는 고정·줄삭제·다음 블록 등장까지 먼저 보여준다.
      const locked = previewBlockBattleLockAndSpawn(player);
      changed ||= locked.changed;
      boardChanged ||= locked.boardChanged;
    }
  }
  // 평상시 이동은 활성 블록 주변만, 고정/줄삭제/새 블록 생성 때만 200칸을 다시 그린다.
  if (paint && changed) paintBlockBattleBoard(player, document, { activeOnly: !boardChanged });
  return changed;
}
...
function syncBlockBattleGravity() {
  ...
  // 서버와 동일한 고정 낙하 주기를 사용한다. 오래 플레이해도 클라이언트가 가속되지 않는다.
...
function queueBlockBattleInput(action) {
  ...
  // 하드드롭 ACK 전 연타가 다음 블록까지 연쇄 하드드롭으로 적용되지 않게 한다.
  if (action === 'hardDrop' && awaitingHardDrop) return false;
  if (action === 'tick') {
    // 자동 낙하는 최대 한 개만 미확정 상태로 둔다. 고정 주기에서도 네트워크
    // 응답이 늦을 때 tick이 쌓여 수동 조작을 밀어내지 못하게 한다.
...
function recordsView() {
  ...
  return `<section class="page-title"><span class="eyebrow">기록</span><h1>${esc(pet.displayName)}의 기록</h1><p>현재 레고의 핵심 기록만 표시합니다.</p></section><section class="section"><div class="record-grid">${compactMetric('세대', `${pet.generation}세대`)}${compactMetric('생존', `${records.days}게임일`)}${compactMetric('레벨', `Lv.${pet.stats.level}`)}${compactMetric('레고력', `${pet.stats.legoPower}`)}${compactMetric('포인트', points(pet.stats.points))}${compactMetric('최고 포인트', points(records.maxPoints))}${compactMetric('현재 경고', `${pet.warnings}회`)}${compactMetric('누적 경고', `${pet.records.warnings}회`)}${compactMetric('영토전 우승', `${records.territoryWins}회`)}${compactMetric('테트리스대전', `${records.blockBattleWins || 0}승 ${records.blockBattleLosses || 0}패`)}${compactMetric('번개 최고', records.bestReactionMs ? `${(records.bestReactionMs/1000).toFixed(3)}초` : '-')}${compactMetric('낚시', `${records.fishing}회`)}</div></section>${app.data.admin.isAdmin ? `<section class="section admin-callout"><div><h2>운영자 관리</h2><p>포인트 지급·회수, 일반 경고, 계정 잠금·해제, 강퇴, 계정 삭제, 상태 초기화와 진행 중 벙·오목방 관리를 할 수 있습니다.</p></div><button class="primary" data-action="open-admin" type="button">관리 열기</button></section>` : ''}<section class="section"><button class="danger-button wide" data-action="logout" type="button">모든 기기에서 로그아웃</button></section>`;
}
...
function adminMembersHtml(admin = app.data?.admin) {
  return (admin?.members ?? []).map((member) => {
    ...
    const lockControl = member.isSelf ? '' : `<button class="${member.accountLocked ? 'soft-button' : 'warning-button'}" data-action="admin-account-lock" data-user-id="${esc(member.userId)}" data-locked="${member.accountLocked ? 'false' : 'true'}" data-name="${esc(member.displayName)}" type="button">${member.accountLocked ? '🔓 잠금 해제' : '🔒 계정 잠금'}</button>`;
    return `<article class="admin-member ${member.accountLocked ? 'account-locked' : ''}" data-admin-user-id="${esc(member.userId)}"><div><strong>${esc(member.displayName)}${member.isSelf ? ' <span class="admin-self-badge">내 계정</span>' : ''}${member.workoutBadge ? ' <span class="admin-workout-label">💪 운동방</span>' : ''}${member.accountLocked ? ' <span class="admin-account-lock-label">🔒 계정 잠김</span>' : ''}</strong><small>${summary}${member.accountLocked && member.accountLockedAt ? ` · 잠금 ${dateText(member.accountLockedAt)}` : ''}</small></div><div class="admin-buttons">${workoutControl}${petControls}${lockControl}${member.isSelf ? '' : `<button class="danger-button admin-delete-account" data-action="admin-delete-account" data-user-id="${member.userId}" type="button">계정 삭제</button>`}</div></article>`;
...
function openAdmin() {
  ...
  const auditLogs = (admin.auditLogs ?? []).map((entry) => {
    ...
    else if (entry.action === 'account_lock') detail = `${esc(entry.adminDisplayName)} → ${esc(entry.targetDisplayName)} 계정 잠금`;
    else if (entry.action === 'account_unlock') detail = `${esc(entry.adminDisplayName)} → ${esc(entry.targetDisplayName)} 잠금 해제`;
...
async function handleAction(button, event = null) {
  ...
  if (action === 'admin-account-lock') {
    ...
    const message = locked
      ? `${targetName} 계정을 잠글까요?\n\n회원 데이터·포인트·기록은 유지되지만 즉시 로그인할 수 없게 되고 현재 로그인 세션도 종료됩니다.`
      : `${targetName} 계정 잠금을 해제할까요?\n\n기존 세션은 되살리지 않으며 회원이 새로 로그인해야 합니다.`;
    if (!confirm(message)) return;
    return perform(`/api/admin/users/${encodeURIComponent(targetUserId)}/lock`, { locked });
  }
...
navigator.serviceWorker?.register?.('/sw.js?v=68946')?.catch(() => {});
