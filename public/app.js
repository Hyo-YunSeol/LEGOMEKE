...
async function loadBootstrap({ silent = false, renderMode = 'full' } = {}) {
  ...
  try {
    ...
    const minesweeperOnlyUpdated = renderMode === 'minesweeper' && refreshMinesweeperMiniOnly();
...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=69203" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=69203`;
...
function davinciTileHtml(tile, { selectable = false, penalty = false } = {}) {
  ...
  const selected = selectable && app.davinciGuessTarget?.targetPetId === tile.ownerPetId && app.davinciGuessTarget?.tileId === tile.id;
  const classes = ['davinci-tile', color, tile.hidden ? 'hidden-tile' : 'known-tile', tile.revealed ? 'revealed' : '', tile.drawn ? 'drawn' : '', selected ? 'selected-target' : ''].filter(Boolean).join(' ');
  if (selectable) return `<button class="${classes} selectable" data-action="davinci-target" data-target-pet="${esc(tile.ownerPetId)}" data-tile-id="${esc(tile.id)}" data-color="${color}" type="button"><b>${esc(value)}</b>${selected ? '<small>선택</small>' : tile.revealed ? '<small>공개</small>' : ''}</button>`;
...
function davinciTurnNumber(room, petId) {
  const order = room.turnOrder?.length ? room.turnOrder : (room.players ?? []).map((player) => player.petId);
  const index = order.indexOf(petId);
  return index >= 0 ? index + 1 : null;
}

function davinciDeckSummaryHtml(room) {
  const total = Number(room.deckCount || 0);
  const black = Number(room.deckColors?.black || 0);
  const white = Number(room.deckColors?.white || 0);
  const title = total > 0 ? `🎴 남은 패 ${total}개` : '🎴 남은 패 0개 · 덱 소진';
  const ended = room.status === 'ended';
  const deckTiles = ended && Array.isArray(room.endedDeck) && room.endedDeck.length
    ? `<div class="davinci-ended-deck">${room.endedDeck.map((tile) => davinciTileHtml(tile)).join('')}</div>`
    : ended ? '<small class="davinci-ended-deck-empty">남아 있던 패가 없습니다.</small>' : '';
  return `<div class="davinci-deck-summary${ended ? ' ended' : ''}"><small>${ended ? '판 종료 · 남은 패 전체 공개' : '가운데 패 더미'}</small><strong>${title}</strong><div class="davinci-deck-colors"><span>⬛ 검정 ${black}</span><span>⬜ 흰색 ${white}</span></div>${deckTiles}</div>`;
}

function davinciPlayerPanelHtml(room, player, { isSelf = false, isPlayer = false } = {}) {
  const current = room.status === 'playing' && room.currentTurnPetId === player.petId;
  const turnNumber = davinciTurnNumber(room, player.petId);
  const turnBadge = turnNumber ? `<span class="davinci-order-badge">${turnNumber}</span>` : '';
  const stateLabel = player.forfeited ? '기권' : player.eliminated ? '💀 코드 해독 완료' : isSelf ? '내 코드' : '';
  const connection = room.status === 'playing' && !player.connected ? '오프라인' : '';
  const canTarget = !isSelf && isPlayer && room.status === 'playing' && room.phase === 'turn' && room.isMyTurn && !room.awaitingDecision && !player.eliminated && !player.forfeited;
  const hand = player.hand.map((tile) => davinciTileHtml(tile, {
    selectable: canTarget && tile.hidden && !tile.revealed,
    penalty: isSelf && room.phase === 'deckPenalty' && room.isMyTurn && !tile.revealed
  })).join('') || '<span class="empty">배치 준비 중</span>';
  const drawn = player.drawnTile ? `<div class="davinci-drawn"><small>${room.status === 'ended' ? '판 종료 시 보유 중이던 뽑은 타일' : '이번 턴 뽑은 타일'}</small>${davinciTileHtml(player.drawnTile)}</div>` : '';
  return `<article class="davinci-player-panel ${isSelf ? 'mine' : 'opponent'} ${current ? 'current-turn' : ''} ${player.eliminated || player.forfeited ? 'eliminated' : ''}"><header><div class="davinci-player-title">${turnBadge}<div><strong>${esc(player.displayName)}${isSelf ? ' · 내 코드' : ''}</strong><small>${esc(stateLabel)}</small></div></div><div class="davinci-player-meta"><b>정답 ${Number(player.correctGuesses || 0)}</b>${current ? '<span class="davinci-turn-badge">▶ 현재 차례</span>' : connection ? `<span>${esc(connection)}</span>` : ''}</div></header><div class="davinci-hand">${hand}</div>${drawn}</article>`;
}
...
function davinciRoomView(room) {
  ...
  const playerPanels = [
    ...(self ? [davinciPlayerPanelHtml(room, self, { isSelf: true, isPlayer })] : []),
    ...activeOthers.map((player) => davinciPlayerPanelHtml(room, player, { isSelf: false, isPlayer }))
  ].join('');

  const isHost = isPlayer && self?.petId === room.hostPetId;
  const waiting = room.status === 'waiting' ? `<div class="davinci-waiting"><div class="davinci-seat-list">${(room.players || []).map((player) => {
    const kick = isHost && player.petId !== room.hostPetId ? `<button class="danger-button davinci-kick-button" data-action="davinci-kick" data-id="${room.id}" data-target-pet="${esc(player.petId)}" data-name="${esc(player.displayName)}" type="button">강퇴</button>` : '';
    return `<div class="davinci-seat"><span>${player.petId === room.hostPetId ? '👑' : '•'}</span><strong>${esc(player.displayName)}</strong><small>${player.petId === room.hostPetId ? '방장' : player.ready ? '준비 완료' : '준비 전'}</small>${kick}</div>`;
  }).join('')}${Array.from({ length: Math.max(0, 4 - (room.players?.length || 0)) }, () => '<div class="davinci-seat empty"><span>＋</span><strong>빈자리</strong><small>참가 대기</small></div>').join('')}</div>${isHost ? '<p class="helper">참가자가 들어오거나 나가거나 강퇴되면 준비 상태가 모두 초기화됩니다.</p>' : ''}</div>` : '';
  ...
  const result = room.status === 'ended' ? `<div class="result-card"><strong>${room.winnerPetId ? `${esc(davinciPlayerName(room, room.winnerPetId))} 승리!` : '게임 종료'}</strong><p>${esc(room.resultReason || '')}</p><small>판이 끝나 모든 참가자의 코드와 남은 패를 공개했습니다. · 총 판돈 ${points(Number(room.stakePoints || 0) * Number(room.players?.length || 0))}</small></div>` : '';
  ...
  const gameBody = room.status === 'waiting' ? waiting : `<div class="davinci-stage"><div class="davinci-burst-slot">${spectatorBurstLayer(room.reactions || [], 'davinci')}</div>${davinciDeckSummaryHtml(room)}<div class="davinci-status"><span>단계 <b>${esc(davinciPhaseLabel(room.phase))}</b></span><span>현재 차례 <b>${room.status === 'ended' ? '-' : esc(currentName)}</b></span><span>남은 시간 <b id="davinci-countdown">-</b></span><span>관전자 <b>${Number(room.spectatorCount || 0)}명</b></span></div>${phaseAction}<div class="davinci-player-stack">${playerPanels}</div>${davinciGuessPanel(room)}${davinciRecentLog(room)}</div>`;
  return `${sectionHeading('다빈치코드', `${room.roomNumber}번방 · ${room.players?.length || 0}명 · 판돈 ${points(room.stakePoints)} · ${davinciStatusLabel(room.status)}`, '<button class="text-button" data-action="davinci-back" type="button">로비 보기</button>')}${gameBody}${result}<div class="button-row">${waitingControls}${rematch}${leave}</div>${room.status === 'playing' && (isPlayer || isSpectator) ? spectatorReactionBar('davinci', room.id, room.reactions || [], true, true) : ''}${isSpectator ? '<p class="helper">관전자는 진행 중에는 공개된 숫자와 조커만 볼 수 있고, 판이 끝나면 전체 코드가 공개됩니다.</p>' : ''}`;
}
...
function startMinesweeperClock() {
  ...
  app.minesweeperClockTimer = null;
  const challenge = app.data?.activeMiniChallenge;
  if (app.modal?.gameId !== 'minesweeper' || challenge?.gameId !== 'minesweeper' || !challenge.startedAt) return;
...
function patchMinesweeperBoard(board, challenge) {
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < cols; col += 1) {
      ...
      if (!cell) continue;
      const state = minesweeperCellState(challenge?.cells?.[row]?.[col] ?? null, row, col, finished);
...
async function drainMinesweeperActionQueue() {
  ...
  if (app.busy) { setTimeout(drainMinesweeperActionQueue, 16); return; }
  const item = app.minesweeperActionQueue.shift();
  if (!item) return;
  const challenge = app.data?.activeMiniChallenge;
  if (!challenge || challenge.gameId !== 'minesweeper' || challenge.id !== item.challengeId || !app.minesweeperSessionActive) {
    item.resolve?.(null);
    return drainMinesweeperActionQueue();
  }
  if (item.action === 'flag' && !challenge.startedAt) {
  ...
  app.minesweeperCurrentAction = item.action;
  let result = null;
  try {
    result = await perform('/api/minigames/minesweeper/action', { challengeId: challenge.id, action: item.action, row: item.row, col: item.col }, null, 'POST', {
      renderMode: 'minesweeper', toastResult: false, preserveControls: true
    });
    if (result?.finished) {
      app.minesweeperSessionActive = false;
      clearMinesweeperActionQueue();
      openMiniResult(result, result.challenge ?? finishedChallenge);
    }
    item.resolve?.(result);
  } finally {
    app.minesweeperActionInFlight = false;
...
async function handleAction(button, event = null) {
  ...
  if (action === 'davinci-kick') {
    const targetPetId = button.dataset.targetPet;
    const name = button.dataset.name || '이 참가자';
    if (!targetPetId || !confirm(`${name}님을 방에서 내보낼까요?`)) return;
    return perform(`/api/davinci/rooms/${encodeURIComponent(idValue)}/kick`, { targetPetId });
  }
...
function beginMinesweeperPointerGesture(event) {
  ...
  gesture.timer = setTimeout(() => {
    ...
    try { if (typeof navigator.vibrate === 'function') navigator.vibrate(25); } catch {}
...
navigator.serviceWorker?.register?.('/sw.js?v=69203')?.catch(() => {});
