...
async function refreshModal() {
  ...
  if (descriptor.type === 'shop') {
    ...
    const keepComposerStable = Boolean(input && (document.activeElement === input || app.loudspeakerEditing || app.loudspeakerComposing));
    // 모바일 키보드/한글 조합 중 실시간 bootstrap이 와도 input DOM을 교체하지 않는다.
    // 입력창 자체가 교체되면 IME 조합이 끊기고 타자가 안 쳐지는 것처럼 보인다.
    if (keepComposerStable) return updateLoudspeakerShopState();
    return openShop();
  }
...
function replayBlockBattlePendingInputs(room, { paint = true } = {}) {
  ...
  // 로컬 중력 기준은 유지한다. 그렇지 않으면 서버의 이전 gravityDueAt이 되살아나
  // 수동 하강 직후 너무 이른 tick → 서버 거절 → 블록이 위로 튀는 현상이 생길 수 있다.
...
function queueBlockBattleInput(action) {
  ...
  // 이미 로컬 화면에 미리 반영한 tick은 수동 입력이 들어와도 전송 큐에서 지우지 않는다.
  // 예전처럼 화면만 한 칸 내려간 tick을 삭제하면 서버는 그 낙하를 모른 채 좌우 입력만
  // 처리해서 다음 상태 수신 때 블록이 한 칸 위로 되튀는 스냅백이 발생한다.
  ...
  if (['softDrop', 'hardDrop', 'tick'].includes(action)) {
