...
function avatar(stage, { mini = false } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=650`;
...
function openStyleShop() {
  ...
  const groups = [
    ['effect', '효과', '닉네임·상태메시지·레고 본체를 꾸밉니다.'],
    ['background', '배경', '도감 그림 자체는 투명하게 유지하고 내 카드 뒤에만 적용됩니다.'],
    ['title', '영구 칭호', '구매하면 기간 제한 없이 장착됩니다. 다른 칭호를 사면 교체됩니다.']
  ];
  const statusFor = (item) => {
    if (item.kind === 'title') return cosmetics.title === item.title ? '장착 중' : '';
  ...
  const sections = groups.map(([kind, title, description]) => {
    ...
    const cards = groupItems.map((item) => {
      ...
      const activeTitle = kind === 'title' && activeText;
      const previewClass = item.background ? ` style-bg-preview cos-bg-${String(item.background).replace(/[^a-z0-9-]/gi, '')}` : '';
      return `<article class="style-shop-item${previewClass}"><div><strong>${esc(item.name)}${activeText ? ` <span class="shop-active">${esc(activeText)}</span>` : ''}</strong><p>${esc(item.description)}</p></div><button class="primary" data-action="buy-cosmetic" data-id="${esc(item.id)}" type="button" ${activeTitle || me.stats.points < item.price ? 'disabled' : ''}>${activeTitle ? '장착 중' : activeText ? `${points(item.price)} · 연장` : points(item.price)}</button></article>`;
    }).join('');
...
function activateBottomNav(event, { fromPointer = false } = {}) {
  const tab = event.target.closest?.('.bottom-nav [data-tab]');
  if (!tab) return false;
  if (fromPointer && !['touch','pen'].includes(event.pointerType)) return false;
  event.preventDefault();
  event.stopPropagation();
  if (fromPointer) app.lastTouchNavAt = Date.now();
  if (app.modal) closeModal();
  switchMainTab(tab.dataset.tab, { smooth: false });
  return true;
}
bottomNav?.addEventListener('pointerup', (event) => activateBottomNav(event, { fromPointer: true }), { capture: true });
bottomNav?.addEventListener('click', (event) => {
  if (Date.now() - app.lastTouchNavAt < 650) {
    event.preventDefault();
    event.stopPropagation();
    return;
  }
  activateBottomNav(event);
}, { capture: true });
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=650').catch(() => {});
