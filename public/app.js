...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610101" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610101`;
...
function restoreSingleTetrisState(challenge) {
  try {
    ...
    // v6.10.1: 싱글도 대전과 동일하게 새 블록은 0행에서 즉시 보인다.
...
function sichuanTileAssetSrc(tile) {
  if (!tile?.src) return '';
  return `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610101`;
}
...
function patchSichuanBoard(player, { interactive = false, compact = false } = {}) {
  ...
  for (let index = 0; index < 80; index += 1) {
    ...
    cell.disabled = !tileId || !interactive;
    if (!tileId) {
      cell.dataset.tile = '';
...
function patchSichuanLiveRoom(room) {
  ...
  if (isPlayer) {
    ...
    if (!patchSichuanBoard(self, { interactive: !app.sichuanActionInFlight })) return false;
  } else if (isSpectator) {
...
async function submitSichuanPair(room, first, second) {
  ...
  try {
    ...
    app.sichuanActionInFlight = false;
    app.sichuanSelectedIndex = null;
    const current = currentSichuanRoom();
    if (current?.status === 'playing') patchSichuanLiveRoom(current);
    toast(error.message, 'error');
    return null;
  } finally {
    app.sichuanActionInFlight = false;
...
navigator.serviceWorker?.register?.('/sw.js?v=610101')?.catch(() => {});
