...
function avatar(stage, { mini = false } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=654`;
  ...
  return `
    ...
    <section class="section">
      ...
      <div class="action-grid">
        ...
        <button class="action-card life-action" data-action="exercise" type="button" ${lifeActionsLocked() ? 'disabled' : ''}><span>🏋️</span><strong>헬스</strong><small>몸집 -2 · 체력 -20 · 배고픔 -${lifeHunger.exercise}</small></button>
...
function territoryViewPage() {
  ...
  const previousResult = last?.goldenCell
    ...
    : '';
  return `<section class="page-title"><span class="eyebrow">6시간 시즌</span><h1>제${territory.seasonNumber}회 레고 영토전</h1><p>본진을 지키면서 주변 8칸으로 땅을 넓히고 상대 영토를 빼앗는 게임입니다.</p></section>${previousResult}<section class="section territory-summary"><div class="metric-grid">${compactMetric('남은 시간', `<span id="territory-countdown">${durationText(new Date(territory.endsAt).getTime() - Date.now())}</span>`, '', { raw: true })}${compactMetric('내 레벨', `Lv.${territory.my.level}`)}${compactMetric('내 영토', `${territory.my.owned}/${territory.my.limit}`)}${compactMetric('현재 순위', territory.my.rank ? `${territory.my.rank}위` : '-')}</div><div class="territory-limit-strip"><span>${esc(territoryLimitHint)}</span><button class="soft-button" data-action="territory-limits" type="button">ⓘ 레벨별 한도</button></div><div class="territory-golden-status">${goldenStatus}</div><div class="territory-golden-status"><b>${battleUnlocked ? '⚔️ 전면전 진행 중 · 본진 탈취 가능' : `🛡️ 본진 보호 중 · ${territory.cells.length}/${mapSize * mapSize}칸 점유`}</b></div><div class="territory-rule"><b>룰:</b> 빈 땅 점령은 무료, 상대 땅 탈취는 20P입니다. 처음에는 🏠 본진이 보호됩니다. 맵 25칸이 한 번이라도 모두 차면 ⚔️ 전면전이 열려 본진도 탈취할 수 있습니다. 전면전 이후 영토가 0칸이 되어도 다른 칸을 차지해 다시 참가할 수 있습니다. 보유 한도가 꽉 차면 빈 땅 추가 점령은 막히지만, 상대 땅은 20P를 내고 가장 오래된 일반 영토 한 칸을 옮기는 방식으로 계속 탈취할 수 있습니다.</div></section><section class="section territory-section">${sectionHeading('공용 맵', `${mapSize} × ${mapSize}`)}<div class="territory-scroll"><div class="territory-map" role="grid" aria-label="${mapSize}행 ${mapSize}열 레고 영토전 맵">${rows}</div></div><p class="helper centered">회색칸은 빈 땅, 색깔 닉네임은 소유자입니다. 🏠 본진은 전면전 전까지만 보호되며, 25/25칸이 모두 차면 이후 탈취 가능합니다. 황금 영토 위치는 종료 전에는 공개되지 않습니다.</p></section><section class="section">${sectionHeading('현재 순위', '단독 1위만 300P를 받습니다. 동률이면 종료 후 공개되는 황금 영토가 최종 승자를 가릅니다.')}<div class="rank-list">${ranking}</div></section>`;
}
...
function openTerritoryLimits() {
  ...
  openModal(`${modalHeader('영토 레벨별 보유 한도', note)}<div class="territory-limit-table">${rows}</div><p class="warning-box">한도가 꽉 차면 빈 땅 추가 점령은 제한됩니다. 상대 영토 탈취는 20P이며, 한도에 꽉 찬 상태에서는 가장 오래된 일반 영토 한 칸을 비우고 탈취한 칸으로 이동해 총 보유 수를 유지합니다. 전면전 전에는 본진은 자동으로 비우지 않습니다.</p>`, { type:'territoryLimits' });
}
...
function socialView() {
  ...
  return `
    ...
    <section class="section">${sectionHeading('레고 주민목록', `${(app.data.residents ?? []).length}명`)}<div class="profile-grid resident-grid">${(app.data.residents ?? []).map(profileMiniCard).join('') || '<div class="empty">가입한 레고가 없습니다.</div>'}</div></section>
...
function ensureTabPanes() {
  ...
  for (const tab of MAIN_TABS) {
    if (view.querySelector(`.tab-pane[data-pane="${tab}"]`)) continue;
...
function updateAppChrome() {
  $$('.nav-item').forEach((button) => {
    const active = button.dataset.tab === app.tab;
...
function showTabPane(tab) {
  const view = ensureTabPanes();
  if (!view) return;
  view.querySelectorAll('.tab-pane').forEach((pane) => pane.classList.toggle('hidden', pane.dataset.pane !== tab));
}

function renderTab(tab, { force = false } = {}) {
  if (!app.data || !MAIN_TABS.includes(tab)) return;
  const view = ensureTabPanes();
  const pane = view?.querySelector(`.tab-pane[data-pane="${tab}"]`);
  if (!pane) return;
  const alreadyRendered = app.renderedTabs instanceof Set && app.renderedTabs.has(tab);
  ...
  if (tab === 'games') {
    requestAnimationFrame(() => {
      ...
      if (app.tab === 'games') syncOmokBoardSquare();
    });
  }
}

function cancelTabWarmup() {
  if (app.tabWarmTimer == null) return;
  if (app.tabWarmKind === 'idle' && 'cancelIdleCallback' in window) window.cancelIdleCallback(app.tabWarmTimer);
  else clearTimeout(app.tabWarmTimer);
  app.tabWarmTimer = null;
  app.tabWarmKind = null;
}

function scheduleTabWarmup(callback) {
  if ('requestIdleCallback' in window) {
    app.tabWarmKind = 'idle';
    app.tabWarmTimer = window.requestIdleCallback(callback, { timeout: 900 });
    return;
  }
  // Safari 등 requestIdleCallback 미지원 환경에서는 첫 화면과 터치 입력이 끝난 뒤 한 탭씩 처리한다.
  app.tabWarmKind = 'timer';
  app.tabWarmTimer = setTimeout(() => callback({ didTimeout: true, timeRemaining: () => 0 }), 140);
}

function warmInactiveTabs() {
  cancelTabWarmup();
  // 숨겨진 탭은 브라우저가 한가할 때만 한 개씩 갱신한다. 사용자가 누르는 순간에는 이 작업을 취소해 터치 반응을 우선한다.
  const queue = MAIN_TABS.filter((tab) => tab !== app.tab && (!app.renderedTabs.has(tab) || app.dirtyTabs.has(tab)));
  const next = () => {
    app.tabWarmTimer = null;
    app.tabWarmKind = null;
    if (!app.data) return;
    let tab = queue.shift();
    while (tab && (tab === app.tab || (app.renderedTabs.has(tab) && !app.dirtyTabs.has(tab)))) tab = queue.shift();
    if (!tab) return;
    renderTab(tab);
    if (queue.length) scheduleTabWarmup(next);
  };
  if (queue.length) scheduleTabWarmup(next);
}

function cancelPendingTabRefresh() {
  if (app.tabRefreshFrame) cancelAnimationFrame(app.tabRefreshFrame);
  if (app.tabRefreshTimer != null) clearTimeout(app.tabRefreshTimer);
  app.tabRefreshFrame = 0;
  app.tabRefreshTimer = null;
}

function scheduleActiveTabRefresh(tabName) {
  cancelPendingTabRefresh();
  const token = ++app.tabSwitchToken;
  // 활성 표시와 캐시 화면을 먼저 한 프레임 그린 직후 최신 DOM으로 교체한다.
  // 기존의 중첩 requestAnimationFrame보다 한 프레임 빠르고, 무거운 렌더가 첫 터치 화면을 막지 않는다.
  app.tabRefreshFrame = requestAnimationFrame(() => {
    app.tabRefreshFrame = 0;
    app.tabRefreshTimer = setTimeout(() => {
      app.tabRefreshTimer = null;
      if (!app.data || app.tab !== tabName || app.tabSwitchToken !== token) return;
...
function render() {
  ...
  ensureTabPanes();
  showTabPane(app.tab);
  // 데이터가 바뀌거나 게임 내부 상태가 바뀐 경우 현재 화면만 즉시 다시 그린다.
  renderTab(app.tab, { force: true });
  updateAppChrome();
  warmInactiveTabs();
}
...
async function openProfile(petId) {
  try {
    ...
    openModal(`${modalHeader('레고 프로필', profile.relationshipLabel || '')}<div class="profile-detail">${workoutBadgeHtml(profile)}${avatar(stage)}<strong class="profile-title">${esc(profile.displayName)}</strong>${seasonBadgesHtml(profile)}<button class="body-stage-profile-link" data-action="body-guide" data-body="${profile.stats.body}" type="button">${esc(stage.label)} · 몸집 ${profile.stats.body} · 단계 보기</button>${profile.statusMessage ? `<p class="profile-status-detail">${esc(profile.statusMessage)}</p>` : ''}<div class="metric-grid">${compactMetric('포인트', points(profile.stats.points))}${compactMetric('레벨', `Lv.${profile.stats.level}`)}${compactMetric('레고력', `${profile.stats.legoPower}`)}${compactMetric('몸집', `${profile.stats.body}`)}${compactMetric('경고', `${profile.warnings}회`)}${compactMetric('상태', profile.partnerPetId ? `커플 D+${profile.coupleDay}` : '솔로')}</div>${relationshipActions}</div>`, { type: 'profile', petId });
...
async function handleAction(button) {
  ...
  if (action === 'apple-new-board') {
    ...
    app.appleNewBoardDismissedKey = null;
    const result = await perform('/api/minigames/apple/new-board', { challengeId: challenge.id, requestId }, null, 'POST', { toastDuration: 900, toastType: 'game-start' });
    if (result?.bootstrap?.activeMiniChallenge && app.modal?.type === 'mini') openMiniGame(result.bootstrap.activeMiniChallenge);
    return result;
  }
...
function switchMainTab(tabName, { smooth = false } = {}) {
  ...
  // 가장 먼저 이미 만들어 둔 탭 DOM을 보여준다. API 요청과 전체 #view 재생성은 하지 않는다.
  ensureTabPanes();
  const pane = $('#view')?.querySelector(`.tab-pane[data-pane="${tabName}"]`);
  if (!app.renderedTabs.has(tabName) && pane) {
    pane.setAttribute('aria-busy', 'true');
    pane.innerHTML = '<div class="tab-loading" role="status">화면을 불러오는 중…</div>';
  }
  showTabPane(tabName);
  updateAppChrome();

  const needsRefresh = !app.renderedTabs.has(tabName) || app.dirtyTabs.has(tabName);
  if (needsRefresh) {
    scheduleActiveTabRefresh(tabName);
  } else if (tabName === 'games') {
    requestAnimationFrame(() => syncOmokBoardSquare());
  }
  // 활성 탭 갱신이 필요하면 그 작업이 끝난 뒤에 숨은 탭 예열을 시작한다.
  if (!needsRefresh) warmInactiveTabs();
  window.scrollTo({ top: 0, behavior: smooth ? 'smooth' : 'auto' });
}
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js?v=654').catch(() => {});
