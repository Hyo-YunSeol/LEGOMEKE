...
function flexItemImage(flexItem, { mini = false, shop = false } = {}) {
  ...
  return `<img class="${className}" src="/flex/${esc(flexItem.assetKey)}.svg?v=610001" alt="${esc(flexItem.name)}" draggable="false">`;
}
...
function avatar(stage, { mini = false, flexItem = null } = {}) {
  ...
  const src = `/pets/${esc(stage.assetKey || stage.key)}.svg?v=610001`;
...
function restoreSingleTetrisState(challenge) {
  try {
    ...
    // v6.10.0: 싱글도 대전과 동일하게 새 블록은 0행에서 즉시 보인다.
...
function sichuanBoardHtml(player, { interactive = false, compact = false } = {}) {
  ...
  return `<div class="sichuan-board ${compact ? 'compact' : ''}" role="grid" aria-rowcount="8" aria-colcount="10" aria-label="${esc(player?.displayName || '플레이어')} 사천성 판">${board.map((tileId, index) => {
    ...
    // v6.10.0: CSS Grid의 행 높이 계산을 완전히 사용하지 않는다.
    // 80개 슬롯은 10% × 12.5% 절대좌표에 고정되어 어떤 브라우저에서도 재배치되지 않는다.
    ...
    const tileSrc = tile?.src ? `${tile.src}${tile.src.includes('?') ? '&' : '?'}v=610001` : '';
...
navigator.serviceWorker?.register?.('/sw.js?v=610001')?.catch(() => {});
